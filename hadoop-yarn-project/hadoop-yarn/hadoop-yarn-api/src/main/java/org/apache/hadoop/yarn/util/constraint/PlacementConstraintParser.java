...
block|}
block|}
DECL|method|parseNameSpace (String targetTag)
name|TargetExpression
name|parseNameSpace
parameter_list|(
name|String
...
block|}
block|}
comment|/**    * Constraint parser used to parse a given target expression.    */
DECL|class|NodeConstraintParser
specifier|public
specifier|static
class|class
...
name|ConstraintParser
block|{
DECL|method|NodeConstraintParser (String expression)
specifier|public
name|NodeConstraintParser
parameter_list|(
name|String
...
name|attributeKV
init|=
name|getAttributeOpCodeTokenizer
argument_list|(
name|currentTag
argument_list|)
decl_stmt|;
comment|// Usually there will be only one k=v pair. However in case when
comment|// multiple values are present for same attribute, it will also be
comment|// coming as next token. for example, java=1.8,1.9 or python!=2.
if|if
condition|(
name|attributeKV
operator|.
...
name|PlacementConstraintParseException
argument_list|(
literal|"expecting valid expression like k=v or k!=v, but get "
operator|+
name|constraintEntities
argument_list|)
throw|;
...
return|;
block|}
DECL|method|getAttributeOpCodeTokenizer (String currentTag)
specifier|private
name|StringTokenizer
name|getAttributeOpCodeTokenizer
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|// Try with '!=' delim as well.
if|if
condition|(
name|attributeKV
operator|.
...
return|;
block|}
comment|/**      * Below conditions are validated.      * java=8   : OpCode = EQUALS      * java!=8  : OpCode = NEQUALS      * @param currentTag tag      * @return Attribute op code.      */
DECL|method|getAttributeOpCode (String currentTag)
specifier|private
name|NodeAttributeOpCode
name|getAttributeOpCode
parameter_list|(
...
name|PlacementConstraintParseException
block|{
comment|// Continue handling for application tag based constraint otherwise.
...
comment|// in the source tags, we do overwrite when there is no source tags
comment|// with num_containers argument from commandline. If that is partially
comment|// missed in the constraints, we don't know if it is ought to
comment|// overwritten or not.
if|if
condition|(
name|result
operator|.
...
name|PlacementConstraintParseException
argument_list|(
literal|"Source allocation tags is required for a multi placement"
operator|+
literal|" constraint expression."
argument_list|)
throw|;
block|}
block|}
