...
return|;
block|}
DECL|method|checkForCredentials (Configuration conf, ConfTree tree)
specifier|protected
specifier|static
name|void
name|checkForCredentials
...
block|}
block|}
comment|// using a normal reader instead of a secure one,
comment|// because stdin is not hooked up to the command line
DECL|method|readPassword (String alias, BufferedReader br)
specifier|private
specifier|static
name|char
index|[]
...
argument_list|()
expr_stmt|;
comment|// if the conf dir has a log4j-server.properties, switch to that
if|if
condition|(
name|hasServerLog4jProperties
condition|)
...
argument_list|)
decl_stmt|;
comment|// copy all jars except slider-core-<version>.jar
name|tarGzipFolder
argument_list|(
name|srcFolder
argument_list|,
