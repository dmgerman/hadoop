begin_unit|revision:0.9.5;language:Java;cregit-version:0.0.1
begin_comment
comment|/*  * Licensed to the Apache Software Foundation (ASF) under one  * or more contributor license agreements.  See the NOTICE file  * distributed with this work for additional information  * regarding copyright ownership.  The ASF licenses this file  * to you under the Apache License, Version 2.0 (the  * "License"); you may not use this file except in compliance  * with the License.  You may obtain a copy of the License at  *  *     http://www.apache.org/licenses/LICENSE-2.0  *  * Unless required by applicable law or agreed to in writing, software  * distributed under the License is distributed on an "AS IS" BASIS,  * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.  * See the License for the specific language governing permissions and  * limitations under the License.  */
end_comment

begin_package
DECL|package|org.apache.slider.common.params
package|package
name|org
operator|.
name|apache
operator|.
name|slider
operator|.
name|common
operator|.
name|params
package|;
end_package

begin_import
import|import
name|com
operator|.
name|beust
operator|.
name|jcommander
operator|.
name|Parameter
import|;
end_import

begin_import
import|import
name|com
operator|.
name|beust
operator|.
name|jcommander
operator|.
name|Parameters
import|;
end_import

begin_class
annotation|@
name|Parameters
argument_list|(
name|commandNames
operator|=
block|{
name|SliderActions
operator|.
name|ACTION_PACKAGE
block|}
argument_list|,
name|commandDescription
operator|=
name|SliderActions
operator|.
name|DESCRIBE_ACTION_PACKAGE
argument_list|)
DECL|class|ActionPackageArgs
specifier|public
class|class
name|ActionPackageArgs
extends|extends
name|AbstractActionArgs
block|{
annotation|@
name|Override
DECL|method|getActionName ()
specifier|public
name|String
name|getActionName
parameter_list|()
block|{
return|return
name|SliderActions
operator|.
name|ACTION_PACKAGE
return|;
block|}
annotation|@
name|Parameter
argument_list|(
name|names
operator|=
block|{
name|ARG_INSTALL
block|}
argument_list|,
name|description
operator|=
literal|"Install package in the sub-folder 'package' of the user's Slider base directory"
argument_list|)
DECL|field|install
specifier|public
name|boolean
name|install
decl_stmt|;
annotation|@
name|Parameter
argument_list|(
name|names
operator|=
block|{
name|ARG_PKGDELETE
block|}
argument_list|,
name|description
operator|=
literal|"Delete package operation"
argument_list|)
DECL|field|delete
specifier|public
name|boolean
name|delete
decl_stmt|;
annotation|@
name|Parameter
argument_list|(
name|names
operator|=
block|{
name|ARG_PKGLIST
block|}
argument_list|,
name|description
operator|=
literal|"List of package(s) installed"
argument_list|)
DECL|field|list
specifier|public
name|boolean
name|list
decl_stmt|;
annotation|@
name|Parameter
argument_list|(
name|names
operator|=
block|{
name|ARG_PKGINSTANCES
block|}
argument_list|,
name|description
operator|=
literal|"Lists all application instances referring to package"
argument_list|)
DECL|field|instances
specifier|public
name|boolean
name|instances
decl_stmt|;
annotation|@
name|Parameter
argument_list|(
name|names
operator|=
block|{
name|ARG_PACKAGE
block|}
argument_list|,
name|description
operator|=
literal|"Path to app package on local disk"
argument_list|)
DECL|field|packageURI
specifier|public
name|String
name|packageURI
decl_stmt|;
annotation|@
name|Parameter
argument_list|(
name|names
operator|=
block|{
name|ARG_NAME
block|}
argument_list|,
name|description
operator|=
literal|"Package name"
argument_list|)
DECL|field|name
specifier|public
name|String
name|name
decl_stmt|;
annotation|@
name|Parameter
argument_list|(
name|names
operator|=
block|{
name|ARG_VERSION
block|}
argument_list|,
name|description
operator|=
literal|"Package version"
argument_list|)
DECL|field|version
specifier|public
name|String
name|version
decl_stmt|;
annotation|@
name|Parameter
argument_list|(
name|names
operator|=
block|{
name|ARG_REPLACE_PKG
block|}
argument_list|,
name|description
operator|=
literal|"Overwrite existing package"
argument_list|)
DECL|field|replacePkg
specifier|public
name|boolean
name|replacePkg
init|=
literal|false
decl_stmt|;
annotation|@
name|Parameter
argument_list|(
name|names
operator|=
block|{
name|ARG_OUTPUT
block|,
name|ARG_OUTPUT_SHORT
block|}
argument_list|,
name|description
operator|=
literal|"Output file for package data"
argument_list|)
DECL|field|out
specifier|public
name|String
name|out
decl_stmt|;
comment|/**    * Get the min #of params expected    * @return the min number of params in the {@link #parameters} field    */
DECL|method|getMinParams ()
specifier|public
name|int
name|getMinParams
parameter_list|()
block|{
return|return
literal|0
return|;
block|}
annotation|@
name|Override
DECL|method|getMaxParams ()
specifier|public
name|int
name|getMaxParams
parameter_list|()
block|{
return|return
literal|1
return|;
block|}
block|}
end_class

end_unit

