...
begin_comment
comment|/*  * Common Exit codes  *<p>  * Exit codes from 64 up are application specific.  *<p>  * Many of the exit codes are designed to resemble HTTP error codes,  * squashed into a single byte. e.g 44 , "not found" is the equivalent  * of 404  *<pre>  *    0-10: general command issues  *   30-39: equivalent to the 3XX responses, where those responses are  *          considered errors by the application.  *   40-49: request-related errors  *   50-59: server-side problems. These may be triggered by the request.  *   64-  : application specific error codes  *</pre>  */
end_comment
...
literal|53
decl_stmt|;
comment|/**    * The application does not support, or refuses to support this version: {@value}.    * If raised, this is expected to be raised server-side and likely due    * to client/server version incompatibilities.    *<p>    *<code> 505: Version Not Supported</code>    */
