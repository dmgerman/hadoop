...
block|}
block|}
comment|/**    * Get a single domain of the particular ID. If callerUGI is not the owner    * or the admin of the domain, we need to hide the details from him, and    * only allow him to see the ID.    */
...
return|;
block|}
comment|/**    * Get all the domains that belong to the given owner. If callerUGI is not    * the owner or the admin of the domain, we need to hide the details from    * him, and only allow him to see the ID.    */
...
control|)
block|{
comment|// The owner for each domain is the same, just need to check on
if|if
condition|(
operator|!
name|isChecked
...
return|;
block|}
DECL|method|hideDomainDetails (TimelineDomain domain)
specifier|private
specifier|static
name|void
name|hideDomainDetails
