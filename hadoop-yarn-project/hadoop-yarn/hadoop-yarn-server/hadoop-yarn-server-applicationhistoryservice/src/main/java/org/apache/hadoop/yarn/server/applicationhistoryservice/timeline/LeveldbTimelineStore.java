...
block|}
block|}
comment|/**    * Get the unique start time for a given entity as a byte array that sorts    * the timestamps in reverse order (see {@link    * GenericObjectMapper#writeReverseOrderedLong(long)}). If the start time    * doesn't exist, set it based on the information provided. Should only be    * called when a lock has been obtained on the entity.    *    * @param entityId The id of the entity    * @param entityType The type of the entity    * @param startTime The start time of the entity, or null    * @param events A list of events for the entity, or null    * @return A byte array    * @throws IOException    */
...
return|;
block|}
comment|// warning is suppressed to prevent eclipse from noting unclosed resource
annotation|@
name|SuppressWarnings
argument_list|(
literal|"resource"
...
DECL|method|deleteKeysWithPrefix (WriteBatch writeBatch, byte[] prefix, DBIterator iterator)
specifier|private
name|void
...
annotation|@
name|VisibleForTesting
DECL|method|deleteNextEntity (String entityType, byte[] reverseTimestamp, DBIterator iterator, DBIterator pfIterator, boolean seeked)
name|boolean
