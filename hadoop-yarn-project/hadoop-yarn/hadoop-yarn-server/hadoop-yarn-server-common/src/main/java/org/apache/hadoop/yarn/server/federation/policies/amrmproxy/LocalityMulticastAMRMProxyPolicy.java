...
comment|// any cluster. Pick a random sub-cluster from active and enabled ones.
name|targetId
operator|=
...
return|;
block|}
comment|/**    * For unit test to override.    */
...
argument_list|()
decl_stmt|;
comment|// if we have headroom information for this sub-cluster (and we are safe
...
argument_list|()
decl_stmt|;
DECL|field|maskForRackDeletion
specifier|private
name|Map
argument_list|<
name|SubClusterId
...
argument_list|()
decl_stmt|;
comment|// Store the randomly selected subClusterId for unresolved resource requests
comment|// keyed by requestId
DECL|field|unResolvedRequestLocation
specifier|private
name|Map
argument_list|<
name|Long
...
name|SubClusterId
argument_list|>
name|unResolvedRequestLocation
init|=
operator|new
name|HashMap
argument_list|<>
...
literal|0
decl_stmt|;
DECL|method|reinitialize ( Map<SubClusterId, SubClusterInfo> activeSubclusters, Set<SubClusterId> timedOutSubClusters)
specifier|private
name|void
name|reinitialize
parameter_list|(
...
expr_stmt|;
block|}
comment|/**      * Add a rack-local request to the final answer.      */
...
expr_stmt|;
block|}
DECL|method|internalAddToAnswer (SubClusterId targetId, ResourceRequest partialRR, boolean isRack)
specifier|private
name|void
name|internalAddToAnswer
parameter_list|(
...
expr_stmt|;
block|}
comment|/**      * For requests whose location cannot be resolved, choose an active and      * enabled sub-cluster to forward this requestId to.      */
DECL|method|getSubClusterForUnResolvedRequest (long allocationId)
specifier|private
name|SubClusterId
name|getSubClusterForUnResolvedRequest
parameter_list|(
...
argument_list|(
literal|"Should not be here. activeAndEnabledSC size = "
operator|+
name|activeAndEnabledSC
operator|.
name|size
...
argument_list|()
decl_stmt|;
comment|// Remove redundant rack RR before returning the answer
while|while
condition|(
name|answerIter
operator|.
...
name|info
argument_list|(
literal|"removing {} from output because it has only rack RR"
argument_list|,
name|scId
argument_list|)
expr_stmt|;
