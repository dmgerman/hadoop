...
throw|;
block|}
comment|/**    * Select a random bin according to the weight array for the bins. Only bins    * with positive weights will be considered. If no positive weight found,    * return -1.    *    * @param weights the weight array    * @return the index of the sample in the array    */
DECL|method|getWeightedRandom (ArrayList<Float> weights)
specifier|public
specifier|static
name|int
...
block|}
block|}
comment|// This can only happen if samplePoint is very close to totoalWeight and
comment|// float rounding kicks in during subtractions
return|return
name|lastIndex
return|;
block|}
