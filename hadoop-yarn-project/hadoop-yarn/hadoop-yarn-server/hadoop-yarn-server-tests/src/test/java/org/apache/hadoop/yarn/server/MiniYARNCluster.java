...
return|;
block|}
comment|/**    * In a HA cluster, go through all the RMs and find the Active RM. If none    * of them are active, wait upto 5 seconds for them to transition to Active.    *    * In an non-HA cluster, return the index of the only RM.    *    * @return index of the active RM or -1 if none of them transition to    * active even after 5 seconds of waiting    */
annotation|@
name|InterfaceAudience
operator|.
name|Private
...
expr_stmt|;
block|}
DECL|method|setHARMConfiguration (Configuration conf)
specifier|private
name|void
...
block|}
block|}
comment|/**    * Wait for all the NodeManagers to connect to the ResourceManager.    *    * @param timeout Time to wait (sleeps in 100 ms intervals) in milliseconds.    * @return true if all NodeManagers connect to the (Active)    * ResourceManager, false otherwise.    * @throws YarnException    * @throws InterruptedException    */
