...
literal|true
decl_stmt|;
DECL|method|AdminService (ResourceManager rm)
specifier|public
name|AdminService
parameter_list|(
name|ResourceManager
...
name|IOException
argument_list|(
literal|"Scheduler configuration is mutable. "
operator|+
name|operation
operator|+
literal|" is not allowed in this scenario."
argument_list|)
throw|;
block|}
name|refreshQueues
...
block|}
block|}
DECL|method|isSchedulerMutable ()
specifier|private
name|boolean
name|isSchedulerMutable
parameter_list|()
...
parameter_list|)
block|{
comment|// The interface is from hadoop-common which does not accept YarnException
throw|throw
operator|new
name|IOException
argument_list|(
...
name|IOException
argument_list|(
literal|"Failed to refresh configuration:"
argument_list|,
name|e
argument_list|)
throw|;
...
name|operation
init|=
literal|"mapAttributesToNodes"
decl_stmt|;
specifier|final
name|String
name|msg
init|=
literal|"Map Attributes to Nodes"
decl_stmt|;
name|UserGroupInformation
name|user
init|=
...
argument_list|()
operator|+
literal|" specified in the mapAttributesToNodes request "
argument_list|)
throw|;
block|}
block|}
...
return|;
block|}
DECL|method|validateAttributesExists ( List<NodeToAttributes> nodesToAttributes)
specifier|private
name|void
name|validateAttributesExists
parameter_list|(
...
block|}
block|}
comment|/**    * @param nodesToAttributesMapping input to be validated    * @param failOnUnknownNodes indicates to fail if the nodes are not available.    * @return the map of Node host name to set of NodeAttributes    * @throws IOException if validation fails for node existence or the attribute    *           has a wrong prefix    */
DECL|method|validateAndFetch ( List<NodeToAttributes> nodesToAttributesMapping, boolean failOnUnknownNodes)
specifier|private
name|Map
argument_list|<
name|String
...
name|IOException
argument_list|(
literal|"Invalid Attribute Mapping for the node "
operator|+
name|node
operator|+
literal|". Prefix should be "
operator|+
name|NodeAttribute
operator|.
name|PREFIX_CENTRALIZED
...
name|message
init|=
literal|" Following nodes does not exist : "
operator|+
name|invalidNodes
decl_stmt|;
name|LOG
...
return|;
block|}
comment|/**    * @param node    * @return true if valid else false;    */
DECL|method|validateForInvalidNode (String node, boolean failOnUnknownNodes)
specifier|private
name|boolean
name|validateForInvalidNode
parameter_list|(
