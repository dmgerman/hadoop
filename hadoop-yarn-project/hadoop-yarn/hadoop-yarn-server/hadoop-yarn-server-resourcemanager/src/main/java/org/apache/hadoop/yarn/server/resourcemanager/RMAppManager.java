...
condition|)
block|{
name|removeCompletedAppsFromStateStore
argument_list|()
expr_stmt|;
block|}
if|if
...
condition|)
block|{
name|removeCompletedAppsFromMemory
argument_list|()
expr_stmt|;
block|}
block|}
DECL|method|removeCompletedAppsFromStateStore ()
specifier|private
name|void
name|removeCompletedAppsFromStateStore
parameter_list|()
...
name|maxCompletedAppsInStateStore
operator|+
literal|", but not removing app "
operator|+
name|removeId
operator|+
literal|" from state store as log aggregation have not finished yet."
argument_list|)
expr_stmt|;
block|}
block|}
block|}
DECL|method|removeCompletedAppsFromMemory ()
specifier|private
name|void
name|removeCompletedAppsFromMemory
parameter_list|()
...
name|removeId
operator|+
literal|" from memory as log aggregation have not finished yet."
argument_list|)
expr_stmt|;
block|}
block|}
block|}
DECL|method|shouldDeleteApp (RMApp app)
specifier|private
name|boolean
name|shouldDeleteApp
parameter_list|(
...
block|{
name|boolean
name|applicationTagBasedPlacementEnabled
init|=
name|conf
operator|.
name|getBoolean
...
name|YarnConfiguration
operator|.
name|DEFAULT_APPLICATION_TAG_BASED_PLACEMENT_ENABLED
argument_list|)
decl_stmt|;
name|String
name|usernameUsedForPlacement
...
name|warn
argument_list|(
literal|"User '{}' is not allowed to do placement based "
operator|+
...
init|=
name|getUserNameFromApplicationTag
argument_list|(
name|applicationTags
argument_list|)
decl_stmt|;
...
argument_list|)
decl_stmt|;
comment|// check if the actual user has rights to submit application to the
comment|// user's queue from the application tag
name|String
name|queue
init|=
name|placementManager
...
name|warn
argument_list|(
literal|"User '{}' from application tag does not have access to "
operator|+
literal|" queue '{}'. "
operator|+
literal|"The placement is done for user '{}'"
argument_list|,
name|userNameFromAppTag
argument_list|,
name|queue
...
return|;
block|}
DECL|method|isWhitelistedUser (final String user, final Configuration config)
specifier|private
name|boolean
name|isWhitelistedUser
parameter_list|(
...
return|;
block|}
DECL|method|getUserNameFromApplicationTag (Set<String> applicationTags)
specifier|private
name|String
name|getUserNameFromApplicationTag
parameter_list|(
...
name|warn
argument_list|(
literal|"Found wrongly qualified username in tag"
argument_list|)
expr_stmt|;
block|}
block|}
