...
argument_list|()
decl_stmt|;
comment|// sum of resources used by application masters for applications
comment|// running in this queue
DECL|field|usedAMResources
specifier|private
specifier|final
name|Resource
name|usedAMResources
init|=
name|Resource
operator|.
name|newInstance
...
argument_list|)
decl_stmt|;
DECL|field|consumedAMResources
name|Resource
name|consumedAMResources
init|=
name|Resources
operator|.
name|createResource
...
parameter_list|()
block|{
comment|//? Is this actually used by anything at present?
comment|//  There is a findbugs warning -re lastClusterResource (now excluded),
comment|//  when this is used, verify that the access is mt correct and remove
comment|//  the findbugs exclusion if possible
if|if
condition|(
name|Resources
operator|.
