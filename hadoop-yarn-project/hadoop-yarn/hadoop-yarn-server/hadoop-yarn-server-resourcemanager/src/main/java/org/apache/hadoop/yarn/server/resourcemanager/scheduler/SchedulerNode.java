...
argument_list|)
decl_stmt|;
comment|/** Time stamp for overcommitted resources to time out. */
...
expr_stmt|;
block|}
comment|/**    * Set the timeout for the node to stop overcommitting the resources. After    * this time the scheduler will start killing containers until the resources    * are not overcommitted anymore. This may reset a previous timeout.    * @param timeOut Time out in milliseconds.    */
DECL|method|setOvercommitTimeOut (long timeOut)
specifier|public
specifier|synchronized
name|void
...
name|debug
argument_list|(
literal|"The overcommit timeout for {} was already set to {}"
argument_list|,
name|getNodeID
argument_list|()
argument_list|,
...
block|}
block|}
comment|/**    * Check if the time out has passed.    * @return If the node is overcommitted.    */
DECL|method|isOvercommitTimedOut ()
specifier|public
specifier|synchronized
name|boolean
name|isOvercommitTimedOut
parameter_list|()
block|{
return|return
name|this
...
return|;
block|}
comment|/**    * Check if the node has a time out for overcommit resources.    * @return If the node has a time out for overcommit resources.    */
DECL|method|isOvercommitTimeOutSet ()
specifier|public
specifier|synchronized
name|boolean
name|isOvercommitTimeOutSet
parameter_list|()
block|{
return|return
name|this
...
argument_list|()
decl_stmt|;
comment|// We remove allocation tags when a container is actually
comment|// released on NM. This is to avoid running into situation
comment|// when AM releases a container and NM has some delay to
comment|// actually release it, then the tag can still be visible
comment|// at RM so that RM can respect it during scheduling new containers.
if|if
condition|(
name|rmContext
operator|!=
...
return|;
block|}
comment|/**    * Get the containers running on the node ordered by which to kill first. It    * tries to kill AMs last, then GUARANTEED containers, and it kills    * OPPORTUNISTIC first. If the same time, it uses the creation time.    * @return A copy of the running containers ordered by which to kill first.    */
...
return|;
block|}
comment|/**    * Get the launched containers in the node.    * @return List of launched containers.    */
...
return|;
block|}
DECL|method|updateNodeAttributes (Set<NodeAttribute> attributes)
specifier|public
name|void
