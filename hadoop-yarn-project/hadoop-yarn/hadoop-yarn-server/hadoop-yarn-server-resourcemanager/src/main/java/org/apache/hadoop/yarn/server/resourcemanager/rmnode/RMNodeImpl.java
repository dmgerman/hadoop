...
comment|// it will keep decommissioning.
if|if
condition|(
name|isNodeDecommissioning
condition|)
...
argument_list|()
decl_stmt|;
comment|// no running (and keeping alive) app on this node, get it
comment|// decommissioned.
comment|// TODO may need to check no container is being scheduled on this node
comment|// as well.
if|if
condition|(
operator|(
...
return|;
block|}
comment|// TODO (in YARN-3223) if node in decommissioning, get node resource
comment|// updated if container get finished (keep available resource to be 0)
block|}
name|rmNode
operator|.
name|handleContainerStatus
