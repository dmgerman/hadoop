...
expr_stmt|;
block|}
DECL|method|createAndStartCurator (Configuration conf)
specifier|public
name|CuratorFramework
name|createAndStartCurator
parameter_list|(
...
block|{
comment|// we're dealing with the v.1.x publisher
name|LOG
operator|.
name|info
argument_list|(
literal|"system metrics publisher with the timeline service V1 is "
operator|+
literal|"configured"
argument_list|)
expr_stmt|;
...
name|YarnConfiguration
operator|.
name|RM_WEBAPP_UI2_ADDRESS
argument_list|,
name|YarnConfiguration
operator|.
name|DEFAULT_RM_WEBAPP_UI2_ADDRESS
argument_list|,
name|YarnConfiguration
operator|.
name|DEFAULT_RM_WEBAPP_UI2_PORT
argument_list|)
decl_stmt|;
specifier|final
name|InetSocketAddress
...
name|YarnConfiguration
operator|.
name|RM_WEBAPP_UI2_HTTPS_ADDRESS
argument_list|,
name|YarnConfiguration
operator|.
name|DEFAULT_RM_WEBAPP_UI2_HTTPS_ADDRESS
argument_list|,
name|YarnConfiguration
operator|.
name|DEFAULT_RM_WEBAPP_UI2_HTTPS_PORT
argument_list|)
decl_stmt|;
name|HttpServer2
operator|.
...
parameter_list|()
block|{
comment|// Use the customized yarn filter instead of the standard kerberos filter to
comment|// allow users to authenticate using delegation tokens
comment|// 4 conditions need to be satisfied -
comment|// 1. security is enabled
comment|// 2. http auth type is set to kerberos
comment|// 3. "yarn.resourcemanager.webapp.use-yarn-filter" override is set to true
comment|// 4. hadoop.http.filter.initializers container AuthenticationFilterInitializer
name|Configuration
name|conf
init|=
name|getConfig
...
name|YarnConfiguration
operator|.
name|RM_WEBAPP_UI2_ENABLE
argument_list|,
name|YarnConfiguration
operator|.
name|DEFAULT_RM_WEBAPP_UI2_ENABLE
argument_list|)
condition|)
block|{
try|try
...
name|info
argument_list|(
literal|"Yarn WebApp UI 2 is started"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
name|error
argument_list|(
literal|"Failed to start Yarn web app v2:"
operator|+
name|e
operator|.
name|getMessage
