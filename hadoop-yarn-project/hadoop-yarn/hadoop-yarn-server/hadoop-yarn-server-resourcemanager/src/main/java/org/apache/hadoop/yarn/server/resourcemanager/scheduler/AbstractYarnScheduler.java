...
name|info
argument_list|(
literal|"Update resource on node: {} from: {}, to: {} in {} ms"
argument_list|,
name|node
operator|.
name|getNodeName
...
argument_list|)
expr_stmt|;
name|signalContainersIfOvercommitted
argument_list|(
name|node
argument_list|,
name|timeout
...
parameter_list|)
block|{
comment|// If there is no time out, we don't do anything
if|if
condition|(
operator|!
name|schedulerNode
...
expr_stmt|;
comment|// If it hasn't timed out yet, don't kill
if|if
condition|(
operator|!
name|schedulerNode
...
block|}
block|}
comment|// Check if the node is overcommitted (negative resources)
name|ResourceCalculator
name|rc
init|=
...
name|info
argument_list|(
literal|"{} is overcommitted ({}), preempt/kill containers"
argument_list|,
name|schedulerNode
operator|.
name|getNodeID
...
name|debug
argument_list|(
literal|"Enough unallocated resources {}"
argument_list|,
name|unallocated
argument_list|)
expr_stmt|;
...
block|}
comment|/**    * Gets the apps from a given queue.    *    * Mechanics:    * 1. Get all {@link ApplicationAttemptId}s in the given queue by    * {@link #getAppsInQueue(String)} method.    * 2. Always need to check validity for the given queue by the returned    * values.    *    * @param queueName queue name    * @return a collection of app attempt ids in the given queue, it maybe empty.    * @throws YarnException if {@link #getAppsInQueue(String)} return null, will    * throw this exception.    */
DECL|method|getAppsFromQueue (String queueName)
specifier|private
name|List
argument_list|<
name|ApplicationAttemptId
