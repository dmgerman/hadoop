...
DECL|field|preemptableEntities
specifier|private
name|Map
argument_list|<
name|String
...
literal|"unchecked"
argument_list|)
DECL|method|cleanupStaledKillableContainers (Resource cluster, Set<String> leafQueueNames)
specifier|private
name|void
name|cleanupStaledKillableContainers
parameter_list|(
name|Resource
name|cluster
parameter_list|,
...
control|)
block|{
comment|// When queue's used - killable<= guaranteed and, killable> 0, we need
comment|// to check if any of killable containers needs to be reverted
if|if
condition|(
name|Resources
operator|.
...
condition|)
block|{
comment|// How many killable resources need to be reverted
comment|// need-to-revert = already-marked-killable - (current - ideal)
name|Resource
name|toBeRevertedFromKillable
init|=
name|Resources
operator|.
name|subtract
...
block|{
comment|// This container need to be marked to unkillable
name|Resources
operator|.
name|addTo
argument_list|(
...
expr_stmt|;
block|}
comment|// remove containers from killable list when we want to preempt less resources
comment|// from queue.
name|cleanupStaledKillableContainers
argument_list|(
name|clusterResources
...
condition|)
block|{
comment|// mark container killable
name|rmContext
operator|.
name|getDispatcher
argument_list|()
...
block|}
block|}
DECL|method|getPartitionByRMContainer (RMContainer rmContainer)
specifier|private
name|String
name|getPartitionByRMContainer
parameter_list|(
name|RMContainer
name|rmContainer
parameter_list|)
...
expr_stmt|;
block|}
comment|/**    * Based a resource preemption target drop reservations of containers and    * if necessary select containers for preemption from applications in each    * over-capacity queue. It uses {@link #NATURAL_TERMINATION_FACTOR} to    * account for containers that will naturally complete.    *    * @param leafQueueNames set of leaf queues to preempt from    * @param clusterResource total amount of cluster resources    * @return a map of applciationID to set of containers to preempt    */
...
name|preemptionDisabled
decl_stmt|;
DECL|method|TempQueuePerPartition (String queueName, Resource current, Resource pending, Resource guaranteed, Resource maxCapacity, boolean preemptionDisabled, String partition, Resource killableResource)
name|TempQueuePerPartition
parameter_list|(
name|String
name|queueName
