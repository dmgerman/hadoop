...
argument_list|)
expr_stmt|;
comment|// Do we need to do preemption to balance queue even after queues get satisfied?
name|boolean
...
decl_stmt|;
name|long
name|maximumKillWaitTimeForPreemptionToQueueBalance
init|=
name|config
operator|.
name|getLong
...
literal|"\n"
operator|+
literal|"Preemption-to-balance-queue-enabled = "
operator|+
name|isPreemptionToBalanceRequired
argument_list|)
expr_stmt|;
...
block|}
block|}
DECL|method|preemptOrkillSelectedContainerAfterWait ( Map<PreemptionCandidatesSelector, Map<ApplicationAttemptId, Set<RMContainer>>> toPreemptPerSelector, long currentTime)
specifier|private
name|void
...
comment|// checking if current container should be killed or not
for|for
control|(
name|Map
operator|.
...
control|)
block|{
comment|// if we tried to preempt this for more than maxWaitTime, this
comment|// should be based on custom timeout per container per selector
if|if
condition|(
name|preemptionCandidates
operator|.
...
parameter_list|)
block|{
comment|// If its a ManagedParentQueue, it might not have any children
if|if
condition|(
operator|(
name|q
...
name|format
argument_list|(
literal|"So far, total {0} containers selected to be preempted, {1}"
operator|+
literal|" containers selected this round\n"
argument_list|,
name|totalSelected
argument_list|,
name|curSelected
...
name|ApplicationAttemptId
argument_list|,
DECL|method|getToPreemptCandidatesPerSelector ()
name|Set
argument_list|<
name|RMContainer
argument_list|>
