...
block|}
name|QueueMetricsForCustomResources
name|metricsForCustomResources
init|=
name|scheduler
operator|.
name|getRootQueueMetrics
...
argument_list|()
decl_stmt|;
comment|// We expect all custom resources contained in availableResources,
comment|// so we will loop through all of them.
for|for
control|(
name|Map
operator|.
...
argument_list|()
decl_stmt|;
comment|// We only update the value if fairshare is 0 for that resource.
if|if
condition|(
name|maxResource
operator|.
