...
expr_stmt|;
block|}
comment|// perform initial assignment
name|initIdealAssignment
argument_list|(
name|totGuarant
argument_list|,
name|q
...
argument_list|)
decl_stmt|;
comment|// This value will be used in every round to calculate ideal allocation.
comment|// So make a copy to avoid it changed during calculation.
name|Resource
name|dupUnassignedForTheRound
init|=
name|Resources
operator|.
name|clone
...
argument_list|()
decl_stmt|;
comment|// How much resource we offer to the queue (to increase its ideal_alloc
name|Resource
name|wQavail
init|=
name|Resources
...
argument_list|)
decl_stmt|;
comment|// Make sure it is not beyond unassigned
name|wQavail
operator|=
name|Resources
operator|.
...
argument_list|)
expr_stmt|;
comment|// Make sure unassigned is always larger than 0
name|unassigned
operator|=
name|Resources
operator|.
...
block|}
block|}
comment|/**    * This method is visible to allow sub-classes to override the initialization    * behavior.    *    * @param totGuarant total resources (useful for {@code ResourceCalculator}    *          operations)    * @param q the {@code TempQueuePerPartition} being initialized    * @param initIdealAssigned the proposed initialization value.    */
DECL|method|initIdealAssignment (Resource totGuarant, TempQueuePerPartition q, Resource initIdealAssigned)
specifier|protected
name|void
name|initIdealAssignment
parameter_list|(
