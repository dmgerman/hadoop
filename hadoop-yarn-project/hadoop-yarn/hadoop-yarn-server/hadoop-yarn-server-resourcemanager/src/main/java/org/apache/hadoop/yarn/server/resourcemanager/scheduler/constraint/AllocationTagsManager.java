...
argument_list|()
decl_stmt|;
DECL|method|TypeToCountedTags ()
specifier|public
name|TypeToCountedTags
parameter_list|()
block|{}
DECL|method|TypeToCountedTags (Map<T, Map<String, Long>> tags)
specifier|private
name|TypeToCountedTags
parameter_list|(
name|Map
...
return|;
block|}
comment|/**      * Absorbs the given {@link TypeToCountedTags} to current mapping,      * this will aggregate the count of the tags with same name.      *      * @param target a {@link TypeToCountedTags} to merge with.      */
DECL|method|absorb (final TypeToCountedTags<T> target)
specifier|protected
name|void
name|absorb
parameter_list|(
...
parameter_list|)
block|{
comment|// No opt if the given target is null.
if|if
condition|(
name|target
operator|==
...
control|)
block|{
comment|// Get a mutable copy, do not modify the target reference.
name|Map
argument_list|<
name|String
argument_list|,
...
argument_list|)
decl_stmt|;
comment|// If the target type doesn't exist in the current mapping,
comment|// add as a new entry.
name|Map
argument_list|<
name|String
argument_list|,
...
argument_list|)
decl_stmt|;
comment|// There was a mapping for this target type,
comment|// do proper merging on the operator.
if|if
condition|(
name|existingMapping
operator|!=
...
argument_list|)
decl_stmt|;
comment|// Merge the target map to the inner map.
name|Map
argument_list|<
name|String
argument_list|,
...
block|}
block|}
comment|/**      * @return an immutable copy of current instance.      */
...
expr_stmt|;
block|}
comment|/**    * Aggregates multiple {@link TypeToCountedTags} to a single one based on    * the scope defined in the allocation tags, the values are properly merged.    *    * @param allocationTags {@link AllocationTags}.    * @return an aggregated {@link TypeToCountedTags}.    */
DECL|method|aggregateAllocationTags ( AllocationTags allocationTags, Map<ApplicationId, TypeToCountedTags> mapping)
specifier|private
name|TypeToCountedTags
name|aggregateAllocationTags
parameter_list|(
name|AllocationTags
name|allocationTags
parameter_list|,
...
block|{
comment|// Based on the namespace type of the given allocation tags
...
condition|)
block|{
comment|// If there is only one app, we simply return the mapping
comment|// without any extra computation.
return|return
name|mapping
operator|.
name|get
...
condition|)
block|{
comment|// Make sure ATM state won't be changed.
name|result
operator|.
name|absorb
argument_list|(
...
block|{
comment|// Aggregate app tags cardinality by applications.
name|mapping
operator|=
name|aggregateAllocationTags
argument_list|(
...
block|{
comment|// Aggregates cardinality by rack.
name|mapping
operator|=
name|aggregateAllocationTags
argument_list|(
...
return|;
block|}
comment|/**    * @return all applications that is known to the    * {@link AllocationTagsManager}, along with their application tags.    * The result is a map, where key is an application ID, and value is the    * application-tags attached to this application. If there is no    * application-tag exists for the application, the value is an empty set.    */
DECL|method|getApplicationIdToTags ()
specifier|private
name|Map
argument_list|<
name|ApplicationId
