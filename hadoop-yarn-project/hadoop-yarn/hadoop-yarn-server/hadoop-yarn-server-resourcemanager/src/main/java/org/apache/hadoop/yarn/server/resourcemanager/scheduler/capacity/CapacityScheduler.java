...
argument_list|()
expr_stmt|;
comment|// Register CS specific multi-node policies to common MultiNodeManager
comment|// which will add to a MultiNodeSorter which gives a pre-sorted list of
comment|// nodes to scheduler's allocation.
name|multiNodePlacementEnabled
operator|=
name|this
operator|.
...
literal|"ms"
operator|+
literal|",multiNodePlacementEnabled="
operator|+
name|multiNodePlacementEnabled
argument_list|)
expr_stmt|;
...
argument_list|()
decl_stmt|;
comment|// Get nodeId from allocated container if incoming argument is null.
name|NodeId
name|updatedNodeid
init|=
operator|(
...
return|;
block|}
DECL|method|getCandidateNodeSet ( FiCaSchedulerNode node)
specifier|private
name|CandidateNodeSet
argument_list|<
name|FiCaSchedulerNode
argument_list|>
name|getCandidateNodeSet
parameter_list|(
name|FiCaSchedulerNode
name|node
parameter_list|)
...
argument_list|()
operator|+
literal|" doesn't have sufficient "
operator|+
literal|"available or preemptible resource for minimum allocation"
argument_list|)
expr_stmt|;
name|ActivitiesLogger
operator|.
...
name|debug
argument_list|(
literal|"This partition '{}' doesn't have available or "
operator|+
...
block|}
block|}
DECL|method|updateNodeAttributes ( NodeAttributesUpdateSchedulerEvent attributeUpdateEvent)
specifier|private
name|void
name|updateNodeAttributes
parameter_list|(
...
block|}
block|}
DECL|method|updateAttributesOnNode (List<NodeId> nodeIds, Set<NodeAttribute> attributes)
specifier|private
name|void
name|updateAttributesOnNode
parameter_list|(
...
condition|)
block|{
comment|// Update old partition to list.
name|updateLabels
operator|.
name|add
argument_list|(
...
block|}
block|}
DECL|method|refreshLabelToNodeCache (Set<String> updateLabels)
specifier|private
name|void
name|refreshLabelToNodeCache
parameter_list|(
...
expr_stmt|;
block|}
comment|// recover attributes from store if any.
if|if
condition|(
name|rmContext
operator|.
...
expr_stmt|;
block|}
comment|// Move all reserved containers
for|for
control|(
name|RMContainer
name|rmContainer
...
literal|null
expr_stmt|;
comment|// Decrease unconfirmed resource if app is alive
name|FiCaSchedulerApp
name|app
init|=
name|getApplicationAttempt
