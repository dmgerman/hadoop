...
DECL|method|preCheckForNodeCandidateSet ( Resource clusterResource, FiCaSchedulerNode node, SchedulingMode schedulingMode, ResourceLimits resourceLimits, SchedulerRequestKey schedulerKey)
specifier|private
...
condition|)
block|{
comment|// This is possible when #pending resource decreased by a different
comment|// thread.
name|ActivitiesLogger
operator|.
name|APP
...
return|;
block|}
DECL|method|getLocalityWaitFactor (int uniqAsks, int clusterNodes)
specifier|public
name|float
...
argument_list|)
decl_stmt|;
comment|// available resource for diagnostics collector
name|Resource
name|availableForDC
init|=
name|available
...
argument_list|)
decl_stmt|;
comment|// This could be null when #pending request decreased by another thread.
if|if
condition|(
name|schedulingPS
operator|==
...
block|}
block|}
DECL|method|getResourceDiagnostics (Resource required, Resource available)
specifier|private
name|String
name|getResourceDiagnostics
parameter_list|(
