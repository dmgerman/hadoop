...
return|;
block|}
comment|/**    * Get applicable Node count for AM.    *    * @param rmContext context    * @param conf configuration    * @param amReqs am resource requests    * @return applicable node count    */
DECL|method|getApplicableNodeCountForAM (RMContext rmContext, Configuration conf, List<ResourceRequest> amReqs)
specifier|public
specifier|static
name|int
...
parameter_list|)
block|{
comment|// Determine the list of nodes that are eligible based on the strict
comment|// resource requests
name|Set
argument_list|<
name|NodeId
argument_list|>
...
condition|)
block|{
comment|// Determine the list of nodes that are eligible based on the node label
name|String
...
condition|)
block|{
comment|// If only node labels, strip out any wildcard NodeIds and return
if|if
condition|(
name|nodesForReqs
operator|.
...
block|{
comment|// The NodeIds common to both the strict resource requests and the
comment|// node label is the eligible set
return|return
name|Sets
operator|.
name|intersection
...
block|}
block|}
comment|// If no strict resource request NodeIds nor node label NodeIds, then just
comment|// return the entire cluster
if|if
condition|(
name|nodesForReqs
operator|.
...
return|;
block|}
comment|// No node label NodeIds, so return the strict resource request NodeIds
return|return
name|nodesForReqs
operator|.
name|size
...
return|;
block|}
DECL|method|getNodeIdsForLabel (RMContext rmContext, String label)
specifier|private
specifier|static
name|Set
argument_list|<
...
condition|)
block|{
comment|// NO_LABEL nodes aren't tracked directly
return|return
name|rmContext
operator|.
...
block|}
block|}
DECL|method|convertProfileToResourceCapability ( List<ResourceRequest> asks, Configuration conf, ResourceProfilesManager resourceProfilesManager)
specifier|public
specifier|static
name|void
name|convertProfileToResourceCapability
parameter_list|(
name|List
argument_list|<
name|ResourceRequest
...
block|}
block|}
DECL|method|convertProfileToResourceCapability (ResourceRequest ask, Configuration conf, ResourceProfilesManager resourceProfilesManager)
specifier|public
specifier|static
name|void
name|convertProfileToResourceCapability
...
name|debug
argument_list|(
literal|"Converting profile to resource capability for ask "
operator|+
name|ask
argument_list|)
expr_stmt|;
...
name|debug
argument_list|(
literal|"Converted profile to resource capability for ask "
operator|+
name|ask
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|getOrDefault (Map<String, Long> map, String key, Long defaultValue)
specifier|public
specifier|static
name|Long
name|getOrDefault
