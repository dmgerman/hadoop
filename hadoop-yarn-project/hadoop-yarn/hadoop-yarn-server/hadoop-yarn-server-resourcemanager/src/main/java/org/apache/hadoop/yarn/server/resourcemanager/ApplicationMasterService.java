...
condition|)
block|{
comment|// No recovery supported yet for unmanaged AM. Send the unregister event
comment|// and (falsely) acknowledge state-store write immediately.
name|rmContext
operator|.
name|getDispatcher
argument_list|()
...
block|{
comment|// keep sending the unregister event as RM may crash in the meanwhile.
name|rmContext
operator|.
name|getDispatcher
argument_list|()
