...
name|SchedulerUtils
operator|.
name|MaxResourceValidationResult
import|;
end_import
...
DECL|method|addApplication (ApplicationId applicationId, String queueName, String user, boolean isAppRecovering, ApplicationPlacementContext placementContext)
specifier|protected
name|void
name|addApplication
parameter_list|(
...
parameter_list|)
block|{
comment|// If the  placement was rejected the placementContext will be null.
comment|// We ignore placement rules on recovery.
if|if
condition|(
operator|!
name|isAppRecovering
...
name|user
operator|+
literal|" application rejected by placement rules."
decl_stmt|;
name|rejectApplicationWithMessage
argument_list|(
name|applicationId
argument_list|,
name|message
...
try|try
block|{
comment|// Assign the app to the queue creating and prevent queue delete.
name|FSLeafQueue
name|queue
init|=
name|queueMgr
...
return|return;
block|}
comment|// Enforce ACLs: 2nd check, there could be a time laps between the app
comment|// creation in the RMAppManager and getting here. That means we could
comment|// have a configuration change (prevent race condition)
name|UserGroupInformation
name|userUgi
init|=
name|UserGroupInformation
...
name|error
argument_list|(
literal|"Couldn't find RM app for "
operator|+
name|applicationId
operator|+
literal|" to set queue name on"
argument_list|)
expr_stmt|;
block|}
if|if
...
condition|)
block|{
comment|// Resources.fitsIn would always return false when queueMaxShare is 0
comment|// for any resource, but only using Resources.fitsIn is not enough
comment|// is it would return false for such cases when the requested
comment|// resource is smaller than the max resource but that max resource is
comment|// not zero, e.g. requested vCores = 2, max vCores = 1.
comment|// With this check, we only reject those applications where resource
comment|// requested is greater than 0 and we have 0
comment|// of that resource on the queue.
name|List
argument_list|<
name|MaxResourceValidationResult
argument_list|>
name|invalidAMResourceRequests
init|=
...
name|format
argument_list|(
literal|"Cannot submit application %s to queue %s because "
operator|+
literal|"it has zero amount of resource for a requested "
operator|+
literal|"resource! Invalid requested AM resources: %s, "
operator|+
literal|"maximum queue resources: %s"
argument_list|,
name|applicationId
argument_list|,
name|queueName
...
decl_stmt|;
name|Resource
name|schedulerLevelMaxResourceCapability
init|=
name|getMaximumResourceCapability
argument_list|()
decl_stmt|;
...
block|}
name|Resource
name|queueMaxResourceCapability
init|=
name|queue
operator|.
...
argument_list|)
decl_stmt|;
comment|// We need to be fail-fast here if any invalid ask is detected.
comment|// If we would have thrown exception later, this could be problematic as
comment|// tokens and promoted / demoted containers would have been lost because
comment|// scheduler would clear them right away and AM
comment|// would not get this information.
if|if
condition|(
operator|!
name|invalidAsks
...
name|format
argument_list|(
literal|"Resource request is invalid for application %s because queue %s "
operator|+
literal|"has 0 amount of resource for a resource type! "
operator|+
literal|"Validation result: %s"
argument_list|,
name|applicationId
argument_list|,
name|queue
...
argument_list|)
expr_stmt|;
comment|// TODO, normalize SchedulingRequest
...
argument_list|)
expr_stmt|;
comment|// TODO, handle SchedulingRequest
name|application
operator|.
name|showRequests
argument_list|()
...
return|;
block|}
DECL|method|validateResourceRequests ( List<ResourceRequest> requests, FSLeafQueue queue)
specifier|private
name|List
argument_list|<
name|MaxResourceValidationResult
...
name|trace
argument_list|(
literal|"Validating resource request: "
operator|+
name|resourceRequest
argument_list|)
expr_stmt|;
...
name|SchedulerUtils
operator|.
name|validateResourceRequestsAgainstQueueMaxResource
argument_list|(
name|resourceRequest
argument_list|,
name|queue
...
name|format
argument_list|(
literal|"Queue %s cannot handle resource request"
operator|+
literal|"because it has zero available amount of resource "
operator|+
literal|"for a requested resource type, "
operator|+
literal|"so the resource request is ignored!"
operator|+
literal|" Requested resources: %s, "
operator|+
literal|"maximum queue resources: %s"
argument_list|,
name|queue
...
block|}
block|}
DECL|method|rejectApplicationWithMessage (ApplicationId applicationId, String msg)
specifier|private
name|void
name|rejectApplicationWithMessage
parameter_list|(
...
name|String
argument_list|>
name|removedStaticQueues
init|=
name|getRemovedStaticQueues
argument_list|(
name|queueInfo
argument_list|)
decl_stmt|;
...
block|}
block|}
DECL|method|getRemovedStaticQueues ( AllocationConfiguration queueInfo)
specifier|private
name|Set
argument_list|<
name|String
...
name|YarnException
argument_list|(
literal|"Update application priority is not supported in Fair Scheduler"
argument_list|)
throw|;
block|}
block|}
