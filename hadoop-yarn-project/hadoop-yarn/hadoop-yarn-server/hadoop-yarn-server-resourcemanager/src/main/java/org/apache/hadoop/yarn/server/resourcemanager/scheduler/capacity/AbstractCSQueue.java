...
decl_stmt|;
DECL|field|multiNodeSortingPolicyName
specifier|private
name|String
name|multiNodeSortingPolicyName
init|=
literal|null
decl_stmt|;
DECL|field|acls
...
name|queueCapacities
decl_stmt|;
DECL|field|queueResourceQuotas
name|QueueResourceQuotas
name|queueResourceQuotas
decl_stmt|;
DECL|enum|CapacityConfigType
specifier|protected
enum|enum
name|CapacityConfigType
block|{
DECL|enumConstant|NONE
DECL|enumConstant|PERCENTAGE
DECL|enumConstant|ABSOLUTE_RESOURCE
name|NONE
block|,
name|PERCENTAGE
block|,
...
expr_stmt|;
block|}
DECL|method|AbstractCSQueue (CapacitySchedulerContext cs, CapacitySchedulerConfiguration configuration, String queueName, CSQueue parent, CSQueue old)
specifier|public
name|AbstractCSQueue
parameter_list|(
name|CapacitySchedulerContext
...
argument_list|)
expr_stmt|;
comment|// initialize queueResourceQuotas
name|queueResourceQuotas
operator|=
operator|new
name|QueueResourceQuotas
...
block|}
block|}
comment|/**    * Set maximum capacity    * @param maximumCapacity new max capacity    */
DECL|method|setMaxCapacity (String nodeLabel, float maximumCapacity)
name|void
name|setMaxCapacity
parameter_list|(
name|String
...
expr_stmt|;
block|}
DECL|method|setupQueueConfigs (Resource clusterResource, CapacitySchedulerConfiguration configuration)
specifier|protected
name|void
name|setupQueueConfigs
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// Also fetch minimum/maximum resource constraint for this queue if
comment|// configured.
name|capacityConfigType
operator|=
name|CapacityConfigType
...
name|NONE
expr_stmt|;
name|updateConfigurableResourceRequirement
argument_list|(
name|getQueuePath
argument_list|()
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Setup queue's maximumAllocation respecting the global setting
comment|// and queue setting
name|setupMaximumAllocation
argument_list|(
name|configuration
argument_list|)
expr_stmt|;
comment|// initialized the queue state based on previous state, configured state
comment|// and its parent state.
name|QueueState
name|previous
init|=
name|getState
...
argument_list|)
expr_stmt|;
comment|// Update multi-node sorting algorithm for scheduling as configured.
name|setMultiNodeSortingPolicyName
argument_list|(
name|configuration
operator|.
...
name|userWeights
operator|=
name|getUserWeightsFromHierarchy
argument_list|(
name|configuration
argument_list|)
expr_stmt|;
...
block|}
block|}
DECL|method|setupMaximumAllocation (CapacitySchedulerConfiguration csConf)
specifier|private
name|void
name|setupMaximumAllocation
parameter_list|(
...
name|queue
operator|+
literal|" max allocation per queue: %s"
operator|+
...
condition|)
block|{
comment|// Handle backward compatibility
name|long
name|queueMemory
init|=
name|csConf
...
block|{
comment|// Queue level maximum-allocation can't be larger than cluster setting
for|for
control|(
name|ResourceInformation
name|ri
...
block|}
block|}
DECL|method|getUserWeightsFromHierarchy (CapacitySchedulerConfiguration configuration)
specifier|private
name|Map
argument_list|<
name|String
...
condition|)
block|{
comment|// Inherit all of parent's user's weights
name|unionInheritedWeights
operator|.
name|putAll
argument_list|(
...
expr_stmt|;
block|}
comment|// Insert this queue's user's weights, overriding parent's user's weights if
comment|// there is overlap.
name|unionInheritedWeights
operator|.
name|putAll
argument_list|(
...
return|;
block|}
DECL|method|updateConfigurableResourceRequirement (String queuePath, Resource clusterResource)
specifier|protected
name|void
name|updateConfigurableResourceRequirement
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// If min resource for a resource type is greater than its max resource,
comment|// throw exception to handle such error configs.
if|if
condition|(
operator|!
name|maxResource
...
name|IllegalArgumentException
argument_list|(
literal|"Min resource configuration "
operator|+
name|minResource
operator|+
literal|" is greater than its max value:"
operator|+
name|maxResource
operator|+
literal|" in queue:"
...
throw|;
block|}
comment|// If parent's max resource is lesser to a specific child's max
comment|// resource, throw exception to handle such error configs.
if|if
condition|(
name|parent
operator|!=
...
name|IllegalArgumentException
argument_list|(
literal|"Max resource configuration "
operator|+
name|maxResource
operator|+
literal|" is greater than parents max value:"
operator|+
name|parentMaxRes
operator|+
literal|" in queue:"
...
throw|;
block|}
comment|// If child's max resource is not set, but its parent max resource is
comment|// set, we must set child max resource to its parent's.
if|if
condition|(
name|maxResource
operator|.
...
block|}
block|}
DECL|method|validateAbsoluteVsPercentageCapacityConfig ( Resource minResource)
specifier|private
name|void
name|validateAbsoluteVsPercentageCapacityConfig
parameter_list|(
...
operator|+
literal|" configuration or absolute resource."
argument_list|)
throw|;
block|}
block|}
...
return|;
block|}
DECL|method|initializeQueueState (QueueState previousState, QueueState configuredState, QueueState parentState)
specifier|private
name|void
name|initializeQueueState
parameter_list|(
...
parameter_list|)
block|{
comment|// verify that we can not any value for State other than RUNNING/STOPPED
if|if
condition|(
name|configuredState
operator|!=
...
name|IllegalArgumentException
argument_list|(
literal|"Invalid queue state configuration."
operator|+
literal|" We can only use RUNNING or STOPPED."
argument_list|)
throw|;
block|}
comment|// If we did not set state in configuration, use Running as default state
name|QueueState
name|defaultState
init|=
name|QueueState
...
condition|)
block|{
comment|// If current state of the queue is null, we would inherit the state
comment|// from its parent. If this queue does not has parent, such as root queue,
comment|// we would use the configured state.
if|if
condition|(
name|parentState
operator|==
...
block|{
comment|// when we get a refreshQueue request from AdminService,
if|if
condition|(
name|previousState
operator|==
...
return|;
block|}
DECL|method|getQueueMaxResource (String nodePartition)
name|Resource
name|getQueueMaxResource
parameter_list|(
name|String
...
annotation|@
name|Override
DECL|method|updateQueueState (QueueState queueState)
specifier|public
name|void
name|updateQueueState
parameter_list|(
...
name|queueName
operator|+
literal|" is already in the RUNNING state."
argument_list|)
expr_stmt|;
block|}
elseif|else
...
name|queueName
operator|+
literal|" is in the Stopping process. "
operator|+
literal|"Please wait for the queue getting fully STOPPED."
argument_list|)
throw|;
block|}
else|else
...
argument_list|()
operator|+
literal|" is not running. Please activate the parent queue first"
argument_list|)
throw|;
block|}
block|}
...
return|;
block|}
DECL|method|recoverDrainingState ()
specifier|public
name|void
...
name|info
argument_list|(
literal|"Recover draining state for queue "
operator|+
name|this
operator|.
name|getQueuePath
...
return|;
block|}
DECL|method|setMultiNodeSortingPolicyName (String policyName)
specifier|public
name|void
name|setMultiNodeSortingPolicyName
parameter_list|(
