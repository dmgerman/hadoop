...
annotation|@
name|Override
DECL|method|getMultilevelConstraint (ApplicationId appId, Set<String> sourceTags, PlacementConstraint schedulingRequestConstraint)
specifier|public
name|PlacementConstraint
...
argument_list|()
decl_stmt|;
comment|// Add scheduling request-level constraint.
if|if
condition|(
name|schedulingRequestConstraint
operator|!=
...
expr_stmt|;
block|}
comment|// Add app-level constraint if appId is given.
if|if
condition|(
name|appId
operator|!=
...
expr_stmt|;
block|}
comment|// Remove all null or duplicate constraints.
name|List
argument_list|<
name|PlacementConstraint
operator|.
...
argument_list|)
decl_stmt|;
comment|// Compose an AND constraint
comment|// When merge request(RC), app(AC) and global constraint(GC),
comment|// we do a merge on them with CC=AND(GC, AC, RC) and returns a
comment|// composite AND constraint. Subsequently we check if CC could
comment|// be satisfied. This ensures that every level of constraint
