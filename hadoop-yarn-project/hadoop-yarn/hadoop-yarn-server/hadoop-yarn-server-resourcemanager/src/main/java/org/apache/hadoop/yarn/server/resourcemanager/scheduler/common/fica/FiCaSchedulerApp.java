...
block|}
block|}
DECL|method|allocate (FiCaSchedulerNode node, SchedulerRequestKey schedulerKey, ResourceRequest request, Container container)
specifier|public
name|RMContainer
name|allocate
parameter_list|(
...
return|;
block|}
DECL|method|getResourceChangeRequest ( SchedulerContainer<FiCaSchedulerApp, FiCaSchedulerNode> schedulerContainer)
specifier|private
name|SchedContainerChangeRequest
name|getResourceChangeRequest
parameter_list|(
name|SchedulerContainer
argument_list|<
name|FiCaSchedulerApp
...
return|;
block|}
DECL|method|checkIncreaseContainerAllocation ( ContainerAllocationProposal<FiCaSchedulerApp, FiCaSchedulerNode> allocation, SchedulerContainer<FiCaSchedulerApp, FiCaSchedulerNode> schedulerContainer)
specifier|private
name|boolean
name|checkIncreaseContainerAllocation
parameter_list|(
name|ContainerAllocationProposal
argument_list|<
name|FiCaSchedulerApp
...
parameter_list|)
block|{
comment|// When increase a container
if|if
condition|(
name|schedulerContainer
operator|.
...
name|debug
argument_list|(
literal|"Trying to increase a container, but container="
operator|+
name|schedulerContainer
operator|.
name|getRmContainer
...
argument_list|()
operator|+
literal|" is not in running state."
argument_list|)
expr_stmt|;
block|}
return|return
...
return|;
block|}
comment|// Check if increase request is still valid
name|SchedContainerChangeRequest
...
name|debug
argument_list|(
literal|"Increase request has been changed, reject this proposal"
argument_list|)
expr_stmt|;
block|}
return|return
...
condition|)
block|{
comment|// In addition, if allocation is from a reserved container, check
comment|// if the reserved container has enough reserved space
if|if
condition|(
operator|!
name|Resources
...
expr_stmt|;
block|}
comment|// Update this application for the allocated container
if|if
condition|(
operator|!
name|allocation
...
argument_list|)
decl_stmt|;
comment|// allocate resource for an increase request
comment|// Notify node
name|schedulerContainer
operator|.
name|getSchedulerNode
...
block|{
name|float
name|queueAbsMaxCapPerPartition
init|=
operator|(
operator|(
name|AbstractCSQueue
...
expr_stmt|;
block|}
DECL|method|removedToBeRemovedIncreaseRequests ()
specifier|public
name|void
name|removedToBeRemovedIncreaseRequests
parameter_list|()
block|{
