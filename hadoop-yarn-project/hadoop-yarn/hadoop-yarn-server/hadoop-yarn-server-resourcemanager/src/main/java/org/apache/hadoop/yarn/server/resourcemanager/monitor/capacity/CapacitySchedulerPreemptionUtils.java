...
block|}
block|}
comment|/**    * Invoke this method to preempt container based on resToObtain.    *    * @param rc    *          resource calculator    * @param context    *          preemption context    * @param resourceToObtainByPartitions    *          map to hold resource to obtain per partition    * @param rmContainer    *          container    * @param clusterResource    *          total resource    * @param preemptMap    *          map to hold preempted containers    * @param totalPreemptionAllowed    *          total preemption allowed per round    * @param conservativeDRF    *          should we do conservativeDRF preemption or not.    *          When true:    *            stop preempt container when any major resource type    *            {@literal<=} 0 for to-preempt.    *            This is default preemption behavior of intra-queue preemption    *          When false:    *            stop preempt container when: all major resource type    *            {@literal<=} 0 for to-preempt.    *            This is default preemption behavior of inter-queue preemption    * @return should we preempt rmContainer. If we should, deduct from    *<code>resourceToObtainByPartition</code>    */
DECL|method|tryPreemptContainerAndDeductResToObtain ( ResourceCalculator rc, CapacitySchedulerPreemptionContext context, Map<String, Resource> resourceToObtainByPartitions, RMContainer rmContainer, Resource clusterResource, Map<ApplicationAttemptId, Set<RMContainer>> preemptMap, Map<ApplicationAttemptId, Set<RMContainer>> curCandidates, Resource totalPreemptionAllowed, boolean conservativeDRF)
specifier|public
specifier|static
name|boolean
...
return|;
block|}
comment|// If a toObtain resource type == 0, set it to -1 to avoid 0 resource
comment|// type affect following doPreemption check: isAnyMajorResourceZero
for|for
control|(
name|ResourceInformation
name|ri
...
name|doPreempt
decl_stmt|;
comment|// How much resource left after preemption happen.
name|Resource
name|toObtainAfterPreemption
init|=
name|Resources
operator|.
name|subtract
...
block|{
comment|// When we want to do more aggressive preemption, we will do preemption
comment|// only if:
comment|// - The preempt of the container makes positive contribution to the
comment|//   to-obtain resource. Positive contribution means any positive
comment|//   resource type decreases.
comment|//
comment|//   This is example of positive contribution:
comment|//     * before:<30, 10, 5>, after<20, 10, -10>
comment|//   But this not positive contribution:
comment|//     * before:<30, 10, 0>, after<30, 10, -15>
name|doPreempt
operator|=
name|Resources
operator|.
...
return|;
block|}
DECL|method|addToPreemptMap ( Map<ApplicationAttemptId, Set<RMContainer>> preemptMap, Map<ApplicationAttemptId, Set<RMContainer>> curCandidates, ApplicationAttemptId appAttemptId, RMContainer containerToPreempt)
specifier|protected
specifier|static
name|void
name|addToPreemptMap
