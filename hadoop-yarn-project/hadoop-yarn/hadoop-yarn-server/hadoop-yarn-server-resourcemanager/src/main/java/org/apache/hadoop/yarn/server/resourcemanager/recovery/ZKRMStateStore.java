...
block|}
empty_stmt|;
comment|/**    * Encapsulates full app node path and corresponding split index.    */
DECL|class|AppNodeSplitInfo
specifier|private
specifier|final
specifier|static
class|class
name|AppNodeSplitInfo
block|{
DECL|field|path
specifier|private
specifier|final
...
name|splitIndex
decl_stmt|;
DECL|method|AppNodeSplitInfo (String path, int splitIndex)
name|AppNodeSplitInfo
parameter_list|(
name|String
name|path
...
expr_stmt|;
block|}
comment|/**    * Get parent app node path based on full path and split index supplied.    * @param appIdPath App id path for which parent needs to be returned.    * @param splitIndex split index.    * @return parent app node path.    */
DECL|method|getSplitAppNodeParent (String appIdPath, int splitIndex)
specifier|private
name|String
name|getSplitAppNodeParent
parameter_list|(
name|String
name|appIdPath
parameter_list|,
...
parameter_list|)
block|{
comment|// Calculated as string upto index (appIdPath Length - split index - 1). We
...
return|;
block|}
comment|/**    * Checks if parent app node has no leaf nodes and if it does not have,    * removes it. Called while removing application.    * @param appIdPath path of app id to be removed.    * @param splitIndex split index.    * @throws Exception if any problem occurs while performing ZK operation.    */
DECL|method|checkRemoveParentAppNode (String appIdPath, int splitIndex)
specifier|private
name|void
name|checkRemoveParentAppNode
parameter_list|(
name|String
name|appIdPath
parameter_list|,
...
parameter_list|)
block|{
comment|// It should be fine to swallow this exception as the parent app node we
...
name|debug
argument_list|(
literal|"No leaf app node exists. Removing parent node "
operator|+
name|parentAppNode
argument_list|)
expr_stmt|;
...
parameter_list|)
block|{
comment|// It should be fine to swallow this exception as the parent app node
...
DECL|method|getAlternatePath (String appId)
specifier|private
name|AppNodeSplitInfo
name|getAlternatePath
parameter_list|(
...
DECL|method|getLeafAppIdNodePath (String appId, String rootNode, int appIdNodeSplitIdx, boolean createParentIfNotExists)
specifier|private
name|String
name|getLeafAppIdNodePath
parameter_list|(
