...
literal|null
decl_stmt|;
DECL|field|multiNodeSortPolicyName
specifier|private
name|String
name|multiNodeSortPolicyName
decl_stmt|;
DECL|field|readLock
specifier|private
specifier|final
...
name|debug
argument_list|(
literal|"nodeLookupPolicy used for "
operator|+
name|appSchedulingInfo
operator|.
...
return|;
block|}
comment|// singleNode will be null if Multi-node placement lookup is enabled, and
comment|// hence could consider sorting policies.
return|return
name|multiNodeSortingManager
operator|.
...
argument_list|()
operator|+
literal|" not be able to handle SchedulingRequest, there exists a "
operator|+
literal|"ResourceRequest with the same scheduler key="
operator|+
name|schedulerRequestKey
operator|+
literal|", please send SchedulingRequest with a different allocationId and "
operator|+
literal|"priority"
argument_list|)
throw|;
