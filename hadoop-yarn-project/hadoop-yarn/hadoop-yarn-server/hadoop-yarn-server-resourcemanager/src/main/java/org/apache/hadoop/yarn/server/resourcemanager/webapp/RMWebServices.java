...
return|;
block|}
comment|/**    * If no params are given, returns all active nodes, which includes    * nodes in the NEW and RUNNING states. If state param is "all", returns all    * nodes in all states. Otherwise, if the state param is set to a state name,    * returns all nodes that are in that state.    */
annotation|@
name|GET
annotation|@
name|Path
...
block|}
argument_list|)
DECL|method|getNodes (@ueryParamR) String state)
specifier|public
name|NodesInfo
name|getNodes
parameter_list|(
...
block|}
block|}
comment|// getRMNodes() contains nodes that are NEW, RUNNING OR UNHEALTHY
name|NodesInfo
name|allNodes
init|=
operator|new
...
block|}
block|}
comment|// getInactiveNodes() contains nodes that are DECOMMISSIONED, LOST, OR REBOOTED
if|if
condition|(
name|all
operator|||
