...
parameter_list|)
block|{
comment|// Previously we keep a sorted list for default partition, it is not good
comment|// when multi-threading scheduler is enabled, so to make a simpler code
comment|// now re-sort queue every time irrespective to node partition.
name|partitionQueueComparator
operator|.
...
block|}
block|}
DECL|method|internalReleaseResource (Resource clusterResource, FiCaSchedulerNode node, Resource releasedResource, boolean changeResource)
specifier|private
name|void
