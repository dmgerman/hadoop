...
argument_list|()
expr_stmt|;
comment|// This should fail: make sure that we do test correctly
comment|// TimSort which is used does not handle the concurrent modification of
comment|// objects it is sorting.
try|try
block|{
name|Collections
operator|.
...
name|fail
argument_list|(
literal|"Sorting should have failed and did not"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
argument_list|()
argument_list|,
literal|"Comparison method violates its general contract!"
argument_list|)
expr_stmt|;
block|}
try|try
...
expr_stmt|;
block|}
comment|// clean up and try again using TreeSet which should work
name|schedulableList
operator|.
name|clear
argument_list|()
