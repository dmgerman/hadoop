...
expr_stmt|;
block|}
DECL|method|assertMasterKeysAreSaved ( Set<DelegationKey> rmDTMasterKeyState, RMDelegationTokenSecretManager dtSecretManager)
specifier|private
specifier|static
name|void
name|assertMasterKeysAreSaved
parameter_list|(
name|Set
argument_list|<
name|DelegationKey
...
expr_stmt|;
block|}
comment|// Test removing token without key from state-store.
annotation|@
name|Test
argument_list|(
name|timeout
...
literal|15000
argument_list|)
DECL|method|testUnknownKeyTokensOnRecover ()
specifier|public
name|void
name|testUnknownKeyTokensOnRecover
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// Need RM to get the secret manager and call recover
name|MockRM
name|rm1
init|=
operator|new
...
argument_list|()
decl_stmt|;
comment|// short cut to generate a basic token with unknown key
name|RMDelegationTokenIdentifier
name|rmDT
init|=
operator|new
...
argument_list|)
decl_stmt|;
comment|// set a master key which is not used
name|rmDT
operator|.
name|setMasterKeyId
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// give it time to process
name|GenericTestUtils
operator|.
name|waitFor
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Cannot recover while running: stop and clear
name|dtSecretManager
operator|.
name|stopThreads
argument_list|()
...
name|assertEquals
argument_list|(
literal|"Secret manager should have no tokens"
argument_list|,
name|dtSecretManager
operator|.
name|getAllTokens
...
name|assertEquals
argument_list|(
literal|"Secret manager should have no keys"
argument_list|,
name|dtSecretManager
operator|.
name|getAllMasterKeys
...
name|assertEquals
argument_list|(
literal|"Token should have been expired but is not"
argument_list|,
literal|0L
argument_list|,
name|dtSecretManager
...
argument_list|)
expr_stmt|;
comment|// The remover thread should immediately do its work,
comment|// still give it some time to process
name|dtSecretManager
operator|.
name|startThreads
argument_list|()
