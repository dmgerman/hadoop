...
annotation|@
name|Test
DECL|method|testActiveUsersWithOnlyPendingApps ()
specifier|public
name|void
name|testActiveUsersWithOnlyPendingApps
parameter_list|()
throws|throws
name|Exception
block|{
...
literal|60000
argument_list|)
DECL|method|testUnreserveWhenClusterResourceHasEmptyResourceType ()
specifier|public
name|void
name|testUnreserveWhenClusterResourceHasEmptyResourceType
parameter_list|()
throws|throws
name|Exception
block|{
comment|/**      * Test case:      * Create a cluster with two nodes whose node resource both are      *<8GB, 8core, 0>, create queue "a" whose max-resource is<8GB, 8 core, 0>,      * submit app1 to queue "a" whose am use<1GB, 1 core, 0> and launch on nm1,      * submit app2 to queue "b" whose am use<1GB, 1 core, 0> and launch on nm1,      * app1 asks two<7GB, 1core> containers and nm1 do 1 heartbeat,      * then scheduler reserves one container on nm1.      *      * After nm2 do next node heartbeat, scheduler should unreserve the reserved      * container on nm1 then allocate a container on nm2.      */
name|TestResourceUtils
operator|.
...
argument_list|)
expr_stmt|;
comment|// Set maximum capacity of queue "a" to 50
name|newConf
operator|.
name|setMaximumCapacity
argument_list|(
...
argument_list|)
decl_stmt|;
comment|// launch an app to queue "a", AM container should be launched on nm1
name|RMApp
name|app1
init|=
name|rm1
...
argument_list|)
decl_stmt|;
comment|// launch another app to queue "b", AM container should be launched on nm1
name|RMApp
name|app2
init|=
name|rm1
...
argument_list|)
decl_stmt|;
comment|// Do nm1 heartbeats 1 times, will reserve a container on nm1 for app1
name|cs
operator|.
name|handle
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Do nm2 heartbeats 1 times, will unreserve a container on nm1
comment|// and allocate a container on nm2 for app1
name|cs
operator|.
name|handle
argument_list|(
...
literal|60000
argument_list|)
DECL|method|testAllocationCannotBeBlockedWhenFormerQueueReachedItsLimit ()
specifier|public
name|void
name|testAllocationCannotBeBlockedWhenFormerQueueReachedItsLimit
parameter_list|()
throws|throws
name|Exception
block|{
comment|/**      * Queue structure:      *<pre>      *             Root      *            /  |  \      *           a   b   c      *          10   20  70      *                   |  \      *                  c1   c2      *           10(max=10)  90      *</pre>      * Test case:      * Create a cluster with two nodes whose node resource both are      *<10GB, 10core>, create queues as above, among them max-capacity of "c1"      * is 10 and others are all 100, so that max-capacity of queue "c1" is      *<2GB, 2core>,      * submit app1 to queue "c1" and launch am1(resource=<1GB, 1 core>) on nm1,      * submit app2 to queue "b" and launch am2(resource=<1GB, 1 core>) on nm1,      * app1 and app2 both ask one<2GB, 1core> containers      *      * Now queue "c" has lower capacity percentage than queue "b", the      * allocation sequence will be "a" -> "c" -> "b", queue "c1" has reached      * queue limit so that requests of app1 should be pending      *      * After nm1 do 1 heartbeat, scheduler should allocate one container for      * app2 on nm1.      */
name|CapacitySchedulerConfiguration
name|newConf
init|=
operator|(
...
argument_list|)
decl_stmt|;
comment|// launch an app to queue "c1", AM container should be launched on nm1
name|RMApp
name|app1
init|=
name|rm1
...
argument_list|)
decl_stmt|;
comment|// Do nm1 heartbeats 1 times, will allocate a container on nm1 for app2
name|cs
operator|.
name|handle
argument_list|(
...
literal|60000
argument_list|)
DECL|method|testContainerRejectionWhenAskBeyondDynamicMax ()
specifier|public
name|void
name|testContainerRejectionWhenAskBeyondDynamicMax
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|()
expr_stmt|;
comment|// before any node registered or before registration timeout,
comment|// submit an app beyond queue max leads to failure.
name|boolean
name|submitFailed
init|=
literal|false
...
argument_list|)
expr_stmt|;
comment|// Add a new node, now the cluster maximum should be refreshed to 3GB.
name|CapacityScheduler
name|cs
init|=
operator|(
...
argument_list|)
expr_stmt|;
comment|// Now ask 4 GB will fail
name|submitFailed
operator|=
literal|false
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// But ask 3 GB succeeded.
name|am1
operator|.
name|allocate
argument_list|(
