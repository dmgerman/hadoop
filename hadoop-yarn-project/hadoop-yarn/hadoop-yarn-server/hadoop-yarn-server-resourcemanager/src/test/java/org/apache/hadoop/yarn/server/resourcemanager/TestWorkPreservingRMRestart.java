...
argument_list|)
expr_stmt|;
comment|// Until YARN-1959 is resolved
if|if
condition|(
name|scheduler
operator|.
...
parameter_list|)
block|{
comment|//we're catching it because we want to verify the message
comment|//and we don't want to set it as an expected exception for the
comment|//test because we only want it to happen here
name|assertTrue
argument_list|(
name|e
operator|.
