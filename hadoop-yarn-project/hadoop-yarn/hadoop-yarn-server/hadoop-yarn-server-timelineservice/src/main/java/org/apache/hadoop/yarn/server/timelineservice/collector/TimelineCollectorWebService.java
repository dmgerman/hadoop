...
name|error
argument_list|(
literal|"Unexpected error while putting entities"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * @param req    Servlet request.    * @param res    Servlet response.    * @param domain timeline domain to be put.    * @param appId Application Id to which the domain to be put belong to. If    *     appId is not there or it cannot be parsed, HTTP 400 will be sent back.    * @return a Response with appropriate HTTP status.    */
annotation|@
name|PUT
annotation|@
name|Path
...
block|}
argument_list|)
DECL|method|putDomain ( @ontext HttpServletRequest req, @Context HttpServletResponse res, @QueryParam(R) String appId, TimelineDomain domain)
specifier|public
name|Response
name|putDomain
parameter_list|(
