...
return|;
block|}
comment|/**    * Return a single flow run for given UID which is a delimited string    * containing clusterid, userid, flow name and flowrun id.    *    * @param req Servlet request.    * @param res Servlet response.    * @param uId a delimited string containing clusterid, userid, flow name and    *     flowrun id which are extracted from UID and then used to query backend    *     (Mandatory path param).    *    * @return If successful, a HTTP 200(OK) response having a JSON representing a    *     {@link FlowRunEntity} instance is returned. By default, all metrics for    *     the flow run will be returned.    *     On failures,    *     If any problem occurs in parsing request or UID is incorrect,    *     HTTP 400(Bad Request) is returned.    *     If flow run for the given flow run id cannot be found, HTTP 404    *     (Not Found) is returned.    *     For all other errors while retrieving data, HTTP 500(Internal Server    *     Error) is returned.    */
annotation|@
name|GET
annotation|@
name|Path
