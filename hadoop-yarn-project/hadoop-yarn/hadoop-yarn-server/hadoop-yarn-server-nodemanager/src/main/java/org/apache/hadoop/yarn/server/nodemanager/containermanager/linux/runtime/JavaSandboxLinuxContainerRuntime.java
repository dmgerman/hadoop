...
block|}
block|}
comment|/**    *  Prior to environment from being written locally need to generate    *  policy file which limits container access to a small set of directories.    *  Additionally the container run command needs to be modified to include    *  flags to enable the java security manager with the generated policy.    *<br>    *  The Java Sandbox will be circumvented if the user is a member of the    *  group specified in:    *  {@value    *  org.apache.hadoop.yarn.conf.YarnConfiguration#YARN_CONTAINER_SANDBOX_WHITELIST_GROUP}    *  and if they do not include the JVM flag    *<code>-Djava.security.manager</code>.    *    * @param ctx The {@link ContainerRuntimeContext} containing container    *            setup properties.    * @throws ContainerExecutionException Exception thrown if temporary policy    * file directory can't be created, or if any exceptions occur during policy    * file parsing and generation.    */
annotation|@
name|Override
...
name|String
argument_list|>
name|groupPolicyFiles
init|=
name|getGroupPolicyFiles
argument_list|(
name|configuration
argument_list|,
name|ctx
...
block|}
block|}
comment|/**    * Determine if JVMSandboxLinuxContainerRuntime should be used.  This is    * decided based on the value of    * {@value    * org.apache.hadoop.yarn.conf.YarnConfiguration#YARN_CONTAINER_SANDBOX}    * @param env the environment variable settings for the operation    * @return true if Sandbox is requested, false otherwise    */
annotation|@
name|Override
...
return|;
block|}
DECL|method|getGroupPolicyFiles (Configuration conf, String user)
specifier|private
specifier|static
name|List
argument_list|<
...
return|;
block|}
comment|/**    * Determine if the container should be whitelisted (i.e. exempt from the    * Java Security Manager).    * @param username The name of the user running the container    * @param commands The list of run commands for the container    * @return boolean value denoting whether the container should be whitelisted.    * @throws ContainerExecutionException If container user can not be resolved    */
DECL|method|isSandboxContainerWhitelisted (String username, List<String> commands)
specifier|private
name|boolean
name|isSandboxContainerWhitelisted
parameter_list|(
...
DECL|method|generatePolicyFile (OutputStream policyOutStream, List<String> localDirs, List<String> groupPolicyPaths, Map<org.apache.hadoop.fs.Path, List<String>> resources, Configuration conf)
specifier|static
name|void
name|generatePolicyFile
parameter_list|(
