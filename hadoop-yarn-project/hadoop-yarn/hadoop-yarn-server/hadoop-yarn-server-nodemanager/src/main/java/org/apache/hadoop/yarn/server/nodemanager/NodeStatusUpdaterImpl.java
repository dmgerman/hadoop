...
decl_stmt|;
DECL|field|nodeAttributesHandler
specifier|private
name|NMNodeAttributesHandler
name|nodeAttributesHandler
decl_stmt|;
...
argument_list|()
decl_stmt|;
comment|// Update configured resources via plugins.
name|updateConfiguredResourcesViaPlugins
argument_list|(
name|totalResource
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Nodemanager resources is set to: "
operator|+
name|totalResource
argument_list|)
expr_stmt|;
...
name|nodeAttributesHandler
operator|=
name|createNMNodeAttributesHandler
argument_list|(
name|nodeAttributesProvider
argument_list|)
expr_stmt|;
...
return|;
block|}
DECL|method|updateConfiguredResourcesViaPlugins ( Resource configuredResource)
specifier|private
name|void
name|updateConfiguredResourcesViaPlugins
parameter_list|(
...
condition|)
block|{
comment|// Update configured resource
for|for
control|(
name|ResourcePlugin
name|resourcePlugin
...
name|nodeAttributesHandler
operator|.
name|getNodeAttributesForRegistration
argument_list|()
decl_stmt|;
...
name|nodeAttributesHandler
operator|.
name|verifyRMRegistrationResponseForNodeAttributes
argument_list|(
name|regNMResponse
argument_list|)
argument_list|)
...
block|}
block|}
comment|/**    * Returns a handler based on the configured node attributes provider.    * returns null if no provider is configured.    * @param provider    * @return attributes handler    */
DECL|method|createNMNodeAttributesHandler ( NodeAttributesProvider provider)
specifier|private
name|NMNodeAttributesHandler
name|createNMNodeAttributesHandler
parameter_list|(
...
return|return
operator|new
name|NMCentralizedNodeAttributesHandler
argument_list|()
return|;
block|}
else|else
...
return|return
operator|new
name|NMDistributedNodeAttributesHandler
argument_list|(
name|provider
argument_list|,
name|this
...
block|}
block|}
DECL|class|CachedNodeDescriptorHandler
specifier|private
specifier|static
specifier|abstract
class|class
name|CachedNodeDescriptorHandler
parameter_list|<
name|T
parameter_list|>
block|{
...
literal|0L
decl_stmt|;
DECL|field|isValueSented
specifier|private
name|boolean
name|isValueSented
decl_stmt|;
DECL|method|CachedNodeDescriptorHandler (T defaultValue, long resyncInterval)
name|CachedNodeDescriptorHandler
parameter_list|(
name|T
name|defaultValue
...
expr_stmt|;
block|}
DECL|method|getValueFromProvider ()
specifier|public
specifier|abstract
name|T
name|getValueFromProvider
parameter_list|()
function_decl|;
DECL|method|getValueForRegistration ()
specifier|public
name|T
name|getValueForRegistration
parameter_list|()
block|{
name|T
name|value
...
return|;
block|}
DECL|method|getValueForHeartbeat ()
specifier|public
name|T
name|getValueForHeartbeat
parameter_list|()
block|{
name|T
name|value
...
argument_list|()
decl_stmt|;
comment|// if the provider returns null then consider default value are set
if|if
condition|(
name|defaultValue
operator|!=
...
expr_stmt|;
block|}
comment|// take some action only on modification of value
name|boolean
name|isValueUpdated
init|=
name|isValueUpdated
...
literal|false
expr_stmt|;
comment|// When value updated or resync time is elapsed will send again in
comment|// heartbeat.
if|if
condition|(
name|isValueUpdated
...
parameter_list|)
block|{
comment|// take previous value to replace invalid value, so that invalid
comment|// value are not verified for every HB, and send empty set
comment|// to RM to have same value which was earlier set.
name|value
operator|=
literal|null
expr_stmt|;
...
block|{
comment|// if value have not changed then no need to send
name|value
operator|=
literal|null
expr_stmt|;
...
name|IOException
function_decl|;
DECL|method|isValueUpdated (T value)
specifier|protected
specifier|abstract
name|boolean
name|isValueUpdated
...
return|;
block|}
DECL|method|getLastSendMills ()
specifier|public
name|long
name|getLastSendMills
parameter_list|()
block|{
return|return
name|lastSendMills
...
block|}
block|}
DECL|interface|NMNodeAttributesHandler
specifier|private
interface|interface
name|NMNodeAttributesHandler
block|{
comment|/**      * validates nodeAttributes From Provider and returns it to the caller. Also      * ensures that if provider returns null then empty set is considered      */
DECL|method|getNodeAttributesForRegistration ()
name|Set
argument_list|<
name|NodeAttribute
argument_list|>
...
parameter_list|()
function_decl|;
comment|/**      * @return the node attributes of this node manager.      */
DECL|method|getNodeAttributesForHeartbeat ()
name|Set
argument_list|<
name|NodeAttribute
argument_list|>
name|getNodeAttributesForHeartbeat
parameter_list|()
function_decl|;
comment|/**      * @return RMRegistration Success message and on failure will log      *         independently and returns empty string      */
DECL|method|verifyRMRegistrationResponseForNodeAttributes ( RegisterNodeManagerResponse regNMResponse)
name|String
name|verifyRMRegistrationResponseForNodeAttributes
parameter_list|(
name|RegisterNodeManagerResponse
...
parameter_list|)
function_decl|;
comment|/**      * check whether if updated attributes sent to RM was accepted or not.      * @param response      */
DECL|method|verifyRMHeartbeatResponseForNodeAttributes ( NodeHeartbeatResponse response)
name|void
name|verifyRMHeartbeatResponseForNodeAttributes
parameter_list|(
name|NodeHeartbeatResponse
name|response
parameter_list|)
function_decl|;
block|}
comment|/**    * In centralized configuration, NM need not send Node attributes or process    * the response.    */
DECL|class|NMCentralizedNodeAttributesHandler
specifier|private
specifier|static
class|class
name|NMCentralizedNodeAttributesHandler
...
block|}
block|}
DECL|class|NMDistributedNodeAttributesHandler
specifier|private
specifier|static
class|class
name|NMDistributedNodeAttributesHandler
...
name|NMNodeAttributesHandler
block|{
DECL|field|attributesProvider
specifier|private
specifier|final
name|NodeAttributesProvider
name|attributesProvider
decl_stmt|;
DECL|method|NMDistributedNodeAttributesHandler ( NodeAttributesProvider provider, Configuration conf)
specifier|protected
name|NMDistributedNodeAttributesHandler
parameter_list|(
name|NodeAttributesProvider
...
annotation|@
name|Override
DECL|method|validate (Set<NodeAttribute> nodeAttributes)
specifier|protected
name|void
name|validate
parameter_list|(
...
name|error
argument_list|(
literal|"Invalid node attribute(s) from Provider : "
operator|+
name|e
operator|.
name|getMessage
...
annotation|@
name|Override
DECL|method|isValueUpdated (Set<NodeAttribute> value)
specifier|protected
name|boolean
name|isValueUpdated
parameter_list|(
...
block|{
name|StringBuilder
name|successfulNodeAttributesRegistrationMsg
init|=
operator|new
name|StringBuilder
argument_list|()
...
name|append
argument_list|(
literal|" and with following Node attribute(s) : {"
argument_list|)
operator|.
name|append
argument_list|(
...
block|{
comment|// case where provider is set but RM did not accept the node attributes
name|String
name|errorMsgFromRM
init|=
name|regNMResponse
...
name|error
argument_list|(
literal|"Node attributes sent from NM while registration were"
operator|+
literal|" rejected by RM. "
operator|+
operator|(
operator|(
name|errorMsgFromRM
...
operator|)
condition|?
literal|"Seems like RM is configured with Centralized Attributes."
else|:
...
block|{
comment|// case where updated node attributes from NodeAttributesProvider
comment|// is sent to RM and RM rejected the attributes
name|LOG
operator|.
name|error
argument_list|(
literal|"NM node attributes {"
operator|+
name|getPreviousValue
argument_list|()
operator|+
...
return|;
block|}
DECL|method|validate (Set<NodeLabel> nodeLabels)
specifier|protected
name|void
name|validate
parameter_list|(
...
annotation|@
name|Override
DECL|method|isValueUpdated (Set<NodeLabel> value)
specifier|protected
name|boolean
name|isValueUpdated
parameter_list|(
...
name|NodeAttribute
argument_list|>
name|nodeAttributesForHeartbeat
init|=
name|nodeAttributesHandler
operator|.
name|getNodeAttributesForHeartbeat
