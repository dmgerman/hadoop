...
literal|"user"
decl_stmt|;
DECL|field|NMSS_AMRMTOKEN_KEY
specifier|private
specifier|static
specifier|final
name|String
name|NMSS_AMRMTOKEN_KEY
init|=
literal|"amrmtoken"
decl_stmt|;
DECL|field|server
...
name|registry
decl_stmt|;
comment|/**    * Creates an instance of the service.    *    * @param nmContext NM context    * @param dispatcher NM dispatcher    */
...
expr_stmt|;
block|}
comment|/**    * Recover from NM state store. Called after serviceInit before serviceStart.    *    * @throws IOException if recover fails    */
DECL|method|recover ()
specifier|public
name|void
name|recover
...
name|info
argument_list|(
literal|"Recovering AMRMProxyService"
argument_list|)
expr_stmt|;
name|RecoveredAMRMProxyState
name|state
...
name|info
argument_list|(
literal|"Recovering {} running applications for AMRMProxy"
argument_list|,
name|state
operator|.
name|getAppContexts
...
name|info
argument_list|(
literal|"Recovering app attempt {}"
argument_list|,
name|attemptId
argument_list|)
expr_stmt|;
comment|// Try recover for the running application attempt
try|try
block|{
name|String
name|user
...
name|IOException
argument_list|(
literal|"No amrmToken found for app attempt "
operator|+
name|attemptId
argument_list|)
throw|;
...
name|IOException
argument_list|(
literal|"No user found for app attempt "
operator|+
name|attemptId
argument_list|)
throw|;
block|}
comment|// Regenerate the local AMRMToken for the AM
name|Token
argument_list|<
name|AMRMTokenIdentifier
argument_list|>
...
argument_list|)
decl_stmt|;
comment|// Retrieve the AM container credentials from NM context
name|Credentials
name|amCred
init|=
literal|null
...
name|info
argument_list|(
literal|"AM container {} found in context, has credentials: {}"
argument_list|,
name|container
operator|.
name|getContainerId
...
name|error
argument_list|(
literal|"No credentials found for AM container of {}. "
operator|+
literal|"Yarn registry access might not work"
argument_list|,
name|attemptId
argument_list|)
expr_stmt|;
block|}
comment|// Create the intercepter pipeline for the AM
...
name|error
argument_list|(
literal|"Exception when recovering "
operator|+
name|attemptId
operator|+
literal|", removing it from NMStateStore and move on"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * Callback from the ContainerManager implementation for initializing the    * application request processing pipeline.    *    * @param request - encapsulates information for starting an AM    * @throws IOException if fails    * @throws YarnException if fails    */
...
expr_stmt|;
block|}
comment|/**    * Initializes the request intercepter pipeline for the specified application.    *    * @param applicationAttemptId attempt id    * @param user user name    * @param amrmToken amrmToken issued by RM    * @param localToken amrmToken issued by AMRMProxy    * @param recoveredDataMap the recovered states for AMRMProxy from NMSS    * @param isRecovery whether this is to recover a previously existing pipeline    */
...
condition|)
block|{
comment|// TODO: revisit in AMRMProxy HA in YARN-6128
comment|// Remove the existing pipeline
name|LOG
operator|.
name|info
argument_list|(
literal|"Remove the previous pipeline for ApplicationId: "
operator|+
name|applicationAttemptId
operator|.
name|toString
...
name|error
argument_list|(
literal|"Error removing AMRMProxy application context for "
operator|+
name|applicationAttemptId
argument_list|,
name|e
...
name|YarnRuntimeException
argument_list|(
literal|"null recoveredDataMap recieved for recover"
argument_list|)
throw|;
block|}
name|interceptorChain
...
block|}
block|}
comment|/**    * Shuts down the request processing pipeline for the specified application    * attempt id.    *    * @param applicationId application id    */
...
expr_stmt|;
block|}
comment|// Remove the app context from NMSS after the interceptors are shutdown
if|if
condition|(
name|this
operator|.
...
argument_list|)
decl_stmt|;
comment|// Update the AMRMToken in context map, and in NM state store if it is
comment|// different
if|if
condition|(
name|context
...
block|}
block|}
DECL|method|createApplicationMasterContext ( Context context, ApplicationAttemptId applicationAttemptId, String user, Token<AMRMTokenIdentifier> amrmToken, Token<AMRMTokenIdentifier> localToken, Credentials credentials, RegistryOperations registryImpl)
specifier|private
...
return|;
block|}
comment|/**    * Returns the comma separated intercepter class names from the configuration.    *    * @param conf configuration    * @return the intercepter class names as an instance of ArrayList    */
...
return|;
block|}
comment|/**    * Authorizes the request and returns the application specific request    * processing pipeline.    *    * @return the the intercepter wrapper instance    * @throws YarnException if fails    */
...
name|applicationAttemptId
decl_stmt|;
comment|/**      * Initializes the wrapper with the specified parameters.      *       * @param rootInterceptor the root request intercepter      * @param applicationAttemptId attempt id      */
