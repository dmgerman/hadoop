...
decl_stmt|;
comment|/**    * Stores the AllocateResponses that are received asynchronously from all the    * sub-cluster resource managers, including home RM, but not merged and    * returned back to AM yet.    */
...
decl_stmt|;
comment|/**    * Remembers the last allocate response from all known sub-clusters. This is    * used together with sub-cluster timeout to assemble entries about    * cluster-wide info (e.g. AvailableResource, NumClusterNodes) in the allocate    * response back to AM.    */
DECL|field|lastSCResponse
specifier|private
name|Map
argument_list|<
name|SubClusterId
...
name|lastSCResponse
decl_stmt|;
comment|/**    * The async UAM registration result that is not consumed yet.    */
...
name|subClusterResolver
decl_stmt|;
comment|/**    * Records the last time a successful heartbeat response received from a known    * sub-cluster. lastHeartbeatTimeStamp.keySet() should be in sync with    * uamPool.getAllUAMIds().    */
DECL|field|lastSCResponseTime
specifier|private
name|Map
argument_list|<
name|SubClusterId
...
decl_stmt|;
DECL|field|lastAMHeartbeatTime
specifier|private
name|long
name|lastAMHeartbeatTime
decl_stmt|;
...
argument_list|)
decl_stmt|;
comment|// Set sub-cluster to be timed out initially
name|lastSCResponseTime
operator|.
name|put
argument_list|(
...
name|response
init|=
name|generateBaseAllocationResponse
argument_list|()
decl_stmt|;
...
name|info
argument_list|(
literal|"Shutting down FederationInterceptor for {}"
argument_list|,
name|this
operator|.
name|attemptId
...
name|error
argument_list|(
literal|"Error shutting down all UAM clients without killing them"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * Prepare the base allocation response. Use lastSCResponse and    * lastHeartbeatTimeStamp to assemble entries about cluster-wide info, e.g.    * AvailableResource, NumClusterNodes.    */
DECL|method|generateBaseAllocationResponse ()
specifier|protected
name|AllocateResponse
name|generateBaseAllocationResponse
parameter_list|()
...
condition|)
block|{
comment|// Skip expired sub-clusters
continue|continue;
block|}
name|AllocateResponse
name|response
...
return|;
block|}
DECL|method|getTimedOutSCs (boolean verbose)
specifier|protected
name|Set
argument_list|<
name|SubClusterId
...
condition|)
block|{
comment|// AM haven't heartbeat to us (and thus we to all SCs) for a long time,
comment|// should not consider the SC as timed out
continue|continue;
block|}
name|long
name|duration
...
name|warn
argument_list|(
literal|"Subcluster {} doesn't have a successful heartbeat"
operator|+
literal|" for {} seconds for {}"
argument_list|,
name|entry
operator|.
name|getKey
