...
return|;
block|}
comment|/**    * Calculates the pending number of retries.    *<p>    * When failuresValidityInterval is> 0, it also removes time entries from    *<code>restartTimes</code> which are outside the validity interval.    *    * @return the pending retries.    */
DECL|method|calculatePendingRetries (RetryContext retryContext)
specifier|private
name|int
name|calculatePendingRetries
parameter_list|(
...
block|}
block|}
comment|/**    * Updates remaining retries and the restart time when    * required in the retryContext.    */
DECL|method|updateRetryContext (RetryContext retryContext, int pendingRetries)
specifier|private
name|void
name|updateRetryContext
parameter_list|(
