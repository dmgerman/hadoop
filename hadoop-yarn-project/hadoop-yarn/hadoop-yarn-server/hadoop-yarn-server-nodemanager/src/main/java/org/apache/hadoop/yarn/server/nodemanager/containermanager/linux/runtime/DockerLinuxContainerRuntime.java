...
operator|.
name|Private
DECL|field|ENV_DOCKER_CONTAINER_HOSTNAME
specifier|public
specifier|static
specifier|final
name|String
...
operator|.
name|Private
DECL|field|ENV_DOCKER_CONTAINER_TMPFS_MOUNTS
specifier|public
specifier|static
specifier|final
name|String
...
init|=
literal|"YARN_CONTAINER_RUNTIME_DOCKER_TMPFS_MOUNTS"
decl_stmt|;
annotation|@
name|InterfaceAudience
operator|.
name|Private
DECL|field|ENV_DOCKER_CONTAINER_DELAYED_REMOVAL
specifier|public
specifier|static
specifier|final
name|String
...
operator|.
name|Private
DECL|field|ENV_DOCKER_CONTAINER_PORTS_MAPPING
specifier|public
specifier|static
specifier|final
name|String
name|ENV_DOCKER_CONTAINER_PORTS_MAPPING
init|=
literal|"YARN_CONTAINER_RUNTIME_DOCKER_PORTS_MAPPING"
decl_stmt|;
annotation|@
name|InterfaceAudience
operator|.
...
operator|.
name|Private
DECL|field|ENV_DOCKER_CONTAINER_DOCKER_RUNTIME
specifier|public
specifier|static
specifier|final
name|String
...
init|=
literal|"YARN_CONTAINER_RUNTIME_DOCKER_RUNTIME"
decl_stmt|;
annotation|@
name|InterfaceAudience
operator|.
name|Private
DECL|field|ENV_DOCKER_CONTAINER_DOCKER_SERVICE_MODE
specifier|public
specifier|static
specifier|final
name|String
name|ENV_DOCKER_CONTAINER_DOCKER_SERVICE_MODE
init|=
literal|"YARN_CONTAINER_RUNTIME_DOCKER_SERVICE_MODE"
decl_stmt|;
annotation|@
name|InterfaceAudience
operator|.
...
name|privilegedOperationExecutor
decl_stmt|;
DECL|field|defaultImageName
specifier|private
name|String
name|defaultImageName
decl_stmt|;
DECL|field|defaultImageUpdate
specifier|private
name|Boolean
...
decl_stmt|;
DECL|field|enableUserReMapping
specifier|private
name|boolean
name|enableUserReMapping
decl_stmt|;
DECL|field|userRemappingUidThreshold
specifier|private
name|int
name|userRemappingUidThreshold
decl_stmt|;
DECL|field|userRemappingGidThreshold
specifier|private
name|int
name|userRemappingGidThreshold
decl_stmt|;
...
argument_list|()
decl_stmt|;
DECL|field|defaultTmpfsMounts
specifier|private
name|Set
argument_list|<
name|String
argument_list|>
name|defaultTmpfsMounts
init|=
operator|new
name|HashSet
argument_list|<>
argument_list|()
decl_stmt|;
comment|/**    * Return whether the given environment variables indicate that the operation    * is requesting a Docker container.  If the environment contains a key    * called {@code YARN_CONTAINER_RUNTIME_TYPE} whose value is {@code docker},    * this method will return true.  Otherwise it will return false.    *    * @param daemonConf the NodeManager daemon configuration    * @param env the environment variable settings for the operation    * @return whether a Docker container is requested    */
DECL|method|isDockerContainerRequested (Configuration daemonConf, Map<String, String> env)
specifier|public
specifier|static
name|boolean
...
name|message
operator|+
literal|". Please check configuration"
argument_list|)
expr_stmt|;
block|}
throw|throw
...
throw|;
block|}
comment|// initialize csi adaptors if necessary
...
name|capabilities
operator|=
name|getDockerCapabilitiesFromConf
argument_list|()
expr_stmt|;
name|delayedRemovalAllowed
operator|=
...
name|msg
init|=
literal|"Mixing capabilities with the none keyword is"
operator|+
...
argument_list|()
operator|+
literal|", docker volume output for "
operator|+
name|dockerVolumeCommand
operator|+
literal|": "
...
name|error
argument_list|(
literal|"Error when writing command to temp file, command="
operator|+
name|dockerVolumeCommand
argument_list|,
name|e
...
name|error
argument_list|(
literal|"Error when executing command, command="
operator|+
name|dockerVolumeCommand
argument_list|,
name|e
...
block|}
block|}
DECL|method|checkDockerVolumeCreated ( DockerVolumeCommand dockerVolumeCreationCommand, Container container)
specifier|private
name|void
name|checkDockerVolumeCreated
parameter_list|(
name|DockerVolumeCommand
name|dockerVolumeCreationCommand
parameter_list|,
name|Container
name|container
parameter_list|)
...
argument_list|)
decl_stmt|;
comment|// Parse output line by line and check if it matches
name|String
name|volumeName
init|=
name|dockerVolumeCreationCommand
...
condition|)
block|{
comment|// Good we found it.
name|LOG
operator|.
name|info
argument_list|(
literal|"Docker volume-name="
operator|+
name|volumeName
operator|+
literal|" driver-name="
...
name|driverName
operator|+
literal|" already exists for container="
operator|+
name|container
operator|.
name|getContainerId
...
block|}
block|}
comment|// Couldn't find the volume
name|String
name|message
init|=
literal|" Couldn't find volume="
operator|+
name|volumeName
operator|+
literal|" driver="
...
argument_list|()
operator|+
literal|", please check error message in log to understand "
operator|+
literal|"why this happens."
decl_stmt|;
name|LOG
operator|.
name|error
...
throw|;
block|}
comment|/** Set a DNS friendly hostname.    *  Only add hostname if network is not host or if hostname is    *  specified via YARN_CONTAINER_RUNTIME_DOCKER_CONTAINER_HOSTNAME    *  in host network mode    */
DECL|method|setHostname (DockerRunCommand runCommand, String containerIdStr, String network, String name)
specifier|private
name|void
name|setHostname
parameter_list|(
...
name|info
argument_list|(
literal|"setting hostname in container to: "
operator|+
name|name
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * Check if system is default to disable docker override or    * user requested a Docker container with ENTRY_POINT support.    *    * @param environment - Docker container environment variables    * @return true if Docker launch command override is disabled    */
DECL|method|checkUseEntryPoint (Map<String, String> environment)
specifier|private
name|boolean
...
argument_list|)
expr_stmt|;
comment|// Add ports mapping value.
if|if
condition|(
name|environment
operator|.
...
name|ContainerExecutionException
argument_list|(
literal|"Container requests for volume resource but we are failed"
operator|+
literal|" to publish volumes on this node"
argument_list|)
throw|;
block|}
if|if
...
expr_stmt|;
block|}
name|addDockerClientConfigToRunCommand
argument_list|(
name|ctx
argument_list|,
name|runCommand
...
expr_stmt|;
block|}
comment|// In service mode, the YARN log dirs are not mounted into the container.
comment|// As a result, the container fails to start due to stdout and stderr output
comment|// being sent to a file in a directory that does not exist. In service mode,
comment|// only supply the command with no stdout or stderr redirection.
name|List
argument_list|<
name|String
argument_list|>
...
block|}
block|}
comment|// use plugins to create volume and update docker run command.
if|if
condition|(
name|nmContext
operator|!=
...
condition|)
block|{
comment|// Create volumes when needed.
name|DockerVolumeCommand
name|dockerVolumeCommand
init|=
name|dockerCommandPlugin
...
argument_list|)
expr_stmt|;
comment|// After volume created, run inspect to make sure volume properly
comment|// created.
if|if
condition|(
name|dockerVolumeCommand
...
argument_list|()
decl_stmt|;
comment|// Check to see if the container already exists for relaunch
name|DockerCommandExecutor
operator|.
...
name|ContainerExecutionException
argument_list|(
literal|"Container is not in a startable "
operator|+
literal|"state, unable to relaunch: "
operator|+
name|containerIdStr
argument_list|)
throw|;
block|}
block|}
comment|/**    * Signal the docker container.    *    * Signals are used to check the liveliness of the container as well as to    * stop/kill the container. The following outlines the docker container    * signal handling.    *    *<ol>    *<li>If the null signal is sent, run kill -0 on the pid. This is used    *     to check if the container is still alive, which is necessary for    *     reacquiring containers on NM restart.</li>    *<li>If SIGTERM, SIGKILL is sent, attempt to stop and remove the docker    *     container.</li>    *<li>If the docker container exists and is running, execute docker    *     stop.</li>    *<li>If any other signal is sent, signal the container using docker    *     kill.</li>    *</ol>    *    * @param ctx the {@link ContainerRuntimeContext}.    * @throws ContainerExecutionException if the signaling fails.    */
annotation|@
name|Override
...
block|}
block|}
comment|/**    * Reap the docker container.    *    * @param ctx the {@link ContainerRuntimeContext}.    * @throws ContainerExecutionException if the removal fails.    */
annotation|@
name|Override
...
name|ContainerExecutionException
block|{
comment|// Clean up the Docker container
name|handleContainerRemove
argument_list|(
name|ctx
operator|.
name|getContainer
...
argument_list|)
expr_stmt|;
comment|// Cleanup volumes when needed.
if|if
condition|(
name|nmContext
operator|!=
...
block|}
block|}
comment|/**    * Perform docker exec command into running container.    *    * @param ctx container exec context    * @return IOStreams of docker exec    * @throws ContainerExecutionException    */
annotation|@
name|Override
...
name|containerId
operator|+
literal|", docker exec output for "
operator|+
name|dockerExecCommand
operator|+
literal|": "
...
condition|)
block|{
comment|// Report back node manager IP in the event where docker
comment|// inspect reports no IP address.  This is for bridging a gap for
comment|// docker environment to run with host network.
name|InetAddress
name|address
decl_stmt|;
try|try
...
name|error
argument_list|(
literal|"Can not determine IP for container:"
operator|+
name|containerId
argument_list|)
expr_stmt|;
...
return|;
block|}
DECL|method|buildLaunchOp (ContainerRuntimeContext ctx, String commandFile, DockerCommand command)
specifier|private
name|PrivilegedOperation
name|buildLaunchOp
parameter_list|(
...
block|}
block|}
DECL|method|pullImageFromRemote (String containerIdStr, String imageName)
specifier|public
name|void
name|pullImageFromRemote
parameter_list|(
...
expr_stmt|;
block|}
DECL|method|executeLivelinessCheck (ContainerRuntimeContext ctx)
specifier|private
name|void
name|executeLivelinessCheck
parameter_list|(
...
name|msg
init|=
literal|"Liveliness check failed for PID: "
operator|+
name|pid
operator|+
literal|". Container may have already completed."
decl_stmt|;
throw|throw
operator|new
name|ContainerExecutionException
...
block|}
block|}
comment|/**    * Handles a docker container stop by first finding the {@code STOPSIGNAL}    * using docker inspect and then executing    * {@code docker kill --signal=<STOPSIGNAL>}.    * It doesn't rely on the docker stop because that sends a {@code SIGKILL}    * to the root process in the container after the {@code STOPSIGNAL}.The grace    * period which the docker stop uses has granularity in seconds. However, NM    * is designed to explicitly send a {@code SIGKILL} to the containers after a    * grace period which has a granularity of millis. It doesn't want the docker    * stop to send {@code SIGKILL} but docker stop has no option to disallow    * that.    *    * @param containerId container id    * @param env         env    * @throws ContainerExecutionException    */
DECL|method|handleContainerStop (ContainerId containerId, Map<String, String> env)
specifier|private
name|void
name|handleContainerStop
parameter_list|(
...
name|debug
argument_list|(
literal|"{} inspect failed, skipping stop"
argument_list|,
name|containerId
argument_list|,
name|e
...
name|debug
argument_list|(
literal|"{} status is {}, skipping stop"
argument_list|,
name|containerId
argument_list|,
name|containerStatus
...
block|}
block|}
DECL|method|executeDockerInspect (ContainerId containerId, DockerInspectCommand inspectCommand)
specifier|private
name|String
name|executeDockerInspect
parameter_list|(
...
return|;
block|}
DECL|method|handleContainerKill (ContainerRuntimeContext ctx, Map<String, String> env, ContainerExecutor.Signal signal)
specifier|private
name|void
name|handleContainerKill
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|// Only need to check whether the container was asked to be privileged.
comment|// If the container had failed the permissions checks upon launch, it
comment|// would have never been launched and thus we wouldn't be here
comment|// attempting to signal it.
if|if
condition|(
...
name|debug
argument_list|(
literal|"Container status is {}, skipping kill - {}"
argument_list|,
name|containerStatus
operator|.
name|getName
...
name|ContainerExecutionException
argument_list|(
literal|"Signal container failed using "
operator|+
literal|"signal: "
operator|+
name|signal
...
block|}
block|}
DECL|method|handleContainerRemove (String containerId, Map<String, String> env)
specifier|private
name|void
name|handleContainerRemove
parameter_list|(
...
block|}
block|}
DECL|method|addDockerClientConfigToRunCommand (ContainerRuntimeContext ctx, DockerRunCommand dockerRunCommand)
specifier|private
name|void
name|addDockerClientConfigToRunCommand
parameter_list|(
...
name|ContainerExecutionException
argument_list|(
literal|"Unable to write Docker client credentials to "
operator|+
name|dockerConfigPath
argument_list|)
throw|;
