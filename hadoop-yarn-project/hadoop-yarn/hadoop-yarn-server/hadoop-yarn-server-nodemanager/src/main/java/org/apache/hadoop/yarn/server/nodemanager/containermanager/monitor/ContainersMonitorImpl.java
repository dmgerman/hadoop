...
name|monitoringThread
decl_stmt|;
DECL|field|oomListenerThread
specifier|private
name|CGroupElasticMemoryController
name|oomListenerThread
decl_stmt|;
...
decl_stmt|;
DECL|field|elasticMemoryEnforcement
specifier|private
name|boolean
name|elasticMemoryEnforcement
decl_stmt|;
DECL|field|strictMemoryEnforcement
specifier|private
name|boolean
...
condition|)
block|{
comment|// Test for availability outside the constructor
comment|// to be able to write non-Linux unit tests for
comment|// CGroupElasticMemoryController
throw|throw
operator|new
name|YarnException
argument_list|(
literal|"CGroup Elastic Memory controller enabled but "
operator|+
literal|"it is not available. Exiting."
argument_list|)
throw|;
block|}
else|else
...
argument_list|)
expr_stmt|;
comment|// Publish the container utilization metrics to node manager
...
condition|)
block|{
comment|// We enforce the overall memory usage instead of individual containers
return|return;
block|}
name|boolean
