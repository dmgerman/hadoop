...
decl_stmt|;
DECL|field|publishContainerMetricsToTimelineService
specifier|private
name|boolean
name|publishContainerMetricsToTimelineService
decl_stmt|;
...
decl_stmt|;
comment|// Tracks the aggregated allocation of the currently allocated containers
...
name|info
argument_list|(
literal|"NodeManager has been configured to publish container "
operator|+
literal|"metrics to Timeline Service V2."
argument_list|)
expr_stmt|;
name|threadPool
operator|=
...
name|warn
argument_list|(
literal|"NodeManager has not been configured to publish container "
operator|+
literal|"metrics to Timeline Service V2."
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// if currentPmemUsage data is available
if|if
condition|(
name|currentPmemUsage
operator|!=
...
expr_stmt|;
block|}
comment|// if cpuUsageTotalCoresPercentage data is available
if|if
condition|(
name|cpuUsageTotalCoresPercentage
operator|!=
...
name|error
argument_list|(
literal|"Exception in ContainersMonitorImpl in putting "
operator|+
literal|"resource usage metrics to timeline service."
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
block|}
block|}
DECL|method|putEntityWithoutBlocking (final TimelineClient timelineClient, final TimelineEntity entity)
specifier|private
name|void
name|putEntityWithoutBlocking
parameter_list|(
...
return|;
block|}
comment|/**    * @return true if there are available allocated resources for the given    *         container to start.    */
annotation|@
name|Override
...
expr_stmt|;
block|}
comment|/**    * Calculates the vCores CPU usage that is assigned to the given    * {@link ProcessTreeInfo}. In particular, it takes into account the number of    * vCores that are allowed to be used by the NM and returns the CPU usage    * as a normalized value between {@literal>=} 0 and {@literal<=} 1.    */
DECL|method|allocatedCpuUsage (ProcessTreeInfo pti)
specifier|private
name|float
name|allocatedCpuUsage
parameter_list|(
