...
expr_stmt|;
block|}
comment|/**    * Prepare a launch Context for container upgrade and request the    * Container Manager to re-initialize a running container using the    * new launch context.    * @param failCmd injects a start script that intentionally fails.    * @param failLoc injects a bad file Location that will fail localization.    */
DECL|method|prepareContainerUpgrade (boolean failCmd, boolean failLoc, ContainerId cId, File startFile)
specifier|private
name|void
name|prepareContainerUpgrade
parameter_list|(
...
name|contains
argument_list|(
literal|"Cannot perform UPGRADE"
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
expr_stmt|;
block|}
DECL|method|prepareContainerLaunchContext (File scriptFile, String destFName, boolean putBadFile)
specifier|private
name|ContainerLaunchContext
name|prepareContainerLaunchContext
parameter_list|(
