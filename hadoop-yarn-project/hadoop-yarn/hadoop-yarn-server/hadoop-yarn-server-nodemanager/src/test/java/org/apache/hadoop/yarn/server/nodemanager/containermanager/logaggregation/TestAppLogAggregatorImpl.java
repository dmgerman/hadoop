...
block|}
block|}
DECL|method|createAppLogAggregator ( ApplicationId applicationId, String rootLogDir, YarnConfiguration config, Context context, long recoveredLogInitedTimeMillis, DeletionService deletionServiceWithFilesToExpect, LogAggregationTFileController tFileController)
specifier|private
specifier|static
name|AppLogAggregatorInTest
name|createAppLogAggregator
...
annotation|@
name|Test
DECL|method|testDFSQuotaExceeded ()
specifier|public
name|void
name|testDFSQuotaExceeded
parameter_list|()
throws|throws
name|Exception
block|{
comment|// the expectation is that no log files are deleted if the quota has
comment|// been exceeded, since that would result in loss of logs
name|DeletionService
name|deletionServiceWithExpectedFiles
init|=
name|createDeletionServiceWithExpectedFile2Delete
...
argument_list|)
decl_stmt|;
comment|// we need a LogAggregationTFileController that throws a
comment|// LogAggregationDFSException
name|LogAggregationTFileController
name|format
init|=
name|Mockito
...
argument_list|()
expr_stmt|;
comment|// verify that no files have been uploaded
name|ArgumentCaptor
argument_list|<
name|LogValue
argument_list|>
