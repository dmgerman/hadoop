...
name|class
argument_list|)
comment|//Test that DockerContainerExecutor doesn't successfully init on a secure
comment|//cluster
...
name|class
argument_list|)
comment|//Test that when the image name is null, the container launch throws an
...
name|class
argument_list|)
comment|//Test that when the image name is invalid, the container launch throws an
comment|//IllegalArgumentException
...
annotation|@
name|Test
comment|//Test that a container launch correctly wrote the session script with the
comment|//commands we expected
