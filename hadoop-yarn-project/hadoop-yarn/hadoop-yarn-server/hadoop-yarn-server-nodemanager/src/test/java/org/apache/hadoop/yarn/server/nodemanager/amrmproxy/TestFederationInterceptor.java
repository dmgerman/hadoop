...
argument_list|)
expr_stmt|;
comment|// Set sub-cluster timeout to 500ms
name|conf
operator|.
name|setLong
argument_list|(
...
expr_stmt|;
block|}
DECL|method|checkAMRMToken (Token amrmToken)
specifier|private
name|void
name|checkAMRMToken
parameter_list|(
...
condition|)
block|{
comment|// The token should be the one issued by home MockRM
name|Assert
operator|.
name|assertTrue
argument_list|(
...
expr_stmt|;
block|}
DECL|method|testRecover (final RegistryOperations registryObj)
specifier|protected
name|void
name|testRecover
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// Make sure all async hb threads are done
name|interceptor
operator|.
name|drainAllAsyncQueue
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// SC1 should be initialized to be timed out
name|Assert
operator|.
name|assertEquals
argument_list|(
...
annotation|@
name|Test
DECL|method|testSubClusterTimeOut ()
specifier|public
name|void
name|testSubClusterTimeOut
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// Let all SC timeout (home and SC-1), without an allocate from AM
name|Thread
operator|.
name|sleep
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Should not be considered timeout, because there's no recent AM
comment|// heartbeat
name|allocateResponse
operator|=
name|interceptor
...
argument_list|)
expr_stmt|;
comment|// Generate a duplicate heartbeat from AM, so that it won't really
comment|// trigger an heartbeat to all SC
name|AllocateRequest
name|allocateRequest
init|=
name|Records
...
argument_list|)
decl_stmt|;
comment|// Set to lastResponseId - 1 so that it will be considered a duplicate
comment|// heartbeat and thus not forwarded to all SCs
name|allocateRequest
operator|.
name|setResponseId
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Should be considered timeout
name|allocateResponse
operator|=
name|interceptor
operator|.
...
argument_list|)
expr_stmt|;
comment|// Update the ugi with new attemptId
name|ugi
operator|=
name|interceptor
operator|.
...
annotation|@
name|Test
DECL|method|testMergeAllocateResponse ()
specifier|public
name|void
name|testMergeAllocateResponse
parameter_list|()
block|{
name|ContainerId
name|cid
...
name|setPreemptionMessage
argument_list|(
name|createDummyPreemptionMessage
argument_list|(
name|ContainerId
operator|.
name|newContainerId
...
expr_stmt|;
block|}
DECL|method|createDummyPreemptionMessage ( ContainerId containerId)
specifier|private
name|PreemptionMessage
name|createDummyPreemptionMessage
parameter_list|(
