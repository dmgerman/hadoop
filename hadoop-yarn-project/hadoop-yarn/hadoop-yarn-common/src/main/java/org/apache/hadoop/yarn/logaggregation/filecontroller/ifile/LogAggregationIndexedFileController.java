...
condition|)
block|{
comment|// Get an error when parse the UUID from existed log file.
comment|// Simply OverWrite the existed log file and re-create the
comment|// UUID.
name|fsDataOStream
operator|=
name|fc
...
block|}
block|}
comment|// if the remote log file exists, but we do not have any
comment|// indexedLogsMeta. We need to re-load indexedLogsMeta from
comment|// the existing remote log file. If the re-load fails, we simply
comment|// re-create a new indexedLogsMeta object. And will re-load
comment|// the indexedLogsMeta from checksum file later.
if|if
condition|(
name|indexedLogsMeta
operator|==
...
argument_list|()
decl_stmt|;
comment|// only check the check-sum file when we are in append mode
if|if
condition|(
name|context
operator|.
...
condition|)
block|{
comment|// check whether the checksum file exists to figure out
comment|// whether the previous log aggregation process is successful
comment|// and the aggregated log file is corrupted or not.
name|remoteLogCheckSumFile
operator|=
operator|new
name|Path
...
annotation|@
name|Private
DECL|method|filterFiles ( List<FileStatus> fileList, final String suffix)
specifier|public
name|Map
argument_list|<
name|String
...
annotation|@
name|Private
DECL|method|loadIndexedLogsMeta (Path remoteLogPath, long end)
specifier|public
name|IndexedLogsMeta
name|loadIndexedLogsMeta
parameter_list|(
...
block|}
block|}
DECL|method|loadIndexedLogsMeta (Path remoteLogPath)
specifier|private
name|IndexedLogsMeta
name|loadIndexedLogsMeta
parameter_list|(
...
annotation|@
name|Private
DECL|method|loadIndexedLogsCheckSum (Path remoteLogCheckSumPath)
specifier|public
name|long
name|loadIndexedLogsCheckSum
parameter_list|(
name|Path
name|remoteLogCheckSumPath
parameter_list|)
throws|throws
name|IOException
block|{
