...
return|;
block|}
comment|/**    * Divides lhs by rhs.    * If both lhs and rhs are having a value of 0, then we return 0.    * This is to avoid division by zero and return NaN as a result.    * If lhs is zero but rhs is not, Float.infinity will be returned    * as the result.    * @param lhs    * @param rhs    * @return    */
DECL|method|divideSafelyAsFloat (long lhs, long rhs)
specifier|public
specifier|static
name|float
...
parameter_list|)
function_decl|;
comment|/**    * Check if resource has any major resource types (which are all NodeManagers    * included) has a {@literal>} 0 value.    *    * @param resource resource    * @return returns true if any resource is {@literal>} 0    */
