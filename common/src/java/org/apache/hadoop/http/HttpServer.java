...
name|info
argument_list|(
literal|"Adding Kerberos filter to "
operator|+
name|name
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/**    * Configure an ssl listener on the server.    * @param addr address to listen on    * @param sslConf conf to retrieve ssl options    * @param needCertsAuth whether x509 certificate authentication is required    * @param needKrbAuth whether to allow kerberos auth    */
DECL|method|addSslListener (InetSocketAddress addr, Configuration sslConf, boolean needCertsAuth, boolean needKrbAuth)
specifier|public
name|void
name|addSslListener
parameter_list|(
...
comment|// Default to certificates
name|mode
operator|=
name|MODE
operator|.
...
name|Exception
argument_list|(
literal|"Exepected webserver's listener to be started "
operator|+
literal|"previously but wasn't"
argument_list|)
throw|;
comment|// And skip all the port rolling issues.
name|webServer
operator|.
name|start
argument_list|()
