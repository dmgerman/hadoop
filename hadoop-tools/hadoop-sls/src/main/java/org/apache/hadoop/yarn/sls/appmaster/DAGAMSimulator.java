...
begin_comment
comment|/**  * AMSimulator that simulates DAG - it requests for containers  * based on the delay specified. It finishes when all the tasks  * are completed.  *  * Vocabulary Used:  * pending -> requests which are NOT yet sent to RM.  * scheduled -> requests which are sent to RM but not yet assigned.  * assigned -> requests which are assigned to a container.  * completed -> request corresponding to which container has completed.  * Containers are requested based on the request delay.  */
end_comment
