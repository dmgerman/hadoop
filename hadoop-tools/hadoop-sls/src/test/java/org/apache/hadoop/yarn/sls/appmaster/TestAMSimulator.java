...
annotation|@
name|Test
DECL|method|testPackageRequests ()
specifier|public
name|void
name|testPackageRequests
parameter_list|()
block|{
name|MockAMSimulator
name|app
...
argument_list|)
decl_stmt|;
comment|// total 4 resource requests: any -> 1, rack -> 1, node -> 2
comment|// All resource requests for any would be packaged into 1.
comment|// All resource requests for racks would be packaged into 1 as all of them
comment|// are for same rack.
comment|// All resource requests for nodes would be packaged into 2 as there are
comment|// two different nodes.
name|Assert
operator|.
name|assertEquals
argument_list|(
...
expr_stmt|;
name|int
name|anyRequestCount
init|=
literal|0
decl_stmt|;
name|int
name|rackRequestCount
init|=
literal|0
decl_stmt|;
name|int
name|nodeRequestCount
init|=
literal|0
decl_stmt|;
for|for
...
argument_list|)
expr_stmt|;
comment|// total 7 resource requests: any -> 2, rack -> 2, node -> 3
comment|// All resource requests for any would be packaged into 2 as there are
comment|// two different allocation id.
comment|// All resource requests for racks would be packaged into 2 as all of them
comment|// are for same rack but for two different allocation id.
comment|// All resource requests for nodes would be packaged into 3 as either node
comment|// or allocation id is different for each request.
name|Assert
operator|.
name|assertEquals
argument_list|(
...
argument_list|()
decl_stmt|;
comment|// allocation id should be either 1 or 2
name|Assert
operator|.
name|assertTrue
argument_list|(
