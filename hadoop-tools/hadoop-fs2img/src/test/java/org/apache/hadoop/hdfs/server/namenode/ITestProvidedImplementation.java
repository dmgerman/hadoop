...
argument_list|)
expr_stmt|;
comment|// format the shared edits dir with the proper VERSION file.
name|NameNode
operator|.
...
annotation|@
name|Test
DECL|method|testBootstrapAliasMap ()
specifier|public
name|void
...
argument_list|)
expr_stmt|;
comment|// make NN with index 0 the active, shutdown and delete the directories
comment|// of others. This will delete the aliasmap on these namenodes as well.
name|cluster
operator|.
name|transitionToActive
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// delete the namenode directories including alias map.
for|for
control|(
name|URI
name|u
...
block|}
block|}
comment|// start the other namenodes and bootstrap them
for|for
control|(
name|int
name|index
...
control|)
block|{
comment|// add some content to aliasmap dir
name|File
name|aliasMapDir
init|=
operator|new
...
argument_list|)
decl_stmt|;
comment|// create a directory inside aliasMapDir
if|if
condition|(
operator|!
operator|new
...
argument_list|)
expr_stmt|;
comment|// without force this should fail as aliasmap is not empty.
name|int
name|rc
init|=
name|BootstrapStandby
...
argument_list|)
expr_stmt|;
comment|// force deletes the contents of the aliasmap.
name|rc
operator|=
name|BootstrapStandby
operator|.
...
expr_stmt|;
block|}
comment|// check if aliasmap files are the same on all NNs
name|checkInMemoryAliasMapContents
argument_list|(
literal|0
argument_list|,
name|numNamenodes
argument_list|)
expr_stmt|;
comment|// restart the killed namenodes.
for|for
control|(
name|int
name|i
...
argument_list|()
expr_stmt|;
comment|// transition to namenode 1 as the active
name|int
name|nextNN
init|=
literal|1
...
argument_list|)
expr_stmt|;
comment|// all files must be accessible from nextNN.
name|verifyFileSystemContents
argument_list|(
name|nextNN
argument_list|)
expr_stmt|;
block|}
comment|/**    * Check if the alias map contents of the namenodes are the same as the base.    *    * @param baseNN index of the namenode to compare against.    * @param numNamenodes total number of namenodes in the cluster.    */
DECL|method|checkInMemoryAliasMapContents (int baseNN, int numNamenodes)
specifier|private
name|void
name|checkInMemoryAliasMapContents
parameter_list|(
...
block|}
block|}
comment|/**    * Verify that the contents of the aliasmaps are the same.    *    * @param aliasMap1    * @param aliasMap2    */
DECL|method|verifyAliasMapEquals (InMemoryLevelDBAliasMapServer aliasMap1, InMemoryLevelDBAliasMapServer aliasMap2)
specifier|private
name|void
name|verifyAliasMapEquals
parameter_list|(
...
expr_stmt|;
block|}
comment|/**    * Get all the aliases the aliasmap contains.    *    * @param aliasMap aliasmap to explore.    * @return set of all aliases.    * @throws IOException    */
DECL|method|getFileRegions (InMemoryLevelDBAliasMapServer aliasMap)
specifier|private
name|Set
argument_list|<
name|FileRegion
