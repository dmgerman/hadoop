...
return|;
block|}
comment|/**    * Tests the rename file operation to ensure that when there are multiple    * attempts to rename a file to the same destination, only one rename    * operation is successful (HADOOP-15086).    */
annotation|@
name|Test
DECL|method|testMultipleRenameFileOperationsToSameDestination ()
specifier|public
name|void
name|testMultipleRenameFileOperationsToSameDestination
parameter_list|()
throws|throws
name|IOException
throws|,
...
argument_list|)
decl_stmt|;
comment|// Run 10 threads to rename multiple files to the same target path
name|List
argument_list|<
name|Thread
argument_list|>
...
name|info
argument_list|(
literal|"rename succeeded for thread "
operator|+
name|threadNumber
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Wait for threads to start and wait on latch
name|Thread
operator|.
name|sleep
argument_list|(
...
name|info
argument_list|(
literal|"Success, only one rename operation succeeded!"
argument_list|)
expr_stmt|;
block|}
annotation|@
