...
argument_list|)
decl_stmt|;
comment|// skip the check if the chown allowed users config value is set as '*'
if|if
condition|(
operator|!
name|shouldSkipUserCheck
...
name|IllegalArgumentException
argument_list|(
literal|"User list must contain "
operator|+
literal|"either '*' or list of user names, but not both."
argument_list|)
throw|;
block|}
elseif|else
...
name|format
argument_list|(
literal|"user '%s' does not have the privilege to change the ownership of files/folders."
argument_list|,
name|actualUgi
operator|.
name|getShortUserName
