...
annotation|@
name|VisibleForTesting
DECL|field|KEY_INPUT_STREAM_VERSION
specifier|static
specifier|final
name|String
name|KEY_INPUT_STREAM_VERSION
init|=
literal|"fs.azure.input.stream.version.for.internal.use.only"
decl_stmt|;
...
decl_stmt|;
DECL|field|KEY_AUTO_THROTTLE_ENABLE
specifier|private
specifier|static
specifier|final
name|String
name|KEY_AUTO_THROTTLE_ENABLE
init|=
...
name|pageBlobDirs
decl_stmt|;
comment|/**    * Configuration key to indicate the set of directories in WASB where we    * should store files as block blobs with block compaction enabled.    *    * Entries can be directory paths relative to the container (e.g. "/path") or    * fully qualified wasb:// URIs (e.g.    * wasb://container@example.blob.core.windows.net/path)    */
DECL|field|KEY_BLOCK_BLOB_WITH_COMPACTION_DIRECTORIES
specifier|public
specifier|static
specifier|final
name|String
...
init|=
literal|"fs.azure.block.blob.with.compaction.dir"
decl_stmt|;
comment|/**    * The set of directories where we should store files as block blobs with    * block compaction enabled.    */
DECL|field|blockBlobWithCompationDirs
specifier|private
name|Set
argument_list|<
name|String
argument_list|>
name|blockBlobWithCompationDirs
decl_stmt|;
...
specifier|final
name|int
name|DEFAULT_INPUT_STREAM_VERSION
init|=
literal|2
decl_stmt|;
...
literal|1.0f
decl_stmt|;
DECL|field|DEFAULT_AUTO_THROTTLE_ENABLE
specifier|private
specifier|static
specifier|final
name|boolean
name|DEFAULT_AUTO_THROTTLE_ENABLE
init|=
literal|false
decl_stmt|;
...
name|DEFAULT_UPLOAD_BLOCK_SIZE
decl_stmt|;
DECL|field|inputStreamVersion
specifier|private
name|int
name|inputStreamVersion
init|=
name|DEFAULT_INPUT_STREAM_VERSION
decl_stmt|;
...
decl_stmt|;
DECL|field|autoThrottlingEnabled
specifier|private
name|boolean
name|autoThrottlingEnabled
decl_stmt|;
...
name|setProperty
argument_list|(
literal|"org.eclipse.jetty.util.log.announce"
argument_list|,
literal|"false"
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Extract the directories that should contain block blobs with compaction
name|blockBlobWithCompationDirs
operator|=
name|getDirectorySet
argument_list|(
...
name|debug
argument_list|(
literal|"Block blobs with compaction directories:  {}"
argument_list|,
name|setToString
argument_list|(
name|blockBlobWithCompationDirs
...
block|{
comment|// cannot enable both self-throttling and client-throttling
name|autoThrottlingEnabled
operator|=
literal|false
expr_stmt|;
...
return|;
block|}
comment|/**    * Checks if the given key in Azure Storage should be stored as a block blobs    * with compaction enabled instead of normal block blob.    *    * @param key blob name    * @return true, if the file is in directory with block compaction enabled.    */
DECL|method|isBlockBlobWithCompactionKey (String key)
specifier|public
name|boolean
name|isBlockBlobWithCompactionKey
parameter_list|(
name|String
name|key
parameter_list|)
...
return|;
block|}
DECL|method|matchAsteriskPattern (String pathName, String pattern)
specifier|private
name|boolean
name|matchAsteriskPattern
parameter_list|(
...
continue|continue;
block|}
comment|// ASTERISK_SYMBOL
comment|// 1. * is used in path name: *a/b,a*/b, a/*b, a/b*
if|if
condition|(
name|patternIndex
operator|>
...
continue|continue;
block|}
comment|// 2. * is used as wildcard: */a, a/*/b, a/*
name|patternIndex
operator|++
expr_stmt|;
comment|// find next path separator
while|while
condition|(
name|pathIndex
operator|<
...
block|}
block|}
comment|// Ensure it is not a file/dir which shares same prefix as pattern
comment|// Eg: pattern: /A/B, pathName: /A/BBB should not match
return|return
name|patternIndex
operator|==
name|pattern
...
name|debug
argument_list|(
literal|"Using stream seek algorithm {}"
argument_list|,
name|inputStreamVersion
argument_list|)
expr_stmt|;
...
name|IOException
argument_list|(
literal|"Unknown seek algorithm: "
operator|+
name|inputStreamVersion
argument_list|)
throw|;
