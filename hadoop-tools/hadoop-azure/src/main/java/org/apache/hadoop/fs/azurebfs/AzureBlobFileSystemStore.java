...
specifier|final
name|String
name|TOKEN_DATE_PATTERN
init|=
literal|"yyyy-MM-dd'T'HH:mm:ss.SSSSSSS'Z'"
decl_stmt|;
...
literal|500
decl_stmt|;
DECL|field|GET_SET_AGGREGATE_COUNT
specifier|private
specifier|static
specifier|final
name|int
name|GET_SET_AGGREGATE_COUNT
init|=
literal|2
decl_stmt|;
...
name|startTracking
argument_list|(
literal|"getFilesystemProperties"
argument_list|,
literal|"getFilesystemProperties"
argument_list|)
init|)
...
name|startTracking
argument_list|(
literal|"setFilesystemProperties"
argument_list|,
literal|"setFilesystemProperties"
argument_list|)
init|)
...
block|}
block|}
comment|/**    * @param path The list path.    * @return the entries in the path.    * */
...
return|;
block|}
comment|/**    * @param path Path the list path.    * @param startFrom the entry name that list results should start with.    *                  For example, if folder "/folder" contains four files: "afile", "bfile", "hfile", "ifile".    *                  Then listStatus(Path("/folder"), "hfile") will return "/folder/hfile" and "folder/ifile"    *                  Notice that if startFrom is a non-existent entry name, then the list response contains    *                  all entries after this non-existent entry in lexical order:    *                  listStatus(Path("/folder"), "cfile") will return "/folder/hfile" and "/folder/ifile".    *    * @return the entries in the path start from  "startFrom" in lexical order.    * */
annotation|@
name|InterfaceStability
operator|.
name|Unstable
DECL|method|listStatus (final Path path, final String startFrom)
specifier|public
name|FileStatus
index|[]
name|listStatus
...
name|debug
argument_list|(
literal|"listStatus filesystem: {} path: {}, startFrom: {}"
argument_list|,
name|client
operator|.
name|getFileSystem
...
literal|null
decl_stmt|;
comment|// generate continuation token if a valid startFrom is provided.
if|if
condition|(
name|startFrom
operator|!=
...
argument_list|()
condition|?
name|generateContinuationTokenForXns
argument_list|(
name|startFrom
argument_list|)
else|:
name|generateContinuationTokenForNonXns
argument_list|(
name|path
operator|.
name|isRoot
...
return|;
block|}
comment|// generate continuation token for xns account
DECL|method|generateContinuationTokenForXns (final String firstEntryName)
specifier|private
name|String
name|generateContinuationTokenForXns
parameter_list|(
...
argument_list|)
argument_list|,
literal|"startFrom must be a dir/file name and it can not be a full path"
argument_list|)
expr_stmt|;
name|StringBuilder
name|sb
...
return|;
block|}
comment|// generate continuation token for non-xns account
DECL|method|generateContinuationTokenForNonXns (final String path, final String firstEntryName)
specifier|private
name|String
name|generateContinuationTokenForNonXns
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// Notice: non-xns continuation token requires full path (first "/" is not included) for startFrom
specifier|final
name|String
name|startFrom
init|=
...
name|debug
argument_list|(
literal|"access for filesystem: {}, path: {}, mode: {}"
argument_list|,
name|this
operator|.
name|client
...
name|debug
argument_list|(
literal|"Returning; either check access is not enabled or the account"
operator|+
literal|" used is not namespace enabled"
argument_list|)
expr_stmt|;
return|return;
block|}
...
return|;
block|}
DECL|method|startTracking (String callerName, String calleeName)
specifier|private
name|AbfsPerfInfo
name|startTracking
parameter_list|(
