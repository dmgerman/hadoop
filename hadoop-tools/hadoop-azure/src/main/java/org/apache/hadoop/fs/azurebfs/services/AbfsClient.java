...
parameter_list|)
block|{
comment|// Suppress exception. Failure to init DelegatingSSLSocketFactory would have only performance impact.
block|}
block|}
name|this
operator|.
...
return|;
block|}
comment|/**    * Talks to the server to check whether the permission specified in    * the rwx parameter is present for the path specified in the path parameter.    *    * @param path  Path for which access check needs to be performed    * @param rwx   The permission to be checked on the path    * @return      The {@link AbfsRestOperation} object for the operation    * @throws AzureBlobFileSystemException in case of bad requests    */
DECL|method|checkAccess (String path, String rwx)
specifier|public
name|AbfsRestOperation
name|checkAccess
parameter_list|(
