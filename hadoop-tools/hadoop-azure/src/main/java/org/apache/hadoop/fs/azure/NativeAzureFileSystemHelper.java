...
return|;
block|}
comment|/*    * Determines if a conditional request failed because the blob already    * exists.    *    * @param e - the storage exception thrown by the failed operation.    *    * @return true if a conditional request failed because the blob already    * exists; otherwise, returns false.    */
DECL|method|isBlobAlreadyExistsConflict (StorageException e)
specifier|static
name|boolean
