...
block|}
block|}
comment|/**    * An unexpected HTTP response was raised, such as text coming back    * from what should be an OAuth endpoint.    */
DECL|class|UnexpectedResponseException
specifier|public
specifier|static
class|class
...
name|HttpException
block|{
DECL|method|UnexpectedResponseException (final int httpErrorCode, final String requestId, final String message, final String url, final String contentType, final String body)
specifier|public
name|UnexpectedResponseException
parameter_list|(
specifier|final
...
name|debug
argument_list|(
literal|"Requesting an OAuth token by {} to {}"
argument_list|,
name|httpMethod
argument_list|,
name|authEndpoint
...
condition|)
block|{
comment|// no error stream, try the original input stream
name|stream
operator|=
name|conn
operator|.
...
name|operation
init|=
literal|"AADToken: HTTP connection to "
operator|+
name|authEndpoint
operator|+
literal|" failed for getting token from AzureAD."
decl_stmt|;
name|String
name|logMessage
init|=
...
condition|)
block|{
comment|// 200 is returned by some of the sign-on pages, but can also
comment|// come from proxies, utterly wrong URLs, etc.
throw|throw
operator|new
name|UnexpectedResponseException
argument_list|(
...
operator|+
literal|" Check configuration, URLs and proxy settings."
operator|+
literal|" proxies="
operator|+
name|proxies
...
condition|)
block|{
comment|// the HTTP request returned an empty body
return|return
literal|""
return|;
block|}
