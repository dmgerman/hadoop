...
annotation|@
name|Test
DECL|method|testUpdateUseTrashDeepDirectoryStructureToRemote ()
specifier|public
name|void
name|testUpdateUseTrashDeepDirectoryStructureToRemote
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
name|distCpUpdateUseTrashDeepDirectoryStructure
argument_list|(
name|remoteDir
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * Do a distcp -update -delete -useTrash.    * @param destDir output directory used by the initial distcp    * @return the distcp job    */
DECL|method|distCpUpdateUseTrashDeepDirectoryStructure (final Path destDir)
specifier|protected
name|Job
name|distCpUpdateUseTrashDeepDirectoryStructure
parameter_list|(
...
name|describe
argument_list|(
literal|"Incremental update with deletion-use-trash of missing files"
argument_list|)
expr_stmt|;
name|Path
name|srcDir
...
name|job
init|=
name|distCpUpdateDeleteUseTrash
argument_list|(
name|inputDir
argument_list|,
name|inputDirUnderOutputDir
...
name|remoteFS
argument_list|,
literal|"Path delete does not use trash"
argument_list|,
name|trashRootDir
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * Run distcp -update -delete -useTrash.    * @param srcDir local source directory    * @param destDir remote destination directory.    * @return the completed job    * @throws Exception any failure.    */
DECL|method|distCpUpdateDeleteUseTrash (final Path srcDir, final Path destDir)
specifier|private
name|Job
name|distCpUpdateDeleteUseTrash
parameter_list|(
...
expr_stmt|;
block|}
comment|/**    * Run the distcp job.    * @param options distcp options    * @return the job. It will have already completed.    * @throws Exception failure    */
...
name|describe
argument_list|(
literal|"copy file from local to remote using direct write option"
argument_list|)
expr_stmt|;
name|directWrite
argument_list|(
...
name|describe
argument_list|(
literal|"copy file from local to remote without using direct write "
operator|+
literal|"option"
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|/**    * Executes a test with support for using direct write option.    *    * @param srcFS source FileSystem    * @param srcDir source directory    * @param dstFS destination FileSystem    * @param dstDir destination directory    * @param directWrite whether to use -directwrite option    * @throws Exception if there is a failure    */
DECL|method|directWrite (FileSystem srcFS, Path srcDir, FileSystem dstFS, Path dstDir, boolean directWrite)
specifier|private
name|void
name|directWrite
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// Verify copied file contents
name|verifyFileContents
argument_list|(
name|dstFS
argument_list|,
...
expr_stmt|;
block|}
comment|/**    * Run distcp -direct srcDir destDir.    * @param srcDir local source directory    * @param destDir remote destination directory    * @return the completed job    * @throws Exception any failure.    */
DECL|method|runDistCpDirectWrite (final Path srcDir, final Path destDir)
specifier|private
name|Job
name|runDistCpDirectWrite
parameter_list|(
