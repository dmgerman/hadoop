...
DECL|method|getRenameAndDeleteDiffs (Path targetDir)
specifier|private
name|DiffInfo
index|[]
name|getRenameAndDeleteDiffs
...
return|;
block|}
comment|/**    * For a given source path, get its target path based on the rename item.    * @return target path    */
DECL|method|getTargetPath (Path sourcePath, DiffInfo renameItem)
specifier|private
name|Path
name|getTargetPath
parameter_list|(
...
return|;
block|}
comment|/**    * Prepare the diff list.    * This diff list only includes created or modified files/directories, since    * delete and rename items are synchronized already.    *    * If the parent or self of a source path is renamed, we need to change its    * target path according the correspondent rename item.    * @return a diff list    */
DECL|method|prepareDiffList ()
specifier|public
name|ArrayList
argument_list|<
name|DiffInfo
