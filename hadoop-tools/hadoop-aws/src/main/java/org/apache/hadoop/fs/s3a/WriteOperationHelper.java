...
name|owner
decl_stmt|;
comment|/**    * Invoker for operations; uses the S3A retry policy and calls int    * {@link #operationRetried(String, Exception, int, boolean)} on retries.    */
DECL|field|invoker
specifier|private
specifier|final
name|Invoker
name|invoker
decl_stmt|;
comment|/** Configuration of the owner. This is a reference, not a copy. */
DECL|field|conf
specifier|private
specifier|final
name|Configuration
name|conf
decl_stmt|;
comment|/** Bucket of the owner FS. */
DECL|field|bucket
specifier|private
specifier|final
name|String
...
return|;
block|}
comment|/**    * Finalize a multipart PUT operation.    * This completes the upload, and, if that works, calls    * {@link S3AFileSystem#finishedWrite(String, long, String, String, BulkOperationState)}    * to update the filesystem.    * Retry policy: retrying, translated.    * @param destKey destination of the commit    * @param uploadId multipart operation Id    * @param partETags list of partial uploads    * @param length length of the upload    * @param retrying retrying callback    * @param operationState (nullable) operational state for a bulk update    * @return the result of the operation.    * @throws IOException on problems.    */
annotation|@
name|Retries
operator|.
name|RetryTranslated
DECL|method|finalizeMultipartUpload ( String destKey, String uploadId, List<PartETag> partETags, long length, Retried retrying, @Nullable BulkOperationState operationState)
specifier|private
name|CompleteMultipartUploadResult
...
operator|.
name|RetryTranslated
DECL|method|commitUpload ( String destKey, String uploadId, List<PartETag> partETags, long length, @Nullable BulkOperationState operationState)
specifier|public
name|CompleteMultipartUploadResult
name|commitUpload
parameter_list|(
...
return|;
block|}
comment|/**    * Initiate a commit operation through any metastore.    * @param path path under which the writes will all take place.    * @return an possibly null operation state from the metastore.    * @throws IOException failure to instantiate.    */
...
return|;
block|}
comment|/**    * Get the configuration of this instance; essentially the owning    * filesystem configuration.    * @return the configuration.    */
DECL|method|getConf ()
specifier|public
name|Configuration
name|getConf
...
return|;
block|}
comment|/**    * Create a S3 Select request for the destination path.    * This does not build the query.    * @param path pre-qualified path for query    * @return the request    */
DECL|method|newSelectRequest (Path path)
specifier|public
name|SelectObjectContentRequest
...
return|;
block|}
comment|/**    * Execute an S3 Select operation.    * On a failure, the request is only logged at debug to avoid the    * select exception being printed.    * @param source source for selection    * @param request Select request to issue.    * @param action the action for use in exception creation    * @return response    * @throws IOException failure    */
annotation|@
name|Retries
operator|.
name|RetryTranslated
DECL|method|select ( final Path source, final SelectObjectContentRequest request, final String action)
specifier|public
name|SelectObjectContentResult
name|select
parameter_list|(
...
name|debug
argument_list|(
literal|"Initiating select call {} {}"
argument_list|,
name|source
argument_list|,
name|request
...
name|error
argument_list|(
literal|"Failure of S3 Select request against {}"
argument_list|,
name|source
argument_list|)
expr_stmt|;
...
name|debug
argument_list|(
literal|"S3 Select request against {}:\n{}"
argument_list|,
name|source
argument_list|,
name|SelectBinding
