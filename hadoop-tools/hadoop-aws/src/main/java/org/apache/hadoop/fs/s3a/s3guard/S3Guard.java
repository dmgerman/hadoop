...
return|;
block|}
comment|/**    * Given directory listing metadata from both the backing store and the    * MetadataStore, merge the two sources of truth to create a consistent    * view of the current directory contents, which can be returned to clients.    *    * Also update the MetadataStore to reflect the resulting directory listing.    *    * @param ms MetadataStore to use.    * @param path path to directory    * @param backingStatuses Directory listing from the backing store.    * @param dirMeta  Directory listing from MetadataStore.  May be null.    * @param isAuthoritative State of authoritative mode    * @return Final result of directory listing.    * @throws IOException if metadata store update failed    */
...
block|}
block|}
DECL|method|addAncestors (MetadataStore metadataStore, Path qualifiedPath, String username)
specifier|public
specifier|static
name|void
name|addAncestors
...
block|}
block|}
comment|/**    * This interface is defined for testing purposes.    * TTL can be tested by implementing this interface and setting is as    * {@code S3Guard.ttlTimeProvider}. By doing this, getNow() can return any    * value preferred and flaky tests could be avoided.    */
...
parameter_list|()
function_decl|;
DECL|method|getAuthoritativeDirTtl ()
name|long
...
expr_stmt|;
block|}
DECL|method|listChildrenWithTtl (MetadataStore ms, Path path, ITtlTimeProvider timeProvider)
specifier|public
specifier|static
name|DirListingMetadata
