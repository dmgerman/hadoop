...
block|}
block|}
comment|/**    * Fsck - check for consistency between S3 and the metadatastore.    */
DECL|class|Fsck
specifier|static
class|class
name|Fsck
...
literal|"check"
decl_stmt|;
DECL|field|DDB_MS_CONSISTENCY_FLAG
specifier|public
specifier|static
specifier|final
name|String
...
name|PURPOSE
init|=
literal|"Compares S3 with MetadataStore, and "
operator|+
literal|"returns a failure status if any rules or invariants are violated. "
operator|+
literal|"Only works with DynamoDB metadata stores."
decl_stmt|;
DECL|field|USAGE
specifier|private
specifier|static
...
name|CHECK_FLAG
operator|+
literal|" Check the metadata store for errors, but do "
operator|+
literal|"not fix any issues.\n"
operator|+
literal|"  -"
operator|+
name|DDB_MS_CONSISTENCY_FLAG
operator|+
literal|" Check the dynamodb metadata store "
operator|+
literal|"for internal consistency.\n"
decl_stmt|;
DECL|method|Fsck (Configuration conf)
name|Fsck
parameter_list|(
name|Configuration
name|conf
...
argument_list|()
decl_stmt|;
comment|// check if there's more than one arguments
name|int
name|flags
init|=
literal|0
...
name|invalidArgs
argument_list|(
literal|"There should be only one parameter used for checking."
argument_list|)
throw|;
block|}
name|String
...
name|errorln
argument_list|(
literal|"Failed to initialize S3AFileSystem from path: "
operator|+
name|s3Path
argument_list|)
expr_stmt|;
...
name|s3Path
operator|+
literal|" path uses metadata store: "
operator|+
name|ms
argument_list|)
expr_stmt|;
...
name|NAME
operator|+
literal|" can be only used with a DynamoDB backed s3a bucket."
argument_list|)
expr_stmt|;
name|errorln
argument_list|(
...
name|errorln
argument_list|(
literal|"No supported operation is selected."
argument_list|)
expr_stmt|;
name|errorln
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// We fail if there were compare pairs, as the returned compare pairs
comment|// contain issues.
if|if
condition|(
name|violations
operator|==
...
parameter_list|)
block|{
comment|// Bucket doesn't exist or similar - return code of 44, "404".
name|errorln
argument_list|(
name|e
operator|.
