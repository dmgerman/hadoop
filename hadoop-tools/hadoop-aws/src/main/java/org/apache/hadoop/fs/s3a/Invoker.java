...
expr_stmt|;
block|}
comment|/**    * Execute a void operation with retry processing when doRetry=true, else    * just once.    * @param doRetry true if retries should be performed    * @param action action to execute (used in error messages)    * @param path path of work (used in error messages)    * @param idempotent does the operation have semantics    * which mean that it can be retried even if was already executed?    * @param retrying callback on retries    * @param operation operation to execute    * @throws IOException any IOE raised, or translated exception    */
annotation|@
name|Retries
operator|.
name|RetryTranslated
DECL|method|maybeRetry (boolean doRetry, String action, String path, boolean idempotent, Retried retrying, VoidOperation operation)
specifier|public
name|void
name|maybeRetry
parameter_list|(
...
expr_stmt|;
block|}
comment|/**    * Execute a void operation with the default retry callback invoked when    * doRetry=true, else just once.    * @param doRetry true if retries should be performed    * @param action action to execute (used in error messages)    * @param path path of work (used in error messages)    * @param idempotent does the operation have semantics    * which mean that it can be retried even if was already executed?    * @param operation operation to execute    * @throws IOException any IOE raised, or translated exception    */
annotation|@
name|Retries
operator|.
name|RetryTranslated
DECL|method|maybeRetry ( boolean doRetry, String action, String path, boolean idempotent, VoidOperation operation)
specifier|public
name|void
name|maybeRetry
parameter_list|(
...
return|;
block|}
comment|/**    * Execute a function with retry processing when doRetry=true, else just once.    * Uses {@link #once(String, String, Operation)} as the inner    * invocation mechanism before retry logic is performed.    * @param<T> type of return value    * @param doRetry true if retries should be performed    * @param action action to execute (used in error messages)    * @param path path of work (used in error messages)    * @param idempotent does the operation have semantics    * which mean that it can be retried even if was already executed?    * @param retrying callback on retries    * @param operation operation to execute    * @return the result of the call    * @throws IOException any IOE raised, or translated exception    */
annotation|@
name|Retries
operator|.
name|RetryTranslated
DECL|method|maybeRetry ( boolean doRetry, String action, @Nullable String path, boolean idempotent, Retried retrying, Operation<T> operation)
specifier|public
parameter_list|<
name|T
parameter_list|>
