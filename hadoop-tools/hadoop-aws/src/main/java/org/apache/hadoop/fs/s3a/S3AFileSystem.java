...
return|;
block|}
comment|/**    * Get the region of a bucket.    * @param bucketName the name of the bucket    * @return the region in which a bucket is located    * @throws IOException on any failure.    */
...
block|}
block|}
comment|/**    * List the next set of objects.    * @param request last list objects request to continue    * @param prevResult last paged result to continue from    * @return the next result object    */
...
return|;
block|}
comment|/**    * Raw version of {@link FileSystem#exists(Path)} which uses S3 only:    * S3Guard MetadataStore, if any, will be skipped.    * @return true if path exists in S3    */
