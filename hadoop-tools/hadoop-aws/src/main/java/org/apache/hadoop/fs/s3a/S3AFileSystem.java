...
name|credentials
init|=
name|getAWSCredentialsProvider
argument_list|(
name|name
argument_list|,
name|conf
...
expr_stmt|;
block|}
DECL|method|initAmazonS3Client (Configuration conf, AWSCredentialsProvider credentials, ClientConfiguration awsConf)
specifier|private
name|void
name|initAmazonS3Client
parameter_list|(
...
return|;
block|}
comment|/**    * Create the standard credential provider, or load in one explicitly    * identified in the configuration.    * @param binding the S3 binding/bucket.    * @param conf configuration    * @return a credential provider    * @throws IOException on any problem. Class construction issues may be    * nested inside the IOE.    */
DECL|method|getAWSCredentialsProvider (URI binding, Configuration conf)
specifier|private
name|AWSCredentialsProvider
name|getAWSCredentialsProvider
parameter_list|(
...
return|;
block|}
comment|/**    * Upload part of a multi-partition file.    * Increments the write and put counters    * @param request request    * @return the result of the operation.    */
