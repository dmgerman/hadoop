...
name|assumeTrue
argument_list|(
literal|"No metadata store in test filesystem"
argument_list|,
name|getFileSystem
argument_list|()
operator|.
...
block|}
block|}
comment|/**    * Delete an array of paths; log exceptions.    * @param paths paths to delete    */
DECL|method|deletePathsQuietly (Path...paths)
specifier|private
name|void
name|deletePathsQuietly
parameter_list|(
name|Path
modifier|...
name|paths
...
name|describedAs
argument_list|(
literal|"Expected deleted files to be excluded"
argument_list|)
operator|.
name|doesNotContain
argument_list|(
...
name|describe
argument_list|(
literal|"Verify that delete adds tombstones which block entries"
operator|+
literal|" returned in (inconsistent) listings"
argument_list|)
expr_stmt|;
comment|// Test only implemented for v2 S3 list API
name|assumeV2ListAPI
argument_list|()
expr_stmt|;
name|S3AFileSystem
name|fs
...
name|path
argument_list|(
literal|"testInconsistentS3ClientDeletes-"
operator|+
name|DEFAULT_DELAY_KEY_SUBSTRING
argument_list|)
decl_stmt|;
...
name|info
argument_list|(
literal|"Listing objects before executing delete()"
argument_list|)
expr_stmt|;
name|ListObjectsV2Result
...
name|info
argument_list|(
literal|"Deleting the directory {}"
argument_list|,
name|root
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Delete completed; listing results which must exclude deleted"
operator|+
literal|" paths"
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Executing Deep listing"
argument_list|)
expr_stmt|;
name|ListObjectsV2Result
...
expr_stmt|;
block|}
comment|/**    * Require the v2 S3 list API.    */
DECL|method|assumeV2ListAPI ()
specifier|protected
name|void
name|assumeV2ListAPI
parameter_list|()
...
expr_stmt|;
block|}
comment|/**    * Verify that delayed S3 listings doesn't stop the FS from deleting    * a directory tree. This has not always been the case; this test    * verifies the fix and prevents regression.    */
annotation|@
name|Test
DECL|method|testDeleteUsesS3Guard ()
specifier|public
name|void
name|testDeleteUsesS3Guard
parameter_list|()
throws|throws
name|Throwable
block|{
name|describe
argument_list|(
literal|"Verify that delete() uses S3Guard to get a consistent"
operator|+
literal|" listing of its directory structure"
argument_list|)
expr_stmt|;
name|assumeV2ListAPI
argument_list|()
...
expr_stmt|;
block|}
comment|// we now expect the listing to miss these
name|String
name|key
init|=
name|fs
...
literal|"/"
decl_stmt|;
comment|// verify that the inconsistent listing does not show these
name|LOG
operator|.
name|info
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// now go through every file and verify that it is not there.
comment|// if you comment out the delete above and run this test case,
comment|// the assertion will fail; this is how the validity of the assertions
comment|// were verified.
name|clearInconsistency
argument_list|(
name|fs
argument_list|)
...
expr_stmt|;
block|}
DECL|method|objectsToPaths (ListObjectsV2Result r)
specifier|private
name|List
argument_list|<
name|Path
...
return|;
block|}
comment|/**    * Tests that the file's eTag and versionId are preserved in recursive    * listings.    */
annotation|@
name|Test
DECL|method|testListingReturnsVersionMetadata ()
specifier|public
name|void
name|testListingReturnsVersionMetadata
parameter_list|()
throws|throws
name|Throwable
block|{
...
expr_stmt|;
block|}
comment|// get individual file status
name|FileStatus
index|[]
name|fileStatuses
init|=
...
argument_list|()
decl_stmt|;
comment|// get status through recursive directory listing
name|RemoteIterator
argument_list|<
name|LocatedFileStatus
argument_list|>
...
argument_list|)
expr_stmt|;
comment|// ensure eTag and versionId are preserved in directory listing
