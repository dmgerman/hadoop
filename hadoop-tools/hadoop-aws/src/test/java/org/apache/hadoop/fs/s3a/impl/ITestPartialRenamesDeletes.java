...
expr_stmt|;
block|}
comment|/**    * Directory cleanup includes pruning everything under the path.    * This ensures that any in the tree from failed tests don't fill up    * the store with many, many, deleted entries.    * @throws IOException failure.    */
annotation|@
name|Override
...
name|warn
argument_list|(
literal|"When pruning the test directory {}"
argument_list|,
name|path
argument_list|,
name|e
...
argument_list|)
expr_stmt|;
comment|// do this prune in the test as well as teardown, so that the test
comment|// reporting includes it in the runtime of a successful run.
name|prune
argument_list|(
name|basePath
argument_list|)
...
expr_stmt|;
block|}
comment|/**    * Prune the store for everything under the test path.    * @param path path.    * @throws IOException on failure.    */
DECL|method|prune (Path path)
specifier|private
name|void
name|prune
parameter_list|(
