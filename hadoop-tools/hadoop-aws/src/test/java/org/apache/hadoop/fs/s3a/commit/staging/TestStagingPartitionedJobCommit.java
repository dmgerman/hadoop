...
expr_stmt|;
block|}
comment|/**      * Generate pending uploads to commit.      * This is quite complex as the mock pending uploads need to be saved      * to a filesystem for the next stage of the commit process.      * To simulate multiple commit, more than one .pendingset file is created,      * @param context job context      * @return an active commit containing a list of paths to valid pending set      * file.      * @throws IOException IO failure      */
annotation|@
name|Override
...
argument_list|)
decl_stmt|;
comment|// need to create some pending entries.
for|for
control|(
name|String
name|dateint
...
argument_list|)
expr_stmt|;
comment|// register the upload so commit operations are not rejected
name|getMockResults
argument_list|()
operator|.
name|addUpload
