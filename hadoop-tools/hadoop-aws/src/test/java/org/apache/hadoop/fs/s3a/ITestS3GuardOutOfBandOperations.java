...
expr_stmt|;
block|}
comment|/**    * Tests that tombstone expiry is implemented. If a file is created raw    * while the tombstone exist in ms for with the same name then S3Guard will    * check S3 for the file.    *    * Seq: create guarded; delete guarded; create raw (same path); read guarded;    * This will fail if no tombstone expiry is set    *    * @throws Exception    */
annotation|@
name|Test
...
block|}
block|}
comment|/**    * Test that a tombstone won't hide an entry after it's expired in the    * listing.    */
annotation|@
name|Test
DECL|method|testRootTombstones ()
specifier|public
name|void
name|testRootTombstones
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// the rawFS will include the file=
name|LambdaTestUtils
operator|.
name|eventually
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// it will be hidden because of the tombstone
name|checkListingDoesNotContainPath
argument_list|(
name|guardedFs
argument_list|,
