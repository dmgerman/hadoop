...
decl_stmt|;
DECL|field|WEB_UGI_PROPERTY_NAME
specifier|final
specifier|static
specifier|public
name|String
name|WEB_UGI_PROPERTY_NAME
init|=
name|DFSConfigKeys
operator|.
...
return|;
block|}
comment|/**    * See    * {@link JspHelper#getUGI(ServletContext, HttpServletRequest, Configuration)}    * , ServletContext is passed as null.    */
...
return|;
block|}
comment|/**    * Get {@link UserGroupInformation} and possibly the delegation token out of    * the request.    * @param context the ServletContext that is serving this request.    * @param request the http request    * @return a new user from the request    * @throws AccessControlException if the request has no token    */
...
name|info
argument_list|(
literal|"Setting service in token: "
operator|+
operator|new
name|Text
argument_list|(
...
name|IOException
argument_list|(
literal|"Security enabled but user not "
operator|+
literal|"authenticated by filter"
argument_list|)
throw|;
block|}
name|ugi
...
block|{
comment|// Security's not on, pull from url
name|String
name|user
init|=
name|request
