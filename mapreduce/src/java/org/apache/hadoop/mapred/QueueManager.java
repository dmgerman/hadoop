...
return|;
block|}
comment|/**    * ONLY FOR TESTING - Do not use in production code.    * This method is used for setting up of leafQueues only.    * We are not setting the hierarchy here.    *    * @param queues    */
DECL|method|setQueues (Queue[] queues)
specifier|synchronized
name|void
name|setQueues
parameter_list|(
...
expr_stmt|;
block|}
comment|/**    * Return an array of {@link JobQueueInfo} objects for the root    * queues configured in the system.    *<p/>    * Root queues are queues that are at the top-most level in the    * hierarchy of queues in mapred-queues.xml, or they are the queues    * configured in the mapred.queue.names key in mapred-site.xml.    *    * @return array of JobQueueInfo objects for root level queues.    */
...
return|;
block|}
comment|/**    * Get the complete hierarchy of children for queue    * queueName    *    * @param queueName    * @return    */
...
block|}
block|}
comment|/**    * Used only for testing purposes .    * This method is unstable as refreshQueues would leave this    * data structure in unstable state.    *    * @param queueName    * @return    */
...
return|;
block|}
comment|/**    * Returns the specific queue ACL for the given queue.    * Returns null if the given queue does not exist or the acl is not    * configured for that queue.    * If acls are disabled(mapreduce.cluster.acls.enabled set to false), returns    * ACL with all users.    */
DECL|method|getQueueACL (String queueName, QueueACL qACL)
specifier|synchronized
name|AccessControlList
name|getQueueACL
parameter_list|(
