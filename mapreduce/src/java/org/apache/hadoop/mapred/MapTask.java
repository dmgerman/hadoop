...
argument_list|)
expr_stmt|;
comment|// split.dta/split.info files are used only by IsolationRunner.
comment|// Write the split file to the local disk if it is a normal map task (not a
comment|// job-setup or a job-cleanup task) and if the user wishes to run
comment|// IsolationRunner either by setting keep.failed.tasks.files to true or by
comment|// using keep.tasks.files.pattern
if|if
condition|(
...
condition|)
block|{
comment|// localize the split meta-information
name|Path
name|localSplitMeta
init|=
operator|new
...
name|debug
argument_list|(
literal|"Writing local split to "
operator|+
name|localSplitMeta
argument_list|)
expr_stmt|;
...
annotation|@
name|Override
DECL|method|createRunner (TaskTracker tracker, TaskTracker.TaskInProgress tip)
specifier|public
name|TaskRunner
name|createRunner
parameter_list|(
...
literal|1
decl_stmt|;
DECL|method|TrackedRecordReader (RecordReader<K,V> raw, TaskReporter reporter)
name|TrackedRecordReader
parameter_list|(
name|RecordReader
argument_list|<
...
literal|1
decl_stmt|;
DECL|method|SkippingRecordReader (RecordReader<K,V> raw, TaskUmbilicalProtocol umbilical, TaskReporter reporter)
name|SkippingRecordReader
parameter_list|(
name|RecordReader
argument_list|<
...
name|reporter
decl_stmt|;
DECL|method|NewTrackingRecordReader (org.apache.hadoop.mapreduce.RecordReader<K,V> real, TaskReporter reporter)
name|NewTrackingRecordReader
parameter_list|(
name|org
operator|.
...
literal|"unchecked"
argument_list|)
DECL|method|DirectMapOutputCollector (TaskUmbilicalProtocol umbilical, JobConf job, TaskReporter reporter)
specifier|public
name|DirectMapOutputCollector
parameter_list|(
name|TaskUmbilicalProtocol
...
literal|"unchecked"
argument_list|)
DECL|method|MapOutputBuffer (TaskUmbilicalProtocol umbilical, JobConf job, TaskReporter reporter )
specifier|public
name|MapOutputBuffer
parameter_list|(
name|TaskUmbilicalProtocol
...
return|;
block|}
comment|/**      * Swap logical indices st i, j MOD offset capacity.      * @see IndexedSortable#swap      */
...
argument_list|)
decl_stmt|;
comment|// calculate the length of the value
name|int
name|vallen
init|=
operator|(
