...
condition|)
block|{
comment|// NLineInputFormat uses LineRecordReader, which always reads
comment|// (and consumes) at least one character out of its upper split
comment|// boundary. So to make sure that each mapper gets N lines, we
comment|// move back the upper split limits of each split
comment|// by one character here.
if|if
condition|(
name|begin
operator|==
