...
argument_list|)
expr_stmt|;
comment|// We use a pre and post job object here because we need the post job object
comment|// to get the new values set during uploadResources, but we need the pre job
comment|// to set the job jar because JobResourceUploader#uploadJobJar uses the Job
comment|// interface not the JobConf. The post job is automatically created in
comment|// validateResourcePaths.
name|Job
name|jobPre
init|=
name|Job
...
expr_stmt|;
block|}
DECL|method|uploadResources (JobResourceUploader uploader, JobConf jConf, Job job)
specifier|private
name|void
name|uploadResources
parameter_list|(
...
expr_stmt|;
block|}
DECL|method|validateResourcePaths (JobConf jConf, String[] expectedFiles, String[] expectedArchives, String expectedJobJar, Job preJob)
specifier|private
name|void
name|validateResourcePaths
parameter_list|(
