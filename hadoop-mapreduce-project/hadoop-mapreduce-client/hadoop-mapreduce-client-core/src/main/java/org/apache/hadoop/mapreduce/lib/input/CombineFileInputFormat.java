...
return|;
block|}
comment|// Convert them to Paths first. This is a costly operation and
comment|// we should do it first, otherwise we will incur doing it multiple
comment|// times, one time each for each pool in the next loop.
name|List
argument_list|<
name|Path
argument_list|>
...
DECL|method|getMoreSplits (JobContext job, Path[] paths, long maxSize, long minSizeNode, long minSizeRack, List<InputSplit> splits)
specifier|private
name|void
name|getMoreSplits
parameter_list|(
...
DECL|method|OneFileInfo (Path path, Configuration conf, boolean isSplitable, HashMap<String, List<OneBlockInfo>> rackToBlocks, HashMap<OneBlockInfo, String[]> blockToNodes, HashMap<String, List<OneBlockInfo>> nodeToBlocks, HashMap<String, Set<String>> rackToNodes, long maxSize)
name|OneFileInfo
parameter_list|(
name|Path
name|path
