...
name|FAILED
decl_stmt|;
comment|/* notes that get delegation token was called. Again this is hack for oozie     * to make sure we add history server delegation tokens to the credentials    *  for the job. Since the api only allows one delegation token to be returned,     *  we have to add this hack.    */
DECL|field|getDelegationTokenCalled
specifier|private
name|boolean
...
name|DelegationTokenIdentifier
operator|.
name|MAPREDUCE_DELEGATION_KIND
operator|.
name|equals
argument_list|(
name|kind
...
block|}
block|}
DECL|method|getJobStatusUsingCluster (final JobID jobId)
specifier|private
name|JobStatus
name|getJobStatusUsingCluster
parameter_list|(
specifier|final
name|JobID
name|jobId
...
return|;
block|}
comment|/**    * Get {@link JobStatus} of a job. Returns null if the id does not correspond    * to any known job.    *     * @param jobid    *          the jobid of the job.    * @return the {@link JobStatus} object to retrieve the job stats, null if the    *<code>jobid</code> doesn't correspond to any known job.    * @throws IOException    */
