...
block|}
block|}
comment|/**    * Display the information about a job's tasks, of a particular type and    * in a particular state    *     * @param job the job    * @param type the type of the task (map/reduce/setup/cleanup)    * @param state the state of the task     * (pending/running/completed/failed/killed)    */
...
name|println
argument_list|(
literal|"JobId\tState\tStartTime\t"
operator|+
literal|"UserName\tQueue\tPriority\tMaps\tReduces\tUsedContainers\t"
operator|+
literal|"RsvdContainers\tUsedMem\tRsvdMem\tNeededMem\tAM info"
argument_list|)
expr_stmt|;
for|for
control|(
...
name|printf
argument_list|(
literal|"%s\t%s\t%d\t%s\t%s\t%s\t%d\t%d\t%d\t%d\t%dM\t%dM\t%dM\t%s\n"
argument_list|,
name|job
operator|.
name|getJobID
