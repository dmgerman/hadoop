...
condition|)
block|{
comment|// "/" needs special treatment
name|root
operator|=
literal|true
expr_stmt|;
...
block|{
comment|// If originalPath ends in a "/", then remove it so
comment|// that originalPath.getName() does not return an empty string
name|String
name|uriString
init|=
name|originalPath
...
return|;
block|}
DECL|method|disableErasureCodingForPath (Path path)
specifier|private
name|void
name|disableErasureCodingForPath
parameter_list|(
...
name|debug
argument_list|(
literal|"Ignore disabling erasure coding for path {} because method "
operator|+
literal|"disableErasureCodingForPath doesn't exist, probably "
operator|+
literal|"talking to a lower version HDFS."
argument_list|,
name|path
operator|.
name|toString
