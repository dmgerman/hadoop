...
decl_stmt|;
comment|/* usually is false unless the jobclient get delegation token is     *  called. This is a hack wherein we do return a token from RM     *  on getDelegationtoken but due to the restricted api on jobclient    *  we just add a job history DT token when submitting a job.    */
DECL|field|DEFAULT_HS_DELEGATION_TOKEN_REQUIRED
specifier|private
specifier|static
specifier|final
name|boolean
name|DEFAULT_HS_DELEGATION_TOKEN_REQUIRED
init|=
literal|false
decl_stmt|;
