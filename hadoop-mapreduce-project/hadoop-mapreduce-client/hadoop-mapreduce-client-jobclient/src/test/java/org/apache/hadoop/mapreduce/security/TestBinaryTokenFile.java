...
argument_list|)
expr_stmt|;
comment|// NB: the MRJobConfig.MAPREDUCE_JOB_CREDENTIALS_BINARY key now gets deleted from config,
comment|// so it's not accessible in the job's config. So, we use another key to pass the file name into the job configuration:
name|job
operator|.
name|getConfiguration
argument_list|()
