...
block|}
block|}
comment|// This is to test a race condition where JobEventType.JOB_KILL is generated
comment|// right after TaskAttemptEventType.TA_DONE is generated.
comment|// TaskImpl's state machine might receive both T_ATTEMPT_SUCCEEDED
comment|// and T_ATTEMPT_KILLED from the same attempt.
annotation|@
name|Test
...
argument_list|)
expr_stmt|;
comment|// The order in the dispatch event queue, from the oldest to the newest
comment|// TA_DONE
comment|// JOB_KILL
comment|// CONTAINER_REMOTE_CLEANUP ( from TA_DONE's handling )
comment|// T_KILL ( from JOB_KILL's handling )
comment|// TA_CONTAINER_CLEANED ( from CONTAINER_REMOTE_CLEANUP's handling )
comment|// TA_KILL ( from T_KILL's handling )
comment|// T_ATTEMPT_SUCCEEDED ( from TA_CONTAINER_CLEANED's handling )
comment|// T_ATTEMPT_KILLED ( from TA_KILL's handling )
comment|// Finish map
name|app
operator|.
name|getContext
