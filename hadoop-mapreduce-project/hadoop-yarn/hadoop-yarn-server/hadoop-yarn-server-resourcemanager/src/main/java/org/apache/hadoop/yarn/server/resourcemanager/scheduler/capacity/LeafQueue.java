...
expr_stmt|;
block|}
DECL|method|setupQueueConfigs ( float capacity, float absoluteCapacity, float maxCapacity, float absoluteMaxCapacity, int userLimit, float userLimitFactor, int maxApplications, int maxApplicationsPerUser, QueueState state, Map<QueueACL, AccessControlList> acls)
specifier|private
...
condition|)
block|{
comment|// First: Do we need containers on this rack?
if|if
condition|(
name|rackLocalRequest
operator|!=
