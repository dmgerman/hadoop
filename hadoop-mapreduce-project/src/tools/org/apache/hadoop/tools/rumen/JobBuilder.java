...
argument_list|(
literal|"JobBuilder.process(HistoryEvent event) called after LoggedJob built"
argument_list|)
throw|;
block|}
...
name|IllegalStateException
argument_list|(
literal|"JobBuilder.process(Properties conf) called after LoggedJob built"
argument_list|)
throw|;
block|}
...
expr_stmt|;
block|}
comment|/**    * Request the builder to build the final object. Once called, the    * {@link JobBuilder} would accept no more events or job-conf properties.    *     * @return Parsed {@link LoggedJob} object.    */
DECL|method|build ()
specifier|public
name|LoggedJob
name|build
...
argument_list|)
expr_stmt|;
comment|// job queue name is set when conf file is processed.
comment|// See JobBuilder.process(Properties) method for details.
block|}
