...
expr_stmt|;
block|}
comment|/**    * Tries to get the local host IP Address as trace prefix    * for creating trace IDs, otherwise uses a random UUID for it.    */
DECL|method|getTracePrefix ()
specifier|private
specifier|static
name|String
name|getTracePrefix
...
return|;
block|}
comment|/**    * We create a trace ID to make it easy to debug issues.    * A trace ID is in IPAddress:UserName:VolumeName:blockID:second format.    *    * This will get written down on the data node if we get any failures, so    * with this trace ID we can correlate cBlock failures across machines.    *    * @param blockID - Block ID    * @return trace ID    */
DECL|method|getTraceID (File dbPath, long blockID)
specifier|public
name|String
name|getTraceID
parameter_list|(
