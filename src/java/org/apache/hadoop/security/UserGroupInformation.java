...
name|registry
argument_list|,
literal|"Rate of successful kerberos logins and time taken in milliseconds"
argument_list|)
expr_stmt|;
name|loginFailure
operator|=
...
name|registry
argument_list|,
literal|"Rate of failed kerberos logins and time taken in milliseconds"
argument_list|)
expr_stmt|;
specifier|final
name|MetricsContext
...
parameter_list|)
block|{
comment|// If no security configuration is on the classpath, then
comment|// we catch this exception, and we don't need to delegate
comment|// to anyone
block|}
if|if
condition|(
...
name|info
argument_list|(
literal|"JAAS Configuration already set up for Hadoop, not re-installing."
argument_list|)
expr_stmt|;
block|}
else|else
...
name|parent
decl_stmt|;
DECL|method|HadoopConfiguration (javax.security.auth.login.Configuration parent)
name|HadoopConfiguration
parameter_list|(
name|javax
operator|.
