...
expr_stmt|;
block|}
comment|/**    * Returns the raw bytes; however, only data up to {@link #getLength()} is    * valid.    */
...
return|;
block|}
comment|/**    * Read a string, but check it for sanity. The format consists of a vint    * followed by the given number of bytes.    * @param in the stream to read from    * @param maxLength the largest acceptable length of string    * @return the bytes as a string    * @throws IOException if reading from the DataInput fails    * @throws IllegalArgumentException if the string length is negative or     *         larger than maxSize. Only the vint is read.    */
