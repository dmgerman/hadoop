...
expr_stmt|;
block|}
comment|/**    * Get blocks in the specified range.    * Fetch them from the namenode if not cached.    *     * @param offset    * @param length    * @return consequent segment of located blocks    * @throws IOException    */
...
argument_list|()
decl_stmt|;
comment|/* if requested length is greater than current file length        * then, it could possibly be from the current block being        * written to. First get the finalized block range and then        * if necessary, get the length of last block being written        * to.        */
if|if
condition|(
name|readPastEnd
condition|)
...
argument_list|)
expr_stmt|;
comment|/* requested length is greater than what finalized blocks         * have.        */
if|if
condition|(
name|readPastEnd
condition|)
