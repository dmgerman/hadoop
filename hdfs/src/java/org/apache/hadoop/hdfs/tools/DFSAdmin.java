...
decl_stmt|;
comment|//
comment|// If we are waiting for safemode to exit, then poll and
comment|// sleep till we are out of safemode.
comment|//
if|if
condition|(
name|waitExitSafe
...
operator|+
literal|"hadoop dfsadmin [-report] [-safemode<enter | leave | get | wait>]\n"
operator|+
...
name|report
init|=
literal|"-report: \tReports basic filesystem information and statistics.\n"
decl_stmt|;
name|String
name|safemode
init|=
...
init|=
literal|"-upgradeProgress<status|details|force>: \n"
operator|+
literal|"\t\trequest current distributed upgrade status, \n"
operator|+
literal|"\t\ta detailed status or force the upgrade to proceed.\n"
decl_stmt|;
name|String
name|metaSave
init|=
...
return|;
block|}
comment|/**    * Command to request current distributed upgrade status,     * a detailed status, or to force the upgrade to proceed.    *     * Usage: java DFSAdmin -upgradeProgress [status | details | force]    * @exception IOException     */
DECL|method|upgradeProgress (String[] argv, int idx)
specifier|public
name|int
name|upgradeProgress
parameter_list|(
