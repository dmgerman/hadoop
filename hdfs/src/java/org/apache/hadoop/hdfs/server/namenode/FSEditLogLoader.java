...
literal|"deprecation"
argument_list|)
DECL|method|loadEditRecords (int logVersion, DataInputStream in, Checksum checksum, boolean closeOnExit, long expectedStartingTxId)
name|int
name|loadEditRecords
parameter_list|(
name|int
...
block|}
block|}
comment|/**    * Return the number of valid transactions in the file. If the file is    * truncated during the header, returns a value indicating that there are    * 0 valid transactions.    * @throws IOException if the file cannot be read due to an IO error (eg    *                     if the log does not exist)    */
DECL|method|validateEditLog (File f)
specifier|static
...
name|debug
argument_list|(
literal|"Unable to read header from "
operator|+
name|f
operator|+
literal|" -> no valid transactions in this file."
argument_list|)
expr_stmt|;
return|return
operator|new
