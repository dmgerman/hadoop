...
DECL|method|getFiles (NameNodeDirType dirType, String fileName)
name|List
argument_list|<
name|File
argument_list|>
...
block|}
block|}
comment|/**    * Iterate over all of the storage dirs, reading their contents to determine    * their layout versions. Returns an FSImageStorageInspector which has    * inspected each directory.    *     *<b>Note:</b> this can mutate the storage info fields (ctime, version, etc).    * @throws IOException if no valid storage dirs are found    */
...
name|maxLayoutVersion
assert|;
comment|// If we have any storage directories with the new layout version
...
name|warn
argument_list|(
literal|"Ignoring one or more storage directories with old layouts"
argument_list|)
expr_stmt|;
block|}
block|}
