...
begin_comment
comment|/**  * This class generates the data that is needed to be displayed on cluster web   * console by connecting to each namenode through JMX.  */
end_comment
...
literal|"Dead"
decl_stmt|;
comment|/**    * JSP helper function that generates cluster health report.  When     * encountering exception while getting Namenode status, the exception will     * be listed in the page with corresponding stack trace.    */
...
return|;
block|}
comment|/**    * Helper function that generates the decommissioning report.    */
...
return|;
block|}
comment|/**    * Class for connecting to Namenode over JMX and get attributes    * exposed by the MXBean.    */
...
name|mxbeanProxy
operator|=
name|getNamenodeMxBean
argument_list|()
expr_stmt|;
block|}
DECL|method|getNamenodeMxBean ()
specifier|private
name|NameNodeMXBean
name|getNamenodeMxBean
parameter_list|()
...
name|MalformedObjectNameException
block|{
comment|// Get an MBeanServerConnection on the remote VM.
name|MBeanServerConnection
name|remote
init|=
name|connector
...
return|;
block|}
comment|/**      * Process JSON string returned from JMX connection to get the number of      * live datanodes.      *       * @param json JSON output from JMX call that contains live node status.      * @param nn namenode status to return information in      */
...
block|}
block|}
comment|/**      * Count the number of dead datanode based on the JSON string returned from      * JMX call.      *       * @param nn namenode      * @param json JSON string returned from JMX call      */
...
return|;
block|}
comment|/**      * Connect to namenode to get decommission node information.      * @param statusMap data node status map      * @param connector JMXConnector      */
DECL|method|getDecomNodeInfoForReport ( Map<String, Map<String, String>> statusMap)
specifier|private
name|void
name|getDecomNodeInfoForReport
parameter_list|(
...
block|}
block|}
comment|/**      * Process the JSON string returned from JMX connection to get the dead      * datanode information. Store the information into datanode status map and      * Decommissionnode.      *       * @param statusMap map with key being datanode, value being an      *          inner map (key:namenode, value:decommisionning state).      * @param host datanode hostname      * @param decomnode      * @param json      * @throws IOException      */
...
block|}
block|}
comment|/**      * We process the JSON string returned from JMX connection to get the      * decommisioning datanode information.      *       * @param dataNodeStatusMap map with key being datanode, value being an      *          inner map (key:namenode, value:decommisionning state).      * @param host datanode      * @param decomnode Decommissionnode      * @param json JSON string returned from JMX connection      */
...
parameter_list|)
block|{
comment|// log failure of close jmx connection
name|LOG
operator|.
name|warn
argument_list|(
literal|"Unable to close JMX connection. "
operator|+
name|StringUtils
operator|.
