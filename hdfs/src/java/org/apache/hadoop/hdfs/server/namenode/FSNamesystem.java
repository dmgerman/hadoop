...
decl_stmt|;
comment|// FSNamesystemMetrics counter variables
...
literal|null
decl_stmt|;
comment|// HeartbeatMonitor thread
DECL|field|lmthread
specifier|public
name|Daemon
name|lmthread
...
literal|true
decl_stmt|;
DECL|field|systemStart
name|long
name|systemStart
init|=
literal|0
decl_stmt|;
comment|// heartbeatRecheckInterval is how often namenode checks for expired datanodes
DECL|field|heartbeatRecheckInterval
specifier|private
name|long
name|heartbeatRecheckInterval
decl_stmt|;
comment|// heartbeatExpireInterval is how long namenode waits for datanode to report
comment|// heartbeat
...
decl_stmt|;
comment|//resourceRecheckInterval is how often namenode checks for the disk space availability
...
name|fsLock
decl_stmt|;
comment|/**    * FSNamesystem constructor.    */
DECL|method|FSNamesystem (Configuration conf)
name|FSNamesystem
parameter_list|(
name|Configuration
name|conf
...
block|}
block|}
comment|/**    * Initialize FSNamesystem.    */
DECL|method|initialize (Configuration conf, FSImage fsImage)
specifier|private
name|void
name|initialize
parameter_list|(
...
block|}
block|}
DECL|method|activateSecretManager ()
name|void
...
block|}
block|}
comment|/**    * Activate FSNamesystem daemons.    */
...
argument_list|)
expr_stmt|;
comment|/* If the dns to switch mapping supports cache, resolve network      * locations of those hosts in the include list,       * and store the mapping in the cache; so future calls to resolve      * will be fast.      */
if|if
condition|(
name|dnsToSwitchMapping
operator|instanceof
...
return|;
block|}
comment|/**    * dirs is a list of directories where the filesystem directory state     * is stored    */
DECL|method|FSNamesystem (FSImage fsImage, Configuration conf)
name|FSNamesystem
parameter_list|(
name|FSImage
name|fsImage
...
expr_stmt|;
block|}
comment|/**    * Create FSNamesystem for {@link BackupNode}.    * Should do everything that would be done for the NameNode,    * except for loading the image.    *     * @param bnImage {@link BackupImage}    * @param conf configuration    * @throws IOException    */
DECL|method|FSNamesystem (Configuration conf, BackupImage bnImage)
name|FSNamesystem
parameter_list|(
name|Configuration
name|conf
...
block|}
block|}
comment|/**    * Initializes some of the members from configuration    */
DECL|method|setConfigurationParameters (Configuration conf)
specifier|private
name|void
name|setConfigurationParameters
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|//use conf value if it is set.
name|this
operator|.
name|blockInvalidateLimit
operator|=
...
name|info
argument_list|(
literal|"isBlockTokenEnabled="
operator|+
name|isBlockTokenEnabled
operator|+
literal|" blockKeyUpdateInterval="
operator|+
name|blockKeyUpdateInterval
operator|/
operator|(
...
operator|)
operator|+
literal|" min(s), blockTokenLifetime="
operator|+
name|blockTokenLifetime
operator|/
operator|(
...
name|warn
argument_list|(
literal|"Exception shutting down FSNamesystem"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
argument_list|(
literal|"Unexpected exception.  Got getBlocks message for datanode "
operator|+
name|datanode
operator|.
name|getName
argument_list|()
operator|+
literal|", but there is no info for it"
argument_list|)
throw|;
block|}
name|int
...
block|}
DECL|method|getBlockLocationsInternal (INodeFile inode, long offset, long length, boolean needBlockToken)
name|LocatedBlocks
name|getBlockLocationsInternal
parameter_list|(
name|INodeFile
...
return|;
block|}
DECL|method|setReplicationInternal (String src, short replication)
specifier|private
name|boolean
name|setReplicationInternal
parameter_list|(
...
literal|null
condition|)
comment|// file not found or is a directory
return|return
literal|false
return|;
name|int
...
name|info
argument_list|(
literal|"Reducing replication for file "
operator|+
name|src
operator|+
literal|". New replication is "
operator|+
name|replication
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Increasing replication for file "
operator|+
name|src
operator|+
literal|". New replication is "
...
DECL|method|markBlockAsCorrupt (ExtendedBlock blk, DatanodeInfo dn)
specifier|public
name|void
...
argument_list|)
expr_stmt|;
comment|// update lease with new filename
return|return
literal|true
return|;
block|}
...
condition|)
block|{
comment|// Perform small deletes right away
name|removeBlocks
argument_list|(
name|collectedBlocks
argument_list|)
...
condition|)
block|{
comment|// If this commit does not want to close the file, persist
comment|// blocks only if append is supported
name|dir
operator|.
name|persistBlocks
argument_list|(
...
block|}
block|}
comment|/** @see #registerDatanode(DatanodeRegistration) */
DECL|method|registerDatanodeInternal (DatanodeRegistration nodeReg)
specifier|public
name|void
name|registerDatanodeInternal
parameter_list|(
...
expr_stmt|;
block|}
comment|// check if the datanode is allowed to be connect to the namenode
if|if
condition|(
operator|!
name|verifyNodeRegistration
argument_list|(
name|nodeReg
argument_list|,
name|dnAddress
...
block|}
block|}
comment|/** @see #handleHeartbeat(DatanodeRegistration, long, long, long, long, int, int, int) */
DECL|method|handleHeartbeatInternal (DatanodeRegistration nodeReg, long capacity, long dfsUsed, long remaining, long blockPoolUsed, int xceiverCount, int xmitsInProgress, int failedVolumes)
name|DatanodeCommand
index|[]
name|handleHeartbeatInternal
parameter_list|(
...
block|}
block|}
comment|//check distributed upgrade
name|cmd
operator|=
...
return|;
block|}
DECL|method|updateStats (DatanodeDescriptor node, boolean isAdded)
specifier|private
name|void
name|updateStats
parameter_list|(
...
block|{
comment|//
comment|// The statistics are protected by the heartbeat lock
comment|// For decommissioning/decommissioned nodes, only used capacity
comment|// is counted.
comment|//
assert|assert
operator|(
...
block|}
block|}
comment|/**    * Periodically calls heartbeatCheck() and updateBlockKey()    */
...
comment|/////////////////////////////////////////////////////////
comment|//
comment|// These methods are called by the Namenode system, to see
comment|// if there is any work for registered datanodes.
comment|//
comment|/////////////////////////////////////////////////////////
...
return|;
block|}
DECL|method|setNodeReplicationLimit (int limit)
specifier|public
name|void
...
expr_stmt|;
block|}
comment|/**    * Remove a datanode descriptor.    * @param nodeID datanode ID.    * @throws IOException    */
DECL|method|removeDatanode (DatanodeID nodeID)
specifier|public
name|void
name|removeDatanode
parameter_list|(
...
name|warn
argument_list|(
literal|"BLOCK* NameSystem.removeDatanode: "
operator|+
name|nodeID
operator|.
name|getName
...
expr_stmt|;
block|}
name|unprotectedRemoveDatanode
argument_list|(
name|nodeInfo
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
DECL|method|unprotectedRemoveDatanode (DatanodeDescriptor nodeDescr)
name|void
name|unprotectedRemoveDatanode
parameter_list|(
name|DatanodeDescriptor
...
name|debug
argument_list|(
literal|"BLOCK* NameSystem.unprotectedRemoveDatanode: "
operator|+
name|nodeDescr
operator|.
name|getName
...
name|info
argument_list|(
literal|"BLOCK* NameSystem.heartbeatCheck: "
operator|+
literal|"lost heartbeat from "
operator|+
name|nodeInfo
operator|.
name|getName
...
block|}
block|}
comment|/**    * The given node is reporting all its blocks.  Use this info to     * update the (machine-->blocklist) and (block-->machinelist) tables.    */
DECL|method|processReport (DatanodeID nodeID, String poolId, BlockListAsLongs newReport)
specifier|public
name|void
name|processReport
parameter_list|(
...
name|info
argument_list|(
literal|"BLOCK* NameSystem.processReport: "
operator|+
...
name|info
argument_list|(
literal|"BLOCK* NameSystem.processReport: from "
operator|+
name|nodeID
operator|.
name|getName
...
literal|null
decl_stmt|;
comment|// check if we can del delNodeHint
if|if
condition|(
name|firstOne
operator|&&
...
DECL|method|blockReceived (DatanodeID nodeID, String poolId, Block block, String delHint )
specifier|public
name|void
name|blockReceived
parameter_list|(
...
name|IOException
argument_list|(
literal|"Got blockReceived message from unregistered or dead node "
operator|+
name|block
argument_list|)
throw|;
...
return|;
block|}
comment|/**    * Total used space by data nodes as percentage of total capacity    */
...
block|}
block|}
comment|/**    * Total used space by data nodes for non DFS purposes such    * as storing temporary files on the local file system    */
...
return|;
block|}
comment|/**    * Total non-used raw bytes.    */
...
return|;
block|}
comment|/**    * Total remaining space by data nodes as percentage of total capacity    */
...
return|;
block|}
DECL|method|getDatanodeListForReport ( DatanodeReportType type)
specifier|private
name|ArrayList
argument_list|<
name|DatanodeDescriptor
...
block|}
comment|/**    */
DECL|method|DFSNodesStatus (ArrayList<DatanodeDescriptor> live, ArrayList<DatanodeDescriptor> dead)
specifier|public
name|void
name|DFSNodesStatus
parameter_list|(
...
DECL|method|datanodeDump (PrintWriter out)
specifier|private
name|void
name|datanodeDump
parameter_list|(
...
return|;
block|}
DECL|method|getMaxReplication ()
name|short
name|getMaxReplication
parameter_list|()
block|{
return|return
operator|(
...
return|;
block|}
comment|/**    * Clamp the specified replication between the minimum and maximum    * replication levels for this namesystem.    */
...
return|;
block|}
comment|/**     * Keeps track of which datanodes/ipaddress are allowed to connect to the namenode.    */
...
return|;
block|}
comment|/**    * Check if the given node (of DatanodeID or ipAddress) is in the (include or     * exclude) list.  If ipAddress in null, check only based upon the given     * DatanodeID.  If ipAddress is not null, the ipAddress should refers to the    * same host that given DatanodeID refers to.    *     * @param node, DatanodeID, the host DatanodeID    * @param ipAddress, if not null, should refers to the same host    *                   that DatanodeID refers to    * @param hostsList, the list of hosts in the include/exclude file    * @param isExcludeList, boolean, true if this is the exclude list    * @return boolean, if in the list    */
DECL|method|checkInList (DatanodeID node, String ipAddress, Set<String> hostsList, boolean isExcludeList)
specifier|private
name|boolean
name|checkInList
parameter_list|(
...
name|warn
argument_list|(
literal|"Unknown host in host list: "
operator|+
name|ipAddress
argument_list|)
expr_stmt|;
comment|// can't resolve the host name.
if|if
condition|(
name|isExcludeList
condition|)
...
argument_list|()
expr_stmt|;
comment|// Reread the config to get dfs.hosts and dfs.hosts.exclude filenames.
...
expr_stmt|;
block|}
comment|/**    * Checks if the node is not on the hosts list.  If it is not, then    * it will be disallowed from registering.     */
DECL|method|verifyNodeRegistration (DatanodeID nodeReg, String ipAddr)
specifier|private
name|boolean
name|verifyNodeRegistration
parameter_list|(
...
block|}
block|}
comment|/**    * Get data node by storage ID.    *     * @param nodeID    * @return DatanodeDescriptor or null if the node is not found.    * @throws IOException    */
...
return|;
block|}
comment|/** Choose a random datanode    *     * @return a randomly chosen datanode    */
DECL|method|getRandomDatanode ()
name|DatanodeDescriptor
...
argument_list|()
operator|:
literal|"Found a lease for file that is not under construction."
assert|;
name|INodeFileUnderConstruction
name|cons
init|=
...
block|}
block|}
comment|/**    * Moves fsimage.ckpt to fsImage and edits.new to edits    * Reopens the new edits file.    *    * @param sig the signature of this checkpoint (old image)    */
...
DECL|method|unprotectedChangeLease (String src, String dst, HdfsFileStatus dinfo)
name|void
name|unprotectedChangeLease
parameter_list|(
name|String
...
block|}
block|}
comment|/**    * Register a name-node.    *<p>    * Registration is allowed if there is no ongoing streaming to    * another backup node.    * We currently allow only one backup node, but multiple chackpointers     * if there are no backups.    *     * @param registration    * @throws IOException    */
DECL|method|registerBackupNode (NamenodeRegistration registration)
name|void
...
operator|+
literal|"Another node is registered as a backup."
argument_list|)
throw|;
block|}
finally|finally
...
DECL|method|listCorruptFileBlocks (String path, String startBlockAfter)
name|Collection
argument_list|<
name|CorruptFileBlockInfo
argument_list|>
...
argument_list|()
decl_stmt|;
comment|// we need to call DFSNodeStatus to filter out the dead data nodes
name|DFSNodesStatus
argument_list|(
name|liveNodeList
argument_list|,
...
DECL|method|removeDecomNodeFromList (ArrayList<DatanodeDescriptor> nodeList)
name|void
name|removeDecomNodeFromList
parameter_list|(
name|ArrayList
