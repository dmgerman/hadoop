...
name|nameCache
decl_stmt|;
comment|/** Access an existing dfs name directory. */
...
return|;
block|}
comment|/**    * Load the filesystem image into memory.    *    * @param startOpt Startup type as specified by the user.    * @throws IOException If image or editlog cannot be read.    */
...
block|}
comment|/**    */
DECL|method|unprotectedAddFile ( String path, PermissionStatus permissions, BlockInfo[] blocks, short replication, long modificationTime, long atime, long preferredBlockSize)
name|INode
...
block|}
block|}
comment|/**    * Remove a block to the file.    */
...
return|;
block|}
comment|/**    * Get the blocksize of a file    * @param filename the filename    * @return the number of bytes     */
...
name|IOException
argument_list|(
literal|"Getting block size of non-file: "
operator|+
name|filename
argument_list|)
throw|;
...
return|;
block|}
comment|/** Return if a directory is empty or not **/
DECL|method|isDirEmpty (String src)
name|boolean
name|isDirEmpty
parameter_list|(
name|String
...
literal|null
operator|:
literal|"should be taken care in isDir() above"
assert|;
if|if
condition|(
operator|(
...
DECL|method|unprotectedDelete (String src, List<Block> collectedBlocks, long mtime)
name|int
name|unprotectedDelete
parameter_list|(
name|String
...
name|path
operator|+
literal|" does not exist under rootDir."
argument_list|)
throw|;
block|}
name|updateCount
...
name|FileNotFoundException
argument_list|(
literal|"Cannot set quota on a file: "
operator|+
name|srcs
argument_list|)
throw|;
...
DECL|method|unprotectedSymlink (String path, String target, long modTime, long atime, PermissionStatus perm)
name|INodeSymlink
name|unprotectedSymlink
parameter_list|(
name|String
...
parameter_list|)
block|{
comment|/* All UnresolvedLinkExceptions should have been resolved by now, but we        * should re-throw them in case that changes so they are not swallowed         * by catching IOException below.        */
throw|throw
name|e
throw|;
block|}
