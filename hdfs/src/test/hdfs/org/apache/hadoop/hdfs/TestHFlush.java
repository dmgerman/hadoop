...
decl_stmt|;
comment|/** The test uses {@link #doTheJob(Configuration, String, long, short)    * to write a file with a standard block size    */
annotation|@
name|Test
...
expr_stmt|;
block|}
comment|/** The test uses {@link #doTheJob(Configuration, String, long, short)    * to write a file with a custom block size so the writes will be    * happening across block' boundaries    */
annotation|@
name|Test
...
expr_stmt|;
block|}
comment|/** The test uses {@link #doTheJob(Configuration, String, long, short)    * to write a file with a custom block size so the writes will be    * happening across block's and checksum' boundaries    */
annotation|@
name|Test
...
expr_stmt|;
block|}
comment|/**     The method starts new cluster with defined Configuration;     creates a file with specified block_size and writes 10 equal sections in it;     it also calls hflush() after each write and throws an IOException in case of      an error.     @param conf cluster configuration     @param fileName of the file to be created and processed as required     @param block_size value to be used for the file's creation     @param replicas is the number of replicas     @throws IOException in case of any errors     */
DECL|method|doTheJob (Configuration conf, final String fileName, long block_size, short replicas)
specifier|public
specifier|static
name|void
name|doTheJob
...
argument_list|)
expr_stmt|;
comment|// Wait while hflush() pushes all packets through built pipeline
operator|(
operator|(
