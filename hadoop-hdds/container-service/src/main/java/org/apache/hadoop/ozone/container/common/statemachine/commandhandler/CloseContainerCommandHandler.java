...
argument_list|)
expr_stmt|;
comment|// when a closeContainerCommand arrives at a Datanode and if the
comment|// container is open, each replica will be moved to closing state first.
if|if
condition|(
name|containerState
operator|==
...
expr_stmt|;
block|}
comment|// if the container is already closed, it will be just ignored.
comment|// ICR will get triggered to change the replica state in SCM.
name|HddsProtos
operator|.
name|PipelineID
name|pipelineID
