...
condition|)
block|{
name|releaseCommitSpace
argument_list|()
expr_stmt|;
block|}
comment|/**      * commit space when container transitions (back) to Open.      * when? perhaps closing a container threw an exception      */
if|if
condition|(
operator|(
name|state
...
expr_stmt|;
block|}
DECL|method|releaseCommitSpace ()
specifier|private
name|void
name|releaseCommitSpace
parameter_list|()
...
argument_list|()
decl_stmt|;
comment|// only if container size< max size
if|if
condition|(
name|unused
operator|>
...
expr_stmt|;
block|}
comment|/**    * add available space in the container to the committed space in the volume.    * available space is the number of bytes remaining till max capacity.    */
DECL|method|commitSpace ()
specifier|public
name|void
name|commitSpace
parameter_list|()
...
name|cVol
decl_stmt|;
comment|//we don't expect duplicate calls
name|Preconditions
operator|.
name|checkState
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Only Open Containers have Committed Space
if|if
condition|(
name|myState
operator|!=
...
return|return;
block|}
comment|// junit tests do not always set up volume
name|cVol
operator|=
name|getVolume
argument_list|()
...
return|;
block|}
comment|/**    * Increase the number of bytes write into the container.    * Also decrement committed bytes against the bytes written.    * @param bytes the number of bytes write into the container.    */
...
condition|)
block|{
comment|//with this write, container size might breach max size
name|long
name|decrement
init|=
name|Math
