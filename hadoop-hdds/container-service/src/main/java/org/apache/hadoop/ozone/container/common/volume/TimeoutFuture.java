...
begin_comment
comment|/**  * Implementation of {@code Futures#withTimeout}.  *<p>  *<p>Future that delegates to another but will finish early (via a  * {@link TimeoutException} wrapped in an {@link ExecutionException}) if the  * specified duration expires. The delegate future is interrupted and  * cancelled if it times out.  */
end_comment
...
parameter_list|()
block|{
comment|// If either of these reads return null then we must be after a
comment|// successful cancel or another call to this method.
name|TimeoutFuture
argument_list|<
name|V
argument_list|>
...
comment|// timer may be null if this call to run was by the timer task since there
comment|// is no happens-before edge between the assignment to timer and an
comment|// execution of the timer task.
if|if
condition|(
name|localTimer
operator|!=
