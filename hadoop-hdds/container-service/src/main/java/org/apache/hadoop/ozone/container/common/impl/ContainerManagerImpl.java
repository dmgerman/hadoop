...
condition|)
block|{
comment|// This means we were not able read data from the disk when booted the
comment|// datanode. We are going to rely on SCM understanding that we don't
comment|// have valid data for this container when we send container reports.
comment|// Hopefully SCM will ask us to delete this container and rebuild it.
name|LOG
operator|.
name|error
argument_list|(
literal|"Invalid SHA found for container data. Name :{}"
operator|+
literal|"cowardly refusing to read invalid data"
argument_list|,
name|containerName
argument_list|)
expr_stmt|;
