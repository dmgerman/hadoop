...
literal|10
decl_stmt|;
DECL|method|BlockDeletingService (OzoneContainer ozoneContainer, long serviceInterval, long serviceTimeout, TimeUnit timeUnit, Configuration conf)
specifier|public
name|BlockDeletingService
parameter_list|(
name|OzoneContainer
...
return|;
block|}
DECL|method|isDeletionAllowed (ContainerData containerData, ContainerDeletionChoosingPolicy deletionPolicy)
specifier|private
name|boolean
name|isDeletionAllowed
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|// in case te ratis group does not exist, just mark it for deletion.
if|if
condition|(
operator|!
name|ratisServer
...
name|warn
argument_list|(
literal|"Close Container log Index {} is not replicated across all"
operator|+
literal|"the servers in the pipeline {} as the min replicated "
operator|+
literal|"index is {}. Deletion is not allowed in this container "
operator|+
literal|"yet."
argument_list|,
name|containerBCSID
...
parameter_list|)
block|{
comment|// in case of any exception check again whether the pipeline exist
comment|// and in case the pipeline got destroyed, just mark it for deletion
if|if
condition|(
operator|!
name|ratisServer
