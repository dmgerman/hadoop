...
block|{
throw|throw
name|transformServiceException
argument_list|(
name|e
argument_list|)
throw|;
...
return|;
block|}
DECL|method|transformServiceException ( ServiceException se)
specifier|private
name|IOException
name|transformServiceException
parameter_list|(
...
name|IOException
block|{
comment|//TODO SCM has no perfect way to return with business exceptions. All
comment|//the exceptions will be mapped to ServiceException.
comment|//ServiceException is handled in a special way in hadoop rpc: the message
comment|//contains the whole stack trace which is not required for the business
comment|//exception. As of now I remove the stack trace (use first line only).
comment|//Long term we need a proper way of the exception propagation.
name|Throwable
name|cause
init|=
name|se
...
return|;
block|}
DECL|method|useFirstLine (String message)
specifier|private
name|String
name|useFirstLine
parameter_list|(
