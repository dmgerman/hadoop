...
argument_list|)
decl_stmt|;
comment|// TODO: GRPC TLS only for now, netty/hadoop RPC TLS support later.
if|if
condition|(
name|tlsConfig
operator|!=
...
return|;
block|}
comment|// For External gRPC client to server with gRPC TLS.
comment|// No mTLS for external client as SCM CA does not issued certificates for them
DECL|method|createTlsClientConfig (SecurityConfig conf, X509Certificate caCert)
specifier|static
name|GrpcTlsConfig
...
return|;
block|}
comment|// For Internal gRPC client from SCM to DN with gRPC TLS
DECL|method|createTlsClientConfigForSCM (SecurityConfig conf, CertificateServer certificateServer)
specifier|static
name|GrpcTlsConfig
name|createTlsClientConfigForSCM
parameter_list|(
name|SecurityConfig
name|conf
parameter_list|,
...
name|SCMSecurityException
argument_list|(
literal|"Fail to find SCM CA certificate."
argument_list|,
name|ex
argument_list|)
throw|;
...
return|;
block|}
comment|// For gRPC server running DN container service with gPRC TLS
comment|// No mTLS as the channel is shared for for external client, which
comment|// does not have SCM CA issued certificates.
comment|// In summary:
comment|// authenticate from server to client is via TLS.
comment|// authenticate from client to server is via block token (or container token).
DECL|method|createTlsServerConfigForDN (SecurityConfig conf, CertificateClient caClient)
specifier|static
name|GrpcTlsConfig
...
return|;
block|}
DECL|method|getMinReplicatedIndex ( Collection<RaftProtos.CommitInfoProto> commitInfos)
specifier|static
name|Long
name|getMinReplicatedIndex
parameter_list|(
name|Collection
argument_list|<
name|RaftProtos
...
return|;
block|}
DECL|method|int2ByteString (int n)
specifier|static
name|ByteString
name|int2ByteString
parameter_list|(
...
argument_list|(
literal|"Failed to write integer n = "
operator|+
name|n
operator|+
literal|" to a ByteString."
argument_list|,
name|e
argument_list|)
throw|;
