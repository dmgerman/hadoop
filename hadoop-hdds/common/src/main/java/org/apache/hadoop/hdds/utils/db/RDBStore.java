...
argument_list|)
decl_stmt|;
comment|// Only the first record needs to be checked if its seq number<
comment|// ( 1 + passed_in_sequence_number). For example, if seqNumber passed
comment|// in is 100, then we can read from the WAL ONLY if the first sequence
comment|// number is<= 101. If it is 102, then 101 may already be flushed to
comment|// SST. If it 99, we can skip 99 and 100, and then read from 101.
name|boolean
name|checkValidStartingSeqNumber
init|=
literal|true
decl_stmt|;
while|while
...
name|SequenceNumberNotFoundException
argument_list|(
literal|"Unable to read data from"
operator|+
literal|" RocksDB wal to get delta updates. It may have already been"
operator|+
literal|"flushed to SSTs."
argument_list|)
throw|;
block|}
comment|// If the above condition was not satisfied, then it is OK to reset
comment|// the flag.
name|checkValidStartingSeqNumber
operator|=
literal|false
...
name|error
argument_list|(
literal|"Unable to get delta updates since sequenceNumber {} "
argument_list|,
name|sequenceNumber
argument_list|,
name|e
