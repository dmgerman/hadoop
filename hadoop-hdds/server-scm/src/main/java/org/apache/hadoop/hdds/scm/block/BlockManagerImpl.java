...
expr_stmt|;
block|}
comment|/**    * Allocates a block in a container and returns that info.    *    * @param size - Block Size    * @param type Replication Type    * @param factor - Replication Factor    * @param excludeList List of datanodes/containers to exclude during block    *                    allocation.    * @return Allocated block    * @throws IOException on failure.    */
annotation|@
name|Override
DECL|method|allocateBlock (final long size, ReplicationType type, ReplicationFactor factor, String owner, ExcludeList excludeList)
specifier|public
name|AllocatedBlock
name|allocateBlock
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// TODO: if getMachingContainer results in containers which are in exclude
comment|// list, we may end up in this loop forever. This case needs to be
comment|// addressed.
if|if
condition|(
name|containerInfo
...
return|;
block|}
DECL|method|discardContainer (ContainerID containerId, List<ContainerID> containers)
specifier|private
name|boolean
name|discardContainer
parameter_list|(
