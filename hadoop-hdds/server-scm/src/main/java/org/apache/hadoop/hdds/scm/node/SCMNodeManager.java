...
return|;
block|}
comment|/**    * Add an entry to the dnsToUuidMap, which maps hostname / IP to the DNs    * running on that host. As each address can have many DNs running on it,    * this is a one to many mapping.    * @param dnsName String representing the hostname or IP of the node    * @param uuid String representing the UUID of the registered node.    */
annotation|@
name|SuppressFBWarnings
argument_list|(
name|value
...
name|justification
operator|=
literal|"The method is synchronized and this is the only place "
operator|+
literal|"dnsToUuidMap is modified"
argument_list|)
DECL|method|addEntryTodnsToUuidMap ( String dnsName, String uuid)
specifier|private
...
name|debug
argument_list|(
literal|"Processing node report from [datanode={}]"
argument_list|,
name|datanodeDetails
operator|.
name|getHostName
...
name|trace
argument_list|(
literal|"HB is received from [datanode={}]:<json>{}</json>"
argument_list|,
name|datanodeDetails
operator|.
name|getHostName
...
return|;
block|}
comment|/**    * Given datanode uuid, returns the DatanodeDetails for the node.    *    * @param uuid node host address    * @return the given datanode, or null if not found    */
annotation|@
name|Override
...
name|warn
argument_list|(
literal|"Cannot find node for uuid {}"
argument_list|,
name|uuid
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * Given datanode address(Ipaddress or hostname), return a list of    * DatanodeDetails for the datanodes registered on that address.    *    * @param address datanode address    * @return the given datanode, or empty list if none found    */
annotation|@
name|Override
...
name|warn
argument_list|(
literal|"Cannot find node for address {}"
argument_list|,
name|address
argument_list|)
expr_stmt|;
