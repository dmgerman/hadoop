...
argument_list|)
expr_stmt|;
comment|// Updating the storage report for the datanode.
comment|// I dont think we will get NotFound exception, as we are taking
comment|// nodeInfo from nodeStateMap, as I see it is not being removed from
comment|// the map, just we change the states. And during first time
comment|// registration we call this, after adding to nodeStateMap. And also
comment|// from eventhandler it is called only if it has node Report.
name|DatanodeInfo
name|datanodeInfo
init|=
name|nodeStateManager
