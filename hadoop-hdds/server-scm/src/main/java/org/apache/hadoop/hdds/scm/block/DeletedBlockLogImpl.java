...
decl_stmt|;
DECL|method|DeletedBlockLogImpl (Configuration conf, ContainerManager containerManager, SCMMetadataStore scmMetadataStore)
specifier|public
name|DeletedBlockLogImpl
parameter_list|(
name|Configuration
...
condition|)
block|{
comment|// Should we make this an error ? How can we not find the deleted
comment|// TXID?
name|LOG
operator|.
name|warn
argument_list|(
literal|"Deleted TXID not found."
argument_list|)
expr_stmt|;
continue|continue;
block|}
...
argument_list|)
expr_stmt|;
comment|// We do not throw error here, since we don't want to abort the loop.
comment|// Just log and continue processing the rest of txids.
block|}
finally|finally
block|{
name|lock
