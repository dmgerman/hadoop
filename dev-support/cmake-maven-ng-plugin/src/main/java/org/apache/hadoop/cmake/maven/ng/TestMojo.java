...
begin_comment
comment|/**  * Goal which runs a native unit test.  *  * @goal test  * @phase test  */
end_comment
...
name|AbstractMojo
block|{
comment|/**    * Location of the binary to run.    *    * @parameter expression="${binary}"    * @required    */
DECL|field|binary
specifier|private
name|File
name|binary
decl_stmt|;
comment|/**    * Name of this test.    *    * Defaults to the basename of the binary.  So if your binary is /foo/bar/baz,    * this will default to 'baz.'    *    * @parameter expression="${testName}"    */
DECL|field|testName
specifier|private
name|String
name|testName
decl_stmt|;
comment|/**    * Environment variables to pass to the binary.    *    * @parameter expression="${env}"    */
DECL|field|env
specifier|private
name|Map
argument_list|<
...
name|env
decl_stmt|;
comment|/**    * Arguments to pass to the binary.    *    * @parameter expression="${args}"    */
DECL|field|args
specifier|private
name|List
argument_list|<
...
name|args
decl_stmt|;
comment|/**    * Number of seconds to wait before declaring the test failed.    *    * @parameter expression="${timeout}" default-value=600    */
DECL|field|timeout
specifier|private
name|int
name|timeout
decl_stmt|;
comment|/**    * Path to results directory.    *    * @parameter expression="${results}" default-value="cmake-ng-results"    */
DECL|field|results
specifier|private
name|File
name|results
decl_stmt|;
comment|/**    * A list of preconditions which must be true for this test to be run.    *    * @parameter expression="${preconditions}"    */
...
argument_list|()
decl_stmt|;
comment|/**    * If true, pass over the test without an error if the binary is missing.    *    * @parameter expression="${skipIfMissing}" default-value="false"    */
DECL|field|skipIfMissing
specifier|private
name|boolean
name|skipIfMissing
decl_stmt|;
comment|/**    * What result to expect from the test    *    * @parameter expression="${expectedResult}" default-value="success"    *            Can be either "success", "failure", or "any".    */
...
argument_list|()
expr_stmt|;
comment|// Process#getInputStream gets the stdout stream of the process, which
comment|// acts as an input to us.
name|outThread
operator|=
operator|new
name|OutputToFileThread
...
name|println
argument_list|(
literal|"failed to write status file!  Error "
operator|+
name|e
argument_list|)
expr_stmt|;
...
expr_stmt|;
block|}
comment|// Now that we've terminated the process, the threads servicing
comment|// its pipes should receive end-of-file and exit.
comment|// We don't want to terminate them manually or else we might lose
comment|// some output.
if|if
condition|(
name|errThread
...
name|println
argument_list|(
literal|"Interrupted while waiting for errThread"
argument_list|)
expr_stmt|;
name|e
operator|.
...
name|println
argument_list|(
literal|"Interrupted while waiting for outThread"
argument_list|)
expr_stmt|;
name|e
operator|.
