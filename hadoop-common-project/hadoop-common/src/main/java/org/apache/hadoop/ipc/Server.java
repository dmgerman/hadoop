...
argument_list|)
expr_stmt|;
comment|// wait to send response until failure is logged
name|doSaslReply
argument_list|(
name|sendToClient
argument_list|)
...
expr_stmt|;
block|}
comment|// send reply here to avoid a successful auth being logged as a
comment|// failure if response can't be sent
name|doSaslReply
argument_list|(
name|saslResponse
argument_list|)
...
block|}
block|}
DECL|method|processSaslMessage (byte[] buf)
specifier|private
name|RpcSaslProto
name|processSaslMessage
parameter_list|(
...
name|SaslException
argument_list|(
literal|"Client sent non-SASL request"
argument_list|)
throw|;
block|}
specifier|final
...
condition|)
block|{
comment|// the auth method is not supported
if|if
condition|(
name|sentNegotiate
condition|)
...
break|break;
block|}
comment|// fallthru to process sasl token
block|}
case|case
name|RESPONSE
case|:
...
condition|)
block|{
comment|// switch to simple must ignore next negotiate or initiate
if|if
condition|(
name|skipInitialSaslHandshake
condition|)
...
expr_stmt|;
block|}
comment|// else wait for a negotiate or initiate
break|break;
block|}
block|}
return|return
...
return|;
block|}
DECL|method|createSaslServer (String authMethodName)
specifier|private
name|AuthMethod
name|createSaslServer
parameter_list|(
...
block|}
block|}
comment|/**      * Process an RPC Request - the connection headers and context have been      * read      * @param buf - contains the RPC request header and the rpc request      * @throws RpcServerException due to fatal rpc layer issues such as      *   invalid header. In this case a RPC fatal status response is sent back      *   to client.      */
DECL|method|processRpcRequest (byte[] buf)
specifier|private
name|void
name|processRpcRequest
parameter_list|(
...
expr_stmt|;
block|}
DECL|method|respondBadRpcHeader (Call call, String errorClass, String error)
specifier|private
name|void
name|respondBadRpcHeader
parameter_list|(
