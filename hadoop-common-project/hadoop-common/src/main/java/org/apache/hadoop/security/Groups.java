...
expr_stmt|;
block|}
DECL|method|Groups (Configuration conf, Timer timer)
specifier|public
name|Groups
parameter_list|(
name|Configuration
...
block|}
block|}
comment|/**    * Determine whether the CachedGroups is expired.    * @param groups cached groups for one user.    * @return true if groups is expired from useToGroupsMap.    */
DECL|method|hasExpired (CachedGroups groups, long startMs)
specifier|private
name|boolean
name|hasExpired
parameter_list|(
...
condition|)
block|{
comment|// This CachedGroups is in the negative cache, thus it should expire
comment|// sooner.
name|timeout
operator|=
name|negativeCacheTimeout
...
condition|)
block|{
comment|// Even with enabling negative cache, getGroups() has the same behavior
comment|// that throws IOException if the groups for the user is empty.
throw|throw
operator|new
name|IOException
argument_list|(
