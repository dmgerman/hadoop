...
decl_stmt|;
comment|/* Failover if queue is filled up */
...
condition|)
block|{
comment|// Signal clients to failover and try a separate server.
name|ex
operator|=
name|CallQueueOverflowException
operator|.
...
expr_stmt|;
block|}
comment|/**      * Fetch the current call queue from the weak reference delegate. If there      * is no delegate, or the delegate is empty, this will return null.      */
DECL|method|getCallQueue ()
specifier|private
name|FairCallQueue
argument_list|<
name|?
...
name|info
argument_list|(
literal|"FairCallQueueSize_p"
operator|+
name|i
argument_list|,
literal|"FCQ Queue Size"
argument_list|)
argument_list|,
name|currentQueueSizes
index|[
...
name|info
argument_list|(
literal|"FairCallQueueOverflowedCalls_p"
operator|+
name|i
argument_list|,
literal|"FCQ Overflowed Calls"
argument_list|)
argument_list|,
name|currentOverflowedCalls
index|[
