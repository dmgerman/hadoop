...
argument_list|()
decl_stmt|;
DECL|method|getUniqueLocalTraceFileName ()
specifier|private
specifier|static
name|String
...
try|try
block|{
comment|// On Linux we can get a unique local file name by reading the process id
comment|// out of /proc/self/stat.  (There isn't any portable way to get the
comment|// process ID from Java.)
name|reader
operator|=
operator|new
name|BufferedReader
...
condition|)
block|{
comment|// If we can't use the process ID, use a random nonce.
name|nonce
operator|=
name|UUID
operator|.
