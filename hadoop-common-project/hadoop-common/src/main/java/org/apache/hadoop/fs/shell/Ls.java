...
literal|false
decl_stmt|;
DECL|field|aclNotSupportedFsSet
specifier|private
name|Set
argument_list|<
name|URI
argument_list|>
name|aclNotSupportedFsSet
init|=
name|Sets
operator|.
name|newHashSet
...
condition|)
block|{
comment|// This FileSystem failed to run the ACL API in an earlier iteration.
return|return
literal|false
return|;
block|}
...
parameter_list|)
block|{
comment|// If this is a RpcNoSuchMethodException, then the client is connected to
comment|// an older NameNode that doesn't support ACLs.  Keep going.
name|IOException
name|e2
init|=
name|e
...
parameter_list|)
block|{
comment|// The NameNode supports ACLs, but they are not enabled.  Keep going.
name|String
name|message
init|=
name|e
...
name|contains
argument_list|(
literal|"ACLs has been disabled"
argument_list|)
condition|)
block|{
throw|throw
...
parameter_list|)
block|{
comment|// The underlying FileSystem doesn't implement ACLs.  Keep going.
block|}
comment|// Remember that this FileSystem cannot support ACLs.
name|aclNotSupportedFsSet
operator|.
name|add
argument_list|(
