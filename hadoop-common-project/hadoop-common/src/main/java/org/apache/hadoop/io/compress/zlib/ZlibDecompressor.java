...
throw|;
block|}
DECL|method|put (ByteBuffer dst, ByteBuffer src)
specifier|private
name|int
name|put
parameter_list|(
...
parameter_list|)
block|{
comment|// this will lop off data from src[pos:limit] into dst[pos:limit], using the
comment|// min() of both remaining()
name|int
name|l1
init|=
name|src
...
literal|0
decl_stmt|;
comment|/* fast path for clean state and direct buffers */
if|if
condition|(
operator|(
name|src
...
condition|)
block|{
comment|/*        * TODO: fix these assumptions in inflateDirect(), eventually by allowing        * it to read position()/limit() directly        */
name|boolean
name|cleanDst
init|=
operator|(
...
operator|)
decl_stmt|;
comment|/* use the buffers directly */
if|if
condition|(
name|cleanDst
operator|&&
...
condition|)
block|{
comment|// this does not update buffers if we have no userBuf
if|if
condition|(
name|userBufLen
operator|<=
...
literal|0
operator|:
literal|"src.remaining() == 0"
assert|;
block|}
block|}
comment|// if we have drained userBuf, read from src (ideally, do not mix buffer
comment|// modes, but sometimes you can)
if|if
condition|(
name|userBufLen
operator|==
