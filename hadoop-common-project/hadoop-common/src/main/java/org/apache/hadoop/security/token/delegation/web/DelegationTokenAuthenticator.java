...
block|}
block|}
comment|/**    * Requests a delegation token using the configured<code>Authenticator</code>    * for authentication.    *    * @param url the URL to get the delegation token from. Only HTTP/S URLs are    * supported.    * @param token the authentication token being used for the user where the    * Delegation token will be stored.    * @throws IOException if an IO error occurred.    * @throws AuthenticationException if an authentication exception occurred.    */
...
block|}
block|}
DECL|method|doDelegationTokenOperation (URL url, AuthenticatedURL.Token token, DelegationTokenOperation operation, String renewer, Token<?> dToken, boolean hasResponse)
specifier|private
name|Map
name|doDelegationTokenOperation
parameter_list|(
...
name|format
argument_list|(
literal|"HTTP status, expected [%d], got [%d]: %s"
argument_list|,
name|expected
argument_list|,
name|status
