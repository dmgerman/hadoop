...
parameter_list|)
block|{
comment|// this will lop off data from src[pos:limit] into dst[pos:limit]
name|int
name|l1
init|=
name|src
...
comment|/* TODO: reset should free userBuf? */
if|if
condition|(
operator|(
name|src
...
argument_list|()
expr_stmt|;
comment|// we move dst.position() forward, not limit()
comment|// unlike the local buffer case, which moves it when we put() into the dst
name|dst
operator|.
name|position
argument_list|(
...
expr_stmt|;
block|}
comment|// fill the dst buffer from compressedDirectBuf
name|int
name|fill
init|=
name|put
