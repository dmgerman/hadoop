...
return|;
block|}
comment|/**    * Get a protocol proxy that contains a proxy connection to a remote server    * and a set of methods that are supported by the server    *    * @param protocol protocol    * @param clientVersion client's version    * @param addr server address    * @param ticket security ticket    * @param conf configuration    * @param factory socket factory    * @param rpcTimeout max time for each rpc; 0 means no timeout    * @param connectionRetryPolicy retry policy    * @param fallbackToSimpleAuth set to true or false during calls to indicate    *   if a secure client falls back to simple auth    * @param alignmentContext state alignment context    * @return the proxy    * @throws IOException if any error occurs    */
DECL|method|getProtocolProxy (Class<T> protocol, long clientVersion, InetSocketAddress addr, UserGroupInformation ticket, Configuration conf, SocketFactory factory, int rpcTimeout, RetryPolicy connectionRetryPolicy, AtomicBoolean fallbackToSimpleAuth, AlignmentContext alignmentContext)
specifier|public
specifier|static
parameter_list|<
name|T
...
name|verbose
decl_stmt|;
DECL|field|COMPLEX_SERVER_NAME_PATTERN
specifier|private
specifier|static
specifier|final
name|Pattern
name|COMPLEX_SERVER_NAME_PATTERN
init|=
name|Pattern
operator|.
name|compile
