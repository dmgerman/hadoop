...
block|}
block|}
comment|/** Sort nodes array by their distances to<i>reader</i>    * It linearly scans the array, if a local node is found, swap it with    * the first element of the array.    * If a local node group node is found, swap it with the first element     * following the local node.    * If a local rack node is found, swap it with the first element following    * the local node group node.    * If neither local node, node group node or local rack node is found, put a     * random replica location at position 0.    * It leaves the rest nodes untouched.    * @param reader the node that wishes to read a block from one of the nodes    * @param nodes the list of nodes containing data for the reader    */
annotation|@
name|Override
...
condition|)
block|{
comment|// if reader is not a datanode (not in NetworkTopology tree), we will
...
condition|)
block|{
comment|//local node group
name|localNodeGroupNode
operator|=
name|i
expr_stmt|;
comment|// node local and rack local are already found
if|if
condition|(
name|tempIndex
operator|!=
...
block|}
block|}
comment|// swap the local nodegroup node and the node at position tempIndex
if|if
condition|(
name|localNodeGroupNode
operator|!=
...
block|}
block|}
comment|// put a random node at position 0 if there is not a local/local-nodegroup/
comment|// local-rack node
if|if
condition|(
name|tempIndex
operator|==
