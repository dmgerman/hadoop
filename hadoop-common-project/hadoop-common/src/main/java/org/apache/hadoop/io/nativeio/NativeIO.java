...
name|NativeIOException
function_decl|;
comment|/**    * Unbuffered file copy from src to dst without tainting OS buffer cache    * In Linux, it uses sendfile() which uses O_DIRECT flag internally    * In Windows, it uses CopyFileEx with COPY_FILE_NO_BUFFERING flag    *    * Note: This does not support FreeBSD/OSX which have a different sendfile()    * semantic. Also, this simple native wrapper does minimal parameter checking    * It is recommended to use wrapper function like    * the Storage#nativeCopyFileUnbuffered() function in hadoop-hdfs.    *    *    * @param src                  The source path    * @param dst                  The destination path    * @throws IOException    */
