...
block|}
block|}
comment|/**    * Open a (if need be) secure connection to a URL in a secure environment    * that is using SPNEGO to authenticate its URLs. All Namenode and Secondary    * Namenode URLs that are protected via SPNEGO should be accessed via this    * method.    *    * @param url to authenticate via SPNEGO.    * @return A connection that has been authenticated via SPNEGO    * @throws IOException If unable to authenticate via SPNEGO    */
DECL|method|openSecureHttpConnection (URL url)
specifier|public
specifier|static
name|URLConnection
...
name|IOException
argument_list|(
literal|"Exception trying to open authenticated connection to "
operator|+
name|url
argument_list|,
name|e
