...
return|;
block|}
comment|/**    * Open an FSDataInputStream matching the PathHandle instance. The    * implementation may encode metadata in PathHandle to address the    * resource directly and verify that the resource referenced    * satisfies constraints specified at its construciton.    * @param fd PathHandle object returned by the FS authority.    * @throws InvalidPathHandleException If {@link PathHandle} constraints are    *                                    not satisfied    * @throws IOException IO failure    * @throws UnsupportedOperationException If {@link #open(PathHandle, int)}    *                                       not overridden by subclass    */
...
return|;
block|}
comment|/**    * Open an FSDataInputStream matching the PathHandle instance. The    * implementation may encode metadata in PathHandle to address the    * resource directly and verify that the resource referenced    * satisfies constraints specified at its construciton.    * @param fd PathHandle object returned by the FS authority.    * @param bufferSize the size of the buffer to use    * @throws InvalidPathHandleException If {@link PathHandle} constraints are    *                                    not satisfied    * @throws IOException IO failure    * @throws UnsupportedOperationException If not overridden by subclass    */
...
parameter_list|)
block|{
comment|// method is final with a default so clients calling getPathHandle(stat)
comment|// get the same semantics for all FileSystem implementations
if|if
condition|(
literal|null
operator|==
...
return|;
block|}
comment|/**    * Hook to implement support for {@link PathHandle} operations.    * @param stat Referent in the target FileSystem    * @param opt Constraints that determine the validity of the    *            {@link PathHandle} reference.    */
DECL|method|createPathHandle (FileStatus stat, HandleOpt... opt)
specifier|protected
name|PathHandle
name|createPathHandle
parameter_list|(
...
return|;
block|}
comment|/**    * Return an array of {@link FileStatus} objects whose path names match    * {@code pathPattern} and is accepted by the user-supplied path filter.    * Results are sorted by their path names.    *    * @param pathPattern a glob specifying the path pattern    * @param filter a user-supplied path filter    * @return null if {@code pathPattern} has no glob and the path does not exist    *         an empty array if {@code pathPattern} has a glob and no path    *         matches it else an array of {@link FileStatus} objects matching the    *         pattern    * @throws IOException if any I/O error occurs when fetching file status    */
...
argument_list|()
argument_list|,
literal|"Must specify either create, overwrite or append"
argument_list|)
throw|;
block|}
annotation|@
...
operator|.
name|Unstable
DECL|method|openFile (PathHandle pathHandle)
specifier|public
name|FutureDataInputStreamBuilder
name|openFile
parameter_list|(
...
return|;
block|}
comment|/**    * Execute the actual open file operation.    * The base implementation performs a blocking    * call to {@link #open(Path, int)}in this call;    * the actual outcome is in the returned {@code CompletableFuture}.    * This avoids having to create some thread pool, while still    * setting up the expectation that the {@code get()} call    * is needed to evaluate the result.    * @param pathHandle path to the file    * @param mandatoryKeys set of options declared as mandatory.    * @param options options set during the build sequence.    * @param bufferSize buffer size    * @return a future which will evaluate to the opened file.    * @throws IOException failure to resolve the link.    * @throws IllegalArgumentException unknown mandatory key    * @throws UnsupportedOperationException PathHandles are not supported.    * This may be deferred until the future is evaluated.    */
DECL|method|openFileWithOptions ( final PathHandle pathHandle, final Set<String> mandatoryKeys, final Configuration options, final int bufferSize)
specifier|protected
name|CompletableFuture
argument_list|<
name|FSDataInputStream
...
parameter_list|)
block|{
comment|// fail lazily here to ensure callers expect all File IO operations to
comment|// surface later
name|result
operator|.
...
DECL|class|FSDataInputStreamBuilder
specifier|private
specifier|static
class|class
name|FSDataInputStreamBuilder
...
name|FutureDataInputStreamBuilder
block|{
comment|/**      * Path Constructor.      * @param fileSystem owner      * @param path path to open.      */
DECL|method|FSDataInputStreamBuilder ( @onnull final FileSystem fileSystem, @Nonnull final Path path)
specifier|protected
name|FSDataInputStreamBuilder
parameter_list|(
annotation|@
...
expr_stmt|;
block|}
comment|/**      * Construct from a path handle.      * @param fileSystem owner      * @param pathHandle path handle of file to open.      */
DECL|method|FSDataInputStreamBuilder ( @onnull final FileSystem fileSystem, @Nonnull final PathHandle pathHandle)
specifier|protected
name|FSDataInputStreamBuilder
parameter_list|(
annotation|@
...
expr_stmt|;
block|}
comment|/**      * Perform the open operation.      * Returns a future which, when get() or a chained completion      * operation is invoked, will supply the input stream of the file      * referenced by the path/path handle.      * @return a future to the input stream.      * @throws IOException early failure to open      * @throws UnsupportedOperationException if the specific operation      * is not supported.      * @throws IllegalArgumentException if the parameters are not valid.      */
annotation|@
name|Override
DECL|method|build ()
specifier|public
