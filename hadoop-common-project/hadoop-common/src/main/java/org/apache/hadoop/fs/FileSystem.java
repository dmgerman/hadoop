...
condition|)
block|{
comment|// no matches with multiple expansions is a non-matching glob
name|results
operator|=
operator|new
name|FileStatus
...
return|;
block|}
comment|// sort gripes because FileStatus Comparable isn't parameterized...
annotation|@
...
literal|false
decl_stmt|;
comment|// pathPattern has any globs
name|List
argument_list|<
name|FileStatus
argument_list|>
...
argument_list|()
decl_stmt|;
comment|// determine starting point
name|int
name|level
init|=
literal|0
...
literal|1
expr_stmt|;
comment|// need to skip empty item at beginning of split list
name|baseDir
operator|=
name|Path
operator|.
...
expr_stmt|;
block|}
comment|// parse components and determine if it's a glob
name|String
index|[]
name|components
init|=
...
literal|null
expr_stmt|;
comment|// short through to filter check
block|}
block|}
comment|// seed the parent directory path, return if it doesn't exist
try|try
block|{
name|matches
operator|.
...
return|;
block|}
comment|// skip if there are no components other than the basedir
if|if
condition|(
name|components
operator|!=
...
condition|)
block|{
comment|// iterate through each path component
for|for
control|(
name|int
name|i
...
control|)
block|{
comment|// don't look for children in a file matched by a glob
if|if
condition|(
operator|!
name|match
...
condition|)
block|{
comment|// get all children matching the filter
name|FileStatus
index|[]
name|statuses
init|=
...
block|}
block|}
comment|// remove anything that didn't match the filter
if|if
condition|(
operator|!
name|matches
...
block|}
block|}
comment|// no final paths, if there were any globs return empty list
if|if
condition|(
name|matches
operator|.
