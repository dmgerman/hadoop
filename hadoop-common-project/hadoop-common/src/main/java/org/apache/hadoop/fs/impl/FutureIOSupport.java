...
block|{
throw|throw
name|unwrapInnerException
argument_list|(
name|e
argument_list|)
throw|;
block|}
comment|/**    * Extract the cause of a completion failure and rethrow it if an IOE    * or RTE.    * @param e exception.    * @param<T> type of return value.    * @return nothing, ever.    * @throws IOException either the inner IOException, or a wrapper around    * any non-Runtime-Exception    * @throws RuntimeException if that is the inner cause.    */
DECL|method|raiseInnerCause (final CompletionException e)
specifier|public
specifier|static
parameter_list|<
name|T
...
throw|;
block|}
comment|/**    * From the inner cause of an execution exception, extract the inner cause.    * If it is an RTE: throw immediately.    * If it is an IOE: Return.    * If it is a WrappedIOException: Unwrap and return    * Else: create a new IOException.    *    * Recursively handles wrapped Execution and Completion Exceptions in    * case something very complicated has happened.    * @param e exception.    * @return an IOException extracted or built from the cause.    * @throws RuntimeException if that is the inner cause.    */
DECL|method|unwrapInnerException (final Throwable e)
specifier|private
specifier|static
name|IOException
name|unwrapInnerException
...
block|{
comment|// this only happens if there was no cause.
return|return
operator|new
name|IOException
argument_list|(
