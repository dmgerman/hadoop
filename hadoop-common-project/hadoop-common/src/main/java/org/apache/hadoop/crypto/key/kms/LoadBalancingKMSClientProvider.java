...
decl_stmt|;
comment|// credentials alias for token.
...
literal|null
decl_stmt|;
DECL|method|LoadBalancingKMSClientProvider (URI providerUri, KMSClientProvider[] providers, Configuration conf)
specifier|public
name|LoadBalancingKMSClientProvider
parameter_list|(
name|URI
...
expr_stmt|;
block|}
DECL|method|LoadBalancingKMSClientProvider (URI uri, KMSClientProvider[] providers, long seed, Configuration conf)
specifier|private
name|LoadBalancingKMSClientProvider
parameter_list|(
name|URI
...
argument_list|)
expr_stmt|;
comment|// uri is the token service so it can be instantiated for renew/cancel.
name|dtService
operator|=
name|KMSClientProvider
operator|.
...
argument_list|)
expr_stmt|;
comment|// if provider not in conf, new client will alias on uri else addr.
if|if
condition|(
name|KMSUtil
operator|.
...
block|{
comment|// canonical service (credentials alias) will be the first underlying
comment|// provider's service.  must be deterministic before shuffle so multiple
comment|// calls for a token do not obtain another unnecessary token.
name|canonicalService
operator|=
operator|new
name|Text
...
expr_stmt|;
block|}
comment|// shuffle unless seed is 0 which is used by tests for determinism.
name|this
operator|.
name|providers
operator|=
...
name|debug
argument_list|(
literal|"Created LoadBalancingKMSClientProvider for KMS url: {} with {} "
operator|+
literal|"providers. delegation token service: {}, canonical service: {}"
argument_list|,
name|uri
argument_list|,
name|providers
...
argument_list|)
decl_stmt|;
comment|// override sub-providers service with our own so it can be used
comment|// across all providers.
name|token
operator|.
name|setService
argument_list|(
...
name|debug
argument_list|(
literal|"New token service set. Token: ({})"
argument_list|,
name|token
argument_list|)
expr_stmt|;
