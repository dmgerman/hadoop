...
condition|)
block|{
name|checkAccessByFileSystemInteraction
argument_list|(
name|dir
argument_list|)
expr_stmt|;
...
name|DiskErrorException
block|{
comment|// Make sure we can read the directory by listing it.
if|if
condition|(
name|dir
operator|.
...
throw|;
block|}
comment|// Make sure we can write to the directory by creating a temp file in it.
try|try
block|{
name|File
name|tempFile
...
throw|;
block|}
comment|// Make sure the directory is executable by trying to cd into it.  This
comment|// launches a separate process.  It does not change the working directory of
