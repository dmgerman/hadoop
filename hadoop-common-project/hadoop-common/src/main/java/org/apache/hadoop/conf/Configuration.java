...
block|}
block|}
comment|/**    * Unset a previously set property.    * @param name the property name    */
...
expr_stmt|;
block|}
comment|/**    * Set the value of the<code>name</code> property to the given type. This    * is equivalent to<code>set(&lt;name&gt;, value.toString())</code>.    * @param name property name    * @param value new value    * @param<T> enumeration type    */
...
expr_stmt|;
block|}
comment|/**    * Return value matching this enumerated type.    * Note that the returned value is trimmed by this method.    * @param name Property name    * @param defaultValue Value returned if no mapping exists    * @param<T> enumeration type    * @throws IllegalArgumentException If mapping is illegal for the type    * provided    * @return enumeration type    */
...
return|;
block|}
comment|/**    * Try and resolve the provided element name as a credential provider    * alias.    * @param name alias of the provisioned credential    * @return password or null if not found    * @throws IOException when error in fetching password    */
...
block|}
block|}
DECL|method|getStreamReader (Resource wrapper, boolean quiet)
specifier|private
name|XMLStreamReader2
name|getStreamReader
parameter_list|(
...
name|sources
decl_stmt|;
DECL|method|ParsedItem (String name, String key, String value, boolean isFinal, String[] sources)
name|ParsedItem
parameter_list|(
name|String
name|name
...
block|}
block|}
comment|/**    * Parser to consume SAX stream of XML elements from a Configuration.    */
DECL|class|Parser
specifier|private
class|class
name|Parser
...
name|name
decl_stmt|;
DECL|field|nameSingletonArray
specifier|private
specifier|final
name|String
index|[]
name|nameSingletonArray
decl_stmt|;
DECL|field|isRestricted
specifier|private
specifier|final
name|boolean
name|isRestricted
...
argument_list|()
decl_stmt|;
DECL|method|Parser (XMLStreamReader2 reader, Resource wrapper, boolean quiet)
name|Parser
parameter_list|(
name|XMLStreamReader2
name|reader
...
block|}
block|}
DECL|method|handleStartProperty ()
specifier|private
name|void
name|handleStartProperty
parameter_list|()
...
argument_list|)
decl_stmt|;
comment|// This is only called recursively while the lock is already held
comment|// by this thread, but synchronizing avoids a findbugs warning.
synchronized|synchronized
init|(
name|Configuration
operator|.
...
DECL|method|readTagFromConfig (String attributeValue, String confName, String confValue, String[] confSource)
specifier|private
name|void
name|readTagFromConfig
parameter_list|(
...
block|}
block|}
comment|/**    * get keys matching the the regex     * @param regex    * @return {@literal Map<String,String>} with matching keys    */
