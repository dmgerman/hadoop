...
name|EncryptedKeyVersion
argument_list|>
DECL|method|parseJSONEncKeyVersion (String keyName, List valueList)
name|parseJSONEncKeyVersion
parameter_list|(
name|String
name|keyName
...
try|try
block|{
comment|// if current UGI is different from UGI at constructor time, behave as
comment|// proxyuser
name|UserGroupInformation
name|currentUgi
init|=
...
literal|null
decl_stmt|;
comment|// If current UGI contains kms-dt&& is not proxy, doAs it to use its dt.
comment|// Otherwise, create the HTTP connection using the UGI at constructor time
name|UserGroupInformation
name|ugiToUse
init|=
operator|(
...
return|;
block|}
DECL|method|call (HttpURLConnection conn, Map jsonOutput, int expectedResponse, Class<T> klass, int authRetryCount)
specifier|private
parameter_list|<
name|T
parameter_list|>
...
parameter_list|)
block|{
comment|// Ignore the AuthExceptions.. since we are just using the method to
comment|// extract and set the authToken.. (Workaround till we actually fix
comment|// AuthenticatedURL properly to set authToken post initialization)
block|}
name|HttpExceptionUtils
operator|.
