...
name|dtService
decl_stmt|;
DECL|field|copyLegacyToken
specifier|private
specifier|final
name|boolean
name|copyLegacyToken
decl_stmt|;
...
return|;
block|}
DECL|method|createProvider (Configuration conf, URL origUrl, int port, String hostsPart, URI providerUri)
specifier|private
name|KeyProvider
name|createProvider
parameter_list|(
...
block|}
block|}
DECL|method|KMSClientProvider (URI uri, Configuration conf, URI providerUri)
specifier|public
name|KMSClientProvider
parameter_list|(
name|URI
...
annotation|@
name|VisibleForTesting
DECL|method|createKMSAuthenticatedURL ()
name|DelegationTokenAuthenticatedURL
name|createKMSAuthenticatedURL
parameter_list|()
block|{
...
condition|)
block|{
comment|// do not set service for legacy kind, for compatibility.
name|token
operator|.
name|setService
argument_list|(
...
begin_comment
comment|/**    * If {@link CommonConfigurationKeysPublic#KMS_CLIENT_COPY_LEGACY_TOKEN_KEY}    * is true when creating the provider, then copy the passed-in token of    * {@link KMSDelegationToken#TOKEN_KIND} and create a new token of    * {@link KMSDelegationToken#TOKEN_LEGACY_KIND}, and add it to credentials.    *    * @return The legacy token, or null if one should not be created.    */
end_comment

begin_function
DECL|method|createAndAddLegacyToken (Credentials credentials, Token<?> token)
specifier|private
name|Token
argument_list|<
name|?
...
name|debug
argument_list|(
literal|"Not creating legacy token because copyLegacyToken={}, "
operator|+
literal|"token={}"
argument_list|,
name|copyLegacyToken
...
return|;
block|}
comment|// copy a KMS_DELEGATION_TOKEN and create a new kms-dt with the same
comment|// underlying token for backwards-compatibility. Old clients/renewers
comment|// does not parse the new token and can only work with kms-dt.
specifier|final
name|Token
argument_list|<
name|?
...
name|info
argument_list|(
literal|"Copied token to legacy kind: {}"
argument_list|,
name|legacyToken
argument_list|)
expr_stmt|;
...
begin_comment
comment|/**    * Given a list of tokens, return the token that should be used for KMS    * authentication.    */
end_comment

begin_function
annotation|@
name|VisibleForTesting
DECL|method|selectKMSDelegationToken (Credentials creds)
name|Token
name|selectKMSDelegationToken
parameter_list|(
name|Credentials
...
parameter_list|)
block|{
comment|// always look for TOKEN_KIND first
specifier|final
name|TokenSelector
argument_list|<
name|AbstractDelegationTokenIdentifier
...
name|debug
argument_list|(
literal|"Searching service {} found token {}"
argument_list|,
name|dtService
argument_list|,
name|token
...
return|;
block|}
comment|// fall back to look for token by service, regardless of kind.
comment|// this is old behavior, keeping for compatibility reasons (for example,
comment|// even if KMS server is new, if the job is submitted with an old kms
comment|// client, job runners on new version should be able to find the token).
specifier|final
name|InetSocketAddress
name|addr
init|=
