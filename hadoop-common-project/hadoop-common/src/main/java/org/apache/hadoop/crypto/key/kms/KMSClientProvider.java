...
operator|.
name|DelegationTokenExtension
DECL|field|clientTokenProvider
name|clientTokenProvider
init|=
name|this
decl_stmt|;
comment|// the token's service.
...
name|dtService
decl_stmt|;
comment|// alias in the credentials.
DECL|field|canonicalService
specifier|private
specifier|final
name|Text
...
name|debug
argument_list|(
literal|"Creating key provider with token service value {}"
argument_list|,
name|service
argument_list|)
expr_stmt|;
...
return|;
block|}
DECL|method|createProviders (Configuration conf, URL origUrl, int port, String hostsPart)
specifier|private
name|KMSClientProvider
index|[]
name|createProviders
...
argument_list|)
expr_stmt|;
comment|// the token's service so it can be instantiated for renew/cancel.
name|dtService
operator|=
name|getDtService
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// the canonical service is the alias for the token in the credentials.
comment|// typically it's the actual service in the token but older clients expect
comment|// an address.
name|URI
name|serviceUri
init|=
...
name|debug
argument_list|(
literal|"KMSClientProvider created for KMS url: {} delegation token "
operator|+
literal|"service: {} canonical service: {}."
argument_list|,
name|kmsUrl
argument_list|,
name|dtService
...
expr_stmt|;
block|}
DECL|method|getDtService (URI uri)
specifier|protected
specifier|static
name|Text
name|getDtService
...
name|service
decl_stmt|;
comment|// remove fragment for forward compatibility with logical naming.
specifier|final
name|String
name|fragment
init|=
...
return|;
block|}
comment|// note: this is only a crutch for backwards compatibility.
comment|// override the instance that will be used to select a token, intended
comment|// to allow load balancing provider to find a token issued by any of its
comment|// sub-providers.
DECL|method|setClientTokenProvider ( KeyProviderDelegationTokenExtension.DelegationTokenExtension provider)
specifier|protected
name|void
name|setClientTokenProvider
parameter_list|(
name|KeyProviderDelegationTokenExtension
operator|.
name|DelegationTokenExtension
...
annotation|@
name|VisibleForTesting
DECL|method|createAuthenticatedURL ()
name|DelegationTokenAuthenticatedURL
name|createAuthenticatedURL
parameter_list|()
block|{
...
name|debug
argument_list|(
literal|"Looking for delegation token. creds: {}"
argument_list|,
name|creds
operator|.
name|getAllTokens
...
expr_stmt|;
block|}
comment|// clientTokenProvider is either "this" or a load balancing instance.
comment|// if the latter, it will first look for the load balancer's uri
comment|// service followed by each sub-provider for backwards-compatibility.
return|return
name|clientTokenProvider
operator|.
name|selectDelegationToken
...
return|;
block|}
DECL|method|selectDelegationToken (Credentials creds, Text service)
specifier|protected
specifier|static
name|Token
argument_list|<
...
name|debug
argument_list|(
literal|"selected by alias={} token={}"
argument_list|,
name|service
argument_list|,
name|token
...
name|debug
argument_list|(
literal|"selected by service={} token={}"
argument_list|,
name|service
argument_list|,
name|token
