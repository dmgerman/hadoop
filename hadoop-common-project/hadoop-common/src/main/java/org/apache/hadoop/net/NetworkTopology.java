...
name|warn
argument_list|(
literal|"Total Nodes in scope : {} are less than Available Nodes : {}"
argument_list|,
name|totalInScopeNodes
argument_list|,
name|availableNodes
...
expr_stmt|;
block|}
comment|/**    * Sort nodes array by network distance to<i>reader</i> with secondary sort.    *<p>    * In a three-level topology, a node can be either local, on the same rack,    * or on a different rack from the reader. Sorting the nodes based on network    * distance from the reader reduces network traffic and improves    * performance.    *<p>    * As an additional twist, we also randomize the nodes at each network    * distance. This helps with load balancing when there is data skew.    *    * @param reader    Node where data will be read    * @param nodes     Available replicas with the requested data    * @param activeLen Number of active nodes at the front of the array    * @param secondarySort a secondary sorting strategy which can inject into    *     that point from outside to help sort the same distance.    */
DECL|method|sortByDistance (Node reader, T[] nodes, int activeLen, Consumer<List<T>> secondarySort)
specifier|public
parameter_list|<
name|T
extends|extends
...
expr_stmt|;
block|}
comment|/**    * Sort nodes array by network distance to<i>reader</i> with secondary sort.    *<p> using network location. This is used when the reader    * is not a datanode. Sorting the nodes based on network distance    * from the reader reduces network traffic and improves    * performance.    *<p>    *    * @param reader    Node where data will be read    * @param nodes     Available replicas with the requested data    * @param activeLen Number of active nodes at the front of the array    */
...
expr_stmt|;
block|}
comment|/**    * Sort nodes array by network distance to<i>reader</i>.    *<p> using network location. This is used when the reader    * is not a datanode. Sorting the nodes based on network distance    * from the reader reduces network traffic and improves    * performance.    *<p>    *    * @param reader    Node where data will be read    * @param nodes     Available replicas with the requested data    * @param activeLen Number of active nodes at the front of the array    * @param secondarySort a secondary sorting strategy which can inject into    *     that point from outside to help sort the same distance.    */
DECL|method|sortByDistanceUsingNetworkLocation (Node reader, T[] nodes, int activeLen, Consumer<List<T>> secondarySort)
specifier|public
parameter_list|<
name|T
extends|extends
...
DECL|method|sortByDistance (Node reader, T[] nodes, int activeLen, Consumer<List<T>> secondarySort, boolean nonDataNodeReader)
specifier|private
parameter_list|<
name|T
extends|extends
...
expr_stmt|;
block|}
comment|// Sort nodes which have the same weight using secondarySort.
name|int
name|idx
init|=
literal|0
