...
return|;
block|}
comment|/**    * Convert a os-native filename to a path that works for the shell    * and avoids script injection attacks.    * @param file The filename to convert    * @return The unix pathname    * @throws IOException on windows, there can be problems with the subprocess    */
DECL|method|makeSecureShellPath (File file)
specifier|public
specifier|static
name|String
name|makeSecureShellPath
parameter_list|(
name|File
name|file
parameter_list|)
...
condition|)
block|{
comment|// Currently it is never called, but it might be helpful in the future.
throw|throw
operator|new
name|UnsupportedOperationException
argument_list|(
literal|"Not implemented for Windows"
argument_list|)
throw|;
block|}
else|else
...
block|}
block|}
comment|/**    * Given a stream input it will unzip the it in the unzip directory.    * passed as the second parameter    * @param inputStream The zip file as input    * @param toDir The unzip directory where to unzip the zip file.    * @throws IOException an exception occurred    */
DECL|method|unZip (InputStream inputStream, File toDir)
specifier|public
specifier|static
name|void
name|unZip
...
block|}
block|}
comment|/**    * Given a File input it will unzip it in the unzip directory.    * passed as the second parameter    * @param inFile The zip file as input    * @param unzipDir The unzip directory where to unzip the zip file.    * @throws IOException An I/O exception has occurred    */
...
block|}
block|}
comment|/**    * Run a command and send the contents of an input stream to it.    * @param inputStream Input stream to forward to the shell command    * @param command shell command to run    * @throws IOException read or write failed    * @throws InterruptedException command interrupted    * @throws ExecutionException task submit failed    */
DECL|method|runCommandOnStream ( InputStream inputStream, String command)
specifier|private
specifier|static
name|void
name|runCommandOnStream
parameter_list|(
name|InputStream
name|inputStream
parameter_list|,
...
try|try
block|{
comment|// Consume stdout and stderr, to avoid blocking the command
name|executor
operator|=
name|Executors
operator|.
...
try|try
block|{
comment|// Read until the output stream receives an EOF and closed.
if|if
condition|(
name|LOG
operator|.
...
condition|)
block|{
comment|// Log directly to avoid out of memory errors
try|try
init|(
name|BufferedReader
name|reader
...
try|try
block|{
comment|// Read until the error stream receives an EOF and closed.
if|if
condition|(
name|LOG
operator|.
...
argument_list|)
decl_stmt|;
comment|// Pass the input stream to the command to process
try|try
block|{
name|org
operator|.
...
expr_stmt|;
block|}
comment|// Wait for both stdout and stderr futures to finish
name|error
operator|.
name|get
argument_list|()
...
expr_stmt|;
block|}
comment|// Wait to avoid leaking the child process
name|exitCode
operator|=
name|process
operator|.
...
name|format
argument_list|(
literal|"Error executing command. %s "
operator|+
literal|"Process exited with exit code %d."
argument_list|,
name|command
argument_list|,
name|exitCode
...
block|}
block|}
DECL|method|unTarUsingTar (InputStream inputStream, File untarDir, boolean gzipped)
specifier|private
specifier|static
name|void
name|unTarUsingTar
...
block|}
block|}
DECL|method|unTarUsingJava (InputStream inputStream, File untarDir, boolean gzipped)
specifier|private
specifier|static
name|void
name|unTarUsingJava
...
argument_list|()
operator|+
literal|" would create entry outside of "
operator|+
name|outputDir
argument_list|)
throw|;
...
condition|)
block|{
comment|// Create symbolic link relative to tar parent dir
name|Files
operator|.
