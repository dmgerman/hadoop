...
name|path
expr_stmt|;
comment|// Qualify the original and strip out URI fragment via toUri().getPath()
name|Path
name|stripped
init|=
operator|new
...
argument_list|)
decl_stmt|;
comment|// Re-qualify the bare stripped path and store it
name|this
operator|.
name|qualified
operator|=
...
return|;
block|}
comment|/**    * Whether Stat is supported on the current platform    * @return    */
...
literal|"f"
block|,
literal|"%z,%HT,%m,%a,%Op,%Su,%Sg,`link' -> `%Y'"
block|,
name|path
operator|.
name|toString
...
argument_list|()
decl_stmt|;
comment|// FreeBSD has extra digits beyond 4, truncate them
if|if
condition|(
name|octalPerms
operator|.
