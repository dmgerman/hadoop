...
annotation|@
name|Test
DECL|method|testOpenFileDelete ()
specifier|public
name|void
name|testOpenFileDelete
parameter_list|()
throws|throws
name|Throwable
block|{
...
argument_list|)
expr_stmt|;
comment|// now construct the builder.
comment|// even if the open happens in the build operation,
comment|// the failure must not surface until later.
name|CompletableFuture
argument_list|<
name|FSDataInputStream
argument_list|>
...
condition|)
block|{
comment|// support both FileNotFoundException
comment|// and InvalidPathHandleException as different implementations
comment|// support either -and with non-atomic open sequences, possibly
comment|// both
throw|throw
name|ioe
throw|;
...
name|describe
argument_list|(
literal|"openFile fails on a misssng file in the get() and not before"
argument_list|)
expr_stmt|;
name|FileStatus
name|stat
