...
block|}
block|}
comment|// must be set immediately to avoid inconsistent testing issues.
static|static
block|{
...
begin_comment
comment|// verify that getCurrentUser on the same and different subjects can be
end_comment

begin_comment
comment|// concurrent.  Ie. no synchronization.
end_comment
...
literal|8000
argument_list|)
DECL|method|testConcurrentGetCurrentUser ()
specifier|public
name|void
name|testConcurrentGetCurrentUser
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
decl_stmt|;
comment|// swap the User with a spy to allow getCurrentUser to block when the
comment|// spy is called for the user name.
name|Set
argument_list|<
name|Principal
argument_list|>
...
argument_list|)
expr_stmt|;
comment|// wait for the thread to block on the barrier in getCurrentUser.
name|Future
argument_list|<
name|UserGroupInformation
argument_list|>
...
argument_list|()
expr_stmt|;
comment|// old versions of mockito synchronize on returning mocked answers so
comment|// the blocked getCurrentUser will block all other calls to getName.
comment|// workaround this by swapping out the spy with the original User.
name|principals
operator|.
name|remove
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// concurrent getCurrentUser on ugi1 should not be blocked.
name|UserGroupInformation
name|ugi
decl_stmt|;
name|ugi
...
argument_list|)
expr_stmt|;
comment|// concurrent getCurrentUser on ugi2 should not be blocked.
name|ugi
operator|=
name|testUgi2
operator|.
...
argument_list|)
expr_stmt|;
comment|// unblock the original call.
name|barrier
operator|.
name|await
argument_list|()
...
annotation|@
name|Test
DECL|method|testKerberosTicketIsDestroyedChecked ()
specifier|public
name|void
name|testKerberosTicketIsDestroyedChecked
parameter_list|()
throws|throws
name|Exception
block|{
comment|// Create UserGroupInformation
name|GenericTestUtils
operator|.
name|setLogLevel
argument_list|(
...
argument_list|)
decl_stmt|;
comment|// throw IOException in the middle of the autoRenewalForUserCreds
name|doThrow
argument_list|(
operator|new
name|IOException
...
argument_list|()
expr_stmt|;
comment|// Create and destroy the KerberosTicket, so endTime will be null
name|Date
name|d
init|=
operator|new
...
argument_list|()
expr_stmt|;
comment|// run AutoRenewalForUserCredsRunnable with this
name|UserGroupInformation
operator|.
name|AutoRenewalForUserCredsRunnable
name|userCredsRunnable
init|=
name|ugi
operator|.
expr|new
...
argument_list|)
decl_stmt|;
comment|// Set the runnable to not to run in a loop
name|userCredsRunnable
operator|.
...
argument_list|)
expr_stmt|;
comment|// there should be no exception when calling this
name|userCredsRunnable
operator|.
name|run
argument_list|()
expr_stmt|;
comment|// isDestroyed should be called at least once
name|Mockito
operator|.
name|verify
argument_list|(
...
annotation|@
name|Test
DECL|method|testImportTokensFromConfig ()
specifier|public
name|void
name|testImportTokensFromConfig
parameter_list|()
throws|throws
name|IOException
block|{
...
argument_list|()
decl_stmt|;
comment|// Add a base64 token
name|String
name|service0
init|=
literal|"testTokenImportService0"
decl_stmt|;
name|byte
index|[]
name|identity
init|=
literal|"identityImportConfig"
operator|.
name|getBytes
argument_list|()
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Add a token from a file
name|String
name|service1
init|=
literal|"testTokenImportService1"
decl_stmt|;
name|Credentials
name|cred0
init|=
...
argument_list|)
expr_stmt|;
comment|// Check if the tokens were loaded
name|UserGroupInformation
name|ugi
init|=
name|UserGroupInformation
...
name|assertTrue
argument_list|(
literal|"Expected token testTokenService0 not found: "
operator|+
name|outCred
argument_list|,
name|found0
...
name|assertTrue
argument_list|(
literal|"Expected token testTokenService1 not found: "
operator|+
name|outCred
argument_list|,
name|found1
argument_list|)
expr_stmt|;
comment|// Try to add the same token through configuration and file
name|Credentials
name|cred1
init|=
operator|new
...
annotation|@
name|Test
DECL|method|testImportTokensFromProperty ()
specifier|public
name|void
name|testImportTokensFromProperty
parameter_list|()
throws|throws
name|IOException
block|{
...
name|identity
init|=
literal|"identityImportProperty"
operator|.
name|getBytes
argument_list|()
decl_stmt|;
...
name|password
init|=
literal|"passwordImportProperty"
operator|.
name|getBytes
argument_list|()
decl_stmt|;
