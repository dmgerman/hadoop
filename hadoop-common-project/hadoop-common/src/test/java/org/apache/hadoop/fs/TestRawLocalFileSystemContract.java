...
argument_list|)
decl_stmt|;
comment|// These are the string values that DF sees as "Filesystem" for a
comment|// Docker container accessing a Mac or Windows host's filesystem.
DECL|field|FS_TYPE_MAC
specifier|private
specifier|static
specifier|final
name|String
...
literal|"osxfs"
decl_stmt|;
DECL|method|looksLikeMac (String filesys)
specifier|private
specifier|static
name|boolean
name|looksLikeMac
...
argument_list|)
decl_stmt|;
DECL|method|looksLikeWindows (String filesys)
specifier|private
specifier|static
name|boolean
name|looksLikeWindows
parameter_list|(
name|String
name|filesys
parameter_list|)
...
return|;
block|}
comment|// osType is linux or unix-like, but it might be in a container mounting a
comment|// Mac or Windows volume. Use DF to try to determine if this is the case.
name|String
name|rfsPathStr
init|=
...
argument_list|)
decl_stmt|;
comment|// DF.getFilesystem() only provides indirect info about FS type, but it's
comment|// the best we have.  `df -T` would be better, but isn't cross-platform.
name|rfsType
operator|=
operator|(
operator|new
...
name|info
argument_list|(
literal|"DF.Filesystem is {} for path {}"
argument_list|,
name|rfsType
argument_list|,
name|rfsPath
