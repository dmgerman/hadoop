...
argument_list|)
expr_stmt|;
comment|// cleanup trash from previous run if it stuck around
name|fsTarget
operator|.
name|delete
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// create a link for home directory so that trash path works
comment|// set up viewfs's home dir root to point to home dir root on target
...
argument_list|)
expr_stmt|;
comment|// set working dir so that relative paths
comment|//fsView.setWorkingDirectory(new Path(fsTarget.getWorkingDirectory().toUri().getPath()));
name|conf
operator|.
name|set
argument_list|(
