...
block|}
block|}
comment|/**    * A dummy DelegationTokenAuthenticationHandler to verify that the request    * header contains delegation token.    */
DECL|class|HeaderVerifyingDelegationTokenAuthenticationHandler
specifier|public
specifier|static
class|class
name|HeaderVerifyingDelegationTokenAuthenticationHandler
extends|extends
name|DummyDelegationTokenAuthenticationHandler
block|{
annotation|@
...
name|assertNotNull
argument_list|(
literal|"Request header should have delegation token"
argument_list|,
name|request
operator|.
name|getHeader
...
block|}
block|}
comment|/**    * A dummy DelegationTokenAuthenticationFilter that uses a    * {@link HeaderVerifyingDelegationTokenAuthenticationHandler} to verify that    * the request header contains delegation token.    */
DECL|class|HeaderVerifyingFilter
specifier|public
specifier|static
class|class
name|HeaderVerifyingFilter
extends|extends
name|DelegationTokenAuthenticationFilter
block|{
annotation|@
...
literal|120000
argument_list|)
DECL|method|testDelegationTokenAuthenticatorUsingDT ()
specifier|public
name|void
name|testDelegationTokenAuthenticatorUsingDT
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// Create a token that only has dt so that we can test ops when
comment|// authenticating with a delegation token.
name|DelegationTokenAuthenticatedURL
operator|.
name|Token
name|dtOnlyToken
...
argument_list|)
expr_stmt|;
comment|/**      * We're using delegation token, so everything comes from that.      * {@link DelegationTokenAuthenticationHandler#authenticate}.      *      * This means that the special logic we injected at      * {@link DummyAuthenticationHandler#authenticate}      * (check "authenticated" and return 401) wouldn't work any more.      */
name|aUrl
operator|.
name|getDelegationToken
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Verify that after cancelling, we can't renew.
comment|// After cancelling, the dt on token will be set to null. Back it up here.
name|DelegationTokenAuthenticatedURL
operator|.
name|Token
name|cancelledToken
...
name|info
argument_list|(
literal|"Intentional exception caught:"
argument_list|,
name|ex
argument_list|)
expr_stmt|;
