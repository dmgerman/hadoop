...
argument_list|()
decl_stmt|;
comment|// Injector to create providers with different ports. Can only happen in tests
...
DECL|method|createHAProvider (URI[] uris, Configuration conf, String originalUri)
specifier|protected
name|LoadBalancingKMSClientProvider
...
annotation|@
name|AfterClass
DECL|method|shutdownMiniKdc ()
specifier|public
specifier|static
name|void
name|shutdownMiniKdc
parameter_list|()
block|{
if|if
condition|(
...
name|assertTrue
argument_list|(
literal|"KeyProvider should be an instance of KMSClientProvider"
argument_list|,
operator|(
name|provider
operator|instanceof
...
annotation|@
name|Test
DECL|method|doKMSHAZKWithDelegationTokenAccess ()
specifier|public
name|void
name|doKMSHAZKWithDelegationTokenAccess
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// Login as a Kerberos user principal using keytab.
comment|// Connect to KMS to create a delegation token and add it to credentials
specifier|final
name|String
name|keyName
init|=
...
argument_list|)
expr_stmt|;
name|assertTokenIdentifierEquals
argument_list|(
name|credentials
argument_list|)
expr_stmt|;
...
argument_list|)
decl_stmt|;
comment|// verify both tokens can be used to authenticate
for|for
control|(
name|Token
name|t
...
expr_stmt|;
block|}
comment|/**    * Assert that the passed in credentials have 2 tokens, of kind    * {@link KMSDelegationToken#TOKEN_KIND} and    * {@link KMSDelegationToken#TOKEN_LEGACY_KIND}. Assert that the 2 tokens have    * the same identifier.    */
DECL|method|assertTokenIdentifierEquals (Credentials credentials)
specifier|private
name|void
name|assertTokenIdentifierEquals
parameter_list|(
...
name|IOException
block|{
comment|// verify the 2 tokens have the same identifier
name|assertEquals
argument_list|(
literal|2
argument_list|,
...
name|assertEquals
argument_list|(
literal|"KMS DT and legacy dt should have identical identifier"
argument_list|,
name|tokenId
argument_list|,
name|legacyTokenId
...
expr_stmt|;
block|}
comment|/**    * Tests token access with each providers in the    * {@link LoadBalancingKMSClientProvider}. This is to make sure the 2 token    * kinds are compatible and can both be used to authenticate.    */
DECL|method|assertTokenAccess (final LoadBalancingKMSClientProvider lbkp, final String keyName, final Token token)
specifier|private
name|void
name|assertTokenAccess
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|// Verify the tokens can authenticate to any KMS
name|tokenUgi
operator|.
name|addToken
argument_list|(
...
comment|// It should succeed because its delegation token can access any
comment|// KMS instances.
for|for
control|(
name|KMSClientProvider
name|provider
...
condition|)
block|{
comment|// Historically known issue: Legacy token can only work with the
comment|// key provider specified in the token's Service
continue|continue;
block|}
name|LOG
operator|.
name|info
argument_list|(
literal|"Rolling key {} via provider {} with token {}."
argument_list|,
name|keyName
argument_list|,
name|provider
...
annotation|@
name|Test
DECL|method|testKMSHAZKDelegationTokenRenewCancel ()
specifier|public
name|void
name|testKMSHAZKDelegationTokenRenewCancel
parameter_list|()
throws|throws
name|Exception
block|{
...
annotation|@
name|Test
DECL|method|testKMSHAZKDelegationTokenRenewCancelLegacy ()
specifier|public
name|void
name|testKMSHAZKDelegationTokenRenewCancelLegacy
parameter_list|()
throws|throws
name|Exception
block|{
...
expr_stmt|;
block|}
DECL|method|testKMSHAZKDelegationTokenRenewCancel (final Text tokenKind)
specifier|private
name|void
name|testKMSHAZKDelegationTokenRenewCancel
parameter_list|(
...
argument_list|()
decl_stmt|;
comment|// Create a UGI without Kerberos auth. It will be authenticated with
...
argument_list|)
expr_stmt|;
comment|// Test token renewal and cancellation
specifier|final
name|Collection
argument_list|<
name|Token
...
name|assertTrue
argument_list|(
literal|"token should have been canceled"
argument_list|,
name|canceled
argument_list|)
expr_stmt|;
...
name|assertTrue
argument_list|(
literal|"Should have found token kind "
operator|+
name|tokenKind
operator|+
literal|" from "
...
expr_stmt|;
block|}
comment|/**    * Set or unset the key provider configuration based on token kind.    */
DECL|method|setupConfForToken (Text tokenKind, Configuration conf, String lbUri)
specifier|private
name|void
name|setupConfForToken
parameter_list|(
...
block|{
comment|// conf is only required for legacy tokens to create provider,
comment|// new tokens create provider by parsing its own Service field
name|assertEquals
argument_list|(
name|TOKEN_LEGACY_KIND
argument_list|,
