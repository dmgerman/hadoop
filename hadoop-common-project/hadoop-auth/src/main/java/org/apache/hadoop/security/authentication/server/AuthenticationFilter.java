...
name|cookiePath
decl_stmt|;
comment|/**    * Initializes the authentication filter and signer secret provider.    *<p/>    * It instantiates and initializes the specified {@link AuthenticationHandler}.    *<p/>    *    * @param filterConfig filter configuration.    *    * @throws ServletException thrown if the filter or the authentication handler could not be initialized properly.    */
annotation|@
name|Override
...
literal|"unchecked"
argument_list|)
DECL|method|getProviderClass (Properties config)
specifier|private
name|Class
argument_list|<
name|?
...
argument_list|)
decl_stmt|;
comment|// fallback to old behavior
if|if
condition|(
name|signerSecretProviderName
operator|==
