...
name|warn
argument_list|(
literal|"'signature.secret' configuration not set, using a random value as secret"
argument_list|)
expr_stmt|;
block|}
name|signer
...
name|AuthenticationException
argument_list|(
literal|"Missing AuthenticationToken"
argument_list|)
throw|;
block|}
block|}
...
block|}
block|}
comment|/**    * Creates the Hadoop authentiation HTTP cookie.    *<p/>    * It sets the domain and path specified in the configuration.    *    * @param token authentication token for the cookie.    *    * @return the HTTP cookie.    */
DECL|method|createCookie (String token)
specifier|protected
name|Cookie
name|createCookie
parameter_list|(
