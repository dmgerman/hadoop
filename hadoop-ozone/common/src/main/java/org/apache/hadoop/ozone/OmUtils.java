...
return|;
block|}
comment|/**    * Returns true if OZONE_OM_SERVICE_IDS_KEY is defined and not empty.    * @param conf Configuration    * @return true if OZONE_OM_SERVICE_IDS_KEY is defined and not empty;    * else false.    */
DECL|method|isServiceIdsDefined (Configuration conf)
specifier|public
specifier|static
name|boolean
...
return|;
block|}
comment|/**    * Returns true if HA for OzoneManager is configured for the given service id.    * @param conf Configuration    * @param serviceId OM HA cluster service ID    * @return true if HA is configured in the configuration; else false.    */
DECL|method|isOmHAServiceId (Configuration conf, String serviceId)
specifier|public
specifier|static
name|boolean
...
block|}
block|}
comment|/**    * Write OM DB Checkpoint to an output stream as a compressed file (tgz).    * @param checkpoint checkpoint file    * @param destination desination output stream.    * @throws IOException    */
DECL|method|writeOmDBCheckpointToStream (DBCheckpoint checkpoint, OutputStream destination)
specifier|public
specifier|static
name|void
...
name|IOException
argument_list|(
literal|"Can't compress the checkpoint: "
operator|+
name|checkpoint
operator|.
name|getCheckpointLocation
...
name|IOException
block|{
comment|// If this key is in a GDPR enforced bucket, then before moving
comment|// KeyInfo to deletedTable, remove the GDPR related metadata from
comment|// KeyInfo.
if|if
condition|(
name|Boolean
...
condition|)
block|{
comment|//The key doesn't exist in deletedTable, so create a new instance.
name|repeatedOmKeyInfo
operator|=
operator|new
name|RepeatedOmKeyInfo
...
block|{
comment|//The key exists in deletedTable, so update existing instance.
name|repeatedOmKeyInfo
operator|.
name|addOmKeyInfo
argument_list|(
