...
expr_stmt|;
name|boolean
name|needToreleaseOldOwnerLock
init|=
literal|false
decl_stmt|;
try|try
...
argument_list|()
expr_stmt|;
comment|// Release and reacquire lock for now it will not be a problem, as
comment|// applyTransaction serializes the operation's.
comment|// TODO: Revisit this logic once HDDS-1672 checks in.
comment|// releasing volume lock, as to acquire user lock we need to release
comment|// volume lock.
name|omMetadataManager
operator|.
name|getLock
