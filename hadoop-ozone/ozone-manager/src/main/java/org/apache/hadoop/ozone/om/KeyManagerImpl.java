...
argument_list|)
decl_stmt|;
comment|// If om is not managing via ratis, write to db, otherwise write to DB
comment|// will happen via ratis apply transaction.
if|if
condition|(
operator|!
name|isRatisEnabled
...
expr_stmt|;
block|}
comment|// When OM is not managed via ratis we should write in to Om db in
comment|// openKey call.
if|if
condition|(
operator|!
name|isRatisEnabled
...
argument_list|()
decl_stmt|;
comment|// Do we need to call again validateBucket, as this is just called after
comment|// start Transaction from applyTransaction. Can we remove this double
comment|// check?
name|validateBucket
argument_list|(
name|volumeName
...
argument_list|()
decl_stmt|;
comment|// TODO: here if on OM machines clocks are skewed and there is a chance
comment|//  for override of the openKey entries.
try|try
block|{
name|String
name|openKey
...
name|error
argument_list|(
literal|"Apply Open Key failed for volume:{} bucket:{} key:{}"
argument_list|,
name|volumeName
argument_list|,
name|bucketName
...
DECL|method|createKeyInfo (OmKeyArgs keyArgs, List<OmKeyLocationInfo> locations, ReplicationFactor factor, ReplicationType type, long size, FileEncryptionInfo encInfo)
specifier|private
name|OmKeyInfo
name|createKeyInfo
parameter_list|(
