...
name|runCount
decl_stmt|;
DECL|method|KeyDeletingService (OzoneManager ozoneManager, ScmBlockLocationProtocol scmClient, KeyManager manager, long serviceInterval, long serviceTimeout, Configuration conf)
name|KeyDeletingService
parameter_list|(
name|OzoneManager
name|ozoneManager
...
condition|)
block|{
comment|// OzoneManager can be null for testing
return|return
literal|true
return|;
block|}
...
name|Exception
block|{
comment|// Check if this is the Leader OM. If not leader, no need to execute this
comment|// task.
if|if
condition|(
name|shouldRun
...
block|{
comment|// TODO: Once HA and non-HA paths are merged, we should have
comment|//  only one code path here. Purge keys should go through an
comment|//  OMRequest model.
name|delCount
operator|=
name|deleteAllKeys
argument_list|(
...
block|}
block|}
comment|// By design, no one cares about the results of this call back.
return|return
name|EmptyTaskResult
operator|.
name|newResult
...
return|;
block|}
comment|/**      * Submits PurgeKeys request for the keys whose blocks have been deleted      * by SCM.      *      * @param results DeleteBlockGroups returned by SCM.      * @throws IOException      on Error      */
DECL|method|submitPurgeKeysRequest (List<DeleteBlockGroupResult> results)
specifier|public
name|int
name|submitPurgeKeysRequest
parameter_list|(
...
argument_list|()
decl_stmt|;
comment|// Put all keys to be purged in a list
name|int
name|deletedCount
init|=
literal|0
...
condition|)
block|{
comment|// Add key to PurgeKeys list.
name|String
name|deletedKey
init|=
name|result
...
name|debug
argument_list|(
literal|"Key {} set to be purged from OM DB"
argument_list|,
name|deletedKey
argument_list|)
expr_stmt|;
...
argument_list|()
decl_stmt|;
comment|// Submit PurgeKeys request to OM
try|try
block|{
name|ozoneManager
operator|.
...
name|error
argument_list|(
literal|"PurgeKey request failed. Will retry at next run."
argument_list|)
expr_stmt|;
return|return
literal|0
