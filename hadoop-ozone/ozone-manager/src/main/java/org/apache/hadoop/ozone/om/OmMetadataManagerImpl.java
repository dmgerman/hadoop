...
argument_list|)
decl_stmt|;
comment|// During iteration from DB, check in mean time if this bucket is not
comment|// marked for delete.
if|if
condition|(
name|kv
operator|!=
...
argument_list|)
decl_stmt|;
comment|//TODO: When Key ops are converted in to HA model, use cache also to
comment|// determine bucket is empty or not.
try|try
init|(
name|TableIterator
argument_list|<
