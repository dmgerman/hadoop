...
specifier|final
name|String
name|DELEGATION_TOKEN_TABLE
init|=
literal|"dTokenTable"
decl_stmt|;
DECL|field|PREFIX_TABLE
specifier|public
specifier|static
specifier|final
name|String
...
name|s3SecretTable
decl_stmt|;
DECL|field|dTokenTable
specifier|private
name|Table
name|dTokenTable
decl_stmt|;
DECL|field|prefixTable
specifier|private
name|Table
name|prefixTable
decl_stmt|;
...
expr_stmt|;
block|}
comment|/**    * For subclass overriding.    */
DECL|method|OmMetadataManagerImpl ()
specifier|protected
name|OmMetadataManagerImpl
parameter_list|()
block|{
...
block|}
block|}
DECL|method|addOMTablesAndCodecs (DBStoreBuilder builder)
specifier|protected
name|DBStoreBuilder
name|addOMTablesAndCodecs
parameter_list|(
...
return|;
block|}
comment|/**    * Initialize OM Tables.    *    * @throws IOException    */
DECL|method|initializeOmTables ()
specifier|protected
name|void
name|initializeOmTables
parameter_list|()
...
argument_list|)
decl_stmt|;
comment|// First check in bucket table cache.
name|Iterator
argument_list|<
name|Map
operator|.
...
argument_list|()
decl_stmt|;
comment|// Making sure that entry is not for delete bucket request.
if|if
condition|(
name|key
operator|.
...
literal|false
return|;
comment|// we found at least one bucket with this volume
comment|// prefix.
block|}
block|}
else|else
...
literal|false
return|;
comment|// we found at least one key with this vol/bucket
comment|// prefix.
block|}
block|}
else|else
...
literal|0
decl_stmt|;
comment|// For Bucket it is full cache, so we can just iterate in-memory table
comment|// cache.
name|Iterator
argument_list|<
name|Map
...
argument_list|()
decl_stmt|;
comment|// Making sure that entry in cache is not for delete bucket request.
if|if
condition|(
name|omBucketInfo
operator|!=
...
continue|continue;
block|}
comment|// We should return only the keys, whose keys match with prefix and
comment|// the keys after the startBucket.
if|if
condition|(
name|key
operator|.
...
argument_list|()
decl_stmt|;
comment|//TODO: We can avoid this iteration if table cache has stored entries in
comment|// treemap. Currently HashMap is used in Cache. HashMap get operation is an
comment|// constant time operation, where as for treeMap get is log(n).
comment|// So if we move to treemap, the get operation will be affected. As get
comment|// is frequent operation on table. So, for now in list we iterate cache map
comment|// and construct treeMap which match with keyPrefix and are greater than or
comment|// equal to startKey. Later we can revisit this, if list operation
comment|// is becoming slow.
while|while
condition|(
name|iterator
operator|.
...
argument_list|()
decl_stmt|;
comment|// Making sure that entry in cache is not for delete key request.
if|if
condition|(
name|omKeyInfo
operator|!=
...
block|}
block|}
comment|// Get maxKeys from DB if it has.
try|try
init|(
name|TableIterator
argument_list|<
...
argument_list|)
expr_stmt|;
comment|// we need to iterate maxKeys + 1 here because if skipStartKey is true,
comment|// we should skip that entry and return the result.
while|while
condition|(
name|currentCount
operator|<
...
condition|)
block|{
comment|// Entry should not be marked for delete, consider only those
comment|// entries.
if|if
condition|(
operator|!
...
block|}
block|}
comment|// Finally DB entries and cache entries are merged, then return the count
comment|// of maxKeys from the sorted map.
name|currentCount
operator|=
literal|0
expr_stmt|;
...
return|;
block|}
comment|/**    * Update store used by subclass.    *    * @param store DB store.    */
DECL|method|setStore (DBStore store)
specifier|protected
name|void
name|setStore
parameter_list|(
