...
literal|"MD5"
decl_stmt|;
comment|// A common initial MesssageDigest for each key without its UUID
DECL|field|commonInitialMD
specifier|private
name|MessageDigest
name|commonInitialMD
init|=
literal|null
decl_stmt|;
DECL|field|LOG
...
name|description
operator|=
literal|"Specifies whether to validate keys after writing"
argument_list|)
...
name|description
operator|=
literal|"Specifies the buffer size while writing"
argument_list|,
name|defaultValue
operator|=
literal|"4096"
...
decl_stmt|;
DECL|field|totalBucketCount
specifier|private
name|int
name|totalBucketCount
decl_stmt|;
DECL|field|totalKeyCount
specifier|private
name|long
name|totalKeyCount
decl_stmt|;
...
name|volumeCounter
decl_stmt|;
DECL|field|bucketCounter
specifier|private
name|AtomicInteger
name|bucketCounter
decl_stmt|;
DECL|field|keyCounter
specifier|private
name|AtomicLong
name|keyCounter
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Compute the common initial digest for all keys without their UUID
if|if
condition|(
name|validateWrites
condition|)
...
argument_list|()
expr_stmt|;
comment|// wait until all keys are added or exception occurred.
while|while
condition|(
operator|(
name|numberOfKeysAdded
...
name|keyName
decl_stmt|;
comment|/**      * Digest of this key's full value.      */
DECL|field|digest
specifier|private
name|byte
index|[]
name|digest
decl_stmt|;
comment|/**      * Constructs a new ozone keyValidate.      *      * @param bucket    bucket part      * @param keyName   key part      * @param digest    digest of this key's full value      */
DECL|method|KeyValidate (OzoneBucket bucket, String keyName, byte[] digest)
name|KeyValidate
parameter_list|(
name|OzoneBucket
name|bucket
...
block|}
block|}
DECL|method|createVolume (int volumeNumber)
specifier|private
name|boolean
name|createVolume
parameter_list|(
...
block|}
block|}
DECL|method|createBucket (int globalBucketNumber)
specifier|private
name|boolean
name|createBucket
parameter_list|(
name|int
name|globalBucketNumber
parameter_list|)
block|{
name|int
name|volumeNumber
...
name|error
argument_list|(
literal|"Could not create bucket "
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
argument_list|)
DECL|method|createKey (long globalKeyNumber)
specifier|private
name|boolean
name|createKey
parameter_list|(
name|long
name|globalKeyNumber
parameter_list|)
block|{
name|int
name|globalBucketNumber
...
block|}
block|}
DECL|method|getVolume (Integer volumeNumber)
specifier|private
name|OzoneVolume
name|getVolume
parameter_list|(
...
return|;
block|}
DECL|method|getBucket (Integer bucketNumber)
specifier|private
name|OzoneBucket
name|getBucket
parameter_list|(
...
return|;
block|}
comment|/**    * Looks up volume or bucket from the cache.  Waits for it to be created if    * needed (can happen for the last few items depending on the number of    * threads).    *    * @return may return null if this thread is interrupted, or if any other    *   thread encounters an exception (and stores it to {@code exception})    */
DECL|method|waitUntilAddedToMap (Map<Integer, T> map, Integer i)
specifier|private
parameter_list|<
name|T
parameter_list|>
...
annotation|@
name|VisibleForTesting
DECL|method|setKeySize (long keySize)
specifier|public
name|void
name|setKeySize
parameter_list|(
