...
name|IOException
block|{
comment|// TODO : If the block is still not committed and is in the
comment|// pending openBlock Map, it will return BLOCK_NOT_COMMITTED
comment|// exception. We should handle this by retrying the same operation
comment|// n times and update the OzoneManager with the actual block length
comment|// written. At this point of time, we also need to allocate new blocks
