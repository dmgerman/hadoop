...
argument_list|)
expr_stmt|;
comment|// This byteBuffer will be used to cache data until all the blockCommits
comment|// (putBlock) gets replicated to all/majority servers. The idea here is to
comment|// allocate the buffer of size blockSize so that as and when a chunk is
comment|// is replicated to all servers, as a part of discarding the buffer, we
comment|// don't necessarily need to run compaction(buffer.compact() on the buffer
comment|// to actually discard the acknowledged data. Compaction is inefficient so
comment|// it would be a better choice to avoid compaction on the happy I/O path.
name|this
operator|.
name|buffer
operator|=
...
expr_stmt|;
block|}
DECL|method|handleWrite (byte[] b, int off, int len, boolean retry, int pos)
specifier|private
name|void
name|handleWrite
parameter_list|(
...
condition|)
block|{
comment|// for the current iteration, current pos - initialPos gives the
...
argument_list|()
decl_stmt|;
comment|// In case of a failure, read the data from the position till the last
comment|// acknowledgement happened.
if|if
condition|(
name|lastSuccessfulFlushIndex
operator|>
