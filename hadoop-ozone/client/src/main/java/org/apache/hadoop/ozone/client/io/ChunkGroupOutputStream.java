...
argument_list|)
expr_stmt|;
comment|// reset the original length to zero here. It will be updated as and when
comment|// the data gets written.
name|subKeyInfo
operator|.
name|setLength
argument_list|(
...
expr_stmt|;
block|}
DECL|method|incrementBlockLength (int index, long length)
specifier|private
name|void
name|incrementBlockLength
parameter_list|(
name|int
name|index
parameter_list|,
...
literal|0
decl_stmt|;
comment|// TODO : At this point of time, we also need to allocate new blocks
comment|// from a different container and may need to nullify
comment|// all the remaining pre-allocated blocks in case they were
comment|// pre-allocated on the same container which got closed now.This needs
comment|// caching the closed container list on the client itself.
while|while
condition|(
literal|true
condition|)
