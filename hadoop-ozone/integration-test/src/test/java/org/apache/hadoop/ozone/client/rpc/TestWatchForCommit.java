...
begin_comment
comment|/**  * This class verifies the watchForCommit Handling by xceiverClient.  */
end_comment
...
try|try
block|{
comment|// just watch for a log index which in not updated in the commitInfo Map
comment|// as well as there is no logIndex generate in Ratis.
comment|// The basic idea here is just to test if its throws an exception.
name|xceiverClient
operator|.
name|watchForCommit
argument_list|(
...
expr_stmt|;
block|}
comment|// After releasing the xceiverClient, this connection should be closed
...
argument_list|)
expr_stmt|;
comment|// since the timeout value is quite long, the watch request will either
comment|// fail with NotReplicated exceptio, RetryFailureException or
...
argument_list|)
expr_stmt|;
comment|// mark the node stale early so that pipleline gets destroyed quickly
name|conf
operator|.
name|setTimeDuration
argument_list|(
