...
expr_stmt|;
block|}
comment|/**    * Test to create volume without specifying --user or -u.    * @throws Exception    */
annotation|@
name|Test
DECL|method|testCreateVolumeWithoutUser ()
specifier|public
name|void
name|testCreateVolumeWithoutUser
parameter_list|()
throws|throws
name|Exception
block|{
...
block|}
comment|/**    * Execute command, assert exception message and returns true if error    * was thrown.    */
DECL|method|executeWithError (Shell ozoneShell, String[] args, Class exception)
specifier|private
name|void
name|executeWithError
parameter_list|(
...
name|Ignore
argument_list|(
literal|"Can't run without secure cluster."
argument_list|)
DECL|method|testS3Secret ()
specifier|public
name|void
name|testS3Secret
parameter_list|()
...
expr_stmt|;
name|String
name|outputFirstAttempt
decl_stmt|;
name|String
name|outputSecondAttempt
decl_stmt|;
comment|//First attempt: If secrets are not found in database, they will be created
name|String
index|[]
name|args
init|=
...
argument_list|()
expr_stmt|;
comment|//Extracting awsAccessKey& awsSecret value from output
name|String
index|[]
name|output
init|=
...
argument_list|()
expr_stmt|;
comment|//Second attempt: Since secrets were created in previous attempt, it
comment|// should return the same value
name|args
operator|=
operator|new
name|String
...
argument_list|()
expr_stmt|;
comment|//verifying if secrets from both attempts are same
name|assertTrue
argument_list|(
name|outputFirstAttempt
operator|.
...
return|;
block|}
DECL|method|createTestKey (OzoneBucket bucket)
specifier|private
name|OzoneKey
name|createTestKey
parameter_list|(
...
annotation|@
name|Test
DECL|method|testTokenCommands ()
specifier|public
name|void
name|testTokenCommands
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
decl_stmt|;
comment|// Case 1: Execution will fail when security is disabled.
name|shellCommands
operator|.
name|add
argument_list|(
...
name|cmd
argument_list|,
literal|"Error:Token operations "
operator|+
literal|"work only"
argument_list|)
argument_list|)
...
literal|"=true"
decl_stmt|;
comment|// Case 2: Execution of get token will fail when security is enabled but
comment|// OzoneManager is not setup correctly.
name|execute
argument_list|(
operator|new
name|String
...
block|}
argument_list|,
literal|"Error: Get delegation token operation failed."
argument_list|)
expr_stmt|;
...
argument_list|()
expr_stmt|;
comment|// Case 3: Execution of renew/cancel/print token will fail as token file
...
literal|"token "
operator|+
literal|"operation failed as token file:"
argument_list|)
argument_list|)
expr_stmt|;
comment|// Create corrupt token file.
name|File
name|testPath
init|=
name|GenericTestUtils
...
argument_list|()
decl_stmt|;
comment|// Case 4: Execution of renew/cancel/print token will fail if token file
comment|// is corrupt.
name|shellCommands
operator|.
name|add
...
expr_stmt|;
block|}
DECL|method|execute (String[] cmd, String msg)
specifier|private
name|void
name|execute
parameter_list|(
...
return|;
block|}
DECL|method|assertKeyNotExists (OzoneBucket bucket, String keyName)
specifier|private
specifier|static
name|void
name|assertKeyNotExists
...
name|format
argument_list|(
literal|"Key %s should not exist, but it does"
argument_list|,
name|keyName
argument_list|)
argument_list|)
