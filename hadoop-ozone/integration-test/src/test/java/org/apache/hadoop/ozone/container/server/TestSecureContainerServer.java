...
begin_comment
comment|/**  * Test Container servers when security is enabled.  */
end_comment

begin_class
DECL|class|TestSecureContainerServer
specifier|public
class|class
name|TestSecureContainerServer
block|{
DECL|field|TEST_DIR
specifier|static
specifier|final
...
argument_list|()
expr_stmt|;
comment|// Test 1: Test failure in request without block token.
specifier|final
name|ContainerCommandRequestProto
name|request
init|=
...
name|client
decl_stmt|;
comment|// Validation is different for grpc and ratis client.
if|if
condition|(
name|client
operator|instanceof
...
literal|"Failed to"
operator|+
literal|" authenticate with GRPC XceiverServer with Ozone block token"
argument_list|,
parameter_list|()
lambda|->
name|finalClient
...
expr_stmt|;
block|}
comment|// Test 2: Test success in request with valid block token.
name|long
name|expiryTime
init|=
name|Time
