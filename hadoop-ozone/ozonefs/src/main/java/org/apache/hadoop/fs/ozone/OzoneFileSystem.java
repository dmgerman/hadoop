...
block|}
block|}
comment|/**      * Traverse the parent directory structure of keyPath to determine the      * which parent/ grand-parent/.. is the immediate child of the input path f.      * @param keyPath path whose parent directory structure should be traversed.      * @return immediate child path of the input path f.      * @return immediate child path of the input path f.      */
...
return|;
block|}
comment|// Check if the key exists
name|OzoneKey
name|ozoneKey
init|=
name|getKeyInfo
...
name|debug
argument_list|(
literal|"Found exact file for path {}: normal file"
argument_list|,
name|f
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**    * Get the FileStatus for input directory path.    * They key corresponding to input path is appended with a trailing slash    * to return only the corresponding directory key in the bucket.    * @param f directory path    * @return FileStatus for the input directory path    * @throws FileNotFoundException    */
DECL|method|innerGetFileStatusForDir (Path f)
specifier|public
name|FileStatus
name|innerGetFileStatusForDir
parameter_list|(
...
name|debug
argument_list|(
literal|"Found file (with /) for path {}: fake directory"
argument_list|,
name|f
argument_list|)
expr_stmt|;
...
block|{
comment|// Key is a file with trailing slash
name|LOG
operator|.
name|warn
argument_list|(
literal|"Found file (with /) for path {}: real file? should not "
operator|+
literal|"happen"
argument_list|,
name|f
...
return|;
block|}
comment|// File or directory corresponding to input path does not exist.
comment|// Check if there exists a key prefixed with this key.
name|boolean
name|hasChildren
init|=
name|bucket
...
return|;
block|}
comment|/**    *  This class provides an interface to iterate through all the keys in the    *  bucket prefixed with the input path key and process them.    *    *  Each implementing class should define how the keys should be processed    *  through the processKey() function.    */
