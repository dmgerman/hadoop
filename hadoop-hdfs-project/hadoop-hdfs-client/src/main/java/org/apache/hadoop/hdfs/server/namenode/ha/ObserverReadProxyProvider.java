...
begin_comment
comment|/**    * Fetch the service state from a proxy. If it is unable to be fetched,    * assume it is in standby state, but log the exception.    */
end_comment

begin_function
DECL|method|getHAServiceState (NNProxyInfo<T> proxyInfo)
specifier|private
name|HAServiceState
name|getHAServiceState
parameter_list|(
...
parameter_list|)
block|{
comment|// Though a Standby will allow a getHAServiceState call, it won't allow
comment|// delegation token lookup, so if DT is used it throws StandbyException
if|if
condition|(
name|re
operator|.
...
name|debug
argument_list|(
literal|"NameNode {} threw StandbyException when fetching HAState"
argument_list|,
name|proxyInfo
operator|.
name|getAddress
