...
condition|)
block|{
comment|// Check if NN provided uri is not null and ignore property is false.
if|if
condition|(
name|keyProviderUriStr
operator|!=
...
block|}
block|}
comment|// Fallback to configuration.
if|if
condition|(
name|keyProviderUri
operator|==
...
condition|)
block|{
comment|// Either NN is old and doesn't report provider or ignore NN KMS
comment|// provider property is set to true, so use conf.
name|keyProviderUri
operator|=
name|KMSUtil
operator|.
...
return|;
block|}
DECL|method|getKeyProvider (KeyProviderTokenIssuer issuer, Configuration conf)
specifier|public
specifier|static
name|KeyProvider
name|getKeyProvider
