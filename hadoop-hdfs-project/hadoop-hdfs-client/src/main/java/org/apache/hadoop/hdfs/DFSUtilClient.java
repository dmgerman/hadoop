...
return|;
block|}
comment|/**    * Converts a byte array to array of arrays of bytes    * on byte separator.    */
DECL|method|bytes2byteArray (byte[] bytes)
specifier|public
specifier|static
name|byte
index|[]
...
return|;
block|}
comment|// Count the splits. Omit multiple separators and the last one by
comment|// peeking at prior byte.
name|int
name|splits
init|=
literal|0
...
name|nextIndex
decl_stmt|;
comment|// find next separator in the bytes.
while|while
condition|(
name|nextIndex
operator|<
...
name|EMPTY_BYTES
expr_stmt|;
comment|// reuse empty bytes for root.
do|do
block|{
comment|// skip over separators.
name|nextIndex
operator|++
expr_stmt|;
block|}
...
return|;
block|}
comment|/**    * Return address from configuration. Take a list of keys as preference.    * If the address to be returned is the value of DFS_NAMENODE_RPC_ADDRESS_KEY,    * will check to see if auxiliary ports are enabled. If so, call to replace    * address port with auxiliary port. If the address is not the value of    * DFS_NAMENODE_RPC_ADDRESS_KEY, return the original address. If failed to    * find any address, return the given default value.    *    * @param defaultValue the default value if no values found for given keys    * @param suffix suffix to append to keys    * @param conf the configuration    * @param keys a list of keys, ordered by preference    * @return    */
DECL|method|checkKeysAndProcess (String defaultValue, String suffix, Configuration conf, String... keys)
specifier|private
specifier|static
name|String
name|checkKeysAndProcess
...
name|info
argument_list|(
literal|"Using server auxiliary ports "
operator|+
name|Arrays
operator|.
name|toString
...
parameter_list|)
block|{
comment|// return the original address untouched if it is not a valid URI. This
comment|// happens in unit test, as MiniDFSCluster sets the value to
comment|// 127.0.0.1:0, without schema (i.e. "hdfs://"). While in practice, this
comment|// should not be the case. So log a warning message here.
name|LOG
operator|.
name|warn
argument_list|(
literal|"NameNode address is not a valid uri:"
operator|+
name|address
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|// Ignore the port, only take the schema(e.g. hdfs) and host (e.g.
comment|// localhost), then append port
...
argument_list|)
expr_stmt|;
comment|// TODO : currently, only the very first auxiliary port is being used.
comment|// But actually NN supports running multiple auxiliary
name|sb
operator|.
name|append
argument_list|(
...
return|;
block|}
DECL|field|INODE_PATH_MAX_LENGTH
specifier|private
specifier|static
specifier|final
name|int
name|INODE_PATH_MAX_LENGTH
init|=
literal|3
operator|*
name|Path
...
literal|1
decl_stmt|;
comment|/**    * Create the internal unique file path from HDFS file ID (inode ID). Unlike    * a regular file path, this one is guaranteed to refer to the same file at    * all times, across overwrites, etc.    * @param fileId File ID.    * @return The internal ID-based path.    */
DECL|method|makePathFromFileId (long fileId)
specifier|public
specifier|static
name|Path
...
return|;
block|}
comment|/**    * Returns current user home directory under a home directory prefix.    * The home directory prefix can be defined by    * {@link HdfsClientConfigKeys#DFS_USER_HOME_DIR_PREFIX_KEY}.    * User info is obtained from given {@link UserGroupInformation}.    * @param conf configuration    * @param ugi {@link UserGroupInformation} of current user.    * @return the home directory of current user.    */
DECL|method|getHomeDirectory (Configuration conf, UserGroupInformation ugi)
specifier|public
specifier|static
name|Path
