...
expr_stmt|;
block|}
comment|/**    * Verify the metric ProxyOp with RemoteException.    */
annotation|@
name|Test
DECL|method|testProxyOpWithRemoteException ()
specifier|public
name|void
name|testProxyOpWithRemoteException
parameter_list|()
throws|throws
name|IOException
block|{
...
name|testPath
init|=
literal|"/proxy_op/remote_exception.txt"
decl_stmt|;
specifier|final
name|FederationRPCMetrics
name|metrics
...
try|try
block|{
comment|// Create the test file in ns1.
name|createFile
argument_list|(
name|fileSystem1
argument_list|,
...
argument_list|()
decl_stmt|;
comment|// First retry nn0 with remoteException then nn1.
name|getRouterProtocol
argument_list|()
operator|.
...
expr_stmt|;
block|}
comment|/**    * Verify some rpc with previous block not null.    */
annotation|@
name|Test
DECL|method|testPreviousBlockNotNull ()
specifier|public
name|void
name|testPreviousBlockNotNull
parameter_list|()
throws|throws
name|IOException
throws|,
...
name|testPath
init|=
literal|"/getAdditionalData/test.txt"
decl_stmt|;
specifier|final
name|String
name|ns1
...
argument_list|)
expr_stmt|;
comment|// Crate the test file via Router to get file status.
name|HdfsFileStatus
name|status
init|=
name|clientProtocol
...
argument_list|()
decl_stmt|;
comment|// Add a block via router and previous block is null.
name|LocatedBlock
name|blockOne
init|=
name|clientProtocol
...
argument_list|)
expr_stmt|;
comment|// Add a block via router and previous block is not null.
name|LocatedBlock
name|blockTwo
init|=
name|clientProtocol
...
argument_list|)
expr_stmt|;
comment|// Get additionalDatanode via router and block is not null.
name|DatanodeInfo
index|[]
name|exclusions
init|=
...
expr_stmt|;
name|long
name|proxyNumAdditionalDatanode
init|=
name|metrics
operator|.
name|getProcessingOps
...
argument_list|)
expr_stmt|;
comment|// Complete the file via router and last block is not null.
name|clientProtocol
operator|.
name|complete
argument_list|(
...
block|}
block|}
comment|/**    * Test isFileClosed when the result is false.    */
annotation|@
name|Test
