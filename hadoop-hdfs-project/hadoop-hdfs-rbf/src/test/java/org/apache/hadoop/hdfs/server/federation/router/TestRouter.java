begin_unit|revision:0.9.5;language:Java;cregit-version:0.0.1
begin_comment
comment|/**  * Licensed to the Apache Software Foundation (ASF) under one  * or more contributor license agreements.  See the NOTICE file  * distributed with this work for additional information  * regarding copyright ownership.  The ASF licenses this file  * to you under the Apache License, Version 2.0 (the  * "License"); you may not use this file except in compliance  * with the License.  You may obtain a copy of the License at  *  *     http://www.apache.org/licenses/LICENSE-2.0  *  * Unless required by applicable law or agreed to in writing, software  * distributed under the License is distributed on an "AS IS" BASIS,  * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.  * See the License for the specific language governing permissions and  * limitations under the License.  */
end_comment

begin_package
DECL|package|org.apache.hadoop.hdfs.server.federation.router
package|package
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hdfs
operator|.
name|server
operator|.
name|federation
operator|.
name|router
package|;
end_package

begin_import
import|import static
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|test
operator|.
name|GenericTestUtils
operator|.
name|assertExceptionContains
import|;
end_import

begin_import
import|import static
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|test
operator|.
name|LambdaTestUtils
operator|.
name|intercept
import|;
end_import

begin_import
import|import static
name|org
operator|.
name|junit
operator|.
name|Assert
operator|.
name|assertEquals
import|;
end_import

begin_import
import|import static
name|org
operator|.
name|junit
operator|.
name|Assert
operator|.
name|assertNotNull
import|;
end_import

begin_import
import|import static
name|org
operator|.
name|junit
operator|.
name|Assert
operator|.
name|assertNull
import|;
end_import

begin_import
import|import static
name|org
operator|.
name|junit
operator|.
name|Assert
operator|.
name|fail
import|;
end_import

begin_import
import|import static
name|org
operator|.
name|mockito
operator|.
name|Mockito
operator|.
name|mock
import|;
end_import

begin_import
import|import
name|java
operator|.
name|io
operator|.
name|IOException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|net
operator|.
name|InetSocketAddress
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Collection
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|conf
operator|.
name|Configuration
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|fs
operator|.
name|CommonConfigurationKeys
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hdfs
operator|.
name|DFSClient
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hdfs
operator|.
name|DFSConfigKeys
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hdfs
operator|.
name|HdfsConfiguration
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hdfs
operator|.
name|protocol
operator|.
name|HdfsConstants
operator|.
name|DatanodeReportType
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hdfs
operator|.
name|server
operator|.
name|federation
operator|.
name|MockResolver
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hdfs
operator|.
name|server
operator|.
name|federation
operator|.
name|RouterConfigBuilder
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hdfs
operator|.
name|server
operator|.
name|federation
operator|.
name|resolver
operator|.
name|ActiveNamenodeResolver
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|hdfs
operator|.
name|server
operator|.
name|federation
operator|.
name|resolver
operator|.
name|FileSubclusterResolver
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|ipc
operator|.
name|RemoteException
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|hadoop
operator|.
name|service
operator|.
name|Service
operator|.
name|STATE
import|;
end_import

begin_import
import|import
name|org
operator|.
name|junit
operator|.
name|BeforeClass
import|;
end_import

begin_import
import|import
name|org
operator|.
name|junit
operator|.
name|Test
import|;
end_import

begin_comment
comment|/**  * The the safe mode for the {@link Router} controlled by  * {@link SafeModeTimer}.  */
end_comment

begin_class
DECL|class|TestRouter
specifier|public
class|class
name|TestRouter
block|{
DECL|field|conf
specifier|private
specifier|static
name|Configuration
name|conf
decl_stmt|;
annotation|@
name|BeforeClass
DECL|method|create ()
specifier|public
specifier|static
name|void
name|create
parameter_list|()
throws|throws
name|IOException
block|{
comment|// Basic configuration without the state store
name|conf
operator|=
operator|new
name|Configuration
argument_list|()
expr_stmt|;
comment|// 1 sec cache refresh
name|conf
operator|.
name|setInt
argument_list|(
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_CACHE_TIME_TO_LIVE_MS
argument_list|,
literal|1
argument_list|)
expr_stmt|;
comment|// Mock resolver classes
name|conf
operator|.
name|setClass
argument_list|(
name|RBFConfigKeys
operator|.
name|FEDERATION_NAMENODE_RESOLVER_CLIENT_CLASS
argument_list|,
name|MockResolver
operator|.
name|class
argument_list|,
name|ActiveNamenodeResolver
operator|.
name|class
argument_list|)
expr_stmt|;
name|conf
operator|.
name|setClass
argument_list|(
name|RBFConfigKeys
operator|.
name|FEDERATION_FILE_RESOLVER_CLIENT_CLASS
argument_list|,
name|MockResolver
operator|.
name|class
argument_list|,
name|FileSubclusterResolver
operator|.
name|class
argument_list|)
expr_stmt|;
comment|// Bind to any available port
name|conf
operator|.
name|set
argument_list|(
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_RPC_BIND_HOST_KEY
argument_list|,
literal|"0.0.0.0"
argument_list|)
expr_stmt|;
name|conf
operator|.
name|set
argument_list|(
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_RPC_ADDRESS_KEY
argument_list|,
literal|"127.0.0.1:0"
argument_list|)
expr_stmt|;
name|conf
operator|.
name|set
argument_list|(
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_ADMIN_ADDRESS_KEY
argument_list|,
literal|"127.0.0.1:0"
argument_list|)
expr_stmt|;
name|conf
operator|.
name|set
argument_list|(
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_ADMIN_BIND_HOST_KEY
argument_list|,
literal|"0.0.0.0"
argument_list|)
expr_stmt|;
name|conf
operator|.
name|set
argument_list|(
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_HTTP_ADDRESS_KEY
argument_list|,
literal|"127.0.0.1:0"
argument_list|)
expr_stmt|;
name|conf
operator|.
name|set
argument_list|(
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_HTTPS_ADDRESS_KEY
argument_list|,
literal|"127.0.0.1:0"
argument_list|)
expr_stmt|;
name|conf
operator|.
name|set
argument_list|(
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_HTTP_BIND_HOST_KEY
argument_list|,
literal|"0.0.0.0"
argument_list|)
expr_stmt|;
comment|// Simulate a co-located NN
name|conf
operator|.
name|set
argument_list|(
name|DFSConfigKeys
operator|.
name|DFS_NAMESERVICES
argument_list|,
literal|"ns0"
argument_list|)
expr_stmt|;
name|conf
operator|.
name|set
argument_list|(
name|CommonConfigurationKeys
operator|.
name|FS_DEFAULT_NAME_KEY
argument_list|,
literal|"hdfs://"
operator|+
literal|"ns0"
argument_list|)
expr_stmt|;
name|conf
operator|.
name|set
argument_list|(
name|DFSConfigKeys
operator|.
name|DFS_NAMENODE_RPC_ADDRESS_KEY
operator|+
literal|"."
operator|+
literal|"ns0"
argument_list|,
literal|"127.0.0.1:0"
operator|+
literal|0
argument_list|)
expr_stmt|;
name|conf
operator|.
name|set
argument_list|(
name|DFSConfigKeys
operator|.
name|DFS_NAMENODE_HTTP_ADDRESS_KEY
operator|+
literal|"."
operator|+
literal|"ns0"
argument_list|,
literal|"127.0.0.1:"
operator|+
literal|0
argument_list|)
expr_stmt|;
name|conf
operator|.
name|set
argument_list|(
name|DFSConfigKeys
operator|.
name|DFS_NAMENODE_RPC_BIND_HOST_KEY
operator|+
literal|"."
operator|+
literal|"ns0"
argument_list|,
literal|"0.0.0.0"
argument_list|)
expr_stmt|;
block|}
DECL|method|testRouterStartup (Configuration routerConfig)
specifier|private
specifier|static
name|void
name|testRouterStartup
parameter_list|(
name|Configuration
name|routerConfig
parameter_list|)
throws|throws
name|InterruptedException
throws|,
name|IOException
block|{
name|Router
name|router
init|=
operator|new
name|Router
argument_list|()
decl_stmt|;
name|assertEquals
argument_list|(
name|STATE
operator|.
name|NOTINITED
argument_list|,
name|router
operator|.
name|getServiceState
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|RouterServiceState
operator|.
name|UNINITIALIZED
argument_list|,
name|router
operator|.
name|getRouterState
argument_list|()
argument_list|)
expr_stmt|;
name|router
operator|.
name|init
argument_list|(
name|routerConfig
argument_list|)
expr_stmt|;
if|if
condition|(
name|routerConfig
operator|.
name|getBoolean
argument_list|(
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_SAFEMODE_ENABLE
argument_list|,
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_SAFEMODE_ENABLE_DEFAULT
argument_list|)
condition|)
block|{
name|assertEquals
argument_list|(
name|RouterServiceState
operator|.
name|SAFEMODE
argument_list|,
name|router
operator|.
name|getRouterState
argument_list|()
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|assertEquals
argument_list|(
name|RouterServiceState
operator|.
name|INITIALIZING
argument_list|,
name|router
operator|.
name|getRouterState
argument_list|()
argument_list|)
expr_stmt|;
block|}
name|assertEquals
argument_list|(
name|STATE
operator|.
name|INITED
argument_list|,
name|router
operator|.
name|getServiceState
argument_list|()
argument_list|)
expr_stmt|;
name|router
operator|.
name|start
argument_list|()
expr_stmt|;
if|if
condition|(
name|routerConfig
operator|.
name|getBoolean
argument_list|(
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_SAFEMODE_ENABLE
argument_list|,
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_SAFEMODE_ENABLE_DEFAULT
argument_list|)
condition|)
block|{
name|assertEquals
argument_list|(
name|RouterServiceState
operator|.
name|SAFEMODE
argument_list|,
name|router
operator|.
name|getRouterState
argument_list|()
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|assertEquals
argument_list|(
name|RouterServiceState
operator|.
name|RUNNING
argument_list|,
name|router
operator|.
name|getRouterState
argument_list|()
argument_list|)
expr_stmt|;
block|}
name|assertEquals
argument_list|(
name|STATE
operator|.
name|STARTED
argument_list|,
name|router
operator|.
name|getServiceState
argument_list|()
argument_list|)
expr_stmt|;
name|router
operator|.
name|stop
argument_list|()
expr_stmt|;
name|assertEquals
argument_list|(
name|RouterServiceState
operator|.
name|SHUTDOWN
argument_list|,
name|router
operator|.
name|getRouterState
argument_list|()
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|STATE
operator|.
name|STOPPED
argument_list|,
name|router
operator|.
name|getServiceState
argument_list|()
argument_list|)
expr_stmt|;
name|router
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
annotation|@
name|Test
DECL|method|testRouterService ()
specifier|public
name|void
name|testRouterService
parameter_list|()
throws|throws
name|InterruptedException
throws|,
name|IOException
block|{
comment|// Admin only
name|testRouterStartup
argument_list|(
operator|new
name|RouterConfigBuilder
argument_list|(
name|conf
argument_list|)
operator|.
name|admin
argument_list|()
operator|.
name|build
argument_list|()
argument_list|)
expr_stmt|;
comment|// Http only
name|testRouterStartup
argument_list|(
operator|new
name|RouterConfigBuilder
argument_list|(
name|conf
argument_list|)
operator|.
name|http
argument_list|()
operator|.
name|build
argument_list|()
argument_list|)
expr_stmt|;
comment|// Rpc only
name|testRouterStartup
argument_list|(
operator|new
name|RouterConfigBuilder
argument_list|(
name|conf
argument_list|)
operator|.
name|rpc
argument_list|()
operator|.
name|build
argument_list|()
argument_list|)
expr_stmt|;
comment|// Safemode only
name|testRouterStartup
argument_list|(
operator|new
name|RouterConfigBuilder
argument_list|(
name|conf
argument_list|)
operator|.
name|rpc
argument_list|()
operator|.
name|safemode
argument_list|()
operator|.
name|build
argument_list|()
argument_list|)
expr_stmt|;
comment|// Metrics only
name|testRouterStartup
argument_list|(
operator|new
name|RouterConfigBuilder
argument_list|(
name|conf
argument_list|)
operator|.
name|metrics
argument_list|()
operator|.
name|build
argument_list|()
argument_list|)
expr_stmt|;
comment|// Statestore only
name|testRouterStartup
argument_list|(
operator|new
name|RouterConfigBuilder
argument_list|(
name|conf
argument_list|)
operator|.
name|stateStore
argument_list|()
operator|.
name|build
argument_list|()
argument_list|)
expr_stmt|;
comment|// Heartbeat only
name|testRouterStartup
argument_list|(
operator|new
name|RouterConfigBuilder
argument_list|(
name|conf
argument_list|)
operator|.
name|heartbeat
argument_list|()
operator|.
name|build
argument_list|()
argument_list|)
expr_stmt|;
comment|// Run with all services
name|testRouterStartup
argument_list|(
operator|new
name|RouterConfigBuilder
argument_list|(
name|conf
argument_list|)
operator|.
name|all
argument_list|()
operator|.
name|build
argument_list|()
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Test
DECL|method|testRouterRestartRpcService ()
specifier|public
name|void
name|testRouterRestartRpcService
parameter_list|()
throws|throws
name|IOException
block|{
comment|// Start
name|Router
name|router
init|=
operator|new
name|Router
argument_list|()
decl_stmt|;
name|router
operator|.
name|init
argument_list|(
operator|new
name|RouterConfigBuilder
argument_list|(
name|conf
argument_list|)
operator|.
name|rpc
argument_list|()
operator|.
name|build
argument_list|()
argument_list|)
expr_stmt|;
name|router
operator|.
name|start
argument_list|()
expr_stmt|;
comment|// Verify RPC server is running
name|assertNotNull
argument_list|(
name|router
operator|.
name|getRpcServerAddress
argument_list|()
argument_list|)
expr_stmt|;
name|RouterRpcServer
name|rpcServer
init|=
name|router
operator|.
name|getRpcServer
argument_list|()
decl_stmt|;
name|assertNotNull
argument_list|(
name|rpcServer
argument_list|)
expr_stmt|;
name|assertEquals
argument_list|(
name|STATE
operator|.
name|STARTED
argument_list|,
name|rpcServer
operator|.
name|getServiceState
argument_list|()
argument_list|)
expr_stmt|;
comment|// Stop router and RPC server
name|router
operator|.
name|stop
argument_list|()
expr_stmt|;
name|assertEquals
argument_list|(
name|STATE
operator|.
name|STOPPED
argument_list|,
name|rpcServer
operator|.
name|getServiceState
argument_list|()
argument_list|)
expr_stmt|;
name|router
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
annotation|@
name|Test
DECL|method|testRouterRpcWithNoSubclusters ()
specifier|public
name|void
name|testRouterRpcWithNoSubclusters
parameter_list|()
throws|throws
name|IOException
block|{
name|Router
name|router
init|=
operator|new
name|Router
argument_list|()
decl_stmt|;
name|router
operator|.
name|init
argument_list|(
operator|new
name|RouterConfigBuilder
argument_list|(
name|conf
argument_list|)
operator|.
name|rpc
argument_list|()
operator|.
name|build
argument_list|()
argument_list|)
expr_stmt|;
name|router
operator|.
name|start
argument_list|()
expr_stmt|;
name|InetSocketAddress
name|serverAddress
init|=
name|router
operator|.
name|getRpcServerAddress
argument_list|()
decl_stmt|;
name|DFSClient
name|dfsClient
init|=
operator|new
name|DFSClient
argument_list|(
name|serverAddress
argument_list|,
name|conf
argument_list|)
decl_stmt|;
try|try
block|{
name|dfsClient
operator|.
name|create
argument_list|(
literal|"/test.txt"
argument_list|,
literal|false
argument_list|)
expr_stmt|;
name|fail
argument_list|(
literal|"Create with no subclusters should fail"
argument_list|)
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|RemoteException
name|e
parameter_list|)
block|{
name|assertExceptionContains
argument_list|(
literal|"Cannot find locations for /test.txt"
argument_list|,
name|e
argument_list|)
expr_stmt|;
block|}
try|try
block|{
name|dfsClient
operator|.
name|datanodeReport
argument_list|(
name|DatanodeReportType
operator|.
name|LIVE
argument_list|)
expr_stmt|;
name|fail
argument_list|(
literal|"Get datanode reports with no subclusters should fail"
argument_list|)
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|IOException
name|e
parameter_list|)
block|{
name|assertExceptionContains
argument_list|(
literal|"No remote locations available"
argument_list|,
name|e
argument_list|)
expr_stmt|;
block|}
name|dfsClient
operator|.
name|close
argument_list|()
expr_stmt|;
name|router
operator|.
name|stop
argument_list|()
expr_stmt|;
name|router
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
annotation|@
name|Test
DECL|method|testRouterIDInRouterRpcClient ()
specifier|public
name|void
name|testRouterIDInRouterRpcClient
parameter_list|()
throws|throws
name|Exception
block|{
name|Router
name|router
init|=
operator|new
name|Router
argument_list|()
decl_stmt|;
name|router
operator|.
name|init
argument_list|(
operator|new
name|RouterConfigBuilder
argument_list|(
name|conf
argument_list|)
operator|.
name|rpc
argument_list|()
operator|.
name|build
argument_list|()
argument_list|)
expr_stmt|;
name|router
operator|.
name|setRouterId
argument_list|(
literal|"Router-0"
argument_list|)
expr_stmt|;
name|RemoteMethod
name|remoteMethod
init|=
name|mock
argument_list|(
name|RemoteMethod
operator|.
name|class
argument_list|)
decl_stmt|;
name|intercept
argument_list|(
name|IOException
operator|.
name|class
argument_list|,
literal|"Router-0"
argument_list|,
parameter_list|()
lambda|->
name|router
operator|.
name|getRpcServer
argument_list|()
operator|.
name|getRPCClient
argument_list|()
operator|.
name|invokeSingle
argument_list|(
literal|"ns0"
argument_list|,
name|remoteMethod
argument_list|)
argument_list|)
expr_stmt|;
name|router
operator|.
name|stop
argument_list|()
expr_stmt|;
name|router
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
annotation|@
name|Test
DECL|method|testRouterMetricsWhenDisabled ()
specifier|public
name|void
name|testRouterMetricsWhenDisabled
parameter_list|()
throws|throws
name|Exception
block|{
name|Router
name|router
init|=
operator|new
name|Router
argument_list|()
decl_stmt|;
name|router
operator|.
name|init
argument_list|(
operator|new
name|RouterConfigBuilder
argument_list|(
name|conf
argument_list|)
operator|.
name|rpc
argument_list|()
operator|.
name|build
argument_list|()
argument_list|)
expr_stmt|;
name|router
operator|.
name|start
argument_list|()
expr_stmt|;
name|intercept
argument_list|(
name|IOException
operator|.
name|class
argument_list|,
literal|"Namenode metrics is not initialized"
argument_list|,
parameter_list|()
lambda|->
name|router
operator|.
name|getNamenodeMetrics
argument_list|()
operator|.
name|getCacheCapacity
argument_list|()
argument_list|)
expr_stmt|;
name|router
operator|.
name|stop
argument_list|()
expr_stmt|;
name|router
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
annotation|@
name|Test
DECL|method|testSwitchRouter ()
specifier|public
name|void
name|testSwitchRouter
parameter_list|()
throws|throws
name|IOException
block|{
name|assertRouterHeartbeater
argument_list|(
literal|true
argument_list|,
literal|true
argument_list|)
expr_stmt|;
name|assertRouterHeartbeater
argument_list|(
literal|true
argument_list|,
literal|false
argument_list|)
expr_stmt|;
name|assertRouterHeartbeater
argument_list|(
literal|false
argument_list|,
literal|true
argument_list|)
expr_stmt|;
name|assertRouterHeartbeater
argument_list|(
literal|false
argument_list|,
literal|false
argument_list|)
expr_stmt|;
block|}
comment|/**    * Execute the test by specify the routerHeartbeat and nnHeartbeat switch.    *    * @param expectedRouterHeartbeat expect the routerHeartbeat enable state.    * @param expectedNNHeartbeat expect the nnHeartbeat enable state.    */
DECL|method|assertRouterHeartbeater (boolean expectedRouterHeartbeat, boolean expectedNNHeartbeat)
specifier|private
name|void
name|assertRouterHeartbeater
parameter_list|(
name|boolean
name|expectedRouterHeartbeat
parameter_list|,
name|boolean
name|expectedNNHeartbeat
parameter_list|)
throws|throws
name|IOException
block|{
specifier|final
name|Router
name|router
init|=
operator|new
name|Router
argument_list|()
decl_stmt|;
name|Configuration
name|baseCfg
init|=
operator|new
name|RouterConfigBuilder
argument_list|(
name|conf
argument_list|)
operator|.
name|rpc
argument_list|()
operator|.
name|build
argument_list|()
decl_stmt|;
name|baseCfg
operator|.
name|setBoolean
argument_list|(
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_HEARTBEAT_ENABLE
argument_list|,
name|expectedRouterHeartbeat
argument_list|)
expr_stmt|;
name|baseCfg
operator|.
name|setBoolean
argument_list|(
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_NAMENODE_HEARTBEAT_ENABLE
argument_list|,
name|expectedNNHeartbeat
argument_list|)
expr_stmt|;
name|router
operator|.
name|init
argument_list|(
name|baseCfg
argument_list|)
expr_stmt|;
name|RouterHeartbeatService
name|routerHeartbeatService
init|=
name|router
operator|.
name|getRouterHeartbeatService
argument_list|()
decl_stmt|;
if|if
condition|(
name|expectedRouterHeartbeat
condition|)
block|{
name|assertNotNull
argument_list|(
name|routerHeartbeatService
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|assertNull
argument_list|(
name|routerHeartbeatService
argument_list|)
expr_stmt|;
block|}
name|Collection
argument_list|<
name|NamenodeHeartbeatService
argument_list|>
name|namenodeHeartbeatServices
init|=
name|router
operator|.
name|getNamenodeHeartbeatServices
argument_list|()
decl_stmt|;
if|if
condition|(
name|expectedNNHeartbeat
condition|)
block|{
name|assertNotNull
argument_list|(
name|namenodeHeartbeatServices
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|assertNull
argument_list|(
name|namenodeHeartbeatServices
argument_list|)
expr_stmt|;
block|}
name|router
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
annotation|@
name|Test
DECL|method|testNamenodeHeartBeatEnableDefault ()
specifier|public
name|void
name|testNamenodeHeartBeatEnableDefault
parameter_list|()
throws|throws
name|IOException
block|{
name|checkNamenodeHeartBeatEnableDefault
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|checkNamenodeHeartBeatEnableDefault
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
comment|/**    * Check the default value of dfs.federation.router.namenode.heartbeat.enable    * when it isn't explicitly defined.    * @param enable value for dfs.federation.router.heartbeat.enable.    */
DECL|method|checkNamenodeHeartBeatEnableDefault (boolean enable)
specifier|private
name|void
name|checkNamenodeHeartBeatEnableDefault
parameter_list|(
name|boolean
name|enable
parameter_list|)
throws|throws
name|IOException
block|{
specifier|final
name|Router
name|router
init|=
operator|new
name|Router
argument_list|()
decl_stmt|;
try|try
block|{
name|Configuration
name|config
init|=
operator|new
name|HdfsConfiguration
argument_list|()
decl_stmt|;
name|config
operator|.
name|setBoolean
argument_list|(
name|RBFConfigKeys
operator|.
name|DFS_ROUTER_HEARTBEAT_ENABLE
argument_list|,
name|enable
argument_list|)
expr_stmt|;
name|router
operator|.
name|init
argument_list|(
name|config
argument_list|)
expr_stmt|;
if|if
condition|(
name|enable
condition|)
block|{
name|assertNotNull
argument_list|(
name|router
operator|.
name|getNamenodeHeartbeatServices
argument_list|()
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|assertNull
argument_list|(
name|router
operator|.
name|getNamenodeHeartbeatServices
argument_list|()
argument_list|)
expr_stmt|;
block|}
block|}
finally|finally
block|{
name|router
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
block|}
block|}
end_class

end_unit

