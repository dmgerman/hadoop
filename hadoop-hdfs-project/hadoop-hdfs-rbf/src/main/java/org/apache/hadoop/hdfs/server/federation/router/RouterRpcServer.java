...
name|clientProto
decl_stmt|;
comment|/** Other protocol calls. */
DECL|field|routerProto
specifier|private
specifier|final
...
return|;
block|}
comment|/**    * Invokes the method at default namespace, if default namespace is not    * available then at the first available namespace.    * @param<T> expected return type.    * @param method the remote method.    * @return the response received after invoking method.    * @throws IOException    */
DECL|method|invokeAtAvailableNs (RemoteMethod method, Class<T> clazz)
parameter_list|<
name|T
parameter_list|>
name|T
...
return|;
block|}
comment|// If default Ns is not present return result from first namespace.
name|Set
argument_list|<
name|FederationNamespaceInfo
argument_list|>
...
name|IOException
argument_list|(
literal|"No namespace available."
argument_list|)
throw|;
block|}
name|nsId
...
return|;
block|}
comment|/**    * Get the location to create a file. It checks if the file already existed    * in one of the locations.    *    * @param src Path of the file to check.    * @param locations Prefetched locations for the file.    * @return The remote location for this file.    * @throws IOException If the file has no creation location.    */
DECL|method|getCreateLocation ( final String src, final List<RemoteLocation> locations)
name|RemoteLocation
name|getCreateLocation
parameter_list|(
specifier|final
...
return|;
block|}
comment|/**    * Gets the remote location where the file exists.    * @param src the name of file.    * @param locations all the remote locations.    * @return the remote location of the file if it exists, else null.    * @throws IOException in case of any exception.    */
DECL|method|getExistingLocation (String src, List<RemoteLocation> locations)
specifier|private
name|RemoteLocation
name|getExistingLocation
parameter_list|(
...
begin_comment
comment|/**    * Check if a path supports failed subclusters.    *    * @param path Path to check.    * @return If a path should support failed subclusters.    */
end_comment

begin_function
DECL|method|isPathFaultTolerant (final String path)
name|boolean
...
argument_list|)
decl_stmt|;
comment|// If this is a mount point, we need to invoke everywhere.
if|if
condition|(
name|mountPoints
operator|!=
