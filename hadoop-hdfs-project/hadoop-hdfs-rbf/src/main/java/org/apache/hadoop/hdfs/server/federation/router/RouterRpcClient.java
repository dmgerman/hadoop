...
expr_stmt|;
block|}
comment|/**    * Get the configuration for the RPC client. It takes the Router    * configuration and transforms it into regular RPC Client configuration.    * @param conf Input configuration.    * @return Configuration for the RPC client.    */
DECL|method|getClientConfiguration (final Configuration conf)
specifier|private
name|Configuration
name|getClientConfiguration
parameter_list|(
...
return|;
block|}
comment|/**    * JSON representation of the async caller thread pool.    *    * @return String representation of the JSON.    */
DECL|method|getAsyncCallerPoolJson ()
specifier|public
name|String
...
return|;
block|}
comment|/**    * Get ClientProtocol proxy client for a NameNode. Each combination of user +    * NN must use a unique proxy client. Previously created clients are cached    * and stored in a connection pool by the ConnectionManager.    *    * @param ugi User group information.    * @param nsId Nameservice identifier.    * @param rpcAddress RPC server address of the NN.    * @param proto Protocol of the connection.    * @return ConnectionContext containing a ClientProtocol proxy client for the    *         NN + current user.    * @throws IOException If we cannot get a connection to the NameNode.    */
DECL|method|getConnection (UserGroupInformation ugi, String nsId, String rpcAddress, Class<?> proto)
specifier|private
...
block|}
block|}
comment|/**    * Invokes a method against the ClientProtocol proxy server. If a standby    * exception is generated by the call to the client, retries using the    * alternate server.    *    * Re-throws exceptions generated by the remote RPC call as either    * RemoteException or IOException.    *    * @param ugi User group information.    * @param namenodes A prioritized list of namenodes within the same    *                  nameservice.    * @param method Remote ClientProtcol method to invoke.    * @param params Variable list of parameters matching the method.    * @return The result of invoking the method.    * @throws ConnectException If it cannot connect to any Namenode.    * @throws StandbyException If all Namenodes are in Standby.    * @throws IOException If it cannot invoke the method.    */
DECL|method|invokeMethod ( final UserGroupInformation ugi, final List<? extends FederationNamenodeContext> namenodes, final Class<?> protocol, final Method method, final Object... params)
specifier|private
name|Object
name|invokeMethod
parameter_list|(
...
name|error
argument_list|(
literal|"Get connection for {} {} error: {}"
argument_list|,
name|nsId
argument_list|,
name|rpcAddress
...
argument_list|)
expr_stmt|;
comment|// Throw StandbyException so that client can retry
name|StandbyException
name|se
init|=
operator|new
...
name|error
argument_list|(
literal|"Cannot get available namenode for {} {} error: {}"
argument_list|,
name|nsId
argument_list|,
name|rpcAddress
...
return|;
block|}
comment|/**    * Invokes a remote method against the specified namespace.    *    * Re-throws exceptions generated by the remote RPC call as either    * RemoteException or IOException.    *    * @param<T> The type of the remote method return.    * @param nsId Target namespace for the method.    * @param method The remote method and parameters to invoke.    * @param clazz Class for the return type.    * @return The result of invoking the method.    * @throws IOException If the invoke generated an error.    */
DECL|method|invokeSingle (final String nsId, RemoteMethod method, Class<T> clazz)
specifier|public
parameter_list|<
name|T
parameter_list|>
...
return|;
block|}
comment|/**    * Invokes a remote method against the specified extendedBlock.    *    * Re-throws exceptions generated by the remote RPC call as either    * RemoteException or IOException.    *    * @param<T> The type of the remote method return.    * @param extendedBlock Target extendedBlock for the method.    * @param method The remote method and parameters to invoke.    * @param clazz Class for the return type.    * @return The result of invoking the method.    * @throws IOException If the invoke generated an error.    */
DECL|method|invokeSingle (final ExtendedBlock extendedBlock, RemoteMethod method, Class<T> clazz)
specifier|public
parameter_list|<
name|T
parameter_list|>
...
DECL|method|invokeSingle (final RemoteLocationContext location, RemoteMethod remoteMethod, Class<T> clazz)
specifier|public
parameter_list|<
name|T
parameter_list|>
...
return|;
block|}
comment|/**    * Exception messages might contain local subcluster paths. This method    * generates a new exception with the proper message.    * @param ioe Original IOException.    * @param loc Location we are processing.    * @return Exception processed for federation.    */
DECL|method|processException ( IOException ioe, RemoteLocationContext loc)
specifier|private
name|IOException
name|processException
parameter_list|(
...
return|;
block|}
comment|/**    * Process a subcluster message and make it federated.    * @param msg Original exception message.    * @param dst Path in federation.    * @param src Path in the subcluster.    * @return Message processed for federation.    */
annotation|@
name|VisibleForTesting
DECL|method|processExceptionMsg ( final String msg, final String dst, final String src)
specifier|static
name|String
name|processExceptionMsg
parameter_list|(
...
control|)
block|{
comment|// Check if we can replace sub folders
name|String
name|dst1
init|=
name|dst
...
block|}
block|}
comment|/**    * Invoke method in all locations and return success if any succeeds.    *    * @param<T> The type of the remote location.    * @param<R> The type of the remote method return.    * @param locations List of remote locations to call concurrently.    * @param method The remote method and parameters to invoke.    * @return If the call succeeds in any location.    * @throws IOException If any of the calls return an exception.    */
...
return|;
block|}
comment|/**    * Invoke multiple concurrent proxy calls to different clients. Returns an    * array of results.    *    * Re-throws exceptions generated by the remote RPC call as either    * RemoteException or IOException.    *    * @param<T> The type of the remote location.    * @param<R> The type of the remote method return.    * @param locations List of remote locations to call concurrently.    * @param method The remote method and parameters to invoke.    * @throws IOException If all the calls throw an exception.    */
...
expr_stmt|;
block|}
comment|/**    * Invoke multiple concurrent proxy calls to different clients. Returns an    * array of results.    *    * Re-throws exceptions generated by the remote RPC call as either    * RemoteException or IOException.    *    * @param<T> The type of the remote location.    * @param<R> The type of the remote method return.    * @param locations List of remote locations to call concurrently.    * @param method The remote method and parameters to invoke.    * @return Result of invoking the method per subcluster: nsId to result.    * @throws IOException If all the calls throw an exception.    */
...
argument_list|>
argument_list|>
DECL|method|invokeConcurrent (final Collection<T> locations, final RemoteMethod method, boolean standby, long timeOutMs, Class<R> clazz)
name|invokeConcurrent
parameter_list|(
specifier|final
name|Collection
...
name|msg
init|=
literal|"Not enough client threads "
operator|+
name|active
operator|+
literal|"/"
