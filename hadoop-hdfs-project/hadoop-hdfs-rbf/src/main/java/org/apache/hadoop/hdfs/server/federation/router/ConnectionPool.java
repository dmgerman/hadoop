...
literal|0
decl_stmt|;
DECL|method|ConnectionPool (Configuration config, String address, UserGroupInformation user, int minPoolSize, int maxPoolSize)
specifier|protected
name|ConnectionPool
parameter_list|(
name|Configuration
...
return|;
block|}
comment|/**    * Creates a proxy wrapper for a client NN connection. Each proxy contains    * context for a single user/security context. To maximize throughput it is    * recommended to use multiple connection per user+server, allowing multiple    * writes and reads to be dispatched in parallel.    *    * @param conf Configuration for the connection.    * @param nnAddress Address of server supporting the ClientProtocol.    * @param ugi User context.    * @return Proxy for the target ClientProtocol that contains the user's    *         security context.    * @throws IOException If it cannot be created.    */
DECL|method|newConnection (Configuration conf, String nnAddress, UserGroupInformation ugi)
specifier|protected
specifier|static
name|ConnectionContext
name|newConnection
