...
argument_list|)
expr_stmt|;
comment|// For BaseTestHttpFSWith#testFileAclsCustomizedUserAndGroupNames
name|conf
operator|.
name|set
argument_list|(
...
name|status2
decl_stmt|;
comment|// Check HDFS-specific fields
name|assertEquals
argument_list|(
name|hdfsFileStatus2
operator|.
...
condition|)
block|{
comment|// LocalFS doesn't support setQuota so skip here
return|return;
block|}
name|DistributedFileSystem
name|dfs
...
block|,
DECL|enumConstant|GET_SERVERDEFAULTS
DECL|enumConstant|CHECKACCESS
name|GET_SERVERDEFAULTS
block|,
name|CHECKACCESS
block|}
...
name|FILEACLS
case|:
name|testFileAclsCustomizedUserAndGroupNames
argument_list|()
expr_stmt|;
name|testFileAcls
argument_list|()
...
name|GET_SNAPSHOTTABLE_DIRECTORY_LIST
case|:
name|testGetSnapshottableDirListing
argument_list|()
expr_stmt|;
break|break;
case|case
...
argument_list|()
operator|+
literal|" doesn't support getSnapshotDiff"
argument_list|)
expr_stmt|;
block|}
comment|// Verify result with DFS
name|DistributedFileSystem
name|dfs
init|=
operator|(
...
block|}
block|}
DECL|method|testGetSnapshotDiffIllegalParamCase (FileSystem fs, Path path, String oldsnapshotname, String snapshotname)
specifier|private
name|void
name|testGetSnapshotDiffIllegalParamCase
parameter_list|(
name|FileSystem
name|fs
parameter_list|,
...
parameter_list|)
block|{
comment|// Expect SnapshotException, IllegalArgumentException
comment|// or RemoteException(IllegalArgumentException)
if|if
condition|(
name|e
operator|instanceof
...
condition|)
block|{
comment|// Check RemoteException class name, should be IllegalArgumentException
name|Assert
operator|.
name|assertEquals
argument_list|(
...
name|fail
argument_list|(
literal|"getSnapshotDiff illegal param didn't throw Exception"
argument_list|)
expr_stmt|;
block|}
...
block|}
block|}
DECL|method|verifyGetSnapshottableDirListing ( FileSystem fs, DistributedFileSystem dfs)
specifier|private
name|void
name|verifyGetSnapshottableDirListing
parameter_list|(
name|FileSystem
name|fs
parameter_list|,
...
name|Exception
block|{
comment|// Get snapshottable directory list
name|SnapshottableDirectoryStatus
index|[]
name|sds
init|=
...
argument_list|()
operator|+
literal|" doesn't support getSnapshottableDirListing"
argument_list|)
expr_stmt|;
block|}
comment|// Verify result with DFS
...
expr_stmt|;
block|}
DECL|method|testGetSnapshottableDirListing ()
specifier|private
name|void
name|testGetSnapshottableDirListing
parameter_list|()
...
argument_list|()
decl_stmt|;
comment|// Create directories with snapshot allowed
name|Path
name|path1
init|=
operator|new
...
block|}
block|}
DECL|method|testFileAclsCustomizedUserAndGroupNames ()
specifier|private
name|void
name|testFileAclsCustomizedUserAndGroupNames
parameter_list|()
...
return|return;
block|}
comment|// Get appropriate conf from the cluster
name|MiniDFSCluster
name|miniDFSCluster
init|=
operator|(
...
argument_list|)
decl_stmt|;
comment|// If we call getHttpFSFileSystem() without conf from the mini cluster,
comment|// WebHDFS will be initialized with the default ACL string, causing the
comment|// setAcl() later to fail. This is only an issue in the unit test.
name|FileSystem
name|httpfs
init|=
name|getHttpFSFileSystem
...
argument_list|()
operator|+
literal|" doesn't support custom user and group name pattern. "
operator|+
literal|"Only WebHdfsFileSystem and HttpFSFileSystem support it."
argument_list|)
expr_stmt|;
block|}
specifier|final
...
argument_list|)
expr_stmt|;
comment|// Verify getAclStatus responses are the same
name|AclStatus
name|proxyAclStat
init|=
name|proxyFs
...
argument_list|)
expr_stmt|;
comment|// Verify that custom user and group are set.
name|List
argument_list|<
name|String
argument_list|>
...
expr_stmt|;
block|}
DECL|method|verifyGetServerDefaults (FileSystem fs, DistributedFileSystem dfs)
specifier|private
name|void
name|verifyGetServerDefaults
parameter_list|(
name|FileSystem
name|fs
parameter_list|,
...
argument_list|()
operator|+
literal|" doesn't support getServerDefaults"
argument_list|)
expr_stmt|;
block|}
comment|// Verify result with DFS
...
block|}
block|}
DECL|method|verifyAccess (FileSystem fs, DistributedFileSystem dfs)
specifier|private
name|void
name|verifyAccess
parameter_list|(
...
argument_list|()
operator|+
literal|" doesn't support access"
argument_list|)
expr_stmt|;
block|}
block|}
