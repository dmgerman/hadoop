...
name|FSOperations
block|{
comment|/**    * This class is used to group a FileStatus and an AclStatus together.    * It's needed for the GETFILESTATUS and LISTSTATUS calls, which take    * most info from the FileStatus and a wee bit from the AclStatus.    */
DECL|class|StatusPair
specifier|private
specifier|static
class|class
name|StatusPair
...
name|fileStatus
decl_stmt|;
DECL|field|aclStatus
specifier|private
name|AclStatus
name|aclStatus
decl_stmt|;
comment|/**      * Simple constructor      * @param fileStatus Existing FileStatus object      * @param aclStatus Existing AclStatus object      */
DECL|method|StatusPair (FileStatus fileStatus, AclStatus aclStatus)
specifier|public
name|StatusPair
parameter_list|(
name|FileStatus
...
expr_stmt|;
block|}
comment|/**      * Create one StatusPair by performing the underlying calls to      * fs.getFileStatus and fs.getAclStatus      * @param fs The FileSystem where 'path' lives      * @param path The file/directory to query      * @throws IOException      */
DECL|method|StatusPair (FileSystem fs, Path path)
specifier|public
name|StatusPair
parameter_list|(
name|FileSystem
...
parameter_list|)
block|{
comment|/*          * The cause is almost certainly an "ACLS aren't enabled"          * exception, so leave aclStatus at null and carry on.          */
block|}
catch|catch
parameter_list|(
...
parameter_list|)
block|{
comment|/* Ditto above - this is the case for a local file system */
block|}
block|}
comment|/**      * Return a Map suitable for conversion into JSON format      * @return The JSONish Map      */
DECL|method|toJson ()
specifier|public
name|Map
argument_list|<
...
return|;
block|}
comment|/**      * Return in inner part of the JSON for the status - used by both the      * GETFILESTATUS and LISTSTATUS calls.      * @param emptyPathSuffix Whether or not to include PATH_SUFFIX_JSON      * @return The JSONish Map      */
DECL|method|toJsonInner (boolean emptyPathSuffix)
specifier|public
name|Map
argument_list|<
name|String
...
block|}
block|}
comment|/**    * Simple class used to contain and operate upon a list of StatusPair    * objects.  Used by LISTSTATUS.    */
DECL|class|StatusPairs
specifier|private
specifier|static
class|class
name|StatusPairs
block|{
DECL|field|statusPairs
specifier|private
name|StatusPair
index|[]
name|statusPairs
decl_stmt|;
comment|/**      * Construct a list of StatusPair objects      * @param fs The FileSystem where 'path' lives      * @param path The directory to query      * @param filter A possible filter for entries in the directory      * @throws IOException      */
DECL|method|StatusPairs (FileSystem fs, Path path, PathFilter filter)
specifier|public
name|StatusPairs
parameter_list|(
name|FileSystem
...
name|IOException
block|{
comment|/* Grab all the file statuses at once in an array */
name|FileStatus
index|[]
name|fileStatuses
init|=
...
argument_list|)
decl_stmt|;
comment|/* We'll have an array of StatusPairs of the same length */
name|AclStatus
name|aclStatus
init|=
literal|null
...
index|]
expr_stmt|;
comment|/*        * For each FileStatus, attempt to acquire an AclStatus.  If the        * getAclStatus throws an exception, we assume that ACLs are turned        * off entirely and abandon the attempt.        */
name|boolean
name|useAcls
init|=
literal|true
decl_stmt|;
comment|// Assume ACLs work until proven otherwise
for|for
control|(
name|int
name|i
...
parameter_list|)
block|{
comment|/* Almost certainly due to an "ACLs not enabled" exception */
name|aclStatus
operator|=
literal|null
expr_stmt|;
...
block|}
block|}
comment|/**      * Return a Map suitable for conversion into JSON.      * @return A JSONish Map      */
annotation|@
