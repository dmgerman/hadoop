...
name|assertEquals
argument_list|(
literal|"Inconsistent number of applied txns on Standby"
argument_list|,
name|nn1
operator|.
name|getNamesystem
...
annotation|@
name|Test
DECL|method|testTailerBackoff ()
specifier|public
name|void
name|testTailerBackoff
parameter_list|()
throws|throws
name|Exception
block|{
...
annotation|@
name|Test
DECL|method|testRollEditLogIOExceptionForRemoteNN ()
specifier|public
name|void
name|testRollEditLogIOExceptionForRemoteNN
parameter_list|()
throws|throws
name|IOException
block|{
...
argument_list|)
decl_stmt|;
comment|// It should go on to next name node when IOException happens.
name|when
argument_list|(
name|tailer
operator|.
...
parameter_list|()
constructor_decl|;
comment|// MultipleNameNodeProxy uses Round-robin to look for active NN
comment|// to do RollEditLog. If doWork() fails, then IOException throws,
comment|// it continues to try next NN. triggerActiveLogRoll finishes
comment|// either due to success, or using up retries.
comment|// In this test case, there are 2 remote name nodes, default retry is 3.
comment|// For test purpose, doWork() always returns IOException,
comment|// so the total invoked times will be default retry 3 * remote NNs 2 = 6
name|assertEquals
argument_list|(
literal|6
argument_list|,
...
unit|}    @
name|Test
DECL|method|testStandbyTriggersLogRollsWhenTailInProgressEdits ()
specifier|public
name|void
name|testStandbyTriggersLogRollsWhenTailInProgressEdits
parameter_list|()
throws|throws
name|Exception
block|{
comment|// Time in seconds to wait for standby to catch up to edits from active
specifier|final
name|int
name|standbyCatchupWaitTime
init|=
literal|2
decl_stmt|;
comment|// Time in seconds to wait before checking if edit logs are rolled while
comment|// expecting no edit log roll
specifier|final
name|int
name|noLogRollWaitTime
init|=
literal|2
decl_stmt|;
comment|// Time in seconds to wait before checking if edit logs are rolled while
comment|// expecting edit log roll
specifier|final
name|int
name|logRollWaitTime
init|=
literal|3
decl_stmt|;
name|Configuration
...
argument_list|()
decl_stmt|;
name|waitForStandbyToCatchUpWithInProgressEdits
argument_list|(
name|standby
argument_list|,
name|activeTxId
...
begin_function
DECL|method|waitForStandbyToCatchUpWithInProgressEdits ( final NameNode standby, final long activeTxId, int maxWaitSec)
specifier|private
specifier|static
name|void
name|waitForStandbyToCatchUpWithInProgressEdits
...
begin_function
DECL|method|checkForLogRoll (final NameNode active, final long origTxId, int maxWaitSec)
specifier|private
specifier|static
name|void
name|checkForLogRoll
...
begin_function
DECL|method|createMiniDFSCluster (Configuration conf, int nnCount)
specifier|private
specifier|static
name|MiniDFSCluster
name|createMiniDFSCluster
...
literal|2
decl_stmt|;
comment|// By passing in basePort, name node will have IPC port set,
comment|// which is needed for enabling roll log.
name|MiniDFSNNTopology
name|topology
init|=
name|MiniDFSNNTopology
