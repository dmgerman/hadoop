...
argument_list|()
expr_stmt|;
comment|// look for the method name in the stack trace
name|assertTrue
argument_list|(
name|logs
operator|.
...
argument_list|)
expr_stmt|;
comment|// find the held interval time in the log
name|Pattern
name|pattern
init|=
name|Pattern
...
argument_list|)
expr_stmt|;
comment|// only keep the "yyyy-MM-dd" part of date
name|String
name|startTimeStr
init|=
literal|"held at "
...
argument_list|()
expr_stmt|;
comment|// Add one lock hold which is the longest, but occurs under a different
comment|// stack trace, to ensure this is the one that gets logged
name|timer
operator|.
name|advance
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// Assert that stack trace eventually logged is the one for the longest hold
name|String
...
name|contains
argument_list|(
literal|"Number of suppressed read-lock reports: 3"
argument_list|)
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// match the held interval time in the log
name|Pattern
name|pattern
init|=
name|Pattern
