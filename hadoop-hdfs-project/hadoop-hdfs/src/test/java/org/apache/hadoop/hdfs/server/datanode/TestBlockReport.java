...
expr_stmt|;
block|}
comment|/**    * Test writes a file and closes it. Then test finds a block    * and changes its GS to be< of original one.    * New empty block is added to the list of blocks.    * Block report is forced and the check for # of corrupted blocks is performed.    *    * @throws IOException in case of an error    */
annotation|@
name|Test
...
argument_list|)
decl_stmt|;
comment|// The block with modified GS won't be found. Has to be deleted
name|blocks
operator|.
name|get
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// This new block is unknown to NN and will be mark for deletion.
name|blocks
operator|.
name|add
argument_list|(
...
name|assertEquals
argument_list|(
literal|"Wrong number of CorruptedReplica+PendingDeletion "
operator|+
literal|"blocks is found"
argument_list|,
literal|2
argument_list|,
name|cluster
...
expr_stmt|;
block|}
comment|/**    * This test isn't a representative case for BlockReport    * The empty method is going to be left here to keep the naming    * of the test plan in synch with the actual implementation    * @throws IOException in case of errors    */
...
name|IOException
block|{   }
comment|// Client requests new block from NN. The test corrupts this very block
comment|// and forces new block report.
comment|// The test case isn't specific for BlockReport because it relies on
comment|// BlockScanner which is out of scope of this test
comment|// Keeping the name to be in synch with the test plan
comment|//
DECL|method|blockReport_05 ()
specifier|public
name|void
name|blockReport_05
parameter_list|()
...
annotation|@
name|Test
comment|// Currently this test is failing as expected 'cause the correct behavior is
comment|// not yet implemented (9/15/09)
...
argument_list|)
decl_stmt|;
comment|// Get a block and screw its GS
name|Block
name|corruptedBlock
init|=
name|blocks
...
name|assertEquals
argument_list|(
literal|"Wrong number of Corrupted blocks"
argument_list|,
literal|1
argument_list|,
name|cluster
...
argument_list|()
operator|+
comment|// the following might have to be added into the equation if
comment|// the same block could be in two different states at the same time
comment|// and then the expected number of has to be changed to '2'
comment|//        cluster.getNamesystem().getPendingReplicationBlocks() +
name|cluster
operator|.
name|getNamesystem
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// Get another block and screw its length to be less than original
if|if
condition|(
name|randIndex
operator|==
...
name|debug
argument_list|(
literal|"Done corrupting length of "
operator|+
name|corruptedBlock
operator|.
name|getBlockName
...
expr_stmt|;
block|}
DECL|method|corruptBlockLen (final Block block)
specifier|private
name|void
name|corruptBlockLen
parameter_list|(
...
name|IOException
argument_list|(
literal|"Block isn't suppose to be null"
argument_list|)
throw|;
block|}
name|long
...
name|assertTrue
argument_list|(
literal|"Old and new length shouldn't be the same"
argument_list|,
name|block
operator|.
name|getNumBytes
...
block|}
block|}
DECL|method|corruptBlockGS (final Block block)
specifier|private
name|void
name|corruptBlockGS
parameter_list|(
...
name|assertTrue
argument_list|(
literal|"Old and new GS shouldn't be the same"
argument_list|,
name|block
operator|.
name|getGenerationStamp
