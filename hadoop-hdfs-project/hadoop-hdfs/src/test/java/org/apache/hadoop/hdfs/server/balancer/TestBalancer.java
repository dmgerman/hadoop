...
argument_list|)
decl_stmt|;
comment|// fill up the cluster to be `clusterUtilization` full
name|long
name|totalUsedSpace
init|=
call|(
...
literal|100000
argument_list|)
DECL|method|testMaxIterationTime ()
specifier|public
name|void
name|testMaxIterationTime
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// limit the worker thread count of Balancer to have only 1 queue per DN
name|conf
operator|.
name|setInt
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// limit the bandwitdh to 1 packet per sec to emulate slow block moves
name|conf
operator|.
name|setLong
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// set max iteration time to 2 seconds to timeout before moving any block
name|conf
operator|.
name|setLong
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// setup Balancer and run one iteration
name|List
argument_list|<
name|NameNodeConnector
argument_list|>
...
name|assertEquals
argument_list|(
literal|"We expect ExitStatus.IN_PROGRESS to be reported."
argument_list|,
name|ExitStatus
operator|.
name|IN_PROGRESS
...
argument_list|)
expr_stmt|;
comment|// accept runtime if it is under 3.5 seconds, as we need to wait for
comment|// IN_PROGRESS report from DN, and some spare to be able to finish.
comment|// NOTE: This can be a source of flaky tests, if the box is busy,
comment|// assertion here is based on the following: Balancer is already set
comment|// up, iteration gets the blocks from the NN, and makes the decision
comment|// to move 2 blocks. After that the PendingMoves are scheduled, and
comment|// DataNode heartbeats in for the Balancer every second, iteration is
comment|// two seconds long. This means that it will fail if the setup and the
comment|// heartbeat from the DataNode takes more than 500ms, as the iteration
comment|// should end at the 3rd second from start. As the number of
comment|// operations seems to be pretty low, and all comm happens locally, I
comment|// think the possibility of a failure due to node busyness is low.
name|assertTrue
argument_list|(
literal|"Unexpected iteration runtime: "
operator|+
name|runtime
operator|+
literal|"ms> 3.5s"
...
expr_stmt|;
block|}
comment|/**    * Test that makes the Balancer to disperse RPCs to the NameNode    * in order to avoid NN's RPC queue saturation. This not marked as @Test    * because it is run from {@link TestBalancerRPCDelay}.    */
DECL|method|testBalancerRPCDelay (int getBlocksMaxQps)
name|void
name|testBalancerRPCDelay
parameter_list|(
name|int
name|getBlocksMaxQps
parameter_list|)
throws|throws
name|Exception
block|{
...
name|RACK2
argument_list|,
comment|// Use only 1 node and set the starting capacity to 50% to allow the
comment|// balancing to complete in only one iteration. This is necessary
comment|// because the startGetBlocksTime and endGetBlocksTime measures across
comment|// all get block calls, so if two iterations are performed, the duration
comment|// also includes the time it took to perform the block move ops in the
...
name|info
argument_list|(
literal|"Balancer executed {} getBlocks in {} msec (round up to {} sec)"
argument_list|,
name|numGetBlocksCalls
operator|.
name|get
...
name|assertTrue
argument_list|(
literal|"Expected balancer getBlocks calls per second<= "
operator|+
name|getBlocksMaxQps
argument_list|,
name|getBlockCallsPerSecond
