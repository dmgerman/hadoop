...
expr_stmt|;
block|}
DECL|method|runBasicTest (boolean shotdown)
specifier|private
name|void
name|runBasicTest
parameter_list|(
...
return|;
block|}
DECL|method|genStorageTypes (int numDataNodes, int numAllDisk, int numAllArchive)
specifier|private
specifier|static
name|StorageType
index|[]
...
return|;
block|}
DECL|method|genCapacities (int nDatanodes, int numAllDisk, int numAllArchive, long diskCapacity, long archiveCapacity)
specifier|private
specifier|static
name|long
index|[]
...
argument_list|)
expr_stmt|;
comment|// create hot files with replication 3 until not more spaces.
specifier|final
name|short
name|replication
init|=
...
expr_stmt|;
block|}
comment|// create hot files with replication 1 to use up all remaining spaces.
block|{
name|int
name|hotFileCount_r1
init|=
literal|0
decl_stmt|;
try|try
...
name|info
argument_list|(
literal|"Expected: hotFileCount_r1="
operator|+
name|hotFileCount_r1
argument_list|,
name|e
...
argument_list|)
expr_stmt|;
comment|//      DFSTestUtil.waitReplication(test.dfs, file0, newReplication);
name|Thread
operator|.
name|sleep
argument_list|(
...
argument_list|)
expr_stmt|;
comment|//      DFSTestUtil.waitReplication(test.dfs, p, newReplication);
name|Thread
operator|.
name|sleep
argument_list|(
...
comment|//TODO: fix Mover not terminate in the test below
comment|//      final Path file1 = new Path(pathPolicyMap.hot, "file1");
comment|//      test.dfs.rename(file1, pathPolicyMap.warm);
comment|//      test.migrate();
comment|//      test.verify(true);
block|}
name|test
operator|.
name|shutdownCluster
...
argument_list|)
expr_stmt|;
comment|// create cold files with replication 3 until not more spaces.
specifier|final
name|short
name|replication
init|=
...
name|info
argument_list|(
literal|"Expected: coldFileCount="
operator|+
name|coldFileCount
argument_list|,
name|e
...
expr_stmt|;
block|}
comment|// create cold files with replication 1 to use up all remaining spaces.
block|{
name|int
name|coldFileCount_r1
init|=
literal|0
decl_stmt|;
try|try
...
name|info
argument_list|(
literal|"Expected: coldFileCount_r1="
operator|+
name|coldFileCount_r1
argument_list|,
name|e
