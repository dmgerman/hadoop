...
name|getTestDir
argument_list|(
literal|"TestStandbyCheckpoints"
argument_list|)
expr_stmt|;
name|tmpOivImgDir
operator|.
...
expr_stmt|;
block|}
comment|/**    * Test for the case of when there are observer NameNodes, Standby node is    * able to upload fsImage to Observer node as well.    */
annotation|@
name|Test
argument_list|(
name|timeout
...
literal|300000
argument_list|)
DECL|method|testStandbyAndObserverState ()
specifier|public
name|void
name|testStandbyAndObserverState
parameter_list|()
throws|throws
name|Exception
block|{
comment|// Transition 2 to observer
name|cluster
operator|.
...
argument_list|)
expr_stmt|;
comment|// After a rollEditLog, Standby(nn1) 's next checkpoint would be
comment|// ahead of observer(nn2).
name|nns
index|[
literal|0
index|]
...
argument_list|()
expr_stmt|;
comment|// After standby creating a checkpoint, it will try to push the image to
comment|// active and all observer, updating it's own txid to the most recent.
name|HATestUtil
operator|.
name|waitForCheckpoint
argument_list|(
...
argument_list|()
decl_stmt|;
comment|// observer and standby both have this same image.
name|dirs
operator|.
name|addAll
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Restore 2 back to standby
name|cluster
operator|.
name|transitionToStandby
argument_list|(
...
expr_stmt|;
block|}
comment|/**    * Test that checkpointing is still successful even if an issue    * was encountered while writing the legacy OIV image.    */
annotation|@
name|Test
argument_list|(
name|timeout
...
literal|300000
argument_list|)
DECL|method|testCheckpointSucceedsWithLegacyOIVException ()
specifier|public
name|void
name|testCheckpointSucceedsWithLegacyOIVException
parameter_list|()
throws|throws
name|Exception
block|{
comment|// Delete the OIV image dir to cause an IOException while saving
name|FileUtil
operator|.
name|fullyDelete
argument_list|(
