...
block|}
block|}
comment|/**    * Test that server default values are cached on the client size    * and are stale after namenode update.    */
annotation|@
name|Test
DECL|method|testServerDefaultsWithCaching ()
specifier|public
name|void
name|testServerDefaultsWithCaching
parameter_list|()
throws|throws
name|IOException
throws|,
...
block|{
comment|// Create cluster with an explicit block size param
name|Configuration
name|clusterConf
init|=
operator|new
...
argument_list|()
expr_stmt|;
comment|// Set a spy namesystem inside the namenode and return it
name|FSNamesystem
name|spyNamesystem
init|=
name|NameNodeAdapter
...
try|try
block|{
comment|// Create a dfs client and set a long enough validity interval
name|Configuration
name|clientConf
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// Update the namenode with a new parameter
name|long
name|updatedDefaultBlockSize
init|=
name|DFS_BLOCK_SIZE_DEFAULT
operator|*
literal|3
...
argument_list|)
expr_stmt|;
comment|// Another client reads the updated value correctly
name|DFSClient
name|newDfsClient
init|=
operator|new
...
block|}
block|}
comment|/**    * Test that server defaults are updated on the client after cache expiration.    */
annotation|@
name|Test
DECL|method|testServerDefaultsWithMinimalCaching ()
specifier|public
name|void
name|testServerDefaultsWithMinimalCaching
parameter_list|()
throws|throws
name|IOException
throws|,
...
try|try
block|{
comment|// Create a dfs client and set a minimal validity interval
name|Configuration
name|clientConf
init|=
operator|new
...
argument_list|()
decl_stmt|;
comment|// Invalidate cache in at most 1 ms, see DfsClient#getServerDefaults
name|clientConf
operator|.
name|setLong
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Update the namenode with a new FsServerDefaults
name|long
name|updatedDefaultBlockSize
init|=
name|DFS_BLOCK_SIZE_DEFAULT
...
argument_list|()
expr_stmt|;
comment|// Value is updated correctly
name|assertEquals
argument_list|(
name|updatedDefaultBlockSize
argument_list|,
