...
expr_stmt|;
block|}
comment|/**    * DN decommission shouldn't reconstruction busy DN block.    * @throws Exception    */
annotation|@
name|Test
argument_list|(
name|timeout
...
literal|120000
argument_list|)
DECL|method|testDecommissionWithBusyNode ()
specifier|public
name|void
name|testDecommissionWithBusyNode
parameter_list|()
throws|throws
name|Exception
block|{
...
decl_stmt|;
name|byte
name|decommisionDNIndex
init|=
literal|0
decl_stmt|;
comment|//1. create EC file
specifier|final
name|Path
name|ecFile
init|=
...
name|ecDir
argument_list|,
literal|"testDecommissionWithBusyNode"
argument_list|)
decl_stmt|;
name|int
name|writeBytes
...
expr_stmt|;
block|}
comment|//3. decomission one node
name|List
argument_list|<
name|DatanodeInfo
argument_list|>
...
argument_list|)
expr_stmt|;
comment|//4. wait for decommission block to replicate
name|Thread
operator|.
name|sleep
argument_list|(
...
name|assertEquals
argument_list|(
literal|"Busy DN shouldn't be reconstructed"
argument_list|,
name|dnStorageInfos
index|[
name|busyDNIndex
...
argument_list|)
expr_stmt|;
comment|//5. check decommission DN block index, it should be reconstructed again
name|LocatedBlocks
name|lbs
init|=
name|cluster
...
decl_stmt|;
name|int
name|decommissionBlockIndexCount
init|=
literal|0
decl_stmt|;
for|for
...
name|assertEquals
argument_list|(
literal|"Decommission DN block should be reconstructed"
argument_list|,
literal|2
argument_list|,
name|decommissionBlockIndexCount
...
return|;
block|}
DECL|method|writeStripedFile (DistributedFileSystem fs, Path ecFile, int writeBytes)
specifier|private
name|byte
index|[]
name|writeStripedFile
...
return|;
block|}
comment|/**    * Simulate that There are 2 nodes(dn0,dn1) in decommission. Firstly dn0    * replicates in success, dn1 replicates in failure. Decommissions go on.    */
annotation|@
name|Test
argument_list|(
name|timeout
...
literal|120000
argument_list|)
DECL|method|testDecommissionWithFailedReplicating ()
specifier|public
name|void
name|testDecommissionWithFailedReplicating
parameter_list|()
throws|throws
name|Exception
block|{
comment|// Write ec file.
name|Path
name|ecFile
init|=
operator|new
...
name|ecDir
argument_list|,
literal|"firstReplicationFailedFile"
argument_list|)
decl_stmt|;
name|int
name|writeBytes
...
argument_list|)
expr_stmt|;
comment|// Get 2 nodes of ec block and set them in decommission.
comment|// The 2 nodes are not in pendingNodes of DatanodeAdminManager.
name|List
argument_list|<
name|LocatedBlock
argument_list|>
...
argument_list|)
expr_stmt|;
comment|// Replicate dn0 block to another dn
comment|// Simulate that dn0 replicates in success, dn1 replicates in failure.
specifier|final
name|byte
name|blockIndex
init|=
...
argument_list|)
expr_stmt|;
comment|// dn0 replicates in success
name|GenericTestUtils
operator|.
name|waitFor
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// There are 8 live replicas
name|BlockInfoStriped
name|blockInfo
init|=
operator|(
...
argument_list|)
expr_stmt|;
comment|// Add the 2 nodes to pendingNodes of DatanodeAdminManager
name|bm
operator|.
name|getDatanodeManager
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// There are 9 live replicas
name|assertEquals
argument_list|(
literal|9
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// After dn0& dn1 decommissioned, all internal Blocks(0~8) are there
name|Iterator
argument_list|<
name|DatanodeStorageInfo
argument_list|>
...
condition|)
block|{
comment|// Skip decommissioned nodes
continue|continue;
block|}
name|byte
name|index
...
block|}
block|}
comment|/**    * Get a Datanode which does not contain the block.    */
DECL|method|getDatanodeOutOfTheBlock (LocatedStripedBlock blk)
specifier|private
name|DatanodeInfo
name|getDatanodeOutOfTheBlock
parameter_list|(
...
literal|120000
argument_list|)
DECL|method|testDecommissionWithMissingBlock ()
specifier|public
name|void
name|testDecommissionWithMissingBlock
parameter_list|()
throws|throws
name|Exception
block|{
...
name|ecDir
argument_list|,
literal|"missingOneInternalBLockFile"
argument_list|)
decl_stmt|;
name|int
name|writeBytes
...
literal|0
decl_stmt|;
comment|// Add the 4 nodes, and set the 4 nodes decommissioning.
comment|// So that they are decommissioning at the same time
for|for
control|(
name|int
name|i
...
argument_list|)
expr_stmt|;
comment|// Namenode does not handle decommissioning nodes now
name|assertEquals
argument_list|(
literal|0
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Replicate dn1 block to another dn
comment|// So that one of the 4 replicas has been replicated.
specifier|final
name|byte
name|blockIndex
init|=
...
argument_list|)
expr_stmt|;
comment|// Wait for replication success.
name|GenericTestUtils
operator|.
name|waitFor
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Reopen ecFile, get the new locations.
name|lb
operator|=
name|dfs
operator|.
...
argument_list|()
decl_stmt|;
comment|// Now the block has 10 internal blocks.
name|assertEquals
argument_list|(
literal|10
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Stop the dn0(stopNodeIndex) datanode
comment|// So that the internal block from this dn misses
name|DataNode
name|dn
init|=
name|cluster
...
argument_list|)
expr_stmt|;
comment|// So far, there are 4 decommissioning nodes, 1 replica has been
comment|// replicated, and 1 replica misses. There are 8 total internal
comment|// blocks, 5 live and 3 decommissioning internal blocks.
name|assertEquals
argument_list|(
literal|5
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Handle decommission nodes in a new thread.
comment|// Verify that nodes are decommissioned.
specifier|final
name|CountDownLatch
name|decomStarted
init|=
...
argument_list|)
expr_stmt|;
comment|// Wake up to reconstruct the block.
name|BlockManagerTestUtil
operator|.
...
argument_list|)
expr_stmt|;
comment|// Wait for decommissioning
name|GenericTestUtils
operator|.
name|waitFor
argument_list|(
comment|// Whether there are 8 live replicas after decommission.
parameter_list|()
lambda|->
name|bm
operator|.
...
argument_list|)
expr_stmt|;
comment|// There are 9 live replicas, 0 decommissioning replicas.
name|BlockInfoStriped
name|blockInfo
init|=
operator|(
...
argument_list|)
expr_stmt|;
comment|// Adjust internal block locations
comment|// [b0(decommissioning), b1, b2, b3, b4, b5, b6, b7, b8, b0(live)] changed
comment|// to [b0(live), b1, b2, b3, b4, b5, b6, b7, b8, b0(decommissioning)]
name|BlockManagerTestUtil
operator|.
name|removeStorage
argument_list|(
...
expr_stmt|;
block|}
comment|/**    * Test recovery for an ec block, its storage array contains these internal    * blocks which are {b0, b1, b2, b3, null, b5, b6, b7, b8, b0, b1, b2,    * b3}, array[0]{b0} in decommissioning, array[1-3]{b1, b2, b3} are    * in decommissioned. array[4] is null, array[5-12]{b[5-8],b[0-3]} are    * in live.    */
annotation|@
name|Test
argument_list|(
name|timeout
...
literal|120000
argument_list|)
DECL|method|testRecoveryWithDecommission ()
specifier|public
name|void
name|testRecoveryWithDecommission
parameter_list|()
throws|throws
name|Exception
block|{
...
name|ecDir
argument_list|,
literal|"testRecoveryWithDecommission"
argument_list|)
decl_stmt|;
name|int
name|writeBytes
...
argument_list|)
decl_stmt|;
comment|// Decommission datanode dn0 contains block b0
comment|// Aim to add storageinfo of replicated block b0 to storages[9] of ec block
name|List
argument_list|<
name|DatanodeInfo
argument_list|>
...
argument_list|)
expr_stmt|;
comment|// Now storages of ec block are (b0{decommissioned}, b[1-8]{live},
comment|// b0{live})
name|assertEquals
argument_list|(
literal|9
...
expr_stmt|;
name|int
name|decommissionNodesNum
init|=
literal|4
decl_stmt|;
comment|// Decommission nodes contain blocks of b[0-3]
comment|// dn0 has been decommissioned
for|for
control|(
name|int
name|i
...
argument_list|)
expr_stmt|;
comment|// Now storages of ec block are (b[0-3]{decommissioned}, b[4-8]{live},
comment|// b0{live}, b[1-3]{live})
comment|// There are 9 live and 4 decommissioned internal blocks
name|assertEquals
argument_list|(
literal|9
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// There are no reconstruction tasks
name|assertEquals
argument_list|(
literal|0
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Set dn0 in decommissioning
comment|// So that the block on dn0 can be used for reconstruction task
name|DatanodeDescriptor
name|dn0
init|=
name|bm
...
argument_list|()
expr_stmt|;
comment|// Stop the datanode contains b4
name|DataNode
name|dn
init|=
name|cluster
...
argument_list|)
expr_stmt|;
comment|// Now storages of ec block are (b[0]{decommissioning},
comment|// b[1-3]{decommissioned}, null, b[5-8]{live}, b0{live}, b[1-3]{live})
comment|// There are 8 live and 1 decommissioning internal blocks
comment|// Wait for reconstruction EC block.
name|GenericTestUtils
operator|.
name|waitFor
argument_list|(
