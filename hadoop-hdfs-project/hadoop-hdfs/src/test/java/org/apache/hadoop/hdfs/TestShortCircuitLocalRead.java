...
block|}
block|}
DECL|method|checkFileContent (FileSystem fs, Path name, byte[] expected, int readOffset)
specifier|static
name|void
...
expr_stmt|;
block|}
comment|/**    * Verifies that reading a file with the direct read(ByteBuffer) api gives the expected set of bytes.    */
DECL|method|checkFileContentDirect (FileSystem fs, Path name, byte[] expected, int readOffset)
specifier|static
name|void
...
block|}
block|}
comment|/**    * Test to run benchmarks between shortcircuit read vs regular read with    * specified number of threads simultaneously reading.    *<br>    * Run this using the following command:    * bin/hadoop --config confdir \    * org.apache.hadoop.hdfs.TestShortCircuitLocalRead \    *<shortcircuit on?><checsum on?><Number of threads>    */
