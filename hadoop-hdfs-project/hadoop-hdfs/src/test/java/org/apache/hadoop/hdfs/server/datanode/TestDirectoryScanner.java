...
block|}
block|}
comment|/**    * test scan only meta file NOT generate wrong folder structure warn log.    */
annotation|@
name|Test
argument_list|(
name|timeout
...
literal|600000
argument_list|)
DECL|method|testScanDirectoryStructureWarn ()
specifier|public
name|void
name|testScanDirectoryStructureWarn
parameter_list|()
throws|throws
name|Exception
block|{
comment|//add a logger stream to check what has printed to log
name|ByteArrayOutputStream
name|loggerStream
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|//delete thre block file , left the meta file alone
name|deleteBlockFile
argument_list|()
expr_stmt|;
comment|//scan to ensure log warn not printed
name|scan
argument_list|(
literal|1
argument_list|,
...
argument_list|)
expr_stmt|;
comment|//ensure the warn log not appear and missing block log do appear
name|String
name|logContent
init|=
operator|new
...
decl_stmt|;
name|String
name|missingBlockWarn
init|=
literal|"Deleted a metadata file"
operator|+
literal|" for the deleted block"
decl_stmt|;
name|String
name|dirStructureWarnLog
init|=
literal|" found in invalid directory."
operator|+
literal|"  Expected directory: "
decl_stmt|;
name|assertFalse
argument_list|(
literal|"directory check print meaningless warning message"
argument_list|,
name|logContent
operator|.
name|contains
...
name|assertTrue
argument_list|(
literal|"missing block warn log not appear"
argument_list|,
name|logContent
operator|.
name|contains
...
argument_list|)
decl_stmt|;
comment|/**    * Test parsing LocalReplica. We should be able to find the replica's path    * even if the replica's dir doesn't match the idToBlockDir.    */
annotation|@
name|Test
argument_list|(
name|timeout
...
literal|3000
argument_list|)
DECL|method|testLocalReplicaParsing ()
specifier|public
name|void
name|testLocalReplicaParsing
parameter_list|()
block|{
name|String
name|baseDir
...
argument_list|()
decl_stmt|;
comment|// test parsing dir without ./subdir/subdir
name|LocalReplica
operator|.
name|ReplicaDirInfo
name|info
...
argument_list|)
expr_stmt|;
comment|// test when path doesn't match the idToBLockDir.
name|String
name|pathWithOneSubdir
init|=
name|baseDir
operator|+
name|SEP
...
argument_list|)
expr_stmt|;
comment|// test when path matches the idToBlockDir.
name|info
operator|=
name|LocalReplica
operator|.
...
expr_stmt|;
block|}
comment|/**    * Test whether can LocalReplica.updateWithReplica() correct the wrongly    * recorded replica location.    */
annotation|@
name|Test
argument_list|(
name|timeout
...
literal|3000
argument_list|)
DECL|method|testLocalReplicaUpdateWithReplica ()
specifier|public
name|void
name|testLocalReplicaUpdateWithReplica
parameter_list|()
throws|throws
name|Exception
block|{
...
literal|"subdir0"
decl_stmt|;
comment|// the block file on disk
name|File
name|diskBlockDir
init|=
operator|new
...
argument_list|()
decl_stmt|;
comment|// DirectoryScanner find the inconsistent file and try to make it right
name|StorageLocation
name|sl
init|=
name|StorageLocation
