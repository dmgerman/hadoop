...
argument_list|)
expr_stmt|;
comment|// Check that no DNs saw an excess CACHE message
name|int
name|lines
init|=
name|appender
...
name|assertEquals
argument_list|(
literal|"Namenode should not send extra CACHE commands"
argument_list|,
literal|0
argument_list|,
name|lines
...
argument_list|)
expr_stmt|;
comment|// Nothing will get cached, so just force sleep for a bit
name|Thread
operator|.
name|sleep
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Still should not see any excess commands
name|lines
operator|=
name|appender
operator|.
