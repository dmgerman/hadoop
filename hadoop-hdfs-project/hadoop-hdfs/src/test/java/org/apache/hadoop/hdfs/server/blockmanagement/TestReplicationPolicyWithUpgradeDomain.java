...
argument_list|)
expr_stmt|;
comment|// delete hint rejected because deleting storages[1] would have
comment|// cause only two upgrade domains left.
name|delHintNode
operator|=
name|storages
index|[
...
argument_list|)
expr_stmt|;
comment|// No delete hint, different excess type deletion
name|nonExcess
operator|.
name|clear
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// Test SSD related deletion. With different rack settings here, but
comment|// similar to {@link TestReplicationPolicy#testChooseReplicasToDelete}.
comment|// The block was initially created on excessSSD(rack r1, UD 4),
comment|// storages[7](rack r3, UD 2) and storages[8](rack r3, UD 3) with
comment|// ONESSD_STORAGE_POLICY_NAME storage policy. Replication factor = 3.
comment|// Right after balancer moves the block from storages[7] to
comment|// storages[3](rack r2, UD 1), the application changes the storage policy
comment|// from ONESSD_STORAGE_POLICY_NAME to HOT_STORAGE_POLICY_ID. In this case,
comment|// we should be able to delete excessSSD since the remaining
comment|// storages ({storages[3]}, {storages[7], storages[8]})
comment|// are on different racks (r2, r3) and different UDs (1, 2, 3).
name|DatanodeStorageInfo
name|excessSSD
init|=
name|DFSTestUtil
...
argument_list|()
decl_stmt|;
comment|// after the move, the number of racks changes from 1 to 2.
comment|// and number of upgrade domains remains 3.
name|candidates
operator|.
name|add
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// the move would have changed the number of racks from 1 to 2.
comment|// and the number of UDs from 3 to 2.
name|candidates
operator|.
name|clear
argument_list|()
...
comment|// the number of UDs remains 3.
name|candidates
operator|.
name|clear
argument_list|()
...
expr_stmt|;
comment|// after the move, the number of racks remains 2.
comment|// the number of UDs remains 2.
name|candidates
operator|.
name|clear
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// the move would have changed the number of racks from 2 to 3.
comment|// and the number of UDs from 2 to 1.
name|candidates
operator|.
name|clear
argument_list|()
