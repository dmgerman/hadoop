...
annotation|@
name|Test
DECL|method|testWithNonClientProxy ()
specifier|public
name|void
name|testWithNonClientProxy
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// This will initialize all of the instance fields
specifier|final
name|String
name|fakeUser
init|=
...
annotation|@
name|Test
DECL|method|testObserverRetriableException ()
specifier|public
name|void
name|testObserverRetriableException
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|()
expr_stmt|;
comment|// Set the first observer to throw "ObserverRetryOnActiveException" so that
comment|// the request should skip the second observer and be served by the active.
name|namenodeAnswers
index|[
literal|1
index|]
...
expr_stmt|;
block|}
comment|/**    * An {@link Answer} used for mocking of {@link ClientProtocol}.    * Setting the state or unreachability of this    * Answer will make the linked ClientProtocol respond as if it was    * communicating with a NameNode of the corresponding state. It is in Standby    * state by default.    */
...
throw|;
block|}
comment|// retryActive should be checked before getHAServiceState.
comment|// Check getHAServiceState first here only because in test,
comment|// it relies read call, which relies on getHAServiceState
comment|// to have passed already. May revisit future.
if|if
condition|(
name|invocationOnMock
operator|.
...
expr_stmt|;
block|}
DECL|method|setRetryActive (boolean shouldRetryActive)
name|void
name|setRetryActive
parameter_list|(
name|boolean
name|shouldRetryActive
parameter_list|)
block|{
name|retryActive
operator|=
