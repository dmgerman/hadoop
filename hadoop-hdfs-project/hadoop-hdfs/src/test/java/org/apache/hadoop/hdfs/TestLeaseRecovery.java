...
block|}
block|}
comment|/**    * Test that when a client was writing to a file and died, and before the    * lease can be recovered, all the datanodes to which the file was written    * also die, after some time (5 * lease recovery times) the file is indeed    * closed and lease recovered.    * We also check that if the datanode came back after some time, the data    * originally written is not truncated    * @throws IOException    * @throws InterruptedException    */
annotation|@
name|Test
DECL|method|testLeaseRecoveryWithMissingBlocks ()
specifier|public
name|void
name|testLeaseRecoveryWithMissingBlocks
parameter_list|()
throws|throws
name|IOException
throws|,
...
argument_list|()
decl_stmt|;
comment|//Start a cluster with 3 datanodes
name|cluster
operator|=
operator|new
name|MiniDFSCluster
...
argument_list|()
expr_stmt|;
comment|//create a file (with replication 1)
name|Path
name|file
init|=
operator|new
...
argument_list|)
decl_stmt|;
comment|//This keeps count of the number of bytes written (AND is also the data we
comment|//are writing)
name|long
name|writtenBytes
init|=
...
name|println
argument_list|(
literal|"hsynced the data"
argument_list|)
expr_stmt|;
comment|//Kill the datanode to which the file was written.
name|DatanodeInfo
name|dn
init|=
operator|(
...
argument_list|)
decl_stmt|;
comment|//Wait at most 20 seconds for the lease to be recovered
name|LeaseManager
name|lm
init|=
name|NameNodeAdapter
...
name|assertTrue
argument_list|(
literal|"The lease was not recovered"
argument_list|,
name|lm
operator|.
name|countLease
...
argument_list|)
expr_stmt|;
comment|//Make sure we can't read any data because the datanode is dead
name|FSDataInputStream
name|in
init|=
name|dfs
...
name|assertTrue
argument_list|(
literal|"Shouldn't have reached here"
argument_list|,
literal|false
argument_list|)
expr_stmt|;
...
name|println
argument_list|(
literal|"Correctly got BlockMissingException because datanode"
operator|+
literal|" is still dead"
argument_list|)
expr_stmt|;
block|}
comment|//Bring the dead datanode back.
name|cluster
operator|.
name|restartDataNode
argument_list|(
...
name|println
argument_list|(
literal|"Restart datanode"
argument_list|)
expr_stmt|;
comment|//Make sure we can read all the data back (since we hsync'ed).
name|in
operator|=
name|dfs
operator|.
...
name|assertEquals
argument_list|(
literal|"Didn't read the data we wrote"
argument_list|,
name|in
operator|.
name|readLong
...
name|assertEquals
argument_list|(
literal|"Didn't get all the data"
argument_list|,
name|readBytes
argument_list|,
name|writtenBytes
