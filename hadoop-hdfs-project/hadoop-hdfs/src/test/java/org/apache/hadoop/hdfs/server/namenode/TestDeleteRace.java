...
literal|20000
argument_list|)
DECL|method|testOpenRenameRace ()
specifier|public
name|void
name|testOpenRenameRace
parameter_list|()
throws|throws
name|Exception
block|{
...
name|src
init|=
literal|"/dir/src-file"
decl_stmt|;
specifier|final
name|String
name|dst
init|=
literal|"/dir/dst-file"
decl_stmt|;
specifier|final
name|DistributedFileSystem
name|hdfs
...
argument_list|)
decl_stmt|;
comment|// 1.hold writeLock.
comment|// 2.start open thread.
comment|// 3.openSem& yield makes sure open thread wait on readLock.
comment|// 4.start rename thread.
comment|// 5.renameSem& yield makes sure rename thread wait on writeLock.
comment|// 6.release writeLock, it's fair lock so open thread gets read lock.
comment|// 7.open thread unlocks, rename gets write lock and does rename.
comment|// 8.rename thread unlocks, open thread gets write lock and update time.
name|Thread
name|open
init|=
operator|new
...
argument_list|()
expr_stmt|;
comment|// wait open and rename threads finish.
name|open
operator|.
name|join
argument_list|()
