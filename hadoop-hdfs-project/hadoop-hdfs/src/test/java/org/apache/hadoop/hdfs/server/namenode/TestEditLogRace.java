...
argument_list|)
decl_stmt|;
comment|// don't reset fields so instance can be reused.
specifier|final
name|FSEditLogOp
name|reuseOp
init|=
...
argument_list|()
expr_stmt|;
comment|// only job is spam edits.  it will fill the queue when the test
comment|// loop injects the blockingOp.
name|Future
index|[]
name|logSpammers
init|=
...
argument_list|)
expr_stmt|;
comment|// wait until a blocking edit op notifies us to go.
name|startSpamLatch
operator|.
name|await
argument_list|()
...
control|)
block|{
comment|// do not logSync here because we need to congest the queue.
name|editLog
operator|.
name|logEdit
argument_list|(
...
expr_stmt|;
block|}
comment|// the tx id is set while the edit log monitor is held, so this will
comment|// effectively stall the async processing thread which will cause the
comment|// edit queue to fill up.
specifier|final
name|FSEditLogOp
name|blockingOp
init|=
...
name|Throwable
block|{
comment|// flip the latch to unleash the spamming threads to congest
comment|// the queue.
name|startSpamLatch
operator|.
name|countDown
argument_list|()
expr_stmt|;
comment|// wait until unblocked after a synchronized thread is started.
name|blockerSemaphore
operator|.
name|acquire
argument_list|()
...
argument_list|()
expr_stmt|;
comment|// repeatedly overflow the queue and verify it doesn't deadlock.
for|for
control|(
name|int
name|i
...
control|)
block|{
comment|// when the blockingOp is logged, it triggers the latch to unleash the
comment|// spammers to overflow the edit queue, then waits for a permit
comment|// from blockerSemaphore that will be released at the bottom of
comment|// this loop.
name|Future
name|blockingEdit
init|=
...
argument_list|)
decl_stmt|;
comment|// wait for spammers to seize up the edit log.
name|long
name|startTxId
init|=
name|editLog
...
argument_list|)
expr_stmt|;
comment|// callers that synchronize on the edit log while the queue is full
comment|// are prone to deadlock if the locking is incorrect.  at this point:
comment|// 1. the blocking edit is holding the log's monitor.
comment|// 2. the spammers have filled the queue.
comment|// 3. the spammers are blocked waiting to queue another edit.
comment|// Now we'll start another thread to synchronize on the log (simulates
comment|// what log rolling does), unblock the op currently holding the
comment|// monitor, and ensure deadlock does not occur.
name|CountDownLatch
name|readyLatch
init|=
operator|new
...
name|setName
argument_list|(
literal|"Log synchronizer"
argument_list|)
expr_stmt|;
comment|// the sync is CRUCIAL for this test.  it's what causes edit
comment|// log rolling to deadlock when queue is full.
name|readyLatch
operator|.
name|countDown
argument_list|()
...
argument_list|)
decl_stmt|;
comment|// unblock the edit jammed in setting its txid.  queued edits should
comment|// start flowing and the synced edits should complete.
name|readyLatch
operator|.
name|await
argument_list|()
...
expr_stmt|;
block|}
comment|// just make sure everything can be synced.
name|editLog
operator|.
name|logSyncAll
argument_list|()
