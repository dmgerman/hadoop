...
block|}
block|}
comment|/*    * decommission the DN or put the DN into maintenance for datanodeUuid or one    * random node if datanodeUuid is null.    * And wait for the node to reach the given {@code waitForState}.    */
...
return|;
block|}
comment|/*    * decommission the DN or put the DN to maintenance set by datanodeUuid    * Pick randome node if datanodeUuid == null    * wait for the node to reach the given {@code waitForState}.    */
...
expr_stmt|;
block|}
comment|/*    * Wait till node is transitioned to the expected state.    */
