...
argument_list|)
expr_stmt|;
comment|// After appending data, there should be [2, 2, 4, 4] blocks in each volume
...
name|BrokenBarrierException
block|{
comment|// Starts DFS cluster with 3 DataNodes to form a pipeline.
name|startDFSCluster
argument_list|(
literal|1
argument_list|,
...
argument_list|()
expr_stmt|;
comment|// Make FsDatasetSpi#finalizeBlock a time-consuming operation. So if the
comment|// BlockReceiver releases volume reference before finalizeBlock(), the blocks
comment|// on the volume will be removed, and finalizeBlock() throws IOE.
specifier|final
name|FsDatasetSpi
argument_list|<
name|?
...
argument_list|)
expr_stmt|;
comment|// Bypass the argument to FsDatasetImpl#finalizeBlock to verify that
comment|// the block is not removed, since the volume reference should not
comment|// be released at this point.
name|data
operator|.
name|finalizeBlock
argument_list|(
...
argument_list|)
decl_stmt|;
comment|// Remove the first volume.
specifier|final
name|List
argument_list|<
name|Exception
...
argument_list|)
expr_stmt|;
comment|// If an IOException thrown from BlockReceiver#run, it triggers
comment|// DataNode#checkDiskError(). So we can test whether checkDiskError() is called,
comment|// to see whether there is IOException in BlockReceiver#run().
name|assertEquals
argument_list|(
name|lastTimeDiskErrorCheck
argument_list|,
...
return|;
block|}
comment|/**    * Verify that {@link DataNode#checkDiskErrors()} removes all metadata in    * DataNode upon a volume failure. Thus we can run reconfig on the same    * configuration to reload the new volume on the same directory as the failed one.    */
annotation|@
name|Test
argument_list|(
name|timeout
