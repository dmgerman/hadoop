...
expr_stmt|;
block|}
comment|/**    * Test re-adding one volume with some blocks on a running MiniDFSCluster    * with only one NameNode to reproduce HDFS-13677.    */
annotation|@
name|Test
argument_list|(
name|timeout
...
literal|60000
argument_list|)
DECL|method|testReAddVolumeWithBlocks ()
specifier|public
name|void
name|testReAddVolumeWithBlocks
parameter_list|()
throws|throws
name|IOException
throws|,
...
expr_stmt|;
comment|// 2 volumes
comment|// Now remove the second volume
name|DataNode
name|dn
init|=
name|cluster
...
argument_list|)
expr_stmt|;
comment|// Now create another file - the first volume should have 15 blocks
comment|// and 5 blocks on the previously removed volume
name|createFile
argument_list|(
operator|new
name|Path
...
expr_stmt|;
block|}
comment|// Now add the original volume back again and ensure 15 blocks are reported
name|assertThat
argument_list|(
literal|"DN did not update its own config"
argument_list|,
...
expr_stmt|;
comment|// 2 volumes
comment|// The order of the block reports is not guaranteed. As we expect 2, get the
comment|// max block count and the min block count and then assert on that.
name|int
name|minNumBlocks
init|=
name|Integer
