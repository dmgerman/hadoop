...
expr_stmt|;
block|}
comment|/**    * Test if StandbyException can be thrown from StandbyNN, when it's requested for     * password. (HDFS-6475). With StandbyException, the client can failover to try    * activeNN.    */
annotation|@
name|Test
DECL|method|testDelegationTokenStandbyNNAppearFirst ()
specifier|public
name|void
name|testDelegationTokenStandbyNNAppearFirst
parameter_list|()
throws|throws
name|Exception
block|{
comment|// make nn0 the standby NN, and nn1 the active NN
name|cluster
operator|.
name|transitionToStandby
argument_list|(
...
name|fail
argument_list|(
literal|"InvalidToken with cause StandbyException is expected"
operator|+
literal|" since nn0 is standby"
argument_list|)
expr_stmt|;
return|return
name|tmppw
...
block|{
comment|//
comment|// The exception oe caught here is
comment|//     java.lang.SecurityException: Failed to obtain user group
comment|//     information: org.apache.hadoop.security.token.
comment|//     SecretManager$InvalidToken: StandbyException
comment|//
name|HttpServletResponse
name|response
init|=
