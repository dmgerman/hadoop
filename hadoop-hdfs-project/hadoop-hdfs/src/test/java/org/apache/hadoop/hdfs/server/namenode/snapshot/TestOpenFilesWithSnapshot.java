...
expr_stmt|;
block|}
comment|/**    * Verify if the NameNode can restart properly after an OpenForWrite    * file and the only snapshot it was present in were deleted.    *    * @throws Exception    */
annotation|@
name|Test
argument_list|(
name|timeout
...
literal|600000
argument_list|)
DECL|method|testOpenFileDeletionAndNNRestart ()
specifier|public
name|void
name|testOpenFileDeletionAndNNRestart
parameter_list|()
throws|throws
name|Exception
block|{
...
literal|"snap_1"
decl_stmt|;
comment|// Create a file with few blocks. Get its output stream
comment|// for append.
specifier|final
name|Path
name|hbaseFile
...
argument_list|)
expr_stmt|;
comment|// Write more data to the file
name|writeToStream
argument_list|(
name|hbaseOutputStream
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Take a snapshot while the file is open for write
specifier|final
name|Path
name|snap1Dir
init|=
...
name|info
argument_list|(
literal|"Open file status in snap: "
operator|+
name|fs
operator|.
name|getFileStatus
...
argument_list|)
expr_stmt|;
comment|// Delete the open file and the snapshot while
comment|// its output stream is still open.
name|fs
operator|.
name|delete
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Verify file existence after the NameNode restart
name|cluster
operator|.
name|restartNameNode
argument_list|()
