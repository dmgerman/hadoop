...
annotation|@
name|Test
DECL|method|testMultipleReplicasScheduledForUpgradeDomain ()
specifier|public
name|void
name|testMultipleReplicasScheduledForUpgradeDomain
parameter_list|()
throws|throws
name|Exception
block|{
...
block|{
comment|// Create a file with one block with a replication factor of 3
comment|// No upgrade domains are set.
specifier|final
name|FileSystem
name|fs
init|=
...
argument_list|)
decl_stmt|;
comment|// The block should be replicated OK - so Reconstruction Work will be null
...
argument_list|)
expr_stmt|;
comment|// Set the upgradeDomain to "3" for the 3 nodes hosting the block.
comment|// Then alternately set the remaining 3 nodes to have an upgradeDomain
comment|// of 0 or 1 giving a total of 3 upgradeDomains.
for|for
control|(
name|int
name|i
...
block|}
block|}
comment|// Now reconWork is non-null and 2 extra targets are needed
name|work
operator|=
name|bm
operator|.
...
argument_list|)
expr_stmt|;
comment|// Add the block to the replication queue and ensure it is replicated
comment|// correctly.
name|bm
operator|.
...
annotation|@
name|Test
DECL|method|testUnderReplicatedRespectsRacksAndUpgradeDomain ()
specifier|public
name|void
name|testUnderReplicatedRespectsRacksAndUpgradeDomain
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// All hosts are on two racks
name|String
index|[]
name|racks
init|=
...
block|}
block|}
DECL|method|getDnDescriptors (MiniDFSCluster cluster)
specifier|private
name|List
argument_list|<
name|DatanodeDescriptor
...
name|IOException
argument_list|(
literal|"DatanodeDescriptor not found for DN "
operator|+
name|dn
operator|.
name|getDatanodeUuid
