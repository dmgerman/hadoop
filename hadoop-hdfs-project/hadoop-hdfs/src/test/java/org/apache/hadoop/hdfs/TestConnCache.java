...
literal|1
decl_stmt|;
comment|/* create a socket cache. There is only one socket cache per jvm */
name|cache
operator|=
name|SocketCache
operator|.
...
expr_stmt|;
block|}
comment|/**    * Test the SocketCache expiry.    * Verify that socket cache entries expire after the set    * expiry time.    */
annotation|@
name|Test
DECL|method|testSocketCacheExpiry ()
specifier|public
name|void
name|testSocketCacheExpiry
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|()
decl_stmt|;
comment|// Make some sockets to the DN and put in cache
name|Socket
index|[]
name|dnSockets
init|=
...
argument_list|)
expr_stmt|;
comment|//sleep for a second and see if it expired
name|Thread
operator|.
name|sleep
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Client side has no sockets cached
name|assertEquals
argument_list|(
literal|0
argument_list|,
