...
expr_stmt|;
block|}
comment|/**    * File partially fit in RamDisk after eviction.    * RamDisk can fit 2 blocks. Write a file with 5 blocks.    * Expect 2 blocks are on RamDisk whereas other 3 on disk.    * @throws IOException    */
annotation|@
name|Test
argument_list|(
name|timeout
...
expr_stmt|;
block|}
comment|/**    * RamDisk eviction after lazy persist to disk.    * Evicted blocks are still readable with on-disk replicas.    * @throws IOException    * @throws InterruptedException    */
annotation|@
name|Test
argument_list|(
name|timeout
...
name|InterruptedException
block|{
comment|// 1 replica + delta, lazy persist interval every 50 minutes
name|startUpCluster
argument_list|(
name|REPL_FACTOR
argument_list|,
...
literal|300000
argument_list|)
DECL|method|testRamDiskEvictionLRU ()
specifier|public
name|void
name|testRamDiskEvictionLRU
parameter_list|()
throws|throws
name|IOException
throws|,
...
expr_stmt|;
block|}
comment|// No eviction for the first half of files
for|for
control|(
name|int
name|i
...
expr_stmt|;
block|}
comment|// Lazy persist writer persists the first half of files
name|Thread
operator|.
name|sleep
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Create the second half of files with eviction upon each create.
for|for
control|(
name|int
name|i
...
argument_list|)
expr_stmt|;
comment|// path[i-NUM_PATHS/2] is expected to be evicted by LRU
name|triggerBlockReport
argument_list|()
expr_stmt|;
name|ensureFileReplicasOnStorageType
...
name|bFail
decl_stmt|;
DECL|method|WriterRunnable (MiniDFSCluster cluster, int threadIndex, Path[] paths, int seed, CountDownLatch latch, AtomicBoolean bFail)
specifier|public
name|WriterRunnable
parameter_list|(
name|MiniDFSCluster
