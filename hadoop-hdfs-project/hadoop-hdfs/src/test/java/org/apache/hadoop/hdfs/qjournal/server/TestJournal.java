...
argument_list|()
expr_stmt|;
comment|// Enable fetching edits via RPC
name|conf
operator|.
name|setBoolean
argument_list|(
...
expr_stmt|;
block|}
comment|/**    * Test for HDFS-14557 to ensure that a edit file that failed to fully    * allocate and has a header byte of -1 is moved aside to allow startup    * to progress.    */
annotation|@
name|Test
DECL|method|testEmptyEditsInProgressMovedAside ()
specifier|public
name|void
name|testEmptyEditsInProgressMovedAside
parameter_list|()
throws|throws
name|Exception
block|{
comment|// First, write 5 transactions to the journal
name|journal
operator|.
name|startLogSegment
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Create a new segment creating a new edits_inprogress file
name|journal
operator|.
name|startLogSegment
argument_list|(
...
argument_list|)
decl_stmt|;
comment|// Now stop the journal without finalizing the segment
name|journal
operator|.
name|close
argument_list|()
expr_stmt|;
comment|// Now "zero out" the EIP file with -1 bytes, similar to how it would
comment|// appear if the pre-allocation failed
name|RandomAccessFile
name|rwf
init|=
operator|new
...
argument_list|()
expr_stmt|;
comment|// Finally start the Journal again, and ensure the "zeroed out" file
comment|// is renamed with a .empty extension
name|journal
operator|=
operator|new
name|Journal
...
annotation|@
name|Test
DECL|method|testReadFromCache ()
specifier|public
name|void
name|testReadFromCache
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
name|assertJournaledEditsTxnCountAndContents
argument_list|(
literal|1
argument_list|,
literal|7
...
expr_stmt|;
block|}
DECL|method|assertJournaledEditsTxnCountAndContents (int startTxn, int requestedMaxTxns, int expectedEndTxn, int layoutVersion)
specifier|private
name|void
name|assertJournaledEditsTxnCountAndContents
parameter_list|(
