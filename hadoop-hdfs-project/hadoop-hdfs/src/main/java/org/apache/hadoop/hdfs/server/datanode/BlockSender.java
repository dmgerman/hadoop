...
name|block
decl_stmt|;
comment|/** Stream to read block data from */
DECL|field|blockIn
specifier|private
name|InputStream
name|blockIn
...
literal|1
decl_stmt|;
comment|/** Stream to read checksum */
...
name|blockInFd
decl_stmt|;
comment|/** The reference to the volume where the block is located */
...
return|;
block|}
comment|/**    * Wait for rbw replica to reach the length    * @param rbw replica that is being written to    * @param len minimum length to reach    * @throws IOException on failing to reach the len in given wait time    */
DECL|method|waitForMinLength (ReplicaBeingWritten rbw, long len)
specifier|private
specifier|static
name|void
name|waitForMinLength
...
name|IOException
block|{
comment|// Wait for 3 seconds for rbw replica to reach the minimum length
for|for
control|(
name|int
name|i
