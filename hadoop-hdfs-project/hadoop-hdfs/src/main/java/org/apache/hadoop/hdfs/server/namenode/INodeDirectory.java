...
DECL|method|addChild (final INode node, final boolean setModTime, final Snapshot latest)
specifier|public
name|boolean
name|addChild
parameter_list|(
...
return|;
block|}
comment|/**    * Add new INode to the file tree.    * Find the parent and insert     *     * @param path file path    * @param newNode INode to be added    * @return false if the node already exists; otherwise, return true;    * @throws FileNotFoundException if parent does not exist or     * @throws UnresolvedLinkException if any path component is a symbolic link    * is not a directory.    */
DECL|method|addINode (String path, INode newNode )
name|boolean
name|addINode
parameter_list|(
name|String
...
expr_stmt|;
block|}
DECL|method|destroySubtreeAndCollectBlocksRecursively (final Snapshot snapshot, final BlocksMapUpdateInfo collectedBlocks)
specifier|public
name|int
name|destroySubtreeAndCollectBlocksRecursively
parameter_list|(
specifier|final
name|Snapshot
name|snapshot
...
operator|++
expr_stmt|;
comment|//count this dir only if this object is destroyed
name|clearReferences
argument_list|()
expr_stmt|;
block|}
...
annotation|@
name|VisibleForTesting
DECL|method|dumpTreeRecursively (PrintWriter out, StringBuilder prefix, Iterable<Pair<? extends INode, Snapshot>> subs)
specifier|protected
specifier|static
name|void
name|dumpTreeRecursively
