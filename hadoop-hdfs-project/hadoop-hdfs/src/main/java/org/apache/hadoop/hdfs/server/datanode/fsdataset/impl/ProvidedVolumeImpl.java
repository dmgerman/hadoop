...
name|FsVolumeImpl
block|{
comment|/**    * Get a suffix of the full path, excluding the given prefix.    *    * @param prefix a prefix of the path.    * @param fullPath the full path whose suffix is needed.    * @return the suffix of the path, which when resolved against {@code prefix}    *         gets back the {@code fullPath}.    */
annotation|@
name|VisibleForTesting
DECL|method|getSuffix (final Path prefix, final Path fullPath)
specifier|protected
specifier|static
name|String
name|getSuffix
...
name|debug
argument_list|(
literal|"Path {} is not a prefix of the path {}"
argument_list|,
name|prefix
argument_list|,
name|fullPath
...
return|;
block|}
comment|/**    * Class to keep track of the capacity usage statistics for provided volumes.    */
DECL|class|ProvidedVolumeDF
specifier|public
specifier|static
class|class
...
expr_stmt|;
block|}
DECL|method|fetchVolumeMap (ReplicaMap volumeMap, RamDiskReplicaTracker ramDiskReplicaMap, FileSystem remoteFS)
name|void
name|fetchVolumeMap
parameter_list|(
name|ReplicaMap
...
block|}
DECL|method|compileReport (Collection<ScanInfo> report, ReportCompiler reportCompiler)
specifier|public
name|void
name|compileReport
parameter_list|(
...
try|try
block|{
comment|// default to whatever is the space used!
return|return
name|getDfsUsed
argument_list|()
return|;
...
name|warn
argument_list|(
literal|"Exception when trying to get capacity of ProvidedVolume: {}"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
argument_list|()
decl_stmt|;
comment|// do not report less than 0 remaining space for PROVIDED storage
comment|// to prevent marking it as over capacity on NN
if|if
condition|(
name|remaining
operator|<
...
name|warn
argument_list|(
literal|"Volume {} has less than 0 available space"
argument_list|,
name|this
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Compiling report for volume: {}; bpid: {}"
argument_list|,
name|this
argument_list|,
name|bpid
...
annotation|@
name|VisibleForTesting
DECL|method|getFileRegionProvider (String bpid)
name|BlockAliasMap
argument_list|<
name|FileRegion
argument_list|>
