...
parameter_list|)
block|{
comment|// when the given snapshotId is CURRENT_STATE_ID, it is possible that we
comment|// do not know where the corresponding inode belongs, thus we simply
comment|// return the last reference node
if|if
condition|(
name|snapshotId
operator|==
...
return|;
block|}
comment|// otherwise we search the withNameList
for|for
control|(
name|int
name|i
