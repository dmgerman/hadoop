...
parameter_list|()
block|{
comment|// First, match nodes on the same node group if cluster has nodegroup
comment|// awareness
if|if
condition|(
name|cluster
...
condition|)
block|{
name|chooseNodesOnSameNodeGroup
argument_list|()
expr_stmt|;
block|}
...
argument_list|)
expr_stmt|;
comment|// At last, match nodes on different racks
name|chooseNodes
argument_list|(
literal|false
argument_list|)
...
return|;
block|}
comment|/**    * Decide all<source, target> pairs where source and target are     * on the same NodeGroup    */
DECL|method|chooseNodesOnSameNodeGroup ()
specifier|private
name|void
name|chooseNodesOnSameNodeGroup
parameter_list|()
block|{
comment|/* first step: match each overUtilized datanode (source) to      * one or more underUtilized datanodes within same NodeGroup(targets).      */
name|chooseOnSameNodeGroup
argument_list|(
name|overUtilizedDatanodes
argument_list|,
name|underUtilizedDatanodes
argument_list|)
expr_stmt|;
comment|/* match each remaining overutilized datanode (source) to below average       * utilized datanodes within the same NodeGroup(targets).      * Note only overutilized datanodes that haven't had that max bytes to move      * satisfied in step 1 are selected      */
name|chooseOnSameNodeGroup
argument_list|(
name|overUtilizedDatanodes
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* match each remaining underutilized datanode to above average utilized       * datanodes within the same NodeGroup.      * Note only underutilized datanodes that have not had that max bytes to      * move satisfied in step 1 are selected.      */
name|chooseOnSameNodeGroup
argument_list|(
name|underUtilizedDatanodes
argument_list|,
...
expr_stmt|;
block|}
comment|/**    * Match two sets of nodes within the same NodeGroup, one should be source    * nodes (utilization> Avg), and the other should be destination nodes     * (utilization< Avg).    * @param datanodes    * @param candidates    */
specifier|private
parameter_list|<
name|D
extends|extends
...
parameter_list|>
name|void
DECL|method|chooseOnSameNodeGroup (Collection<D> datanodes, Collection<C> candidates)
name|chooseOnSameNodeGroup
parameter_list|(
name|Collection
argument_list|<
...
block|}
block|}
comment|/**    * Match one datanode with a set of candidates nodes within the same NodeGroup.    */
DECL|method|chooseOnSameNodeGroup ( BalancerDatanode dn, Iterator<T> candidates)
specifier|private
parameter_list|<
name|T
extends|extends
...
name|chosen
init|=
name|chooseCandidateOnSameNodeGroup
argument_list|(
name|dn
argument_list|,
name|candidates
...
expr_stmt|;
block|}
comment|/** choose a datanode from<code>candidates</code> within the same NodeGroup     * of<code>dn</code>.    */
DECL|method|chooseCandidateOnSameNodeGroup ( BalancerDatanode dn, Iterator<T> candidates)
specifier|private
parameter_list|<
name|T
extends|extends
...
DECL|method|chooseTargets ( Collection<BalancerDatanode> targetCandidates, boolean onRackTarget )
specifier|private
name|void
name|chooseTargets
parameter_list|(
...
DECL|method|chooseSources ( Collection<Source> sourceCandidates, boolean onRackSource)
specifier|private
name|void
name|chooseSources
parameter_list|(
