...
return|;
block|}
DECL|method|hasExternalErrorOnly ()
specifier|synchronized
name|boolean
name|hasExternalErrorOnly
parameter_list|()
block|{
return|return
name|error
...
argument_list|)
expr_stmt|;
comment|// TODO: use setParents API once it's available from HTrace 3.2
comment|// scope = Trace.startSpan("dataStreamer", Sampler.ALWAYS);
comment|// scope.getSpan().setParents(parents);
block|}
block|}
block|}
...
condition|)
block|{
comment|// block is not yet initialized, handle external error later.
return|return
literal|false
return|;
block|}
...
return|;
block|}
DECL|method|callUpdateBlockForPipeline (ExtendedBlock newBlock)
name|LocatedBlock
name|callUpdateBlockForPipeline
parameter_list|(
name|ExtendedBlock
