...
return|;
block|}
comment|/**    * If we should retry the RPC call.    *    * @param ex Exception reported.    * @param retryCount Number of retries.    * @return Retry decision.    * @throws IOException Original exception if the retry policy generates one.    */
DECL|method|shouldRetry (final IOException ioe, final int retryCount)
specifier|private
name|RetryDecision
name|shouldRetry
parameter_list|(
...
throw|;
block|}
comment|/**    * Invokes a method on the designated object. Catches exceptions specific to    * the invocation.    *    * Re-throws exceptions generated by the remote RPC call as either    * RemoteException or IOException.    *    * @param method Method to invoke    * @param obj Target object for the method    * @param params Variable parameters    * @return Response from the remote server    * @throws IOException    * @throws InterruptedException    */
DECL|method|invoke (int retryCount, final Method method, final Object obj, final Object... params)
specifier|private
name|Object
name|invoke
parameter_list|(
...
DECL|method|invokeSequential ( final List<? extends RemoteLocationContext> locations, final RemoteMethod remoteMethod, Class<?> expectedResultClass, Object expectedResultValue)
specifier|public
name|Object
name|invokeSequential
parameter_list|(
...
return|;
block|}
comment|/**    * Checks if a result matches the required result class.    *    * @param expectedResultClass Required result class, null to skip the check.    * @param result The result to check.    * @return True if the result is an instance of the required class or if the    *         expected class is null.    */
...
block|}
block|}
comment|/**    * Checks if a result matches the expected value.    *    * @param expectedResultValue The expected value, null to skip the check.    * @param result The result to check.    * @return True if the result is equals to the expected value or if the    *         expected value is null.    */
...
return|;
block|}
comment|/**    * Invokes multiple concurrent proxy calls to different clients. Returns an    * array of results.    *    * Re-throws exceptions generated by the remote RPC call as either    * RemoteException or IOException.    *    * @param locations List of remote locations to call concurrently.    * @param remoteMethod The remote method and parameters to invoke.    * @param requireResponse If true an exception will be thrown if all calls do    *          not complete. If false exceptions are ignored and all data results    *          successfully received are returned.    * @param standby If the requests should go to the standby namenodes too.    * @param timeoutMs Timeout for each individual call.    * @return Result of invoking the method per subcluster: nsId -> result.    * @throws IOException If requiredResponse=true and any of the calls throw an    *           exception.    */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"unchecked"
argument_list|)
DECL|method|invokeConcurrent ( final Collection<T> locations, final RemoteMethod method, boolean requireResponse, boolean standby, long timeOutMs)
specifier|public
parameter_list|<
name|T
extends|extends
...
block|}
block|}
comment|/**    * Get a prioritized list of NNs that share the same nameservice ID (in the    * same namespace). NNs that are reported as ACTIVE will be first in the list.    *    * @param nameserviceId The nameservice ID for the namespace.    * @return A prioritized list of NNs to use for communication.    * @throws IOException If a NN cannot be located for the nameservice ID.    */
...
return|;
block|}
comment|/**    * Get a prioritized list of NNs that share the same block pool ID (in the    * same namespace). NNs that are reported as ACTIVE will be first in the list.    *    * @param blockPoolId The blockpool ID for the namespace.    * @return A prioritized list of NNs to use for communication.    * @throws IOException If a NN cannot be located for the block pool ID.    */
