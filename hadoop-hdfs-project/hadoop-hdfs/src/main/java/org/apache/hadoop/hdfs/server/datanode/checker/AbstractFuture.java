...
begin_comment
comment|/**  * Some portions of this class have been modified to make it functional in this  * package.  */
end_comment
...
argument_list|>
block|{
comment|// NOTE: Whenever both tests are cheap and functional, it's faster to use&,
comment|// | instead of&&, ||
...
argument_list|)
decl_stmt|;
comment|// A heuristic for timed gets. If the remaining timeout is less than this,
comment|// spin instead of
...
comment|// The access control checks that ARFU does means the caller class has
comment|// to be AbstractFuture
...
parameter_list|)
block|{
comment|// Some Android 5.0.x Samsung devices have bugs in JDK reflection APIs
comment|// that cause getDeclaredField to throw a NoSuchFieldException when
...
parameter_list|()
block|{
comment|// avoid volatile write, write is made visible by subsequent CAS on
...
expr_stmt|;
block|}
comment|// non-volatile write to the next field. Should be made visible by
comment|// subsequent CAS on waiters field.
...
parameter_list|()
block|{
comment|// This is racy with removeWaiter. The consequence of the race is that
comment|// we may spuriously call unpark even though the thread has already
comment|// removed itself from the list. But even if we did use a CAS, that
comment|// race would still exist (it would just be ever so slightly smaller).
name|Thread
name|w
init|=
name|thread
...
condition|)
block|{
comment|// we aren't unlinking this node, update
comment|// pred.
name|pred
operator|=
name|curr
...
condition|)
block|{
comment|// We are unlinking this node and it has a
comment|// predecessor.
name|pred
operator|.
name|next
...
condition|)
block|{
comment|// We raced with another node that
comment|// unlinked pred. Restart.
continue|continue
name|restart
continue|;
block|}
...
name|executor
decl_stmt|;
comment|// writes to next are made visible by subsequent CAS's on the listeners
comment|// field
DECL|field|next
annotation|@
name|Nullable
...
condition|)
block|{
comment|// nothing to do, we must have been cancelled, don't bother inspecting
comment|// the future.
return|return;
block|}
name|Object
...
block|}
block|}
comment|/**    * This field encodes the current state of the future.    *<p>    *<p>The valid values are:    *<ul>    *<li>{@code null} initial state, nothing has happened.    *<li>{@link Cancellation} terminal state, {@code cancel} was called.    *<li>{@link Failure} terminal state, {@code setException} was called.    *<li>{@link SetFuture} intermediate state, {@code setFuture} was called.    *<li>{@link #NULL} terminal state, {@code set(null)} was called.    *<li>Any other non-null value, terminal state, {@code set} was called with    * a non-null argument.    *</ul>    */
DECL|field|value
specifier|private
specifier|volatile
name|Object
...
comment|// * Don't create Waiter nodes if you aren't going to park, this helps
comment|// reduce contention on the waiters field.
comment|// * Future completion is defined by when #value becomes non-null/non
comment|// SetFuture
comment|// * Future completion can be observed if the waiters field contains a
comment|// TOMBSTONE
comment|// Timed Get
comment|// There are a few design constraints to consider
comment|// * We want to be responsive to small timeouts, unpark() has non trivial
comment|// latency overheads (I have observed 12 micros on 64 bit linux systems to
comment|// wake up a parked thread). So if the timeout is small we shouldn't park().
comment|// This needs to be traded off with the cpu overhead of spinning, so we use
comment|// SPIN_THRESHOLD_NANOS which is what AbstractQueuedSynchronizer uses for
comment|// similar purposes.
comment|// * We want to behave reasonably for timeouts of 0
comment|// * We are more responsive to completion than timeouts. This is because
comment|// parkNanos depends on system scheduling and as such we could either miss
comment|// our deadline, or unpark() could be delayed so that it looks like we
comment|// timed out even though we didn't. For comparison FutureTask respects
comment|// completion preferably and AQS is non-deterministic (depends on where in
comment|// the queue the waiter is). If we wanted to be strict about it, we could
comment|// store the unpark() time in the Waiter node and we could use that to make
comment|// a decision about whether or not we timed out prior to being unparked.
...
name|ExecutionException
block|{
comment|// NOTE: if timeout< 0, remainingNanos will be< 0 and we will fall into
comment|// the while(true) loop at the bottom and throw a timeoutexception.
name|long
name|remainingNanos
init|=
name|unit
...
return|;
block|}
comment|// we delay calling nanoTime until we know we will need to either park or
comment|// spin
specifier|final
name|long
name|endNanos
...
argument_list|)
expr_stmt|;
comment|// Check interruption first, if we woke up due to interruption
comment|// we need to honor that.
if|if
condition|(
name|Thread
operator|.
...
throw|;
block|}
comment|// Otherwise re-read and check doneness. If we loop then it must
comment|// have been a spurious wakeup
name|localValue
operator|=
name|value
expr_stmt|;
...
condition|)
block|{
comment|// Remove the waiter, one way or another we are done parking
comment|// this thread.
name|removeWaiter
argument_list|(
name|node
...
do|;
block|}
comment|// re-read value, if we get here then we must have observed a TOMBSTONE
comment|// while trying to add a waiter.
return|return
name|getDoneValue
argument_list|(
name|value
...
return|;
block|}
comment|// If we get here then we have remainingNanos< SPIN_THRESHOLD_NANOS and
comment|// there is no node on the waiters list
while|while
condition|(
name|remainingNanos
operator|>
...
argument_list|)
expr_stmt|;
comment|// Check interruption first, if we woke up due to interruption we
comment|// need to honor that.
if|if
condition|(
name|Thread
operator|.
...
name|ExecutionException
block|{
comment|// While this seems like it might be too branch-y, simple benchmarking
comment|// proves it to be unmeasurable (comparing done AbstractFutures with
comment|// immediateFuture)
if|if
condition|(
name|obj
operator|instanceof
...
return|;
block|}
comment|/**    * {@inheritDoc}    *<p>    *<p>If a cancellation attempt succeeds on a {@code Future} that had    * previously been {@linkplain#setFuture set asynchronously}, then the    * cancellation will also be propagated to the delegate {@code Future} that    * was supplied in the {@code setFuture} call.    */
annotation|@
name|Override
DECL|method|cancel (boolean mayInterruptIfRunning)
specifier|public
...
condition|)
block|{
comment|// Try to delay allocating the exception. At this point we may still
comment|// lose the CAS, but it is certainly less likely.
name|Throwable
name|cause
init|=
name|GENERATE_CANCELLATION_CAUSES
...
literal|true
expr_stmt|;
comment|// We call interuptTask before calling complete(), which is
comment|// consistent with FutureTask
if|if
condition|(
name|mayInterruptIfRunning
condition|)
...
condition|)
block|{
comment|// propagate cancellation to the future set in setfuture, this is
comment|// racy, and we don't care if we are successful or not.
name|ListenableFuture
argument_list|<
name|?
argument_list|>
...
condition|)
block|{
comment|// If the future is a TrustedFuture then we specifically avoid
comment|// calling cancel() this has 2 benefits
comment|// 1. for long chains of futures strung together with setFuture
comment|// we consume less stack
comment|// 2. we avoid allocating Cancellation objects at every level of
comment|// the cancellation chain
comment|// We can only do this for TrustedFuture, because
comment|// TrustedFuture.cancel is final and does nothing but delegate
comment|// to this method.
name|AbstractFuture
argument_list|<
name|?
argument_list|>
...
condition|)
block|{
comment|// obj cannot be null at this point, because value can only change
comment|// from null to non-null. So if value changed (and it did since we
comment|// lost the CAS), then it cannot be null and since it isn't a
comment|// SetFuture, then the future must be done and we should exit the loop
break|break;
block|}
block|}
block|}
...
do|;
block|}
comment|// If we get here then the Listener TOMBSTONE was set, which means the
comment|// future is done, call the listener.
...
condition|)
block|{
comment|// the listener is responsible for calling completeWithFuture,
comment|// directExecutor is appropriate since all we are doing is unpacking
comment|// a completed future which should be fast.
try|try
block|{
name|future
operator|.
...
parameter_list|)
block|{
comment|// addListener has thrown an exception! SetFuture.run can't throw
comment|// any exceptions so this must have been caused by addListener
comment|// itself. The most likely explanation is a misconfigured mock. Try
comment|// to switch to Failure.
name|Failure
name|failure
decl_stmt|;
try|try
...
expr_stmt|;
block|}
comment|// Note: The only way this CAS could fail is if cancel() has raced
comment|// with us. That is ok.
name|boolean
name|unused
init|=
name|ATOMIC_HELPER
...
block|}
comment|// The future has already been set to something. If it is cancellation we
comment|// should cancel the incoming future.
if|if
condition|(
name|localValue
operator|instanceof
...
return|;
block|}
comment|/**    * Returns a value, suitable for storing in the {@link #value} field. From    * the given future, which is assumed to be done.    *<p>    *<p>This is approximately the inverse of {@link #getDoneValue(Object)}    */
...
condition|)
block|{
comment|// Break encapsulation for TrustedFuture instances since we know that
comment|// subclasses cannot override .get() (since it is final) and therefore
comment|// this is equivalent to calling .get() and unpacking the exceptions
comment|// like we do below (just much faster because it is a single field read
comment|// instead of a read, several branches and possibly creating exceptions).
return|return
operator|(
operator|(
name|AbstractFuture
...
argument_list|()
expr_stmt|;
comment|// We call this before the listeners in order to avoid needing to manage
comment|// a separate stack data structure for them. afterDone() should be
comment|// generally fast and only used for cleanup work... but in theory can
...
name|task
decl_stmt|;
comment|// We unwind setFuture specifically to avoid StackOverflowErrors in
comment|// the case of long chains of SetFutures
comment|// Handling this special case is important because there is no way
comment|// to pass an executor to setFuture, so a user couldn't break the
comment|// chain by doing this themselves.  It is also potentially common
comment|// if someone writes a recursive Futures.transformAsync transformer.
name|future
operator|=
name|setFuture
operator|.
...
argument_list|()
argument_list|,
literal|"Future was expected to be "
operator|+
literal|"done:"
operator|+
literal|" %s"
...
parameter_list|()
block|{   }
comment|/**    * If this future has been cancelled (and possibly interrupted), cancels    * (and possibly interrupts) the given future (if available).    *<p>    *<p>This method should be used only when this future is completed. It is    * designed to be called from {@code done}.    */
...
block|{
comment|// We need to
comment|// 1. atomically swap the listeners with TOMBSTONE, this is because
comment|// addListener uses that to to synchronize with us
comment|// 2. reverse the linked list, because despite our rather clear contract,
comment|// people depend on us executing listeners in the order they were added
...
function_decl|;
block|}
comment|/**    * {@link AtomicHelper} based on {@link sun.misc.Unsafe}.    *<p>    *<p>Static initialization of this class will fail if the    * {@link sun.misc.Unsafe} object cannot be accessed.    */
...
block|}
block|}
comment|/**    * {@link AtomicHelper} based on {@code synchronized} and volatile writes.    *<p>    *<p>This is an implementation of last resort for when certain basic VM    * features are broken (like AtomicReferenceFieldUpdater).    */
