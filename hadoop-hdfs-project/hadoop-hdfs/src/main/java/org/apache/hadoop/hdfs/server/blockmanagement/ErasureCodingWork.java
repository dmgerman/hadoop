...
decl_stmt|;
DECL|field|liveBusyBlockIndicies
specifier|private
specifier|final
name|byte
index|[]
name|liveBusyBlockIndicies
decl_stmt|;
...
name|blockPoolId
decl_stmt|;
DECL|method|ErasureCodingWork (String blockPoolId, BlockInfo block, BlockCollection bc, DatanodeDescriptor[] srcNodes, List<DatanodeDescriptor> containingNodes, List<DatanodeStorageInfo> liveReplicaStorages, int additionalReplRequired, int priority, byte[] liveBlockIndicies, byte[] liveBusyBlockIndicies)
specifier|public
name|ErasureCodingWork
parameter_list|(
name|String
...
parameter_list|()
block|{
comment|// Mark the block in normal node.
name|BlockInfoStriped
name|block
init|=
operator|(
...
block|}
block|}
comment|// If the block is on the node which is decommissioning or
comment|// entering_maintenance, and it doesn't exist on other normal nodes,
comment|// we just add the node into source list.
name|List
argument_list|<
name|Integer
argument_list|>
