...
condition|)
block|{
comment|// we are handling a reference node and its subtree stored in
comment|// the created list of a snapshot diff, which must be associated
comment|// with the latest snapshot of the dst tree before the rename
comment|// operation.
name|destroyDstSnapshot
argument_list|(
name|inode
argument_list|,
name|postSnapshot
...
condition|)
block|{
comment|// the inode should be renamed again. We only need to delete
comment|// postSnapshot in its subtree.
try|try
block|{
name|inode
operator|.
...
name|error
argument_list|(
literal|"Error: should not throw QuotaExceededException"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**      * For a reference node, delete its first snapshot associated with the dst      * tree of a rename operation, i.e., the snapshot diff is associated with      * the latest snapshot of the dst tree before the rename operation. The      * difference between this process and a regular snapshot deletion      * process is that we need to delete everything created after the rename,      * i.e., we should destroy the whole created list of the referred node.      */
DECL|method|destroyDstSnapshot (INode inode, final Snapshot snapshot, Snapshot prior, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes)
specifier|private
name|Quota
operator|.
name|Counts
...
condition|)
block|{
comment|// file and not reference
name|counts
operator|.
name|add
argument_list|(
...
condition|)
block|{
comment|// directory and not reference
if|if
condition|(
name|inode
operator|.
...
condition|)
block|{
comment|// destroy everything in the created list!
name|counts
operator|.
name|add
argument_list|(
...
condition|)
block|{
comment|// if ref is a DstReference instance, we should delete all the files
comment|// created after the rename
name|destroyDstSnapshot
argument_list|(
name|ref
argument_list|,
...
condition|)
block|{
comment|// ref should have been renamed again. We only need to delete
comment|// the snapshot in its subtree.
try|try
block|{
name|ref
operator|.
