...
return|;
block|}
comment|/**    * {{@inheritDoc org.apache.hadoop.hdfs.server.common.Storage#writeAll()}}    */
DECL|method|writeAll (Collection<StorageDirectory> dirs)
specifier|private
name|void
name|writeAll
parameter_list|(
...
block|}
block|}
comment|/**    * Add a list of volumes to be managed by DataStorage. If the volume is empty,    * format it, otherwise recover it from previous transitions if required.    *    * @param datanode the reference to DataNode.    * @param nsInfo namespace information    * @param dataDirs array of data storage directories    * @param startOpt startup option    * @throws IOException    */
...
name|IOException
block|{
comment|// Similar to recoverTransitionRead, it first ensures the datanode level
comment|// format is completed.
name|List
argument_list|<
name|StorageLocation
argument_list|>
...
expr_stmt|;
block|}
comment|// 1. For each data directory calculate its state and check whether all is
comment|// consistent before transitioning. Format and recover.
name|ArrayList
argument_list|<
name|StorageState
argument_list|>
...
block|}
block|}
comment|// 3. Update all successfully loaded storages. Some of them might have just
comment|// been formatted.
name|this
operator|.
name|writeAll
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// 4. Make newly loaded storage directories visible for service.
if|if
condition|(
operator|!
name|isInitialize
...
block|}
block|}
comment|/**    * Remove volumes from DataStorage.    * @param locations a collection of volumes.    */
DECL|method|removeVolumes (Collection<StorageLocation> locations)
specifier|synchronized
name|void
name|removeVolumes
parameter_list|(
