...
argument_list|()
decl_stmt|;
comment|/** Stores the replication index for each priority */
DECL|field|priorityToReplIdx
specifier|private
name|Map
argument_list|<
name|Integer
...
block|}
block|}
comment|/**    * Get a list of block lists to be replicated. The index of block lists    * represents its replication priority. Replication index will be tracked for    * each priority list separately in priorityToReplIdx map. Iterates through    * all priority lists and find the elements after replication index. Once the    * last priority lists reaches to end, all replication indexes will be set to    * 0 and start from 1st priority list to fulfill the blockToProces count.    *     * @param blocksToProcess - number of blocks to fetch from underReplicated blocks.    * @return Return a list of block lists to be replicated. The block list index    *         represents its replication priority.    */
...
condition|)
block|{
comment|// reset all priorities replication index to 0 because there is no
comment|// recently added blocks in any list.
for|for
control|(
name|int
name|i
...
block|}
block|}
comment|/**    * This method is to decrement the replication index for the given priority    *     * @param priority  - int priority level    */
DECL|method|decrementReplicationIndex (int priority)
specifier|public
name|void
