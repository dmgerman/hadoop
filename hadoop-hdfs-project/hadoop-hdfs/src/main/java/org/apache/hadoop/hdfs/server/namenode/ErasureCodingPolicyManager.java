...
decl_stmt|;
DECL|field|userDefinedAllowed
specifier|private
name|boolean
name|userDefinedAllowed
init|=
name|DFSConfigKeys
operator|.
...
name|allPolicies
decl_stmt|;
comment|/**    * All policies in the state as it will be persisted in the fsimage.    *    * The difference between persisted policies and all policies is that    * if a default policy is only enabled at startup,    * it will appear as disabled in the persisted policy list and in the fsimage.    */
DECL|field|allPersistedPolicies
specifier|private
name|Map
argument_list|<
name|Byte
...
return|;
block|}
comment|/**    * Get a {@link ErasureCodingPolicy} by policy ID, including system policy    * and user defined policy.    * @return ecPolicy, or null if not found    */
...
return|;
block|}
comment|/**    * Get a {@link ErasureCodingPolicyInfo} by policy ID, including system policy    * and user defined policy.    */
DECL|method|getPolicyInfoByID (final byte id)
specifier|private
name|ErasureCodingPolicyInfo
name|getPolicyInfoByID
parameter_list|(
...
return|;
block|}
comment|/**    * Get a {@link ErasureCodingPolicy} by policy name, including system    * policy and user defined policy.    * @return ecPolicy, or null if not found    */
...
return|;
block|}
comment|/**    * Get a {@link ErasureCodingPolicy} by policy name, including system    * policy, user defined policy and Replication policy.    * @return ecPolicy, or null if not found    */
DECL|method|getErasureCodingPolicyByName (String name)
specifier|public
name|ErasureCodingPolicy
...
return|;
block|}
comment|/**    * Get a {@link ErasureCodingPolicyInfo} by policy name, including system    * policy and user defined policy.    * @return ecPolicy, or null if not found    */
DECL|method|getPolicyInfoByName (final String name)
specifier|private
name|ErasureCodingPolicyInfo
name|getPolicyInfoByName
parameter_list|(
...
name|HadoopIllegalArgumentException
argument_list|(
literal|"Adding erasure coding "
operator|+
literal|"policy failed because the number of policies stored in the "
operator|+
literal|"system already reached the threshold, which is "
operator|+
name|ErasureCodeConstants
operator|.
name|MAX_POLICY_ID
...
return|;
block|}
DECL|method|getCurrentMaxPolicyID ()
specifier|private
name|byte
name|getCurrentMaxPolicyID
parameter_list|()
...
argument_list|)
expr_stmt|;
comment|/*      * TODO HDFS-12405 postpone the delete removed policy to Namenode restart      * time.      * */
block|}
annotation|@
name|VisibleForTesting
...
condition|)
block|{
comment|// If policy is not supported in current system, set the policy state to
comment|// DISABLED;
name|info
operator|.
name|setState
...
expr_stmt|;
block|}
comment|/**    * Reload erasure coding policies from fsImage.    *    * @param ecPolicies contains ErasureCodingPolicy list    *    */
DECL|method|loadPolicies ( List<ErasureCodingPolicyInfo> ecPolicies, Configuration conf)
specifier|public
specifier|synchronized
name|void
name|loadPolicies
...
expr_stmt|;
block|}
DECL|method|enableDefaultPolicy (Configuration conf)
specifier|private
name|void
name|enableDefaultPolicy
parameter_list|(
...
return|;
block|}
DECL|method|createPolicyInfo (ErasureCodingPolicy p, ErasureCodingPolicyState s)
specifier|private
name|ErasureCodingPolicyInfo
name|createPolicyInfo
parameter_list|(
