...
return|;
block|}
DECL|method|examineSecret (byte[] secret, String bpid)
specifier|private
name|String
name|examineSecret
parameter_list|(
...
block|{
comment|// step 1
name|SaslMessageWithHandshake
name|message
init|=
name|readSaslMessageWithHandshakeSecret
argument_list|(
name|in
argument_list|)
decl_stmt|;
...
condition|)
block|{
comment|// sanity check, if one is null, the other must also not be null
assert|assert
operator|(
name|secret
operator|!=
...
name|error
argument_list|(
literal|"Unable to match secret to a QOP!"
argument_list|)
expr_stmt|;
block|}
block|}
