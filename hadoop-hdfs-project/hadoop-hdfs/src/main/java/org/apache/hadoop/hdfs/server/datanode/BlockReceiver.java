...
name|checksum
decl_stmt|;
comment|// from where chunks of a block can be read
DECL|field|out
specifier|private
name|OutputStream
name|out
...
name|IOException
block|{
comment|/* This dances around buf a little bit, mainly to read       * full packet with single read and to accept arbitarary size        * for next packet at the same time.      */
if|if
condition|(
name|buf
operator|==
...
block|}
comment|/**     * Receives and processes a packet. It can contain many chunks.    * returns the number of data bytes that the packet has.    */
DECL|method|receivePacket (long offsetInBlock, long seqno, boolean lastPacketInBlock, int len, int endOfHeader)
specifier|private
name|int
name|receivePacket
parameter_list|(
...
comment|// Finalize the block. Does this fsync()?
name|datanode
operator|.
name|data
operator|.
...
name|offsetInBlock
decl_stmt|;
DECL|method|Packet (long seqno, boolean lastPacketInBlock, long offsetInBlock)
name|Packet
parameter_list|(
name|long
name|seqno
