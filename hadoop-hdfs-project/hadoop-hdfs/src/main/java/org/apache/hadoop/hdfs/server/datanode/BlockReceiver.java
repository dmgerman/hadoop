...
parameter_list|)
block|{
comment|// Verify that the exception is due to volume removal.
name|FsVolumeSpi
name|volume
decl_stmt|;
synchronized|synchronized
...
condition|)
block|{
comment|// ReplicaInfo has been removed due to the corresponding data
comment|// volume has been removed. Don't need to check disk error.
name|LOG
operator|.
name|info
argument_list|(
name|myString
operator|+
literal|": BlockReceiver is interrupted because the block pool "
operator|+
name|block
operator|.
name|getBlockPoolId
