...
argument_list|)
decl_stmt|;
DECL|field|RECENT_IMAGE_CHECK_ENABLED
specifier|public
specifier|static
specifier|final
name|String
...
init|=
literal|"recent.image.check.enabled"
decl_stmt|;
DECL|field|RECENT_IMAGE_CHECK_ENABLED_DEFAULT
specifier|public
specifier|static
specifier|final
name|boolean
name|RECENT_IMAGE_CHECK_ENABLED_DEFAULT
init|=
literal|true
decl_stmt|;
annotation|@
...
return|;
block|}
DECL|method|getParamStringForAliasMap ( boolean isBootstrapStandby)
specifier|static
name|String
name|getParamStringForAliasMap
parameter_list|(
name|boolean
name|isBootstrapStandby
parameter_list|)
...
decl_stmt|;
name|Object
name|checkRecentImageEnableObj
init|=
name|context
operator|.
name|getAttribute
...
block|{
comment|// This is an error case, but crashing NN due to this
comment|// seems more undesirable. Only log the error and set to default.
name|LOG
operator|.
name|error
argument_list|(
literal|"Expecting boolean obj for setting checking recent image, "
operator|+
literal|"but got "
operator|+
name|checkRecentImageEnableObj
...
literal|". This is "
operator|+
literal|"unexpected! Setting to default."
argument_list|)
expr_stmt|;
name|checkRecentImageEnable
operator|=
...
condition|)
block|{
comment|// only when at least one of two conditions are met we accept
comment|// a new fsImage
comment|// 1. most recent image's txid is too far behind
comment|// 2. last checkpoint time was too old
name|response
operator|.
name|sendError
argument_list|(
...
name|SC_CONFLICT
argument_list|,
literal|"Most recent checkpoint is neither too far behind in "
operator|+
literal|"txid, nor too old. New txnid cnt is "
operator|+
operator|(
name|txid
operator|-
...
name|checkpointTxnCount
operator|+
literal|" unless too long since last upload."
argument_list|)
expr_stmt|;
return|return
literal|null
