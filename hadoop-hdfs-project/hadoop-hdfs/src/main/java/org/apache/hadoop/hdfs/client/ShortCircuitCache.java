...
name|this
operator|+
literal|": finishing cache cleaner run started at "
operator|+
name|curMs
operator|+
literal|".  Demoted "
...
name|setNameFormat
argument_list|(
literal|"ShortCircuitCache Cleaner"
argument_list|)
operator|.
name|build
argument_list|()
...
name|cacheCleaner
decl_stmt|;
comment|/**    * Tree of evictable elements.    *    * Maps (unique) insertion time in nanoseconds to the element.    */
...
name|maxNonMmappedEvictableLifespanMs
decl_stmt|;
comment|/**    * Tree of mmaped evictable elements.    *    * Maps (unique) insertion time in nanoseconds to the element.    */
...
literal|0
decl_stmt|;
comment|/**    * Create a {@link ShortCircuitCache} object from a {@link Configuration}    */
...
return|;
block|}
DECL|method|ShortCircuitCache (int maxTotalSize, long maxNonMmappedEvictableLifespanMs, int maxEvictableMmapedSize, long maxEvictableMmapedLifespanMs, long mmapRetryTimeoutMs, long staleThresholdMs)
specifier|public
name|ShortCircuitCache
parameter_list|(
name|int
...
DECL|method|removeEvictable (ShortCircuitReplica replica, TreeMap<Long, ShortCircuitReplica> map)
specifier|private
name|void
name|removeEvictable
parameter_list|(
...
DECL|method|insertEvictable (Long evictionTimeNs, ShortCircuitReplica replica, TreeMap<Long, ShortCircuitReplica> map)
specifier|private
name|void
name|insertEvictable
parameter_list|(
...
name|this
operator|+
literal|": can't fetchOrCreate "
operator|+
name|key
operator|+
literal|" because the cache is closed."
argument_list|)
expr_stmt|;
block|}
return|return
...
name|this
operator|+
literal|": starting cache cleaner thread which will run "
operator|+
literal|"every "
operator|+
name|rateMs
...
block|}
block|}
DECL|method|getOrCreateClientMmap (ShortCircuitReplica replica)
name|ClientMmap
...
name|this
operator|+
literal|": can't create client mmap for "
operator|+
name|replica
operator|+
literal|" because we failed to "
operator|+
literal|"create one just "
operator|+
name|delta
operator|+
literal|"ms ago."
...
name|this
operator|+
literal|": retrying client mmap for "
operator|+
name|replica
operator|+
literal|", "
...
name|delta
operator|+
literal|" ms after the previous failure."
argument_list|)
expr_stmt|;
block|}
block|}
...
name|append
argument_list|(
literal|"with outstandingMmapCount="
argument_list|)
operator|.
name|append
argument_list|(
