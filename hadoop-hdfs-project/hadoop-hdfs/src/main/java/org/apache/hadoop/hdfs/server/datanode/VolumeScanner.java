...
name|trace
argument_list|(
literal|"{}: updateScannedBytes is zeroing out slot {}.  "
operator|+
...
annotation|@
name|VisibleForTesting
DECL|method|calculateShouldScan (long targetBytesPerSec, long scannedBytesSum)
specifier|static
name|boolean
...
operator|+
literal|"targetBytesPerSec = {}.  shouldScan = {}"
argument_list|,
name|effectiveBytesPerSec
argument_list|,
name|targetBytesPerSec
...
return|;
block|}
comment|/**    * Run an iteration of the VolumeScanner loop.    *    * @return     The number of milliseconds to delay before running the loop    *               again, or 0 to re-run the loop immediately.    */
DECL|method|runLoop ()
specifier|private
name|long
name|runLoop
...
condition|)
block|{
comment|// Take the lock to check if we should stop.
synchronized|synchronized
init|(
name|this
init|)
