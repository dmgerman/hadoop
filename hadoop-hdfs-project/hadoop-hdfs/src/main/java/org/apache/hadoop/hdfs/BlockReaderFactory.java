...
decl_stmt|;
DECL|method|BlockReaderFactory (DFSClient.Conf conf)
specifier|public
name|BlockReaderFactory
parameter_list|(
name|DFSClient
...
return|;
block|}
comment|/**    * Request file descriptors from a DomainPeer.    *    * @return  A ShortCircuitReplica object if we could communicate with the    *          datanode; null, otherwise.     * @throws  IOException If we encountered an I/O exception while communicating    *          with the datanode.    */
DECL|method|requestFileDescriptors (DomainPeer peer)
specifier|private
name|ShortCircuitReplicaInfo
name|requestFileDescriptors
parameter_list|(
...
literal|" while "
operator|+
literal|"attempting to set up short-circuit access. "
operator|+
name|resp
operator|.
name|getMessage
