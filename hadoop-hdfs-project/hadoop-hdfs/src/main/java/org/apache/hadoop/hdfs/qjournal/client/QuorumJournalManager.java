...
argument_list|)
decl_stmt|;
comment|// This config is not publicly exposed
DECL|field|QJM_RPC_MAX_TXNS_KEY
specifier|static
specifier|final
name|String
...
decl_stmt|;
DECL|field|QJM_RPC_MAX_TXNS_DEFAULT
specifier|static
specifier|final
name|int
...
literal|5000
decl_stmt|;
comment|// Maximum number of transactions to fetch at a time when using the
comment|// RPC edit fetch mechanism
DECL|field|maxTxnsPerRpc
specifier|private
specifier|final
name|int
name|maxTxnsPerRpc
...
name|loggers
decl_stmt|;
DECL|field|OUTPUT_BUFFER_CAPACITY_DEFAULT
specifier|private
specifier|static
specifier|final
name|int
name|OUTPUT_BUFFER_CAPACITY_DEFAULT
init|=
literal|512
operator|*
literal|1024
...
decl_stmt|;
comment|/** Limit logging about input stream selection to every 5 seconds max. */
DECL|field|SELECT_INPUT_STREAM_LOG_INTERVAL_MS
specifier|private
specifier|static
specifier|final
name|long
name|SELECT_INPUT_STREAM_LOG_INTERVAL_MS
init|=
literal|5000
decl_stmt|;
DECL|field|selectInputStreamLogHelper
specifier|private
specifier|final
name|LogThrottlingHelper
name|selectInputStreamLogHelper
init|=
operator|new
name|LogThrottlingHelper
argument_list|(
...
argument_list|(
literal|"Attempted to use QJM output buffer "
operator|+
literal|"capacity ("
operator|+
name|size
operator|+
literal|") greater than the IPC max data length ("
operator|+
name|CommonConfigurationKeys
operator|.
name|IPC_MAXIMUM_DATA_LENGTH
...
name|ipcMaxDataLength
operator|+
literal|"). This will cause journals to reject edits."
argument_list|)
throw|;
block|}
name|outputBufferCapacity
...
name|warn
argument_list|(
literal|"Encountered exception while tailing edits>= "
operator|+
name|fromTxnId
operator|+
literal|" via RPC; falling back to streaming."
argument_list|,
name|ioe
argument_list|)
expr_stmt|;
block|}
block|}
name|selectStreamingInputStreams
argument_list|(
name|streams
argument_list|,
name|fromTxnId
...
expr_stmt|;
block|}
comment|/**    * Select input streams from the journals, specifically using the RPC    * mechanism optimized for low latency.    *    * @param streams The collection to store the return streams into.    * @param fromTxnId Select edits starting from this transaction ID    * @param onlyDurableTxns Iff true, only include transactions which have been    *                        committed to a quorum of the journals.    * @throws IOException Upon issues, including cache misses on the journals.    */
DECL|method|selectRpcInputStreams (Collection<EditLogInputStream> streams, long fromTxnId, boolean onlyDurableTxns)
specifier|private
name|void
name|selectRpcInputStreams
parameter_list|(
...
argument_list|()
operator|:
literal|"Quorum call returned without a majority"
assert|;
name|List
argument_list|<
name|Integer
...
name|StringBuilder
argument_list|(
literal|"Requested edits starting from "
argument_list|)
decl_stmt|;
name|msg
operator|.
...
name|IOException
argument_list|(
literal|"Did not get any valid JournaledEdits "
operator|+
literal|"responses: "
operator|+
name|msg
...
name|debug
argument_list|(
literal|"No new edits available in logs; requested starting from "
operator|+
literal|"ID "
operator|+
name|fromTxnId
...
name|info
argument_list|(
literal|"Selected loggers with>= "
operator|+
name|maxAllowedTxns
operator|+
literal|" transactions "
operator|+
literal|"starting from lowest txn ID "
operator|+
name|logAction
operator|.
name|getStats
...
expr_stmt|;
block|}
comment|/**    * Select input streams from the journals, specifically using the streaming    * mechanism optimized for resiliency / bulk load.    */
DECL|method|selectStreamingInputStreams ( Collection<EditLogInputStream> streams, long fromTxnId, boolean inProgressOk, boolean onlyDurableTxns)
specifier|private
name|void
name|selectStreamingInputStreams
parameter_list|(
...
name|selectInputStreamsTimeoutMs
argument_list|,
literal|"selectStreamingInputStreams"
argument_list|)
decl_stmt|;
name|LOG
operator|.
name|debug
argument_list|(
literal|"selectStreamingInputStream manifests:\n"
operator|+
name|Joiner
operator|.
name|on
