...
argument_list|()
decl_stmt|;
comment|// DatanodeStorageInfo must be removed using the iterator to avoid
comment|// ConcurrentModificationException in the underlying storage
name|it
operator|.
name|remove
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// do not select corrupted replica as src. also do not select the block
comment|// that is already in excess map
if|if
condition|(
name|state
operator|==
...
return|;
block|}
DECL|method|shouldProcessOverReplicated (NumberReplicas num, int pendingNum, int expectedNum)
specifier|private
name|boolean
name|shouldProcessOverReplicated
parameter_list|(
...
block|}
block|}
DECL|method|isExcess (DatanodeDescriptor node, BlockInfo block)
specifier|private
name|boolean
name|isExcess
parameter_list|(
