...
block|{
comment|// Violates placement policy. Needed on a new rack or domain etc.
name|BlockPlacementStatus
name|placementStatus
init|=
name|getBlockPlacementStatus
argument_list|(
name|block
argument_list|)
decl_stmt|;
...
block|}
block|}
comment|/**    * Adjust srcNodes and indices which are used to reconstruction block.    * We should guarantee the indexes of first minRequiredSources nodes    + are different.    */
DECL|method|adjustSrcNodesAndIndices (BlockInfoStriped block, DatanodeDescriptor[] srcNodes, List<Byte> indices, DatanodeDescriptor[] newSrcNodes, byte[] newIndices)
specifier|private
name|void
name|adjustSrcNodesAndIndices
parameter_list|(
...
condition|)
block|{
comment|// If the new targets do not meet the placement policy, or at least
comment|// reduce the number of replicas needed, then no use continuing.
return|return
literal|false
return|;
block|}
...
annotation|@
name|VisibleForTesting
DECL|method|chooseSourceDatanodes (BlockInfo block, List<DatanodeDescriptor> containingNodes, List<DatanodeStorageInfo> nodesContainingLiveReplicas, NumberReplicas numReplicas, List<Byte> liveBlockIndices, List<Byte> liveBusyBlockIndices, int priority)
name|DatanodeDescriptor
index|[]
name|chooseSourceDatanodes
parameter_list|(
...
continue|continue;
block|}
comment|// for EC here need to make sure the numReplicas replicates state correct
comment|// because in the scheduleReconstruction it need the numReplicas to check
comment|// whether need to reconstruct the ec internal block
name|byte
name|blockIndex
init|=
operator|-
...
argument_list|)
expr_stmt|;
name|countLiveAndDecommissioningReplicas
argument_list|(
name|numReplicas
argument_list|,
name|state
...
block|}
block|}
comment|/**    * Check block report lease.    * @return true if lease exist and not expire    */
DECL|method|checkBlockReportLease (BlockReportContext context, final DatanodeID nodeID)
specifier|public
name|boolean
name|checkBlockReportLease
parameter_list|(
name|BlockReportContext
name|context
parameter_list|,
...
expr_stmt|;
block|}
comment|/**    * Return the number of nodes hosting a given block, grouped    * by the state of those replicas.    * For a striped block, this includes nodes storing blocks belonging to the    * striped block group. But note we exclude duplicated internal block replicas    * for calculating {@link NumberReplicas#liveReplicas}. If the replica on a    * decommissioning node is the same as the replica on a live node, the    * internal block for this replica is live, not decommissioning.    */
...
block|}
block|}
comment|/**    * Count distinct live and decommission internal blocks with blockIndex.    * If A replica with INDEX is decommissioning, and B replica with INDEX    * is live, the internal INDEX block is live.    */
DECL|method|countLiveAndDecommissioningReplicas (NumberReplicas counters, StoredReplicaState state, BitSet liveBitSet, BitSet decommissioningBitSet, byte blockIndex)
specifier|private
name|void
name|countLiveAndDecommissioningReplicas
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// Sub decommissioning because the index replica is live.
if|if
condition|(
name|decommissioningBitSet
operator|.
...
return|;
block|}
DECL|method|getBlockPlacementStatus (BlockInfo storedBlock)
name|BlockPlacementStatus
name|getBlockPlacementStatus
parameter_list|(
name|BlockInfo
...
return|;
block|}
DECL|method|getBlockPlacementStatus (BlockInfo storedBlock, DatanodeStorageInfo[] additionalStorage)
name|BlockPlacementStatus
name|getBlockPlacementStatus
parameter_list|(
name|BlockInfo
...
condition|)
block|{
comment|// additionalNodes, are potential new targets for the block. If there are
comment|// any passed, include them when checking the placement policy to see if
comment|// the policy is met, when it may not have been met without these nodes.
for|for
control|(
name|DatanodeStorageInfo
name|s
...
name|queue
decl_stmt|;
DECL|method|BlockReportProcessingThread (int size)
name|BlockReportProcessingThread
parameter_list|(
name|int
name|size
