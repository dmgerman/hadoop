...
name|fileIoProvider
decl_stmt|;
DECL|field|addReplicaThreadPool
specifier|private
specifier|static
name|ForkJoinPool
...
literal|null
decl_stmt|;
DECL|field|VOLUMES_REPLICA_ADD_THREADPOOL_SIZE
specifier|private
specifier|static
specifier|final
name|int
name|VOLUMES_REPLICA_ADD_THREADPOOL_SIZE
init|=
name|Runtime
operator|.
name|getRuntime
...
expr_stmt|;
block|}
DECL|method|initializeAddReplicaPool (Configuration conf, FsDatasetImpl dataset)
specifier|private
...
argument_list|)
decl_stmt|;
comment|// Default pool sizes is max of (volume * number of bp_service) and
comment|// number of processor.
name|addReplicaThreadPool
operator|=
operator|new
name|ForkJoinPool
...
name|IOException
argument_list|(
literal|"Failed to start sub tasks to add replica in replica map :"
operator|+
name|e
operator|.
name|getMessage
...
block|}
block|}
comment|/**    * Wait till all the recursive task for add replica to volume completed.    *    * @param subTaskQueue    *          {@link AddReplicaProcessor} tasks list.    * @param exceptions    *          exceptions occurred in sub tasks.    * @throws IOException    *           throw if any sub task or multiple sub tasks failed.    */
DECL|method|waitForSubTaskToFinish (Queue<RecursiveAction> subTaskQueue, List<IOException> exceptions)
specifier|private
name|void
name|waitForSubTaskToFinish
parameter_list|(
...
condition|)
block|{
comment|// in case of NULL checksum type consider full file as valid
return|return
name|blockFileLen
return|;
block|}
...
literal|true
expr_stmt|;
comment|// Remove the shutdown hook to avoid any memory leak
if|if
condition|(
name|shutdownHook
operator|!=
...
return|;
block|}
comment|/**    * Recursive action for add replica in map.    */
DECL|class|AddReplicaProcessor
class|class
name|AddReplicaProcessor
extends|extends
name|RecursiveAction
...
name|dir
decl_stmt|;
DECL|field|lazyWriteReplicaMap
specifier|private
name|RamDiskReplicaTracker
name|lazyWriteReplicaMap
decl_stmt|;
DECL|field|isFinalized
specifier|private
name|boolean
name|isFinalized
decl_stmt|;
...
name|subTaskQueue
decl_stmt|;
comment|/**      * @param volumeMap      *          the replicas map      * @param dir      *          an input directory      * @param lazyWriteReplicaMap      *          Map of replicas on transient storage.      * @param isFinalized      *          true if the directory has finalized replicas; false if the      *          directory has rbw replicas      * @param exceptions      *          List of exception which need to return to parent thread.      * @param subTaskQueue      *          queue of sub tasks      */
DECL|method|AddReplicaProcessor (ReplicaMap volumeMap, File dir, RamDiskReplicaTracker lazyWriteReplicaMap, boolean isFinalized, List<IOException> exceptions, Queue<RecursiveAction> subTaskQueue)
name|AddReplicaProcessor
parameter_list|(
name|ReplicaMap
name|volumeMap
...
name|warn
argument_list|(
literal|"Caught exception while adding replicas from "
operator|+
name|volume
operator|+
literal|" in subtask. Will throw later."
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * Return the size of fork pool used for adding replica in map.    */
annotation|@
name|VisibleForTesting
DECL|method|getAddReplicaForkPoolSize ()
specifier|public
specifier|static
name|int
name|getAddReplicaForkPoolSize
parameter_list|()
block|{
return|return
name|addReplicaThreadPool
...
annotation|@
name|VisibleForTesting
DECL|method|getAddReplicaThreadPool ()
specifier|public
name|ForkJoinPool
name|getAddReplicaThreadPool
parameter_list|()
block|{
return|return
name|addReplicaThreadPool
...
annotation|@
name|VisibleForTesting
DECL|method|reInitializeAddReplicaThreadPool ()
specifier|public
specifier|static
name|void
name|reInitializeAddReplicaThreadPool
parameter_list|()
block|{
name|addReplicaThreadPool
operator|.
