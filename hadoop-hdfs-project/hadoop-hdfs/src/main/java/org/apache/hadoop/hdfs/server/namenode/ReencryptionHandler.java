...
return|;
block|}
comment|/**    * get the tracker without holding the FSDirectory lock. This is only used for    * testing, when updater checks about pausing.    */
...
DECL|method|addDummyTracker (final long zoneId)
name|void
...
argument_list|)
expr_stmt|;
comment|// This also removes the zone from reencryptionStatus
name|submissions
operator|.
name|remove
argument_list|(
...
block|}
block|}
comment|/**    * Resolve the cursor of re-encryption to an inode.    *<p>    * The parent of the lowest level startAfter is returned. If somewhere in the    * middle of startAfters changed, the parent of the lowest unchanged level is    * returned.    *    * @param zoneId      Id of the EZ inode.    * @param startAfters the cursor, represented by a list of path bytes.    * @return the parent inode corresponding to the startAfters, or null if    * the EZ node (furthest parent) is deleted.    */
DECL|method|resolvePaths (final long zoneId, List<byte[]> startAfters)
specifier|private
name|INode
name|resolvePaths
parameter_list|(
...
block|}
block|}
comment|/**    * Iterates the parent directory, and add direct children files to    * current batch. If batch size meets configured threshold, a Callable    * is created and sent to the thread pool, which will communicate to the KMS    * to get new edeks.    *<p>    * Locks could be released and reacquired when a Callable is created.    *    * @param zoneId Id of the EZ INode    * @return The inode which was just processed, if lock is held in the entire    * process. Null if lock is released.    * @throws IOException    * @throws InterruptedException    */
DECL|method|reencryptDirInt (final long zoneId, INode curr, List<byte[]> startAfters, final String ezKeyVerName)
specifier|private
name|INode
name|reencryptDirInt
parameter_list|(
...
name|debug
argument_list|(
literal|"Re-encrypting directory {}"
argument_list|,
name|parent
operator|.
name|getFullPathName
...
condition|)
block|{
comment|// inode wasn't added for re-encryption. Recurse down if it's a dir,
...
DECL|method|reencryptINode (final INode inode, final String ezKeyVerName)
specifier|private
name|boolean
name|reencryptINode
parameter_list|(
...
DECL|method|checkZoneReady (final long zoneId)
name|void
name|checkZoneReady
parameter_list|(
specifier|final
