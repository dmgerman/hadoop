...
name|warn
argument_list|(
literal|"Error reading number of transactions from "
operator|+
name|candidate
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|/**        * If all candidates either threw a CorruptionException or        * found 0 transactions, then a gap exists.         */
if|if
condition|(
name|corruption
operator|!=
...
name|IOException
argument_list|(
literal|"Gap exists in logs from "
operator|+
name|fromTxId
argument_list|,
name|corruption
...
block|}
argument_list|,
literal|"recovering unclosed streams"
argument_list|)
expr_stmt|;
block|}
