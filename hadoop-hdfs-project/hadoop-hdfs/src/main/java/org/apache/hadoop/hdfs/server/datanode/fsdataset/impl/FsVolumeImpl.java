...
name|IOException
block|{
comment|// outside tests, usage created in ReservedSpaceCalculator.Builder
name|this
argument_list|(
name|dataset
argument_list|,
...
expr_stmt|;
block|}
DECL|method|FsVolumeImpl (FsDatasetImpl dataset, String storageID, StorageDirectory sd, FileIoProvider fileIoProvider, Configuration conf, DF usage)
name|FsVolumeImpl
parameter_list|(
name|FsDatasetImpl
name|dataset
...
name|warn
argument_list|(
literal|"Setting reserved to null as usage is null"
argument_list|)
expr_stmt|;
name|cachedCapacity
operator|=
...
name|debug
argument_list|(
literal|"The reference count for {} is {}, wait to be 0."
argument_list|,
name|this
argument_list|,
name|reference
...
return|;
block|}
comment|/**    * Unplanned Non-DFS usage, i.e. Extra usage beyond reserved.    *    * @return Disk usage excluding space used by HDFS and excluding space    * reserved for blocks open for write.    * @throws IOException    */
...
name|bytesToRelease
expr_stmt|;
comment|// Fail-safe, this should never be less than zero in practice, but if it
comment|// does, do not advertise more space than is have available.
name|newReservation
operator|=
name|Math
operator|.
...
name|trace
argument_list|(
literal|"getNextSubDir({}, {}): picking next subdirectory {} within {}"
argument_list|,
name|storageID
argument_list|,
name|bpid
...
name|checksum
decl_stmt|;
comment|// copy the last partial checksum if the replica is originally
comment|// in finalized or rbw state.
switch|switch
condition|(
name|replicaInfo
operator|.
...
argument_list|()
decl_stmt|;
comment|// Only a finalized replica can be appended.
name|FinalizedReplica
name|finalized
init|=
operator|(
...
return|;
block|}
comment|/**    * Filter for block file names stored on the file system volumes.    */
...
block|}
block|}
DECL|method|compileReport (File bpFinalizedDir, File dir, Collection<ScanInfo> report, ReportCompiler reportCompiler)
specifier|private
name|void
name|compileReport
parameter_list|(
