...
argument_list|)
decl_stmt|;
comment|// Step 2. Load children nodes under parent
name|loadChildren
argument_list|(
name|parent
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// Step 3. Load snapshots if parent is snapshottable
name|int
name|numSnapshots
init|=
name|in
...
expr_stmt|;
block|}
comment|// Step 4. load SnapshotDiff list
name|int
...
name|parent
decl_stmt|;
comment|// load SnapshotDiff list
name|SnapshotFSImageFormat
operator|.
...
condition|)
block|{
comment|// to indicate INodeFileWithSnapshot, blocks may be set as null while
comment|// numBlocks is set to 0
name|blocks
operator|=
name|LayoutVersion
operator|.
...
expr_stmt|;
block|}
comment|// 1. Print prefix (parent directory name)
name|int
name|prefixLen
init|=
name|currentDirName
...
expr_stmt|;
block|}
comment|// 2. Write children INode
name|dirNum
operator|+=
name|saveChildren
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// 3. Write INodeDirectorySnapshottable#snapshotsByNames to record all
comment|// Snapshots
if|if
condition|(
name|current
...
block|}
comment|// 4. Write SnapshotDiff lists.
if|if
condition|(
name|current
operator|instanceof
