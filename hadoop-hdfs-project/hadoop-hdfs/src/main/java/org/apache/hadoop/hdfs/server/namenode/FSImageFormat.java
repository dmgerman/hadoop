...
argument_list|)
expr_stmt|;
comment|// load Files Under Construction
name|this
operator|.
...
literal|null
decl_stmt|;
comment|// for older versions, a blocklist of size 0
comment|// indicates a directory.
if|if
condition|(
operator|(
operator|-
...
comment|// first block as the blocksize. Otherwise use the default block size.
comment|//
if|if
condition|(
operator|-
...
return|;
block|}
DECL|method|loadDatanodes (DataInputStream in)
specifier|private
name|void
name|loadDatanodes
parameter_list|(
...
literal|3
condition|)
comment|// pre datanode image version
return|return;
if|if
condition|(
name|imgVersion
...
block|{
return|return;
comment|// new versions do not store the datanodes any more.
block|}
name|int
name|size
init|=
...
control|)
block|{
comment|// We don't need to add these descriptors any more.
name|FSImageSerialization
operator|.
name|DatanodeImage
operator|.
...
literal|13
condition|)
comment|// pre lease image version
return|return;
name|int
name|size
init|=
...
name|IOException
argument_list|(
literal|"Found lease for non-existent file "
operator|+
name|path
argument_list|)
throw|;
...
name|IOException
argument_list|(
literal|"Found lease for directory "
operator|+
name|path
argument_list|)
throw|;
