...
expr_stmt|;
block|}
comment|/**    * Create a snapshot of subtrees for recursively coping the directory    * structure from the source directory to the snapshot destination directory.    * This creation algorithm requires O(N) running time and O(N) memory,    * where N = # files + # directories + # symlinks.     */
DECL|class|SnapshotCreation
class|class
name|SnapshotCreation
block|{
comment|/** The source root directory path where the snapshot is taken. */
DECL|field|srcRoot
specifier|final
name|INodeDirectorySnapshottable
name|srcRoot
decl_stmt|;
comment|/**       * Constructor.      * @param path The path must be a snapshottable directory.      */
DECL|method|SnapshotCreation (final String path)
specifier|private
name|SnapshotCreation
parameter_list|(
specifier|final
...
expr_stmt|;
block|}
DECL|method|run (final String name)
name|void
name|run
parameter_list|(
specifier|final
