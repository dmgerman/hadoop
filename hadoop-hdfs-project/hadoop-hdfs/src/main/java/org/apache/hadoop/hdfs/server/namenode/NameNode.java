...
argument_list|)
decl_stmt|;
DECL|field|notBecomeActiveInSafemode
specifier|private
specifier|final
name|boolean
name|notBecomeActiveInSafemode
decl_stmt|;
DECL|field|HEALTH_MONITOR_WARN_THRESHOLD_MS
specifier|private
specifier|final
specifier|static
name|int
name|HEALTH_MONITOR_WARN_THRESHOLD_MS
init|=
literal|5000
decl_stmt|;
comment|/** httpServer */
...
annotation|@
name|Override
DECL|method|verifyToken (DelegationTokenIdentifier id, byte[] password)
specifier|public
name|void
name|verifyToken
parameter_list|(
...
name|info
argument_list|(
literal|"Clients should use {} to access"
operator|+
literal|" this namenode/service."
argument_list|,
name|clientNamenodeAddress
...
return|;
block|}
comment|/**    * NameNodeHttpServer, used by unit tests to ensure a full shutdown,    * so that no bind exception is thrown during restart.    */
annotation|@
name|VisibleForTesting
...
argument_list|()
expr_stmt|;
comment|// Abort NameNode format if reformat is disabled and if
comment|// meta-dir already exists
if|if
condition|(
name|conf
operator|.
...
argument_list|(
literal|"NameNode format aborted as reformat is disabled for "
operator|+
...
condition|)
block|{
comment|// log a warning if it take>= 5 seconds.
name|LOG
operator|.
name|warn
argument_list|(
literal|"Remote IP {} checking available resources took {}ms"
argument_list|,
name|Server
operator|.
name|getRemoteIp
...
name|HealthCheckFailedException
argument_list|(
literal|"The NameNode is configured to "
operator|+
literal|"report UNHEALTHY to ZKFC in Safemode."
argument_list|)
throw|;
block|}
block|}
...
argument_list|()
operator|+
literal|" still not leave safemode"
argument_list|)
throw|;
block|}
name|state
...
block|{
return|return
name|reconfReplicationParameters
argument_list|(
name|newVal
argument_list|,
name|property
...
block|}
block|}
DECL|method|reconfReplicationParameters (final String newVal, final String property)
specifier|private
name|String
name|reconfReplicationParameters
parameter_list|(
...
name|property
operator|+
literal|"in reconfReplicationParameters"
argument_list|)
throw|;
block|}
name|LOG
...
name|info
argument_list|(
literal|"RECONFIGURE* changed {} to {}"
argument_list|,
name|property
argument_list|,
name|newSetting
...
block|}
block|}
DECL|method|adjustNewVal (int defaultVal, String newVal)
specifier|private
name|int
name|adjustNewVal
parameter_list|(
...
name|state
operator|+
literal|" NameNode is not allowed"
argument_list|)
argument_list|)
throw|;
block|}
