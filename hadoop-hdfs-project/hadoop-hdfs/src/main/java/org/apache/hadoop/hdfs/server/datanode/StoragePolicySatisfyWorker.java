...
control|)
block|{
comment|// Iterating backwards. This is to ensure that all the block src location
comment|// which doesn't have a target node will be marked as failure before
comment|// scheduling the block movement to valid target nodes.
for|for
control|(
name|int
name|i
...
condition|)
block|{
comment|// Since there is no target selected for scheduling the block,
comment|// just mark this block storage movement as failure. Later, namenode
comment|// can take action on this.
name|movementTracker
operator|.
