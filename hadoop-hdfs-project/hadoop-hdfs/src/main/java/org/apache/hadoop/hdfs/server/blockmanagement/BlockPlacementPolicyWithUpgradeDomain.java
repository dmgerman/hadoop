...
return|;
block|}
DECL|method|getUpgradeDomainMap ( Collection<T> storagesOrDataNodes)
specifier|private
parameter_list|<
name|T
parameter_list|>
...
return|;
block|}
DECL|method|combine ( Collection<DatanodeStorageInfo> moreThanOne, Collection<DatanodeStorageInfo> exactlyOne)
specifier|private
name|Collection
argument_list|<
name|DatanodeStorageInfo
...
condition|)
block|{
comment|// If BlockPlacementPolicyDefault doesn't allow useDelHint, there is no
comment|// point checking with upgrade domain policy.
return|return
literal|false
return|;
block|}
return|return
name|isMovableBasedOnUpgradeDomain
argument_list|(
name|combine
argument_list|(
name|moreThanOne
...
return|;
block|}
comment|// Check if moving from source to target will preserve the upgrade domain
comment|// policy.
DECL|method|isMovableBasedOnUpgradeDomain (Collection<T> all, T source, T target)
specifier|private
parameter_list|<
name|T
parameter_list|>
...
argument_list|)
decl_stmt|;
comment|// shareUDSet includes datanodes that share same upgrade
comment|// domain with another datanode.
name|List
argument_list|<
name|T
argument_list|>
...
argument_list|)
decl_stmt|;
comment|// check if removing source reduces the number of upgrade domains
if|if
condition|(
...
literal|true
return|;
comment|// existing number of upgrade domain exceeds the limit.
block|}
else|else
block|{
return|return
literal|false
return|;
comment|// removing source reduces the number of UDs.
block|}
block|}
annotation|@
name|Override
