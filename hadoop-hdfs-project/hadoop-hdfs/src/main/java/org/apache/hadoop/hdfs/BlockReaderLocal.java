...
name|fisCache
decl_stmt|;
DECL|field|clientMmap
specifier|private
name|ClientMmap
name|clientMmap
decl_stmt|;
DECL|field|mmapDisabled
specifier|private
name|boolean
name|mmapDisabled
decl_stmt|;
DECL|method|getSlowReadBufferNumChunks (int bufSize, int bytesPerChecksum)
specifier|private
specifier|static
name|int
...
argument_list|(
literal|"Configured BlockReaderLocal buffer size ("
operator|+
name|bufSize
operator|+
...
return|;
block|}
DECL|method|BlockReaderLocal (DFSClient.Conf conf, String filename, ExtendedBlock block, long startOffset, long length, FileInputStream dataIn, FileInputStream checksumIn, DatanodeID datanodeID, boolean verifyChecksum, FileInputStreamCache fisCache)
specifier|public
name|BlockReaderLocal
parameter_list|(
name|DFSClient
...
try|try
block|{
comment|// Reposition both input streams to the beginning of the chunk
comment|// containing startOffset
name|this
operator|.
name|dataIn
operator|.
...
name|debug
argument_list|(
literal|"Created BlockReaderLocal for file "
operator|+
name|filename
operator|+
literal|" block "
...
name|debug
argument_list|(
literal|"putting FileInputStream for "
operator|+
name|filename
operator|+
literal|" back into FileInputStreamCache"
argument_list|)
expr_stmt|;
block|}
name|fisCache
...
name|debug
argument_list|(
literal|"closing FileInputStream for "
operator|+
name|filename
argument_list|)
expr_stmt|;
...
annotation|@
name|Override
DECL|method|readZeroCopy (HdfsZeroCopyCursor cursor, LocatedBlock curBlock, long blockPos, int toRead, ClientMmapManager mmapManager)
specifier|public
name|boolean
name|readZeroCopy
parameter_list|(
...
name|error
argument_list|(
literal|"Interrupted while setting up mmap for "
operator|+
name|filename
argument_list|,
name|e
...
name|error
argument_list|(
literal|"unable to set up mmap for "
operator|+
name|filename
argument_list|,
name|e
...
condition|)
block|{
comment|/*        * In Java, ByteBuffers use a 32-bit length, capacity, offset, etc.        * This limits our mmap'ed regions to 2 GB in length.        * TODO: we can implement zero-copy for larger blocks by doing multiple        * mmaps.        */
name|mmapDisabled
operator|=
literal|true
expr_stmt|;
