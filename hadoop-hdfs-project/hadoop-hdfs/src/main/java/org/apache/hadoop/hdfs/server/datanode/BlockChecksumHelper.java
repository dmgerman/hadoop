...
name|datanode
decl_stmt|;
DECL|field|blockChecksumOptions
specifier|private
specifier|final
name|BlockChecksumOptions
name|blockChecksumOptions
...
literal|1
decl_stmt|;
DECL|method|AbstractBlockChecksumComputer ( DataNode datanode, BlockChecksumOptions blockChecksumOptions)
name|AbstractBlockChecksumComputer
parameter_list|(
name|DataNode
name|datanode
...
return|;
block|}
DECL|method|getBlockChecksumOptions ()
name|BlockChecksumOptions
...
name|checksum
decl_stmt|;
DECL|method|BlockChecksumComputer (DataNode datanode, ExtendedBlock block, BlockChecksumOptions blockChecksumOptions)
name|BlockChecksumComputer
parameter_list|(
name|DataNode
name|datanode
...
name|BlockChecksumComputer
block|{
DECL|method|ReplicatedBlockChecksumComputer (DataNode datanode, ExtendedBlock block, BlockChecksumOptions blockChecksumOptions)
name|ReplicatedBlockChecksumComputer
parameter_list|(
name|DataNode
name|datanode
...
name|format
argument_list|(
literal|"Unrecognized BlockChecksumType: %s"
argument_list|,
name|type
argument_list|)
argument_list|)
...
return|;
block|}
DECL|method|computeCompositeCrc (long stripeLength)
specifier|private
name|void
name|computeCompositeCrc
parameter_list|(
...
argument_list|()
decl_stmt|;
comment|// Whether getting the checksum for the entire block (which itself may
comment|// not be a full block size and may have a final chunk smaller than
comment|// getBytesPerCRC()), we begin with a number of full chunks, all of size
comment|// getBytesPerCRC().
name|long
name|numFullChunks
init|=
name|checksumDataLength
...
argument_list|)
expr_stmt|;
comment|// There may be a final partial chunk that is not full-sized. Unlike the
comment|// MD5 case, we still consider this a "partial chunk" even if
comment|// getRequestLength() == getVisibleLength(), since the CRC composition
comment|// depends on the byte size of that final chunk, even if it already has a
comment|// precomputed CRC stored in metadata. So there are two cases:
comment|//   1. Reading only part of a block via getRequestLength(); we get the
comment|//      crcPartialBlock() explicitly.
comment|//   2. Reading full visible length; the partial chunk already has a CRC
comment|//      stored in block metadata, so we just continue reading checksumIn.
name|long
...
name|debug
argument_list|(
literal|"block={}, getBytesPerCRC={}, crcPerBlock={}, compositeCrc={}"
argument_list|,
name|getBlock
argument_list|()
argument_list|,
...
argument_list|()
decl_stmt|;
comment|// Keeps track of the positions within blockChecksumBuf where each data
comment|// block's checksum begins; for fixed-size block checksums this is easily
comment|// calculated as a multiple of the checksum size, but for striped block
comment|// CRCs, it's less error-prone to simply keep track of exact byte offsets
comment|// before each block checksum is populated into the buffer.
DECL|field|blockChecksumPositions
specifier|private
specifier|final
name|int
index|[]
name|blockChecksumPositions
decl_stmt|;
DECL|method|BlockGroupNonStripedChecksumComputer ( DataNode datanode, StripedBlockInfo stripedBlockInfo, long requestedNumBytes, BlockChecksumOptions blockChecksumOptions)
name|BlockGroupNonStripedChecksumComputer
parameter_list|(
name|DataNode
name|datanode
...
control|)
block|{
comment|// Before populating the blockChecksum at this index, record the byte
comment|// offset where it will begin.
name|blockChecksumPositions
index|[
name|idx
index|]
...
name|digest
init|=
name|reassembleNonStripedCompositeCrc
argument_list|(
name|checksumLen
argument_list|)
decl_stmt|;
...
block|}
block|}
comment|/**      * @param checksumLen The sum of bytes associated with the block checksum      *     data being digested into a block-group level checksum.      */
DECL|method|reassembleNonStripedCompositeCrc (long checksumLen)
specifier|private
name|byte
index|[]
name|reassembleNonStripedCompositeCrc
...
argument_list|)
decl_stmt|;
comment|// This should hold all the cell-granularity checksums of blk0
comment|// followed by all cell checksums of blk1, etc. We must unstripe the
comment|// cell checksums in order of logical file bytes. Also, note that the
comment|// length of this array may not equal the the number of actually valid
comment|// bytes in the buffer (blockChecksumBuf.getLength()).
name|byte
index|[]
name|flatBlockChecksumData
init|=
name|blockChecksumBuf
operator|.
name|getData
argument_list|()
decl_stmt|;
comment|// Initialize byte-level cursors to where each block's checksum begins
comment|// inside the combined flattened buffer.
name|int
index|[]
name|blockChecksumCursors
init|=
operator|new
name|int
index|[
...
expr_stmt|;
block|}
comment|// Reassemble cell-level CRCs in the right order.
name|long
name|numFullCells
init|=
name|checksumLen
...
name|debug
argument_list|(
literal|"flatBlockChecksumData.length={}, numDataUnits={}, "
operator|+
literal|"checksumLen={}, digest={}"
argument_list|,
name|flatBlockChecksumData
operator|.
name|length
...
comment|// A BlockGroupCheckum of type COMPOSITE_CRC uses underlying
comment|// BlockChecksums also of type COMPOSITE_CRC but with
comment|// stripeLength == ecPolicy.getCellSize().
name|BlockChecksumOptions
name|childOptions
decl_stmt|;
name|BlockChecksumType
name|groupChecksumType
init|=
name|getBlockChecksumOptions
argument_list|()
operator|.
...
name|debug
argument_list|(
literal|"got reply from datanode:{} for blockIdx:{}, checksum:{}"
argument_list|,
name|targetDatanode
argument_list|,
name|blockIdx
...
name|debug
argument_list|(
literal|"Recalculated checksum for the block index:{}, checksum={}"
argument_list|,
name|errBlkIndex
argument_list|,
name|checksumRecon
...
name|format
argument_list|(
literal|"BlockChecksumType COMPOSITE_CRC doesn't support MIXED "
operator|+
literal|"underlying types; previous block was %s, next block is %s"
argument_list|,
name|getCrcType
argument_list|()
argument_list|,
