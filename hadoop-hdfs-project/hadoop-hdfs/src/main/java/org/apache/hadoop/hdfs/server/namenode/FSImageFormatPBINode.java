...
argument_list|)
decl_stmt|;
DECL|field|DIRECTORY_ENTRY_BATCH_SIZE
specifier|private
specifier|static
specifier|final
name|int
name|DIRECTORY_ENTRY_BATCH_SIZE
init|=
literal|1000
decl_stmt|;
...
name|parent
decl_stmt|;
DECL|field|cacheNameMapLock
specifier|private
name|ReentrantLock
name|cacheNameMapLock
decl_stmt|;
DECL|field|blockMapLock
specifier|private
name|ReentrantLock
name|blockMapLock
decl_stmt|;
...
expr_stmt|;
block|}
DECL|method|loadINodeDirectorySectionInParallel (ExecutorService service, ArrayList<FileSummary.Section> sections, String compressionCodec)
name|void
...
name|info
argument_list|(
literal|"Loading the INodeDirectory section in parallel with {} sub-"
operator|+
literal|"sections"
argument_list|,
name|sections
...
name|error
argument_list|(
literal|"An exception occurred loading INodeDirectories in "
operator|+
literal|"parallel"
argument_list|,
name|e
...
name|warn
argument_list|(
literal|"Failed to close the input stream, ignoring"
argument_list|,
name|ioe
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Interrupted waiting for countdown latch"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"{} exceptions occurred loading INodeDirectories"
argument_list|,
name|exceptions
operator|.
name|size
...
name|info
argument_list|(
literal|"Completed loading all INodeDirectory sub-sections"
argument_list|)
expr_stmt|;
block|}
...
name|warn
argument_list|(
literal|"Failed to add the inode {} to the directory {}"
argument_list|,
name|child
operator|.
name|getId
...
name|warn
argument_list|(
literal|"Failed to add the inode reference {} to the directory {}"
argument_list|,
name|ref
operator|.
name|getId
...
expr_stmt|;
block|}
DECL|method|addToCacheAndBlockMap (ArrayList<INode> inodeList)
specifier|private
name|void
name|addToCacheAndBlockMap
parameter_list|(
...
name|info
argument_list|(
literal|"Successfully loaded {} inodes"
argument_list|,
name|totalLoaded
argument_list|)
expr_stmt|;
block|}
DECL|method|loadINodesInSection (InputStream in, Counter counter)
specifier|private
name|int
name|loadINodesInSection
parameter_list|(
...
name|IOException
block|{
comment|// As the input stream is a LimitInputStream, the reading will stop when
comment|// EOF is encountered at the end of the stream.
name|int
name|cntr
init|=
literal|0
...
return|;
block|}
DECL|method|loadINodeSectionHeader (InputStream in, StartupProgress prog, Step currentStep)
specifier|private
name|long
name|loadINodeSectionHeader
parameter_list|(
...
return|;
block|}
DECL|method|loadINodeSectionInParallel (ExecutorService service, ArrayList<FileSummary.Section> sections, String compressionCodec, StartupProgress prog, Step currentStep)
name|void
...
name|info
argument_list|(
literal|"Loading the INode section in parallel with {} sub-sections"
argument_list|,
name|sections
operator|.
name|size
...
condition|)
block|{
comment|// The first inode section has a header which must be processed first
name|expectedInodes
operator|=
name|loadINodeSectionHeader
argument_list|(
...
name|error
argument_list|(
literal|"An exception occurred loading INodes in parallel"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"{} exceptions occurred loading INodes"
argument_list|,
name|exceptions
operator|.
name|size
...
name|info
argument_list|(
literal|"Completed loading all INode sections. Loaded {} inodes."
argument_list|,
name|totalLoaded
operator|.
name|get
...
block|}
block|}
DECL|method|addToParent (INodeDirectory parentDir, INode child)
specifier|private
name|boolean
name|addToParent
parameter_list|(
