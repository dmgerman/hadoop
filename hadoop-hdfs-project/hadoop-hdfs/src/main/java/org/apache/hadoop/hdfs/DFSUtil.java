...
argument_list|()
decl_stmt|;
comment|// Configuration with a single namenode
if|if
condition|(
name|nameserviceIds
operator|==
...
block|{
comment|// Get the namenodes for all the configured nameServiceIds
for|for
control|(
name|String
name|nameserviceId
...
condition|)
block|{
comment|// client should try {@link isDefaultNamenodeAddress} instead
return|return
literal|null
return|;
block|}
comment|// Get the candidateAddresses for all the configured nameServiceIds
for|for
control|(
name|String
name|nameserviceId
...
return|;
block|}
comment|/**    * return server http or https address from the configuration    * @param conf    * @param namenode - namenode address    * @param httpsAddress -If true, and if security is enabled, returns server     *                      https address. If false, returns server http address.    * @return server http or https address    */
DECL|method|getInfoServer ( InetSocketAddress namenode, Configuration conf, boolean httpsAddress)
specifier|public
specifier|static
name|String
name|getInfoServer
...
return|;
block|}
comment|/**    * @return key specific to a nameserviceId from a generic key    */
DECL|method|getNameServiceIdKey (String key, String nameserviceId)
specifier|public
specifier|static
name|String
name|getNameServiceIdKey
...
DECL|method|setGenericConf (Configuration conf, String nameserviceId, String... keys)
specifier|public
specifier|static
name|void
name|setGenericConf
...
return|;
block|}
comment|/**    * Get name service Id for the {@link NameNode} based on namenode RPC address    * matching the local node address.    */
...
return|;
block|}
comment|/**    * Get name service Id for the BackupNode based on backup node RPC address    * matching the local node address.    */
...
return|;
block|}
comment|/**    * Get name service Id for the secondary node based on secondary http address    * matching the local node address.    */
...
condition|)
block|{
comment|// Not federation configuration, hence no nameservice Id
return|return
literal|null
return|;
block|}
comment|// Match the rpc address with that of local address
name|int
name|found
init|=
literal|0
...
name|HadoopIllegalArgumentException
argument_list|(
literal|"Configuration has multiple RPC addresses that matches "
operator|+
literal|"the local node's address. Please configure the system with "
operator|+
...
name|addressKey
operator|+
literal|" is missing in configuration with name service Id"
argument_list|)
throw|;
block|}
return|return
