...
block|}
block|}
comment|// If true, then replace the port numbers in the final address list
comment|// with the default service RPC port.
name|boolean
name|replacePortNumbers
init|=
literal|false
decl_stmt|;
comment|// First try to lookup using the service RPC address keys.
name|Map
argument_list|<
name|String
argument_list|,
...
argument_list|)
decl_stmt|;
comment|// Next try to lookup using the RPC address key.
if|if
condition|(
name|addressList
operator|.
...
expr_stmt|;
block|}
comment|// Finally, fallback to the default address.
comment|// This will not yield the correct address in a federated/HA setup.
if|if
condition|(
name|addressList
operator|.
...
condition|)
block|{
comment|// Replace the RPC port(s) with the default service RPC port(s)
name|addressList
operator|.
name|forEach
argument_list|(
