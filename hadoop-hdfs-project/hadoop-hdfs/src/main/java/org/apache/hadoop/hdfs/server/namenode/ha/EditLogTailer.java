...
decl_stmt|;
comment|/**    * How often the tailer should check if there are new edit log entries    * ready to be consumed. This is the initial delay before any backoff.    */
...
name|sleepTimeMs
decl_stmt|;
comment|/**    * The maximum time the tailer should wait between checking for new edit log    * entries. Exponential backoff will be applied when an edit log tail is    * performed but no edits are available to be read. If this is less than or    * equal to 0, backoff is disabled.    */
DECL|field|maxSleepTimeMs
specifier|private
specifier|final
name|long
name|maxSleepTimeMs
...
name|warn
argument_list|(
literal|"{} was configured to be {} ms, but this is less than {}."
operator|+
literal|"Disabling backoff when tailing edit logs."
argument_list|,
name|DFSConfigKeys
operator|.
name|DFS_HA_TAILEDITS_PERIOD_BACKOFF_MAX_KEY
...
annotation|@
name|VisibleForTesting
DECL|method|sleep (long sleepTimeMillis)
name|void
name|sleep
parameter_list|(
name|long
...
condition|)
block|{
comment|// If no edits were tailed, apply exponential backoff
comment|// before tailing again. Double the current sleep time on each
comment|// empty response, but don't exceed the max. If the sleep time
comment|// was configured as 0, start the backoff at 1 ms.
name|currentSleepTimeMs
operator|=
name|Math
operator|.
...
name|sleepTimeMs
expr_stmt|;
comment|// reset to initial sleep time
block|}
name|EditLogTailer
operator|.
name|this
