...
return|;
block|}
comment|/**    * Retrieve existing INodes from a path. For non-snapshot path,    * the number of INodes is equal to the number of path components. For    * snapshot path (e.g., /foo/.snapshot/s1/bar), the number of INodes is    * (number_of_path_components - 1).    *     * An UnresolvedPathException is always thrown when an intermediate path     * component refers to a symbolic link. If the final path component refers     * to a symbolic link then an UnresolvedPathException is only thrown if    * resolveLink is true.      *     *<p>    * Example:<br>    * Given the path /c1/c2/c3 where only /c1/c2 exists, resulting in the    * following path components: ["","c1","c2","c3"]    *     *<p>    *<code>getExistingPathINodes(["","c1","c2"])</code> should fill    * the array with [rootINode,c1,c2],<br>    *<code>getExistingPathINodes(["","c1","c2","c3"])</code> should    * fill the array with [rootINode,c1,c2,null]    *     * @param startingDir the starting directory    * @param components array of path component name    * @param resolveLink indicates whether UnresolvedLinkException should    *        be thrown when the path refers to a symbolic link.    * @return the specified number of existing INodes in the path    */
DECL|method|resolve (final INodeDirectory startingDir, final byte[][] components, final boolean resolveLink)
specifier|static
name|INodesInPath
name|resolve
parameter_list|(
...
condition|)
block|{
comment|// for snapshot path shrink the inode array. however, for path ending with
comment|// .snapshot, still keep last the null inode in the array
name|INode
index|[]
name|newNodes
init|=
...
return|;
block|}
comment|/**    * @param length number of ancestral INodes in the returned INodesInPath    *               instance    * @return the INodesInPath instance containing ancestral INodes    */
...
decl_stmt|;
name|int
name|dotSnapshotIndex
init|=
name|getDotSnapshotIndex
argument_list|()
decl_stmt|;
if|if
condition|(
...
return|;
block|}
DECL|method|getDotSnapshotIndex ()
specifier|private
name|int
name|getDotSnapshotIndex
parameter_list|()
...
argument_list|()
operator|+
literal|" is a snapshot path but does not contain "
operator|+
name|HdfsConstants
operator|.
