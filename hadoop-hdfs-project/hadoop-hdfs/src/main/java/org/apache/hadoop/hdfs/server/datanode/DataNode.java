...
if|if
condition|(
name|isNetworkRelatedException
argument_list|(
name|e
argument_list|)
condition|)
...
block|}
block|}
comment|/**    * Check if the provided exception looks like it's from a network error    * @param e the exception from a checkDiskError call    * @return true if this exception is network related, false otherwise    */
DECL|method|isNetworkRelatedException (Exception e)
specifier|protected
name|boolean
name|isNetworkRelatedException
parameter_list|(
...
name|error
argument_list|(
literal|"Bad command line arguments"
argument_list|)
expr_stmt|;
name|printUsage
argument_list|(
...
name|error
argument_list|(
literal|"Missing or unrecognized option to "
operator|+
name|StartupOption
operator|.
name|ROLLINGUPGRADE
...
name|info
argument_list|(
literal|"Rolling upgrade rollback requested via startup option"
argument_list|)
expr_stmt|;
block|}
else|else
