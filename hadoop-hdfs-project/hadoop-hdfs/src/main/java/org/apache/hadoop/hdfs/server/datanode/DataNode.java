...
decl_stmt|;
block|}
comment|/**    * Parse the new DFS_DATANODE_DATA_DIR value in the configuration to detect    * changed volumes.    * @return changed volumes.    * @throws IOException if none of the directories are specified in the    * configuration.    */
annotation|@
name|VisibleForTesting
DECL|method|parseChangedVolumes ()
name|ChangedVolumes
...
return|;
block|}
comment|/**    * Attempts to reload data volumes with new configuration.    * @param newVolumes a comma separated string that specifies the data volumes.    * @throws Exception    */
...
name|info
argument_list|(
literal|"Loading volumes for namesapce: "
operator|+
name|nsInfo
operator|.
name|getNamespaceID
...
argument_list|()
decl_stmt|;
comment|// Clean all failed volumes.
for|for
control|(
name|StorageLocation
name|location
...
block|{
comment|// The http socket is created externally using JSVC, we add it in
comment|// directly.
name|builder
operator|.
name|setConnector
...
annotation|@
name|Override
DECL|method|getHdfsBlocksMetadata ( String bpId, long[] blockIds, List<Token<BlockTokenIdentifier>> tokens)
specifier|public
name|HdfsBlocksMetadata
