...
DECL|method|unprotectedTruncate (String src, String clientName, String clientMachine, long newLength, long mtime)
name|void
...
block|}
block|}
comment|/**    * Truncate has the following properties:    * 1.) Any block deletions occur now.    * 2.) INode length is truncated now â clients can only read up to new length.    * 3.) INode will be set to UC and lastBlock set to UNDER_RECOVERY.    * 4.) NN will trigger DN truncation recovery and waits for DNs to report.    * 5.) File is considered UNDER_RECOVERY until truncation recovery completes.    * 6.) Soft and hard Lease expiration require truncation recovery to complete.    *    * @return true if on the block boundary or false if recovery is need    */
...
argument_list|)
expr_stmt|;
comment|// If on block boundary, then return
name|long
name|lastBlockDelta
init|=
name|remainingLength
...
literal|true
return|;
comment|// Set new last block length
name|BlockInfo
name|lastBlock
init|=
name|file
