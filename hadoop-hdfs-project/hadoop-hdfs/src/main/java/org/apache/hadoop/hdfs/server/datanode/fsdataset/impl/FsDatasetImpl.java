...
name|volumeMap
decl_stmt|;
DECL|field|lazyWriteReplicaTracker
specifier|final
...
expr_stmt|;
block|}
comment|/**    * Add an array of StorageLocation to FsDataset.    *    * @pre dataStorage must have these volumes.    * @param volumes    * @throws IOException    */
annotation|@
name|Override
...
name|warn
argument_list|(
literal|"Attempt to add an invalid volume: "
operator|+
name|vol
operator|.
name|getFile
...
block|}
block|}
comment|/**    * Removes a collection of volumes from FsDataset.    * @param volumes the root directories of the volumes.    *    * DataNode should call this function before calling    * {@link DataStorage#removeVolumes(java.util.Collection)}.    */
annotation|@
name|Override
...
block|}
block|}
comment|// Delete blocks from the block scanner in batch.
name|datanode
operator|.
...
block|}
block|}
comment|/**    * Attempt to evict one or more transient block replicas we have at least    * spaceNeeded bytes free.    *    * @return true if we were able to free up at least spaceNeeded bytes, false    *          otherwise.    */
DECL|method|tryToEvictBlocks (final String bpid, final long spaceNeeded)
specifier|private
name|boolean
name|tryToEvictBlocks
parameter_list|(
specifier|final
name|String
name|bpid
...
name|info
argument_list|(
literal|"Attempting to evict blocks from transient storage"
argument_list|)
expr_stmt|;
comment|// Reverse the map so we can iterate in order of replica creation times,
comment|// evicting oldest replicas one at a time until we have sufficient space.
name|TreeMultimap
argument_list|<
name|Long
argument_list|,
...
literal|0
decl_stmt|;
comment|// TODO: It is really inefficient to do this with the Object lock held!
comment|// TODO: This logic is here just for prototyping.
comment|// TODO: We should replace it with proactive discard when ram_disk free space
comment|// TODO:   falls below a low watermark. That way we avoid fs operations on the
comment|// TODO:   hot path with the lock held.
synchronized|synchronized
init|(
name|this
init|)
...
name|info
argument_list|(
literal|"RAM_DISK: Evicting blockId="
operator|+
name|lazyWriteReplica
operator|.
name|blockId
...
argument_list|)
expr_stmt|;
comment|// Move the persisted replica to the finalized directory of
...
argument_list|)
decl_stmt|;
comment|// Update the volumeMap entry. This removes the old entry.
name|volumeMap
operator|.
name|add
argument_list|(
...
name|info
argument_list|(
literal|"RAM_DISK: freed up "
operator|+
name|spaceNeeded
operator|+
literal|" bytes for new block"
argument_list|)
expr_stmt|;
name|isAvailable
operator|=
...
condition|)
block|{
comment|// Eviction did not work, we'll just fallback to DEFAULT storage.
name|LOG
operator|.
name|info
argument_list|(
literal|"RAM_DISK: Failed to free up "
operator|+
name|b
operator|.
name|getNumBytes
argument_list|()
operator|+
literal|" bytes for new block. Will fallback to DEFAULT "
operator|+
literal|"storage"
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**    * Invalidate a block but does not delete the actual on-disk block file.    *    * It should only be used for decommissioning disks.    *    * @param bpid the block pool ID.    * @param blockId the ID of the block.    */
DECL|method|invalidate (String bpid, long blockId)
specifier|public
name|void
name|invalidate
parameter_list|(
...
name|info
argument_list|(
literal|"LazyWriter invoked to save blockId="
operator|+
name|blockId
operator|+
literal|"; bpid="
