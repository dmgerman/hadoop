...
expr_stmt|;
block|}
DECL|method|addVolumeAndBlockPool (Collection<StorageLocation> dataLocations, Storage.StorageDirectory sd, final Collection<String> bpids)
specifier|private
name|void
name|addVolumeAndBlockPool
parameter_list|(
name|Collection
argument_list|<
name|StorageLocation
...
condition|)
block|{
comment|// The states of FsDatasteImpl are not modified, thus no need to rolled back.
throw|throw
name|MultipleIOException
operator|.
...
expr_stmt|;
block|}
comment|/**    * Add an array of StorageLocation to FsDataset.    *    * @pre dataStorage must have these volumes.    * @param volumes an array of storage locations for adding volumes.    * @param bpids block pool IDs.    * @return an array of successfully loaded volumes.    */
annotation|@
name|Override
DECL|method|addVolumes ( final List<StorageLocation> volumes, final Collection<String> bpids)
specifier|public
specifier|synchronized
name|List
argument_list|<
...
name|warn
argument_list|(
literal|"Caught exception when parsing storage URL."
argument_list|,
name|ioe
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"Caught exception when adding volume "
operator|+
name|vol
argument_list|,
name|e
...
name|warn
argument_list|(
literal|"Caught InterruptedException when adding volume."
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
DECL|method|copyBlockFiles (long blockId, long genStamp, File srcMeta, File srcFile, File destRoot)
specifier|static
name|File
index|[]
name|copyBlockFiles
...
expr_stmt|;
block|}
DECL|method|discardRamDiskReplica (RamDiskReplica replica, boolean deleteSavedCopies)
name|void
name|discardRamDiskReplica
parameter_list|(
name|RamDiskReplica
...
decl_stmt|;
DECL|field|lowWatermarkFreeSpaceReplicas
specifier|final
name|int
name|lowWatermarkFreeSpaceReplicas
decl_stmt|;
...
expr_stmt|;
block|}
DECL|method|moveReplicaToNewVolume (String bpid, long blockId, long creationTime)
specifier|private
name|void
...
expr_stmt|;
block|}
comment|// Drop the FsDatasetImpl lock for the file copy.
name|File
index|[]
name|savedFiles
init|=
...
block|}
block|}
comment|// Before deleting the files from transient storage we must notify the
...
argument_list|)
expr_stmt|;
comment|// Remove the old replicas from transient storage.
if|if
condition|(
name|blockFile
operator|.
