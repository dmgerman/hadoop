...
expr_stmt|;
block|}
comment|/**    * List open files in the system in batches. prevId is the cursor INode id and    * the open files returned in a batch will have their INode ids greater than    * this cursor. Open files can only be requested by super user and the the    * list across batches does not represent a consistent view of all open files.    *    * @param prevId the cursor INode id.    * @throws IOException    */
...
block|}
block|}
DECL|method|reencryptEncryptionZoneInt (final String zone, final ReencryptAction action, final boolean logRetryCache)
specifier|private
name|void
name|reencryptEncryptionZoneInt
parameter_list|(
...
argument_list|()
decl_stmt|;
comment|// get keyVersionName out of the lock. This keyVersionName will be used
comment|// as the target keyVersion for the entire re-encryption.
comment|// This means all edek's keyVersion will be compared with this one, and
comment|// kms is only contacted if the edek's keyVersion is different.
specifier|final
name|KeyVersion
name|kv
init|=
