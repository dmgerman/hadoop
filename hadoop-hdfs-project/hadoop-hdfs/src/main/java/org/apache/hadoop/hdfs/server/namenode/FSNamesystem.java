...
block|}
block|}
comment|/**    * If the file is within an encryption zone, select the appropriate     * CipherSuite from the list provided by the client. Since the client may     * be newer, need to handle unknown CipherSuites.    *    * @param src path of the file    * @param cipherSuites client-provided list of supported CipherSuites,     *                     in desired order.    * @return chosen CipherSuite, or null if file is not in an EncryptionZone    * @throws IOException    */
DECL|method|chooseCipherSuite (String src, List<CipherSuite> cipherSuites)
specifier|private
name|CipherSuite
...
DECL|method|startFileInternal (FSPermissionChecker pc, String src, PermissionStatus permissions, String holder, String clientMachine, boolean create, boolean overwrite, boolean createParent, short replication, long blockSize, List<CipherSuite> cipherSuites, boolean logRetryEntry)
specifier|private
name|void
name|startFileInternal
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// TODO: fill in actual key/iv in HDFS-6474
comment|// For now, populate with dummy data
name|byte
index|[]
name|key
init|=
...
DECL|method|prepareFileForWrite (String src, INodeFile file, String leaseHolder, String clientMachine, DatanodeDescriptor clientNode, boolean writeToEditLog, int latestSnapshot, boolean logRetryCache)
name|LocatedBlock
name|prepareFileForWrite
parameter_list|(
name|String
...
block|}
block|}
DECL|method|createEncryptionZoneInt (final String srcArg, String keyId, final boolean logRetryCache)
specifier|private
name|void
name|createEncryptionZoneInt
parameter_list|(
...
name|src
operator|+
literal|" is already in an encryption zone. ("
operator|+
name|ez
operator|.
name|getPath
...
expr_stmt|;
block|}
DECL|method|createNewKey (String src)
specifier|private
name|String
name|createNewKey
parameter_list|(
...
return|;
block|}
comment|/**    * Delete the encryption zone on directory src.    *    * @param src the path of a directory which is the root of the encryption    * zone. The directory must be empty and must be marked as an encryption    * zone.    *    * @throws AccessControlException if the caller is not the superuser.    *    * @throws UnresolvedLinkException if the path can't be resolved.    *    * @throws SafeModeException if the Namenode is in safe mode.    */
...
block|}
block|}
DECL|method|deleteEncryptionZoneInt (final String srcArg, final boolean logRetryCache)
specifier|private
name|void
name|deleteEncryptionZoneInt
parameter_list|(
...
name|checkNameNodeSafeMode
argument_list|(
literal|"Cannot delete encryption zone on "
operator|+
name|src
argument_list|)
expr_stmt|;
