...
parameter_list|)
block|{
comment|// In the event we're interrupted while getting the normal FSNS read lock,
...
parameter_list|)
block|{
comment|// In the event we're interrupted while getting the normal FSNS write
comment|// lock, release the long read lock.
name|this
operator|.
name|fsLock
operator|.
...
expr_stmt|;
block|}
comment|// If the latest key version is null, need to fetch it and update
if|if
condition|(
name|latestEZKeyVersion
operator|==
...
argument_list|)
expr_stmt|;
comment|// Generate the EDEK while not holding the lock
name|KeyProviderCryptoExtension
operator|.
...
argument_list|()
expr_stmt|;
comment|/*      * We want to avoid holding any locks while creating a new      * FileEncryptionInfo, since this can be very slow. Since the path can      * flip flop between being in an encryption zone and not in the meantime,      * we need to recheck the preconditions and generate a new      * FileEncryptionInfo in some circumstances.      *      * A special RetryStartFileException is used to indicate that we should      * retry creation of a FileEncryptionInfo.      */
try|try
block|{
name|boolean
name|shouldContinue
...
operator|++
expr_stmt|;
comment|// Optimistically generate a FileEncryptionInfo for this path.
name|FileEncryptionInfo
name|feInfo
init|=
name|newFileEncryptionInfo
...
argument_list|)
decl_stmt|;
comment|// Try to create the file with this feInfo
name|writeLock
argument_list|()
expr_stmt|;
try|try
...
DECL|method|startFileInternal (FSPermissionChecker pc, String src, PermissionStatus permissions, String holder, String clientMachine, boolean create, boolean overwrite, boolean createParent, short replication, long blockSize, FileEncryptionInfo feInfo, boolean logRetryEntry)
specifier|private
name|void
name|startFileInternal
parameter_list|(
...
condition|)
block|{
comment|// If the path is not in an EZ, we don't need an feInfo.
comment|// Null it out in case one was already generated.
name|feInfo
operator|=
literal|null
expr_stmt|;
...
block|{
comment|// The path is now within an EZ, but no feInfo. Retry.
if|if
condition|(
name|feInfo
operator|==
...
throw|;
block|}
comment|// It's in an EZ and we have a provided feInfo. Make sure the
comment|// keyVersion of the encryption key used matches one of the keyVersions of
comment|// the key of the encryption zone.
if|if
condition|(
operator|!
name|dir
...
name|path
decl_stmt|;
DECL|method|FileState (INodeFile inode, String fullPath)
specifier|public
name|FileState
parameter_list|(
name|INodeFile
...
argument_list|)
expr_stmt|;
comment|// Blocks/INodes will be handled later
...
DECL|method|mkdirsRecursively (String src, PermissionStatus permissions, boolean inheritPermission, long now)
specifier|private
name|boolean
name|mkdirsRecursively
parameter_list|(
...
annotation|@
name|VisibleForTesting
DECL|method|getLongReadLockForTests ()
specifier|public
name|ReentrantLock
...
expr_stmt|;
block|}
comment|/**    * Create a new key on the KeyProvider for an encryption zone.    *    * @param keyId id of the key    * @param src path of the encryption zone.    * @return KeyVersion of the created key    * @throws IOException    */
DECL|method|createNewKey (String keyId, String src)
specifier|private
name|KeyVersion
name|createNewKey
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// TODO pass in hdfs://HOST:PORT (HDFS-6490)
name|providerOptions
operator|.
name|setDescription
argument_list|(
...
DECL|method|checkXAttrSize (XAttr xAttr)
specifier|private
name|void
name|checkXAttrSize
parameter_list|(
...
expr_stmt|;
block|}
DECL|method|checkXAttrChangeAccess (String src, XAttr xAttr, FSPermissionChecker pc)
specifier|private
name|void
name|checkXAttrChangeAccess
parameter_list|(
