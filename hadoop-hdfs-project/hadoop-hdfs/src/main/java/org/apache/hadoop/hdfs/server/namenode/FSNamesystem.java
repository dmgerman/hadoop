...
expr_stmt|;
block|}
comment|/**    * Truncate file to a lower length.    * Truncate cannot be reverted / recovered from as it causes data loss.    * Truncation at block boundary is atomic, otherwise it requires    * block recovery to truncate the last block of the file.    *    * @return true if and client does not need to wait for block recovery,    * false if client needs to wait for block recovery.    */
...
DECL|method|truncateInternal (String src, long newLength, String clientName, String clientMachine, long mtime, FSPermissionChecker pc)
name|boolean
name|truncateInternal
parameter_list|(
name|String
...
argument_list|()
decl_stmt|;
comment|// Data will be lost after truncate occurs so it cannot support snapshots.
if|if
condition|(
name|file
operator|.
...
name|HadoopIllegalArgumentException
argument_list|(
literal|"Cannot truncate file with snapshot."
argument_list|)
throw|;
...
argument_list|)
expr_stmt|;
comment|// Refresh INode as the file could have been closed
name|iip
operator|=
name|dir
operator|.
...
return|;
block|}
DECL|method|initializeBlockRecovery (INodeFile inodeFile)
name|void
name|initializeBlockRecovery
parameter_list|(
name|INodeFile
