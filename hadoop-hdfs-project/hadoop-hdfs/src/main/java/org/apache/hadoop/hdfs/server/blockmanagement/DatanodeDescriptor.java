...
condition|)
block|{
comment|// to verify provided storage participated in this hb, requires
comment|// check to pass DNDesc.
comment|// e.g., storageInfo.verifyBlockReportId(this, curBlockReportId)
continue|continue;
block|}
if|if
condition|(
...
expr_stmt|;
block|}
DECL|method|updateStorageStats (StorageReport[] reports, long cacheCapacity, long cacheUsed, int xceiverCount, int volFailures, VolumeFailureSummary volumeFailureSummary)
specifier|private
name|void
name|updateStorageStats
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// skip accounting for capacity of PROVIDED storages!
if|if
condition|(
name|StorageType
operator|.
...
block|}
block|}
DECL|method|injectStorage (DatanodeStorageInfo s)
name|void
name|injectStorage
parameter_list|(
name|DatanodeStorageInfo
...
return|;
block|}
comment|/**    * Add the block infos which needs to move its storage locations.    *    * @param blkMovingInfo    *          - storage mismatched block info    */
DECL|method|addBlocksToMoveStorage (BlockMovingInfo blkMovingInfo)
specifier|public
name|void
...
name|debug
argument_list|(
literal|"Adding block move task "
operator|+
name|blkMovingInfo
operator|+
literal|" to "
...
expr_stmt|;
block|}
comment|/**    * Return the number of blocks queued up for movement.    */
DECL|method|getNumberOfBlocksToMoveStorages ()
specifier|public
name|int
...
return|;
block|}
comment|/**    * Get the blocks to move to satisfy the storage media type.    *    * @param numBlocksToMoveTasks    *          total number of blocks which will be send to this datanode for    *          block movement.    *    * @return block infos which needs to move its storage locations or null if    *         there is no block infos to move.    */
DECL|method|getBlocksToMoveStorages (int numBlocksToMoveTasks)
specifier|public
name|BlockMovingInfo
index|[]
