...
block|}
block|}
comment|/**    * Get the best node.    * @param nodes Nodes to choose from.    * @param ignoredNodes Do not chose nodes in this array (may be null)    * @return The DNAddrPair of the best node.    * @throws IOException    */
DECL|method|getBestNodeDNAddrPair (final DatanodeInfo[] nodes, Collection<DatanodeInfo> ignoredNodes)
specifier|private
name|DNAddrPair
name|getBestNodeDNAddrPair
parameter_list|(
...
block|}
block|}
DECL|method|getFromOneDataNode (final DNAddrPair datanode, final LocatedBlock block, final long start, final long end, final ByteBuffer bb, final Map<ExtendedBlock, Set<DatanodeInfo>> corruptedBlockMap, final CountDownLatch latch)
specifier|private
name|Callable
argument_list|<
name|ByteBuffer
...
argument_list|)
expr_stmt|;
comment|// Latch shared by all outstanding reads.  First to finish closes
name|CountDownLatch
name|hasReceivedResult
init|=
operator|new
name|CountDownLatch
argument_list|(
...
literal|null
decl_stmt|;
comment|// futures is null if there is no request already executing.
if|if
condition|(
name|futures
operator|==
...
block|}
block|}
DECL|method|getHedgedReadFuture (final DNAddrPair chosenNode, final LocatedBlock block, long start, final long end, final ByteBuffer bb, final Map<ExtendedBlock, Set<DatanodeInfo>> corruptedBlockMap, final CountDownLatch hasReceivedResult)
specifier|private
name|Future
argument_list|<
name|ByteBuffer
...
return|;
block|}
DECL|method|getFirstToComplete (ArrayList<Future<ByteBuffer>> futures, CountDownLatch latch)
specifier|private
name|ByteBuffer
name|getFirstToComplete
parameter_list|(
...
name|InterruptedException
argument_list|(
literal|"latch has counted down to zero but no"
operator|+
literal|"result available yet, for safety try to request another one from"
operator|+
literal|"outside loop, this should be rare"
argument_list|)
throw|;
block|}
...
DECL|method|bestNode (DatanodeInfo nodes[], AbstractMap<DatanodeInfo, DatanodeInfo> deadNodes, Collection<DatanodeInfo> ignoredNodes)
specifier|static
name|DatanodeInfo
name|bestNode
parameter_list|(
