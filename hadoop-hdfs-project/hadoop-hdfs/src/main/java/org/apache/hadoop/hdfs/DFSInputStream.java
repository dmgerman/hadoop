...
return|;
block|}
comment|// If the legacy local block reader is enabled and we are reading a local
comment|// block, try to create a BlockReaderLocalLegacy.  The legacy local block
comment|// reader implements local reads in the style first introduced by HDFS-2246.
if|if
condition|(
operator|(
name|dfsClient
...
block|}
block|}
comment|// Look for cached domain peers.
name|int
name|cacheTries
init|=
literal|0
...
name|debug
argument_list|(
literal|"Error making BlockReader with DomainSocket. "
operator|+
...
block|}
block|}
comment|// Try to create a DomainPeer.
name|DomainSocket
name|domSock
init|=
name|dsFactory
...
name|IOException
argument_list|(
literal|"TCP reads are disabled."
argument_list|)
throw|;
block|}
comment|// Try to create a new remote peer.
name|Peer
name|peer
init|=
name|newTcpPeer
...
expr_stmt|;
block|}
DECL|method|readZeroCopy (HdfsZeroCopyCursor zcursor, int toRead)
specifier|synchronized
name|void
name|readZeroCopy
parameter_list|(
...
block|}
block|}
comment|/*      * Slow path reads.      *      * readStatistics will be updated when we call back into this      * stream's read methods.      */
name|long
name|prevBlockEnd
init|=
name|blockEnd
...
name|curPos
operator|+
literal|" via the fallback read path.  "
operator|+
literal|"prevBlockEnd = "
operator|+
name|prevBlockEnd
operator|+
literal|", blockEnd = "
