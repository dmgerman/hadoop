...
name|cacheSize
decl_stmt|;
comment|// Track all containers owned by block service.
...
comment|/**    * Load allocated containers from persistent store.    * @throws IOException    */
DECL|method|loadAllocatedContainers ()
specifier|private
name|void
name|loadAllocatedContainers
parameter_list|()
...
name|IOException
block|{
comment|// Pre-allocate empty map entry by state to avoid null check
name|containers
operator|=
operator|new
name|ConcurrentHashMap
...
argument_list|)
decl_stmt|;
comment|// TODO: remove the container from block manager's container DB
comment|// Most likely the allocated container is timeout and cleaned up
comment|// by SCM, we should clean up correspondingly instead of just skip it.
if|if
condition|(
name|containerInfo
operator|==
...
name|debug
argument_list|(
literal|"Loading allocated container: {} used : {} state: {}"
argument_list|,
name|containerName
argument_list|,
name|containerUsed
...
name|warn
argument_list|(
literal|"Failed loading allocated container, continue next..."
argument_list|)
expr_stmt|;
block|}
return|return
...
block|}
block|}
comment|/**    * Pre allocate specified count of containers for block creation.    * @param count - number of containers to allocate.    * @return list of container names allocated.    * @throws IOException    */
DECL|method|allocateContainers (int count)
specifier|private
name|List
argument_list|<
name|String
...
return|;
block|}
comment|/**    * Filter container by states and size.    * @param state the state of the container.    * @param size the minimal available size of the container    * @return allocated containers satisfy both state and size.    */
DECL|method|filterContainers (OzoneProtos.LifeCycleState state, long size)
specifier|private
name|List
argument_list|<
name|String
...
return|;
block|}
DECL|method|getContainer (OzoneProtos.LifeCycleState state, String name)
specifier|private
name|BlockContainerInfo
name|getContainer
parameter_list|(
...
return|;
block|}
comment|// Relies on the caller such as allocateBlock() to hold the lock
comment|// to ensure containers map consistent.
DECL|method|updateContainer (OzoneProtos.LifeCycleState oldState, String name, OzoneProtos.LifeCycleState newState)
specifier|private
name|void
...
name|debug
argument_list|(
literal|"Update container {} from state {} to state {}"
argument_list|,
name|name
argument_list|,
name|oldState
...
expr_stmt|;
block|}
comment|// Refresh containers that have been allocated.
comment|// We may not need to track all the states, just the creating/open/close
comment|// should be enough for now.
DECL|method|refreshContainers ()
specifier|private
name|void
...
condition|)
block|{
comment|// TODO: clean up containers that has been deleted on SCM but
comment|// TODO: still in ALLOCATED state in block manager.
name|LOG
operator|.
name|debug
argument_list|(
...
expr_stmt|;
block|}
comment|// TODO: check containers in other state and refresh as needed.
comment|// TODO: ALLOCATED container that is timeout and DELETED. (unit test)
comment|// TODO: OPEN container that is CLOSE.
block|}
catch|catch
parameter_list|(
name|IOException
...
name|debug
argument_list|(
literal|"Failed to get container info for: {}"
argument_list|,
name|containerName
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|// now we should have some candidates in ALLOCATE state
if|if
condition|(
name|candidates
operator|.
...
block|}
block|}
comment|// Candidates list now should include only ALLOCATE or OPEN containers
name|int
name|randomIdx
init|=
name|rand
...
expr_stmt|;
block|}
comment|// Container must be either OPEN or ALLOCATE state
if|if
condition|(
name|containerInfo
operator|.
