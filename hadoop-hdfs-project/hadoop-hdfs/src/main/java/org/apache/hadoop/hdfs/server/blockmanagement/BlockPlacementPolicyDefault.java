...
return|;
block|}
DECL|method|selectStorageTypes ( final BlockStoragePolicy storagePolicy, final short replication, final Iterable<StorageType> chosen, final EnumSet<StorageType> unavailableStorages, final boolean isNewBlock)
specifier|private
specifier|static
name|List
argument_list|<
...
name|warn
argument_list|(
literal|"Failed to place enough replicas: replication is "
operator|+
name|replication
operator|+
literal|" but only "
...
condition|)
block|{
comment|// Retry chooseTarget with fallback storage types
name|unavailableStorages
operator|.
name|add
argument_list|(
