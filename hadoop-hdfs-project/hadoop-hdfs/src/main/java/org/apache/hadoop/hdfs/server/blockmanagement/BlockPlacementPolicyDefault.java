...
argument_list|)
decl_stmt|;
DECL|field|ONE_RACK_PLACEMENT
specifier|private
specifier|static
specifier|final
name|BlockPlacementStatus
name|ONE_RACK_PLACEMENT
init|=
operator|new
...
annotation|@
name|Override
DECL|method|chooseTarget (String srcPath, int numOfReplicas, Node writer, List<DatanodeStorageInfo> chosen, boolean returnChosenNodes, Set<Node> excludedNodes, long blocksize, BlockStoragePolicy storagePolicy, EnumSet<AddBlockFlag> flags, EnumMap<StorageType, Integer> storageTypes)
specifier|public
name|DatanodeStorageInfo
index|[]
name|chooseTarget
...
name|target
init|=
name|chooseLocalOrFavoredStorage
argument_list|(
name|favoredNode
argument_list|,
literal|true
...
DECL|method|chooseTarget (int numOfReplicas, Node writer, List<DatanodeStorageInfo> chosenStorage, boolean returnChosenNodes, Set<Node> excludedNodes, long blocksize, final BlockStoragePolicy storagePolicy, EnumSet<AddBlockFlag> addBlockFlags, EnumMap<StorageType, Integer> sTypes)
specifier|private
name|DatanodeStorageInfo
index|[]
name|chooseTarget
...
return|;
block|}
comment|/**    * Choose storage of local or favored node.    * @param localOrFavoredNode local or favored node    * @param isFavoredNode if target node is favored node    * @param excludedNodes datanodes that should not be considered as targets    * @param blocksize size of the data to be written    * @param maxNodesPerRack max nodes allowed per rack    * @param results the target nodes already chosen    * @param avoidStaleNodes avoid stale nodes in replica choosing    * @param storageTypes storage type to be considered for target    * @return storage of local or favored node (not chosen node)    * @throws NotEnoughReplicasException    */
DECL|method|chooseLocalOrFavoredStorage ( Node localOrFavoredNode, boolean isFavoredNode, Set<Node> excludedNodes, long blocksize, int maxNodesPerRack, List<DatanodeStorageInfo> results, boolean avoidStaleNodes, EnumMap<StorageType, Integer> storageTypes)
specifier|protected
name|DatanodeStorageInfo
name|chooseLocalOrFavoredStorage
parameter_list|(
name|Node
name|localOrFavoredNode
parameter_list|,
name|boolean
name|isFavoredNode
parameter_list|,
