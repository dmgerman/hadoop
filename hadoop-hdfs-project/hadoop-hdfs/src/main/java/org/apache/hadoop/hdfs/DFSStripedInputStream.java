...
block|}
block|}
DECL|method|fetchOneStripe (LocatedStripedBlock blockGroup, byte[] buf, AlignedStripe alignedStripe, Map<ExtendedBlock, Set<DatanodeInfo>> corruptedBlockMap)
specifier|private
name|void
name|fetchOneStripe
parameter_list|(
...
name|warn
argument_list|(
literal|"Trying to read an empty stripe from"
operator|+
name|blockGroup
argument_list|)
expr_stmt|;
...
name|IOException
argument_list|(
literal|"Too many blocks are missing: "
operator|+
name|alignedStripe
argument_list|)
throw|;
block|}
comment|// When seeing first missing block, initialize decode input buffers
if|if
condition|(
name|decodeInputs
operator|==
...
block|}
block|}
comment|/**    * Schedule a single read request to an internal block    * @param block The internal block    * @param index Index of the internal block in the group    * @param corruptedBlockMap Map of corrupted blocks    */
DECL|method|fetchOneStripingChunk (Map<Future<Void>, Integer> futures, final CompletionService<Void> service, final LocatedBlock block, final AlignedStripe alignedStripe, final int index, Map<ExtendedBlock, Set<DatanodeInfo>> corruptedBlockMap)
specifier|private
name|void
name|fetchOneStripingChunk
parameter_list|(
